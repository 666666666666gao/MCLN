"""Persist one real CPU Sr3D/joint-detection batch for later native preflight."""
import argparse
import datetime
import hashlib
import json
...
def main():
    ...
    selections=json.loads((selection/'preflight_rows.json').read_bytes())['sr3d']
    ...
    print('SR_BATCH_LOADING',flush=True)
    dataset=build_probe_dataset(Joint3DDataset,dataset_args,annotation,selections,'sr3d')
    ...
    assert batch['sample_dataset']==['sr3d']*4+['scannet']*4
    assert batch['language_dataset']==['sr3d']*8
    ...
    receipt=dict(status='pass',time_cst=datetime.datetime.now(datetime.timezone(datetime.timedelta(hours=8))).isoformat(),
        scope='four fixed Sr3D and four joint-detection training rows; CPU integration only, not training-distribution metrics',
    ...
    print('SR_NATIVE_BATCH_COMPLETE '+json.dumps(receipt),flush=True)
