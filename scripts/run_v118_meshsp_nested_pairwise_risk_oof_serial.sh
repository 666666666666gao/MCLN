...
OUTPUT=$BASE/v118_meshsp_nested_pairwise_risk_oof.json
LOG=$BASE/v118_meshsp_nested_pairwise_risk_oof.log
EXIT=$BASE/v118_meshsp_nested_pairwise_risk_oof.exit
SESSION=v118_nested_pairwise_risk_oof
SELF=$(readlink -f "$0")
...
if [[ "${1:-}" == "--inner" ]]; then
    ...
    env CUDA_VISIBLE_DEVICES=0 \
        ...
        "$PYTHON" scripts/run_v118_meshsp_nested_pairwise_risk_oof.py \
...
cd "$REPO"
require_sha a08bb024e7596e259bac556ae9493d3805f3e581d7995cfdee5b89a1553ce649 \
    scripts/run_v118_meshsp_nested_pairwise_risk_oof.py
...
for path in "$OUTPUT" "$LOG" "$EXIT"; do
    if [[ -e "$path" ]]; then
        echo "Refusing to overwrite existing V118 output: $path" >&2
        exit 74
    fi
done
if [[ $(nvidia-smi -L | wc -l) -ne 1 ]]; then
    echo "V118 requires the frozen single-GPU topology" >&2
    exit 75
fi
...
if pgrep -af '[r]un_v118_meshsp_nested_pairwise_risk_oof.py' >/dev/null; then
    echo "V118 process already exists" >&2
    exit 75
fi
if screen -ls 2>/dev/null | grep -q "[.]$SESSION"; then
    echo "V118 screen session already exists" >&2
    exit 75
fi
available_kb=$(df -Pk /root/autodl-tmp | awk 'NR==2 {print $4}')
if [[ "$available_kb" -lt 2621440 ]]; then
    echo "V118 requires at least 2.5 GiB free on /root/autodl-tmp" >&2
    exit 75
fi
...
if ! screen -ls 2>/dev/null | grep -q "[.]$SESSION" \
        && [[ ! -e "$EXIT" ]]; then
    echo "V118 screen failed to remain alive and produced no exit receipt" >&2
    exit 75
fi
