> **2026-09-05 终局修订（master §20.37）**：G0已完成，完整性PASS、预注册科学门FAIL。
> 全体7151行fixed相对old：@.25 +11（修复60/破坏49）、@.50 +116（296/180）；
> raw view-dependent 2718行：@.25为0（20/20），未满足严格增加。因此执行
> `seal_performance_route_keep_data_fix`，封存增强性能路线、保留数据修复，不进入原G0→G1。
> 下文是历史预注册，未启动/full-state等历史状态不代表当前执行状态；科学门未更改。
> 实际起点为同一eval-only E57/E69平均权重，两边新AdamW、四组LR
> 1e-5/1e-4/1e-5/1.25e-5，各完成1611 steps，实际数据顺序一致。
> 两边各612个文件终局复核无变化，唯一组间差异是已登记的数据谓词。
> 该留出只排除新增更新；底层checkpoint已见过完整train，不是全系统未见场景泛化。
>
> 实际文本规范化后增强许可差异为325行（fit253/holdout72）；old允许16432、fixed16179，
> 均与回执吻合。253是fit许可直接变化数，不是所有数值输入变化数，因为worker随机流连续。
> raw CSV的2155保留为raw口径。用户后续独立授权的P2关系读出机制实验单独登记，
> 不改写本G0结论，也不恢复FPR/A-V4、Relation-CF、Density或已取消的长baseline项目。
