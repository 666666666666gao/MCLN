...
OUTPUT=$BASE/v130_meshsp_hyperspherical_semantic_adapter_oof.json
LOG=$BASE/v130_meshsp_hyperspherical_semantic_adapter_oof.log
EXIT=$BASE/v130_meshsp_hyperspherical_semantic_adapter_oof.exit
SESSION=v130_hyperspherical_semantic_adapter_oof
SELF=$(readlink -f "$0")
...
if [[ "${1:-}" == "--inner" ]]; then
    ...
    env CUDA_VISIBLE_DEVICES=0 \
        ...
        "$PYTHON" scripts/run_v130_meshsp_hyperspherical_semantic_adapter_oof.py \
...
cd "$REPO"
require_sha d06c09bd4b17198cc2f057778600205a590af99bff4296233a1958d99483a785 \
    scripts/run_v130_meshsp_hyperspherical_semantic_adapter_oof.py
...
require_sha 376ce0750a5a5a453011f946dd5d8cfef49992879344ec02532d68ea71059b03 \
    tests/test_rec_hyperspherical_semantic_adapter.py
...
for path in "$OUTPUT" "$LOG" "$EXIT"; do
    if [[ -e "$path" ]]; then
        echo "Refusing to overwrite existing V130 output: $path" >&2
        exit 74
    fi
done
if [[ $(nvidia-smi -L | wc -l) -ne 1 ]]; then
    echo "V130 requires the frozen single-GPU topology" >&2
    exit 75
fi
...
if pgrep -af '[r]un_v130_meshsp_hyperspherical_semantic_adapter_oof.py' >/dev/null; then
    echo "V130 process already exists" >&2
    exit 75
fi
if screen -ls 2>/dev/null | grep -q "[.]$SESSION"; then
    echo "V130 screen session already exists" >&2
    exit 75
fi
available_kb=$(df -Pk /root/autodl-tmp | awk 'NR==2 {print $4}')
if [[ "$available_kb" -lt 2621440 ]]; then
    echo "V130 requires at least 2.5 GiB free on /root/autodl-tmp" >&2
    exit 75
fi
...
if ! screen -ls 2>/dev/null | grep -q "[.]$SESSION" \
        && [[ ! -e "$EXIT" ]]; then
    echo "V130 screen failed to remain alive and produced no exit receipt" >&2
    exit 75
fi
