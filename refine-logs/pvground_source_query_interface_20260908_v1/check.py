...
def main():
    ...
    with torch.no_grad():
        for start in (0,2):
            ...
            for key,value in output.items():
                if torch.is_tensor(value):assert torch.equal(value,native[key]),key
                elif isinstance(value,list) and value and all(torch.is_tensor(v) for v in value):
                    assert all(torch.equal(a,b) for a,b in zip(value,native[key])),key
            del native
    ...
    receipt.update(source_query_read=True,module_sha256=sha(Path(__file__).parent/'pvground_source_query.py'),model_source=str(model_source),source_port_sha256=sha(model_source.parent/'source_port.json'),zero_residual_replay_exact=True,new_parameters=sum(p.numel() for p in reader.parameters()))
