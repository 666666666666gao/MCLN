#!/usr/bin/env python
"""Exact train-cache parity audit for V113 materialization versus runtime."""
...
def _readonly_identity(path):
    ...
    if stat.S_ISLNK(entry.st_mode) or not stat.S_ISREG(entry.st_mode):
        raise ValueError("V113 artifact must be a regular file")
    mode = stat.S_IMODE(entry.st_mode)
    if mode != 0o444:
        raise ValueError("V113 artifact must have mode 0444")
    return {
...
def main(argv=None):
    ...
    parser.add_argument("--v113-artifact", required=True)
    ...
    if not _is_sha256(args.expected_artifact_sha256):
        raise ValueError("V113 expected artifact SHA is invalid")
    if _sha256(args.v113_artifact) != args.expected_artifact_sha256:
        raise ValueError("V113 artifact SHA changed")
    ...
    protected_before["v113"] = _readonly_identity(args.v113_artifact)
    ...
    model, artifact = load_rec_hierarchical_runtime_artifact(
        args.v113_artifact, args.device,
        loaded["parent"][0], loaded["geometry_model"],
        loaded["geometry_artifact"],
    )
    if (getattr(model, "_artifact_sha256", None)
            != args.expected_artifact_sha256):
        raise ValueError("V113 runtime loader did not bind the frozen artifact")
    ...
    with torch.no_grad(), _disabled_parent_autocast(device):
        for start in range(0, len(rows), HIERARCHICAL_MATERIALIZATION_BATCH_SIZE):
            ...
            for field in ("member_query_logits", "member_variant_logits"):
                _accumulate(
            ...
            offline_policy = apply_asymmetric_risk_contextual_policy(
                stored_scores, offline_outputs["member_query_logits"],
                offline_outputs["member_variant_logits"],
                offline["query_valid"], offline["variant_valid"],
                float(artifact["policy"]["aggregate_lcb_margin"]),
                min_head_lcb025=artifact["policy"]["min_head_lcb025"],
                min_head_lcb050=artifact["policy"]["min_head_lcb050"],
                risk_lambda025=artifact["policy"]["risk_lambda025"],
                risk_lambda050=artifact["policy"]["risk_lambda050"],
            )
            runtime_policy = apply_asymmetric_risk_contextual_policy(
                flat_scores, runtime_outputs["member_query_logits"],
                runtime_outputs["member_variant_logits"],
                runtime["query_valid"], runtime["variant_valid"],
                float(artifact["policy"]["aggregate_lcb_margin"]),
                min_head_lcb025=artifact["policy"]["min_head_lcb025"],
                min_head_lcb050=artifact["policy"]["min_head_lcb050"],
                risk_lambda025=artifact["policy"]["risk_lambda025"],
            ...
            for field in (
                    ...
                    "switch_mask", "pareto_pass", "member_proposal_indices",
                    "anchor_agreement", "member_head_gain",
                    "anchor_head_gain", "head_risk", "head_lcb",
                    "aggregate_lcb"):
                _accumulate(
    ...
    protected_after["v113"] = _readonly_identity(args.v113_artifact)
    ...
    report = {
        "schema": "rec-v113-train-runtime-parity-audit-v1", "version": 1,
    ...
    if not all_equal:
        raise RuntimeError("V113 offline/runtime train parity failed")
