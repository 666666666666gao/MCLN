"""Matched Mask learning with native versus added point-detail memory."""
...
def main():
    ...
    def verify_inputs():
        ...
        assert file_sha(Path(manifest['detail_preflight_receipt'])) == manifest['detail_preflight_receipt_sha256']

    verify_inputs()
    detail_preflight = json.loads(Path(manifest['detail_preflight_receipt']).read_text())
    assert detail_preflight['status'] == 'complete' and detail_preflight['optimizer_steps'] == 0
    assert detail_preflight['zero_start_identity'] and detail_preflight['native_mask_loss_connected']
    assert detail_preflight['fixed_perturbation_rec_unchanged']
    os.chdir(str(source))
    ...
    from scripts.nr3d_point_detail_memory import PointDetailSuperpointResidual, SuperpointDetailIntervention
    ...
    for arm in ['native', 'detail']:
        ...
        if arm == 'native':
            ...
            detail_addon = PointDetailSuperpointResidual().cuda()
            initial_detail_state = {name: value.detach().cpu().clone()
                                    for name, value in detail_addon.state_dict().items()}
        else:
            detail_attachment = SuperpointDetailIntervention(model, detail_addon)
            parameters.update({'point_detail.' + name: value
                               for name, value in detail_addon.named_parameters()})
        models[arm] = model
        trainable[arm] = parameters
        optimizers[arm] = torch.optim.AdamW(parameters.values(), lr=1e-5, weight_decay=args.weight_decay)
    assert len(trainable['detail']) == 19
    assert list(trainable['native']) == list(trainable['detail'])[:16]
    ...
    def verify_state(before_training=False):
        ...
        for arm, model in models.items():
            ...
            if arm == 'detail':
                for name, value in detail_addon.state_dict().items():
                    same = torch.equal(value.detach().cpu(), initial_detail_state[name])
                    if before_training:
                        assert same, name
                    elif not same:
                        changed[arm].append('point_detail.' + name)
        return changed
    ...
    class SelectedDataset(Joint3DDataset):
        ...
        def __getitem__(self, index):
            ...
            item['detail_row_id'] = np.int64(self.detail_ids[index])
            return item
    ...
    for arm, model in models.items():
        ...
        for name, value in norms.items():
            if name.startswith('point_detail.point_encoder.'):
    ...
    print('POINT DETAIL PAIR PREFLIGHT', json.dumps(preflight), flush=True)
    ...
    def evaluate(stage):
        ...
        with torch.no_grad():
            for index, raw in enumerate(loader('holdout', 1000, 16, False)):
                ...
                require_same_grounding(snapshots['native'], snapshots['detail'])
                for bid, row_id in enumerate(batch['detail_row_id'].tolist()):
                    digest = hashlib.sha256()
                    ...
                    if stage == 'baseline':
                        assert record['native'] == record['detail'], row_id
                    records.append(record)
                if (index + 1) % 50 == 0:
                    print('POINT DETAIL PAIR EVAL', json.dumps({'stage': stage, 'batches': index + 1, 'rows': len(records),
    ...
    for epoch in range(2):
        ...
        for raw in loader('fit', epoch, 4, True):
            ...
            require_same_grounding(snapshots['native'], snapshots['detail'])
            step += 1
            seen_ids.extend(batch['detail_row_id'].tolist())
            if step % 64 == 0:
                print('POINT DETAIL PAIR TRAIN', json.dumps({'step': step, 'epoch': epoch, 'arms': statistics,
                      'elapsed_seconds': time.time() - started}), flush=True)
    assert step == 1024
    for epoch in range(2):
        assert sorted(seen_ids[epoch * 2048:(epoch + 1) * 2048]) == partitions['fit']
    changed = verify_state()
    assert len(changed['native']) == 16 and len(changed['detail']) == 19
    ...
    detail_attachment.remove()
    verify_inputs()
    receipt = {'schema': 'mcln-nr3d-point-detail-pair-v1', 'status': 'complete', 'optimizer_steps_per_arm': step,
    ...
    print('POINT DETAIL PAIR COMPLETE', json.dumps({key: value for key, value in receipt.items() if key != 'fit_order_ids'}), flush=True)
