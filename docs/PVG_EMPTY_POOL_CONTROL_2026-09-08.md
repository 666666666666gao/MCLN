...
## First saved checkpoint metadata (20.195)

19:15:46实时读取原训练controller20870及另外三个原接续controller仍存活，进度512/3723、累计1221.88秒。首个latest.pth已实际保存331104734字节。随后CPU读取保存文件，核对step512/4096训练行、empty_pool_mask=True、模块SHA1eb336f59a1824fa74e778c2e0c293b2357fefbd23bdbfa66f5988cf2767c803、spec SHA dfe684cca10146387195b543281e5dee56ea2df9d8e2b5955909225b7b947a5f及父权重SHA一致；1035个增量状态张量均在CPU。读取前后文件SHA ce22424b67b36e2f2fe335a4f2c74888ae3da74ef5c9273708813beb5143faed一致，未修改权重，CUDA未初始化。此项只是实际保存元数据/设备检查，不冒充恢复后完整前向或终态质量。

保存后磁盘余2297352192字节（约2.14GiB），与预留预算相符；没有删除活跃latest或复制父权重。训练继续按固定预算，预计终态留出约21:34 CST，正式成绩仍无更新。


## 已完成终态更新（20.197）

本轮empty-pool已完成并封存：bbs6138/5549→6154/5514（+16/-35），对同预算native终点+35/-23；独立审计通过，正式按固定规则跳过，Nr/Sr未启动。已删除唯一被替代的3584步latest约331MB，终点与全部保护权重保留。详见[终态报告](PVG_EMPTY_POOL_TERMINAL_2026-09-08.md)。不再等待或重启本实验；普通分源Query读取仍是后续未训练控制，复杂覆盖机制尚无正结果。
