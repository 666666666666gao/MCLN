...
from models.rec_hyperspherical_semantic_adapter import (
    AnchoredHypersphericalSemanticResidualAdapter,
...
def _adapter():
    return AnchoredHypersphericalSemanticResidualAdapter(
...
def test_v130_initial_logits_exactly_equal_the_frozen_anchor():
...
def test_v130_residual_is_bounded_and_padding_stays_zero():
...
def test_v130_recovers_unit_sphere_product_difference_and_cosine():
    inputs = _inputs()
    group = _statistics()["groups"]["query_features"]
...
def test_v130_is_query_permutation_equivariant():
    adapter = _adapter().eval()
    inputs = _inputs()
...
def test_v130_semantic_path_changes_context_without_anchor_gradients():
    adapter = _adapter().eval()
    inputs = _inputs()
...
def test_v130_gpu_semantic_path_optimizes_after_zero_delta_initialization():
