...
def main():
    ...
    def verify_inputs():
        ...
        assert file_sha(Path(manifest['gradient_regression_receipt'])) == manifest['gradient_regression_receipt_sha256']
    ...
    regression = json.loads(Path(manifest['gradient_regression_receipt']).read_text())
    assert regression['status'] == 'pass' and regression['optimizer_steps'] == 0
    assert manifest['shared_gradient_relative_l2_limit'] == 1e-4
    os.chdir(str(source))
    ...
    from scripts.nr3d_shared_gradient_check import shared_gradient_comparison
    ...
    for name, original in shared_gradients['native'].items():
        ...
        shared_differences[name] = shared_gradient_comparison(original, current)
    preflight['shared_gradient_comparisons'] = shared_differences
    preflight['shared_gpu_gradient_norms_exact'] = all(
        preflight['native']['gradient_norms'][name] == preflight['sparse']['gradient_norms'][name]
        for name in trainable['native'])
    # Preserve evidence before asserting, including a failed comparison.
    write_json(addon / 'first_batch_gradient_comparison.json', preflight)
    assert all(value['passed'] for value in shared_differences.values()), shared_differences
    ...
    preflight.update(optimizer_steps=0, trainable_parameters={
                     ...
                     native_warmup_forwards=0, shared_gradient_relative_l2_limit=1e-4)
    ...
    verify_inputs()
    receipt = {'schema': 'mcln-nr3d-sparse-point-pair-v3', 'status': 'complete', 'optimizer_steps_per_arm': step,
               ...
               'native_warmup_forwards': 0, 'shared_gradient_relative_l2_limit': 1e-4,
               'first_batch_gradient_comparison_sha256': file_sha(addon / 'first_batch_gradient_comparison.json'),
