...
def resolve_referit3d_csv(data_path, filename):
    """Resolve ReferIt3D CSVs across the layouts used by public releases."""
    candidates = [
        os.path.join(data_path, "ReferIt3D", filename),
        os.path.join(data_path, "refer_it_3d", filename),
    ]
    for path in candidates:
        if os.path.isfile(path):
            return path
    raise FileNotFoundError(
        "ReferIt3D CSV is missing; checked {}".format(
            ", ".join(candidates)
        )
    )
...
class Joint3DDataset(Dataset):
    ...
    def __init__(self, dataset_dict={'sr3d': 1, 'scannet': 10},
        ...
        for scan in self.scans:
            superpoint_path = os.path.join(
                f'{self.data_path}/superpoints',
                self.split,
                scan + "_superpoint.pth",
            )
            if not os.path.exists(superpoint_path):
                if self.skip_missing_superpoints:
                    missing_superpoints.append(scan)
                    continue
                raise FileNotFoundError(superpoint_path)
            superpoint = torch.load(superpoint_path)
            self.superpoints[scan] = superpoint
        if missing_superpoints:
            missing_set = set(missing_superpoints)
            self.scans = {
                scan: value for scan, value in self.scans.items()
                if scan not in missing_set
            }
            print(
                "Skipping {} scans without superpoints for split {}".format(
                    len(missing_superpoints), self.split
                )
            )
    ...
    def load_annos(self, dset):
        ...
        if self.skip_missing_superpoints:
            annos = [
                ...
                if anno.get('scan_id') in self.scans
            ]
        if self.overfit:
            annos = annos[:128]
        return annos
    ...
    def _literal_list(value):
        ...
        if not isinstance(parsed, list):
            raise ValueError("expected a list annotation")
        return parsed

    def _structured_referit3d_path(self, dset):
        if not self.use_sacr_source or dset.endswith('_spacy'):
            return resolve_referit3d_csv(self.data_path, '%s.csv' % dset)
        return resolve_referit3d_csv(
            self.data_path, '%s_spacy.csv' % dset
        )

    @staticmethod
    def _referit3d_structured_key(record, dset):
        if dset == 'nr3d':
            assignment_id = str(record.get('assignmentid', '')).strip()
            if assignment_id:
                return ('assignmentid', assignment_id)
        return (
            'referit3d',
            str(record.get('stimulus_id', '')),
            str(record.get('scan_id', '')),
            str(record.get('target_id', '')),
            str(record.get('utterance', '')),
        )

    def _referit3d_structured_lookup(self, dset, scan_ids):
        lookup = {}
        with open(self._structured_referit3d_path(dset)) as handle:
            for row in csv.DictReader(handle):
                if row['scan_id'] not in scan_ids:
                    continue
                key = self._referit3d_structured_key(row, dset)
                if key in lookup and lookup[key] != row:
                    raise ValueError(
                        "conflicting structured annotations for {}".format(
                            key
                        )
                    )
                lookup[key] = row
    ...
    def _structured_scanrefer_path(self, split):
        candidates = [
            os.path.join(
                self.data_path,
                'scanrefer',
                'ScanRefer_filtered_%s_spacy.json' % split,
            ),
            os.path.join(
                self.data_path,
                'ScanRefer',
        ...
        for path in candidates:
            if os.path.isfile(path):
                return path
        raise FileNotFoundError(
            "ScanRefer structured annotations are missing; checked {}"
            .format(', '.join(candidates))
        )

    @staticmethod
    def _attach_structured_annotation(anno, record):
        anno['_structured_source_utterance'] = anno.get('utterance', '')
        structured = build_structured_annotation(
            record, anno.get('utterance', '')
        )
        anno.update(structured)
        return anno

    @staticmethod
    def _realign_structured_annotations(annos):
        for anno in annos:
            source_text = anno.pop(
                '_structured_source_utterance', anno.get('utterance', '')
            )
            realign_structured_annotation(
    ...
    def load_sr3d_annos(self, dset='sr3d'):
        ...
        if self.use_sacr_source:
            return self._load_sr3d_structured_annos(dset)
        split = self.split
        if split == 'val':
            split = 'test'
        with open('data/meta_data/sr3d_%s_scans.txt' % split) as f:
            scan_ids = set(eval(f.read()))
        with open(resolve_referit3d_csv(self.data_path, '%s.csv' % dset)) as f:
    ...
    def _load_sr3d_structured_annos(self, dset):
        split = 'test' if self.split == 'val' else self.split
        with open('data/meta_data/sr3d_%s_scans.txt' % split) as handle:
            scan_ids = set(eval(handle.read()))
        structured_lookup = self._referit3d_structured_lookup(
            dset, scan_ids
        )
        annos = []
        with open(resolve_referit3d_csv(
                self.data_path, '%s.csv' % dset)) as handle:
            for row in csv.DictReader(handle):
                if row['scan_id'] not in scan_ids:
                    continue
                if str(row['mentions_target_class']).lower() != 'true':
                    continue
                anno = {
                    'scan_id': row['scan_id'],
                    'target_id': int(row['target_id']),
                    'distractor_ids': self._literal_list(
                        row['distractor_ids']
                    ),
                    'utterance': row['utterance'],
                    'target': row['instance_type'],
                    'anchors': self._literal_list(row['anchors_types']),
                    # These remain object ids for the legacy detection path.
                    'anchor_ids': self._literal_list(row['anchor_ids']),
                    'dataset': dset,
                }
                structured_record = structured_lookup.get(
                    self._referit3d_structured_key(row, dset), {}
                )
                self._attach_structured_annotation(anno, structured_record)
                anno['structured_annotation_hit'] = bool(structured_record)
                annos.append(anno)
        if self.overfit:
            annos = annos[:128]
        self._scene_graph_parse(annos)
    ...
    def load_nr3d_annos(self):
        ...
        if self.use_sacr_source:
            return self._load_nr3d_structured_annos()
        split = self.split
        if split == 'val':
            split = 'test'
        with open('data/meta_data/nr3d_%s_scans.txt' % split) as f:
            scan_ids = set(eval(f.read()))
        with open(resolve_referit3d_csv(self.data_path, 'nr3d.csv')) as f:
            csv_reader = csv.reader(f)
            headers = next(csv_reader)
            headers = {header: h for h, header in enumerate(headers)}
            annos = [
    ...
    def _load_nr3d_structured_annos(self):
        split = 'test' if self.split == 'val' else self.split
        with open('data/meta_data/nr3d_%s_scans.txt' % split) as handle:
            scan_ids = set(eval(handle.read()))
        structured_lookup = self._referit3d_structured_lookup(
            'nr3d', scan_ids
        )
        annos = []
        with open(resolve_referit3d_csv(
                self.data_path, 'nr3d.csv')) as handle:
            for row in csv.DictReader(handle):
                if row['scan_id'] not in scan_ids:
                    continue
                if (
                    split == 'test'
                    and str(row['correct_guess']).lower() != 'true'
                ):
                    continue
                anno = {
                    'scan_id': row['scan_id'],
                ...
                structured_record = structured_lookup.get(
                    self._referit3d_structured_key(row, 'nr3d'), {}
                )
                self._attach_structured_annotation(anno, structured_record)
                anno['structured_annotation_hit'] = bool(structured_record)
                annos.append(anno)
        ...
        for anno in annos:
            anno['distractor_ids'] = [
                index
                for index in range(
                    len(self.scans[anno['scan_id']].three_d_objects)
                )
                if self.scans[anno['scan_id']].get_object_instance_label(
                    index
                ) == anno['target']
                and index != anno['target_id']
            ]
        return annos
    ...
    def load_scanrefer_annos(self):
        ...
        scanrefer_dir = os.path.join(self.data_path, 'ScanRefer')
        if not os.path.isdir(scanrefer_dir):
            scanrefer_dir = os.path.join(self.data_path, 'scanrefer')
        _path = os.path.join(scanrefer_dir, 'ScanRefer_filtered')
        split = self.split
        ...
        if self.use_sacr_source:
            with open(self._structured_scanrefer_path(split)) as handle:
                structured_reader = json.load(handle)
            structured_lookup = build_scanrefer_structured_lookup(
                structured_reader
            )
            for anno in annos:
                key = scanrefer_annotation_key({
                    'scene_id': anno['scan_id'],
                    'object_id': anno['target_id'],
                    'ann_id': anno.get('ann_id', ''),
                })
                structured_record = structured_lookup.get(key, {})
                self._attach_structured_annotation(anno, structured_record)
                anno['structured_annotation_hit'] = bool(structured_record)
        if self.skip_missing_superpoints:
            annos = [
                anno for anno in annos
                if anno['scan_id'] in self.scans
            ]
    ...
    def __getitem__(self, index):
        ...
        # Benchmark identity used by the legacy language-loss recipe.  This is
        # intentionally distinct from the annotation source in joint batches.
        language_dataset = self.test_dataset

        # step Read annotation and point clouds
        anno = self.annos[index]
        sample_dataset = anno.get('dataset')
        if not isinstance(sample_dataset, str) or not sample_dataset:
            raise ValueError("annotation dataset must be a non-empty string")
        ...
        if self.use_sacr_source:
            structured_anchor_ids = list(
                anno.get('structured_anchor_ids', [])
            )[:32]
            structured_anchor_ids += [-1] * (
                32 - len(structured_anchor_ids)
            )
            ret_dict.update({
                'target_spans': anno.get('target_spans', []),
                'entity_spans': anno.get('entity_spans', []),
                'attr_spans': anno.get('attr_spans', []),
                'rel_spans': anno.get('rel_spans', []),
                'structured_anchor_ids': np.asarray(
                    structured_anchor_ids, dtype=np.int64
                ),
                'coverage_stats': anno.get('coverage_stats', {}),
                'parse_confidence': float(
                    anno.get('parse_confidence', 0.0)
                ),
                'decomposition_status': anno.get(
                    'decomposition_status',
                    'global_only_target_unresolved',
                ),
                'decomp_global_only_mask': bool(
                    anno.get('decomp_global_only_mask', True)
                ),
                'decomp_weak_generic_mask': bool(
                    anno.get('decomp_weak_generic_mask', False)
                ),
                'structured_annotation_available': bool(
                    anno.get('structured_annotation_available', False)
                ),
            })
