#!/usr/bin/env python
"""Nested OOF screen of V128 V115-protected listwise rescue."""
...
import models.rec_semantic_antisymmetric_utility as v128_model
...
V128_CRITIC_EPOCHS = 50
V128_ROW_BATCH_SIZE = 512
V128_INFERENCE_BATCH_SIZE = 1024
V128_CRITIC_LEARNING_RATE = 1e-3
V128_CRITIC_WEIGHT_DECAY = 1e-3
...
def predict_pair_components(model, records, statistics, device):
    ...
    with torch.no_grad():
        for start in range(0, len(records), HIERARCHICAL_BATCH_SIZE):
            ...
            model_hit_probability.append(
                probabilities.reshape(
                    len(row_batch), QUERY_COUNT, VARIANT_COUNT, 2
                ).cpu().float()
            )
    ...
    return {
        ...
        "model_hit_probability": torch.cat(
            model_hit_probability
        ).contiguous(),
        "head_gain": head_gain,
...
def _pair_inputs(
        components, rows, proposal_query, proposal_variant,
        baseline_query, baseline_variant, model_gain):
    count = len(rows)
    inputs = {
        "proposal_query_embedding": components[
            "query_embedding"
        ][rows, proposal_query],
        "proposal_variant_embedding": components[
            "variant_embedding"
        ][rows, proposal_query, proposal_variant],
        "baseline_query_embedding": components[
            "query_embedding"
        ][rows, baseline_query],
        "baseline_variant_embedding": components[
            "variant_embedding"
        ][rows, baseline_query, baseline_variant],
        "target_text": components["target_text"][rows],
        "proposal_aux": components[
            "candidate_aux"
        ][rows, proposal_query, proposal_variant],
        "baseline_aux": components[
            "candidate_aux"
        ][rows, baseline_query, baseline_variant],
        "model_gain": model_gain,
    }
    if count <= 0:
        raise ValueError("V128 pair population must be nonempty")
    for value in inputs.values():
        if (value.dtype != torch.float32 or value.device.type != "cpu"
                or value.shape[0] != count
                or not bool(torch.isfinite(value).all().item())):
            raise ValueError("V128 pair input layout changed")
    return {key: value.contiguous() for key, value in inputs.items()}


def build_listwise_training_population(prediction, records):
    """Build full candidate sets and fix-only positive actions per row."""
    components = prediction["semantic_components"]
    count = len(records)
    _validate_semantic_components(components, count)
    probabilities = prediction["model_hit_probability"]
    baselines = prediction["baselines"]
    if (probabilities.dtype != torch.float32
            or tuple(probabilities.shape)
            != (count, QUERY_COUNT, VARIANT_COUNT, 2)
            or baselines.dtype != torch.long
            or tuple(baselines.shape) != (count,)
            or not bool(torch.isfinite(probabilities).all().item())):
        raise ValueError("V128 model probability layout changed")
    rows = torch.arange(count)
    baseline_probability = gather_flat_candidates(
        probabilities, baselines
    )
    gains = probabilities - baseline_probability[:, None, None, :]
    aggregate_gain = 2.0 * gains[..., 0] + gains[..., 1]
    flat_candidates = torch.arange(
        QUERY_COUNT * VARIANT_COUNT
    ).reshape(1, QUERY_COUNT, VARIANT_COUNT)
    eligible = (
        components["candidate_valid"]
        & flat_candidates.ne(baselines[:, None, None])
        & aggregate_gain.ge(v99.V97_MARGIN)
        & gains[..., 0].gt(0.0)
        & gains[..., 1].gt(0.0)
    )
    ious = torch.stack([
    ...
    if (tuple(ious.shape) != (count, QUERY_COUNT, VARIANT_COUNT)
            or not bool(torch.isfinite(ious).all().item())):
        raise ValueError("V128 candidate IoU layout changed")
    baseline_iou = ious.reshape(count, -1)[rows, baselines]
    candidate_hit = torch.stack((
        ious.gt(0.25), ious.gt(0.50)
    ), dim=-1)
    baseline_hit = torch.stack((
        baseline_iou.gt(0.25), baseline_iou.gt(0.50)
    ), dim=-1)
    positive = (
        eligible[..., None]
        & candidate_hit
        & ~baseline_hit[:, None, None, :]
    )
    pair_rows, proposal_query, proposal_variant = eligible.nonzero(
        as_tuple=True
    )
    pair_baselines = baselines[pair_rows]
    baseline_query = pair_baselines.div(
        VARIANT_COUNT, rounding_mode="floor"
    )
    baseline_variant = pair_baselines.remainder(VARIANT_COUNT)
    inputs = _pair_inputs(
        components,
        pair_rows,
        proposal_query,
        ...
        baseline_query,
        baseline_variant,
        gains[pair_rows, proposal_query, proposal_variant],
    )
    pair_positive = positive[
        pair_rows, proposal_query, proposal_variant
    ].contiguous()
    pair_flat = (
        proposal_query * VARIANT_COUNT + proposal_variant
    ).contiguous()
    positive_rows = positive.reshape(count, -1, 2).any(dim=1)
    positive_actions = positive.reshape(count, -1, 2).sum(dim=(0, 1))
    if (not bool(positive_rows.any(dim=0).all().item())
            or not bool((~positive_rows).any(dim=0).all().item())):
        raise ValueError("V128 listwise population lacks action diversity")
    return inputs, pair_rows, pair_flat, pair_positive, {
        "row_count": count,
        "eligible_pair_count": int(eligible.sum().item()),
        "rows_with_candidates": int(
            eligible.reshape(count, -1).any(dim=1).sum().item()
        ),
        "candidate_positive_actions": {
            "025": int(positive_actions[0].item()),
            "050": int(positive_actions[1].item()),
        },
        "candidate_positive_rows": {
            "025": int(positive_rows[:, 0].sum().item()),
            "050": int(positive_rows[:, 1].sum().item()),
        },
...
def listwise_abstain_loss(candidate_logits, candidate_positive):
    """Negative log acceptable-action mass with baseline score fixed at zero."""
    if (candidate_logits.dtype != torch.float32
            or candidate_logits.dim() != 3
            or candidate_logits.shape[-1] != 2
            or candidate_positive.dtype != torch.bool
            or tuple(candidate_positive.shape) != tuple(candidate_logits.shape)
            or bool(torch.isnan(candidate_logits).any().item())
            or bool(torch.isposinf(candidate_logits).any().item())
            or bool(
                (candidate_positive & ~torch.isfinite(candidate_logits))
                .any().item()
            )):
        raise ValueError("V128 listwise loss tensors changed")
    baseline_score = torch.zeros(
        candidate_logits.shape[0], 1, 2,
        dtype=torch.float32, device=candidate_logits.device,
    )
    denominator = torch.logsumexp(
        torch.cat((candidate_logits, baseline_score), dim=1), dim=1
    )
    positive_logits = candidate_logits.masked_fill(
        ~candidate_positive, -torch.inf
    )
    baseline_target = ~candidate_positive.any(dim=1)
    baseline_positive = baseline_score.masked_fill(
        ~baseline_target[:, None, :], -torch.inf
    )
    numerator = torch.logsumexp(
        torch.cat((positive_logits, baseline_positive), dim=1), dim=1
    )
    loss = (denominator - numerator).mean()
    if not bool(torch.isfinite(loss).item()):
        raise RuntimeError("V128 listwise abstain loss became non-finite")
    return loss, baseline_target


def fit_semantic_listwise_critic(prediction, records, device):
    """Fit shared utilities with row-level candidate sets and abstention."""
    inputs, pair_rows, pair_flat, pair_positive, population = (
        build_listwise_training_population(prediction, records)
    )
    pair_count = pair_rows.shape[0]
    if pair_count < 100:
        raise ValueError("V128 listwise candidate population is too small")
    row_count = len(records)
    resolved = torch.device(device)
    _set_hierarchical_seed(resolved)
    model = v128_model.SemanticAntisymmetricUtilityRanker().to(resolved)
    optimizer = torch.optim.AdamW(
        model.parameters(),
        lr=V128_CRITIC_LEARNING_RATE,
        weight_decay=V128_CRITIC_WEIGHT_DECAY,
    )
    shuffle_generator = torch.Generator(device="cpu")
    shuffle_generator.manual_seed(0)
    epoch_receipts = []
    for epoch in range(V128_CRITIC_EPOCHS):
        model.train()
        order = torch.randperm(row_count, generator=shuffle_generator)
        total_loss = 0.0
        total_rows = 0
        for start in range(0, row_count, V128_ROW_BATCH_SIZE):
            batch_rows = order[start:start + V128_ROW_BATCH_SIZE]
            row_lookup = torch.full((row_count,), -1, dtype=torch.long)
            row_lookup[batch_rows] = torch.arange(batch_rows.numel())
            indices = row_lookup[pair_rows].ge(0).nonzero(
                as_tuple=False
            ).flatten()
            if indices.numel() <= 0:
                continue
            local_rows = row_lookup[pair_rows[indices]].to(resolved)
            local_flat = pair_flat[indices].to(resolved)
            batch_inputs = {
                key: value[indices].to(resolved)
                for key, value in inputs.items()
            }
            logits = model(**batch_inputs)
            dense_logits = torch.full(
                (batch_rows.numel(), QUERY_COUNT * VARIANT_COUNT, 2),
                -torch.inf, dtype=torch.float32, device=resolved,
            )
            dense_positive = torch.zeros_like(
                dense_logits, dtype=torch.bool
            )
            dense_logits[local_rows, local_flat] = logits
            dense_positive[local_rows, local_flat] = pair_positive[
                indices
            ].to(resolved)
            loss, _ = listwise_abstain_loss(
                dense_logits, dense_positive
            )
            optimizer.zero_grad(set_to_none=True)
            loss.backward()
            clip_grad_norm_(model.parameters(), 1.0)
            optimizer.step()
            total_loss += float(loss.detach().item()) * batch_rows.numel()
            total_rows += batch_rows.numel()
        epoch_receipts.append({
            "epoch": epoch + 1,
            "loss": total_loss / total_rows,
        })
    model.eval().requires_grad_(False)
    receipt = dict(population)
    receipt.update({
        "epochs": V128_CRITIC_EPOCHS,
        "row_batch_size": V128_ROW_BATCH_SIZE,
        "baseline_score": 0.0,
        "epoch_receipts": epoch_receipts,
        "state_sha256": v99.tensor_sha256(*[
            value for _, value in sorted(model.state_dict().items())
...
def selected_pair_inputs(prediction):
    """Build deployable proposal-vs-baseline inputs without IoU labels."""
    components = prediction["semantic_components"]
    count = prediction["proposals"].shape[0]
    _validate_semantic_components(components, count)
    proposals = prediction["proposals"]
    baselines = prediction["baselines"]
    rows = torch.arange(count)
    proposal_query = proposals.div(
        VARIANT_COUNT, rounding_mode="floor"
    )
    proposal_variant = proposals.remainder(VARIANT_COUNT)
    baseline_query = baselines.div(
        VARIANT_COUNT, rounding_mode="floor"
    )
    baseline_variant = baselines.remainder(VARIANT_COUNT)
    return _pair_inputs(
        components,
        rows,
        proposal_query,
        ...
        baseline_query,
        baseline_variant,
        prediction["head_gain"],
    )


def all_hard_candidate_inputs(prediction):
    """Materialize every deployable V115-Pareto candidate against baseline."""
    components = prediction["semantic_components"]
    probabilities = prediction["model_hit_probability"]
    baselines = prediction["baselines"]
    count = baselines.shape[0]
    _validate_semantic_components(components, count)
    if (probabilities.dtype != torch.float32
            or tuple(probabilities.shape)
            != (count, QUERY_COUNT, VARIANT_COUNT, 2)
            ...
            or tuple(baselines.shape) != (count,)
            or not bool(torch.isfinite(probabilities).all().item())):
        raise ValueError("V128 all-candidate probability layout changed")
    baseline_probability = gather_flat_candidates(probabilities, baselines)
    gains = probabilities - baseline_probability[:, None, None, :]
    aggregate_gain = 2.0 * gains[..., 0] + gains[..., 1]
    flat_candidates = torch.arange(
        QUERY_COUNT * VARIANT_COUNT
    ...
    if pair_rows.numel() <= 0:
        raise ValueError("V128 all-candidate population must be nonempty")
    pair_baselines = baselines[pair_rows]
    baseline_query = pair_baselines.div(
        VARIANT_COUNT, rounding_mode="floor"
    )
...
def predict_pairwise_utility(model, inputs, device):
    count = inputs["model_gain"].shape[0]
    resolved = torch.device(device)
    model.to(resolved).eval()
    logits = []
    with torch.no_grad():
        for start in range(0, count, V128_INFERENCE_BATCH_SIZE):
            stop = min(start + V128_INFERENCE_BATCH_SIZE, count)
            logits.append(model(**{
                key: value[start:stop].to(resolved)
                for key, value in inputs.items()
            }).cpu().float())
    output = torch.cat(logits)
    if (tuple(output.shape) != (count, 2)
            or not bool(torch.isfinite(output).all().item())):
        raise RuntimeError("V128 utility logits changed")
    return output


def select_all_hard_candidates(model, prediction, device):
    """Argmax REC@.25 utility over all eligible candidates, then abstain."""
    population = all_hard_candidate_inputs(prediction)
    pair_logits = predict_pairwise_utility(
        model, population["inputs"], device
    )
    baselines = prediction["baselines"]
    count = baselines.shape[0]
    utility = torch.full(
    ...
    utility[
        population["pair_rows"],
        population["proposal_query"],
        population["proposal_variant"],
    ] = pair_logits
    flat_utility = utility.reshape(count, -1, 2)
    has_candidate = population["eligible"].reshape(count, -1).any(dim=1)
    best_candidate = flat_utility[:, :, 0].argmax(dim=1)
    proposals = torch.where(has_candidate, best_candidate, baselines)
    rows = torch.arange(count)
    selected_logits = flat_utility[rows, proposals].clone()
    selected_logits[~has_candidate] = 0.0
    flat_gains = population["gains"].reshape(count, -1, 2)
    head_gain = flat_gains[rows, proposals].clone()
    head_gain[~has_candidate] = 0.0
    aggregate_gain = 2.0 * head_gain[:, 0] + head_gain[:, 1]
    accepted = has_candidate & selected_logits[:, 0].ge(0.0)
    selected = torch.where(accepted, proposals, baselines)
    eligible_counts = population["eligible"].reshape(count, -1).sum(dim=1)
    if (proposals.dtype != torch.long
            or selected.dtype != torch.long
            or accepted.dtype != torch.bool
            or selected_logits.dtype != torch.float32
            or tuple(selected_logits.shape) != (count, 2)
            or tuple(head_gain.shape) != (count, 2)
            or not bool(torch.isfinite(selected_logits).all().item())
            or not bool(torch.isfinite(head_gain).all().item())
            or not bool(accepted.le(has_candidate).all().item())):
        raise RuntimeError("V128 all-candidate selection contract changed")
    if bool((accepted & proposals.eq(baselines)).any().item()):
        raise RuntimeError("V128 accepted a baseline as a candidate")
    return {
        "proposals": proposals,
        "selected": selected,
        "accepted": accepted,
        "candidate_available": has_candidate,
        "utility_logits": selected_logits,
        "head_gain": head_gain,
        "aggregate_gain": aggregate_gain,
        "eligible_counts": eligible_counts,
        "pair_count": int(population["pair_rows"].numel()),
    }


def select_v115_protected_rescue(model, prediction, device):
    """Keep every V115 switch and add listwise choices only on abstentions."""
    rescue = select_all_hard_candidates(model, prediction, device)
    base_accepted = prediction["base_accepted"]
    if (base_accepted.dtype != torch.bool
            or tuple(base_accepted.shape) != tuple(rescue["accepted"].shape)):
        raise ValueError("V128 V115 acceptance layout changed")
    rescue_accepted = ~base_accepted & rescue["accepted"]
    accepted = base_accepted | rescue_accepted
    proposals = torch.where(
        base_accepted, prediction["proposals"], rescue["proposals"]
    )
    selected = torch.where(
        base_accepted, prediction["proposals"], rescue["selected"]
    )
    head_gain = torch.where(
        base_accepted[:, None], prediction["head_gain"], rescue["head_gain"]
    )
    aggregate_gain = torch.where(
        base_accepted, prediction["aggregate_gain"], rescue["aggregate_gain"]
    )
    if (not torch.equal(selected[base_accepted],
                        prediction["proposals"][base_accepted])
            or not torch.equal(accepted, selected.ne(prediction["baselines"]))
            or bool((rescue_accepted & base_accepted).any().item())):
        raise RuntimeError("V128 protected-rescue contract changed")
    output = dict(rescue)
    output.update({
        "proposals": proposals,
        "selected": selected,
        ...
        "rescue_raw_accepted": rescue["accepted"],
    })
    return output


def pairwise_utility_accept_mask(base_accepted, utility_logits):
    if (base_accepted.dtype != torch.bool or base_accepted.dim() != 1
            or utility_logits.dtype != torch.float32
            or tuple(utility_logits.shape) != (base_accepted.shape[0], 2)
            or not bool(torch.isfinite(utility_logits).all().item())):
        raise ValueError("V128 utility gate tensors changed")
    return base_accepted & utility_logits[:, 0].ge(0.0)
...
def main(argv=None):
    ...
    parser.add_argument("--v121-report", required=True)
    parser.add_argument("--v122-report", required=True)
    parser.add_argument("--v123-report", required=True)
    parser.add_argument("--v124-report", required=True)
    parser.add_argument("--v125-report", required=True)
    parser.add_argument("--v126-report", required=True)
    parser.add_argument("--v127-report", required=True)
    ...
    for name, path_value, expected_sha256, expected_schema, expected_delta in (
            ...
            (
                "v121", args.v121_report, V121_REPORT_SHA256,
                "rec-v121-nested-semantic-candidate-critic-scene-oof-v1",
                (72, 185),
            ),
            (
                "v122", args.v122_report, V122_REPORT_SHA256,
                "rec-v122-nested-semantic-pairwise-utility-scene-oof-v1",
                (57, 137),
            ),
            (
                "v123", args.v123_report, V123_REPORT_SHA256,
                "rec-v123-nested-semantic-antisymmetric-utility-scene-oof-v1",
                (74, 179),
            ),
            (
                "v124", args.v124_report, V124_REPORT_SHA256,
                "rec-v124-nested-semantic-learned-pareto-scene-oof-v1",
                (65, 156),
            ),
            (
                "v125", args.v125_report, V125_REPORT_SHA256,
                "rec-v125-nested-semantic-all-hard-candidate-scene-oof-v1",
                (31, 99),
            ),
            (
                "v126", args.v126_report, V126_REPORT_SHA256,
                "rec-v126-nested-semantic-listwise-abstain-scene-oof-v1",
                (24, 17),
            ),
            (
                "v127", args.v127_report, V127_REPORT_SHA256,
                "rec-v127-nested-semantic-prior-corrected-listwise-scene-oof-v1",
                (21, 21),
            )):
    ...
    rescue_accepted = torch.zeros(EXPECTED_SAMPLE_COUNT, dtype=torch.bool)
    base_accepted = torch.zeros(EXPECTED_SAMPLE_COUNT, dtype=torch.bool)
    candidate_available = torch.zeros(
        EXPECTED_SAMPLE_COUNT, dtype=torch.bool
    )
    eligible_counts = torch.zeros(EXPECTED_SAMPLE_COUNT, dtype=torch.long)
    v115_proposals = baselines.clone()
    utility_logits = torch.zeros(
    ...
    for held in range(5):
        ...
        if (set(records[index]["scan_id"] for index in inner_fit_indices)
            ...
            raise RuntimeError("scene leakage into V128 inner calibration")
        ...
        critic_model, critic_training = fit_semantic_listwise_critic(
            inner_prediction,
            inner_records,
            args.device,
        )
        ...
        if not torch.equal(prediction["baselines"], baselines[held_indices]):
            raise RuntimeError("V128 OOF baseline identity changed")
        fold_selection = select_v115_protected_rescue(
            critic_model, prediction, args.device
        )
        proposals[held_indices] = fold_selection["proposals"]
        selected[held_indices] = fold_selection["selected"]
        aggregate_gain[held_indices] = fold_selection["aggregate_gain"]
        head_gain[held_indices] = fold_selection["head_gain"]
        base_accepted[held_indices] = prediction["base_accepted"]
        candidate_available[held_indices] = fold_selection[
            "candidate_available"
        ]
        eligible_counts[held_indices] = fold_selection["eligible_counts"]
        v115_proposals[held_indices] = prediction["proposals"]
        accepted[held_indices] = fold_selection["accepted"]
        rescue_accepted[held_indices] = fold_selection["rescue_accepted"]
        utility_logits[held_indices] = fold_selection["utility_logits"]
        fold_record = {
            "held_fold": held,
            ...
            "all_hard_candidate_pairs": fold_selection["pair_count"],
            "rows_with_hard_candidates": int(
                fold_selection["candidate_available"].sum().item()
            ),
            "total_accepted_switches": int(
                fold_selection["accepted"].sum().item()
            ),
            "listwise_rescue_switches": int(
                fold_selection["rescue_accepted"].sum().item()
            ),
            "listwise_raw_accepted_switches": int(
                fold_selection["rescue_raw_accepted"].sum().item()
            ),
            "changed_vs_v115_proposal": int(
                (
                    fold_selection["candidate_available"]
                    & fold_selection["proposals"].ne(prediction["proposals"])
                ).sum().item()
            ),
        ...
        del model, critic_model, prediction
        torch.cuda.empty_cache()
    ...
    utility_veto = candidate_available & ~accepted
    veto_reasons = {
        "v115_fixed_proposal_switches": int(base_accepted.sum().item()),
        "listwise_rescue_switches": int(rescue_accepted.sum().item()),
        "rows_with_hard_candidates": int(candidate_available.sum().item()),
        "all_hard_candidate_pairs": int(eligible_counts.sum().item()),
        "accepted_switches": int(accepted.sum().item()),
        "utility_abstentions": int(utility_veto.sum().item()),
        "expanded_beyond_v115_rows": int(
            (candidate_available & ~base_accepted).sum().item()
        ),
        "changed_vs_v115_proposal": int(
            (candidate_available & proposals.ne(v115_proposals)).sum().item()
        ),
        "selected_margin_violations": int(
            (raw_switch & ~original_margin_pass).sum().item()
        ),
        ...
        "v115_protected_identity_violations": int(
            (
                base_accepted
                & selected.ne(v115_proposals)
            ).sum().item()
        ),
    }
    if (veto_reasons["selected_margin_violations"] != 0
            or veto_reasons["nonpositive_delta025"] != 0
            or veto_reasons["nonpositive_delta050"] != 0
            or veto_reasons["v115_protected_identity_violations"] != 0
            or not bool(rescue_accepted.le(~base_accepted).all().item())
            or not bool(accepted.le(candidate_available).all().item())):
        raise RuntimeError("V128 protected-rescue Pareto contract changed")
    protected_after = capture_immutable_artifact_identities(protected_paths)
    if protected_after != protected_before:
        raise RuntimeError("protected artifacts changed during V128")
    protected_metadata_after = {
        name: file_sha256(path)
        for name, path in protected_metadata_paths.items()
    }
    if protected_metadata_after != protected_metadata_before:
        raise RuntimeError("protected V128 cache metadata changed during OOF")
    script_sources = {
        "v128": str(Path(__file__).resolve()),
        ...
        "v128_model": str(Path(v128_model.__file__).resolve()),
        "v108": str(
    ...
    report = {
        "schema": "rec-v128-v115-protected-listwise-rescue-scene-oof-v1",
        "version": 1,
        "validation_data_accessed": False,
        ...
        "protocol": {
            "architecture": (
                ...
                "shared low-capacity semantic utility scorer with strict "
                "candidate-minus-baseline antisymmetry; row-level training "
                "normalizes all deployable hard candidates with an explicit "
                "zero-score baseline abstain action; V115 accepted actions "
                "are immutable and listwise actions are rescue-only"
            ),
            "adapter_objective": "frozen V95 bounded threshold-aligned listwise",
            "critic_objective": (
                "dual-threshold negative log acceptable-action probability; "
                "fixing candidates are acceptable only when baseline misses, "
                "otherwise only the baseline abstain action is acceptable"
            ),
            "proposal_margin": v99.V97_MARGIN,
            "acceptance": (
                "keep every V115 accepted proposal; only on V115 abstentions, "
                "argmax REC@.25 utility across all hard candidates and add "
                "the rescue iff its maximum is nonnegative"
            ),
            "protected_primary_policy": "V115 proposal and acceptance exact",
            "rescue_scope": "V115 abstention rows only",
            "candidate_scope": (
                "all valid nonbaseline candidates with aggregate gain at "
                "least the frozen V99 margin and positive gains at both heads"
            ),
            "candidate_ranking": "maximum_antisymmetric_utility_logit025",
            "candidate_tie_break": "lowest_flat_candidate_index",
            "baseline_abstain_score": 0.0,
            "listwise_positive_set": (
                "all eligible fix actions per row and threshold; baseline "
                "alone when no eligible fix exists"
            ),
            "grid_search": False,
            "selection_data": (
            ...
            "critic_candidate_dim": v128_model.V123_CANDIDATE_DIM,
            "critic_pair_structure": "shared_score_difference",
            "critic_model_gain_scale": "learned_positive_per_threshold",
            "critic_epochs": V128_CRITIC_EPOCHS,
            "critic_row_batch_size": V128_ROW_BATCH_SIZE,
            "critic_inference_batch_size": V128_INFERENCE_BATCH_SIZE,
            "critic_learning_rate": V128_CRITIC_LEARNING_RATE,
            "critic_weight_decay": V128_CRITIC_WEIGHT_DECAY,
            "critic_class_weights": None,
            "critic_neutral_candidates_are_negative_actions": True,
            "critic_acceptance": (
                "v115_primary_union_abstention_only_listwise_rescue"
            ),
        ...
        "prediction_sha256": v99.tensor_sha256(
            ...
            base_accepted, candidate_available, eligible_counts,
            v115_proposals, utility_logits, accepted, rescue_accepted
        ),
        "oof": {
    ...
    try:
        ...
        while written < len(payload):
            ...
            if count <= 0:
                raise OSError("V128 output write made no progress")
            written += count
        os.fsync(descriptor)
    finally:
        os.close(descriptor)
