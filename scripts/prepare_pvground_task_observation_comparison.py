...
root='/root/autodl-tmp/mcln_pvground_task_observation_comparison_20260917_v1'
archive=repo/'refine-logs/pvground_task_observation_comparison_20260917_v1'
candidate='/root/autodl-tmp/mcln_pvground_scanrefer_finetune_20260917_task_observation_v1'
...
controls={'A':'/root/autodl-tmp/mcln_pvground_scanrefer_finetune_20260908_vsaorder_v1',
          ...
          'C':'/root/autodl-tmp/mcln_pvground_scanrefer_finetune_20260909_observation_v1'}
...
files={'compare.py':(repo/'scripts/compare_pvground_task_observation_control.py').read_bytes()}
for label,control in controls.items():
    assert read(control+'/controller.exit').strip()==b'0'
    configs=[read(p+'/spec.json') for p in [control,candidate]]
    pair=dict(native_root=control,candidate_root=candidate,control_source_query=label in ['B','C'],control_observation_state=label=='C',
    ...
    files[label+'_D.json']=(json.dumps(pair,indent=2)+'\n').encode()
...
for stage,filename in [('initial','initial_audit.json'),('terminal','audit.json')]:
    ...
    for label in ['A','B','C']:
        prefix=stage+'_'+label+'_D'
        cmd=['/root/miniconda3/envs/bdetr/bin/python','-u',str(root/'compare.py'),'--spec',str(root/(label+'_D.json')),'--stage',stage,'--output',str(root/(prefix+'.json'))]
...
cmd=['screen','-dmS','mcln_pvg_task_observation_comparison_v1','/root/miniconda3/envs/bdetr/bin/python','-u',root+'/controller.py']
