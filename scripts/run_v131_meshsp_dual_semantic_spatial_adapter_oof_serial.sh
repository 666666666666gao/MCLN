...
OUTPUT=$BASE/v131_meshsp_dual_semantic_spatial_adapter_oof.json
LOG=$BASE/v131_meshsp_dual_semantic_spatial_adapter_oof.log
EXIT=$BASE/v131_meshsp_dual_semantic_spatial_adapter_oof.exit
SESSION=v131_dual_semantic_spatial_adapter_oof
SELF=$(readlink -f "$0")
...
if [[ "${1:-}" == "--inner" ]]; then
    ...
    env CUDA_VISIBLE_DEVICES=0 \
        ...
        "$PYTHON" scripts/run_v131_meshsp_dual_semantic_spatial_adapter_oof.py \
...
cd "$REPO"
require_sha 0899c3b3f9df3bc7ae6ebf2a0264e56173fae5bc9722da28cd83ea31093083d9 \
    scripts/run_v131_meshsp_dual_semantic_spatial_adapter_oof.py
require_sha d33007979fa77930b3364cd0fe9fee36fcf6cf528497c8701578e67b773118d5 \
    models/rec_dual_semantic_spatial_adapter.py
require_sha a46b3a44ab3bcd7eb90b3d1001edaeec55c4f281fdcbe84e6e10355e92d0cb7b \
    tests/test_rec_dual_semantic_spatial_adapter.py
require_sha d1db38703b39733fddecec5e12f3e2813978dcc41dc9dd7cd7d263980cab19ff \
    models/rec_hyperspherical_semantic_adapter.py
...
for path in "$OUTPUT" "$LOG" "$EXIT"; do
    if [[ -e "$path" ]]; then
        echo "Refusing to overwrite existing V131 output: $path" >&2
        exit 74
    fi
done
if [[ $(nvidia-smi -L | wc -l) -ne 1 ]]; then
    echo "V131 requires the frozen single-GPU topology" >&2
    exit 75
fi
...
if pgrep -af '[r]un_v131_meshsp_dual_semantic_spatial_adapter_oof.py' >/dev/null; then
    echo "V131 process already exists" >&2
    exit 75
fi
if screen -ls 2>/dev/null | grep -q "[.]$SESSION"; then
    echo "V131 screen session already exists" >&2
    exit 75
fi
available_kb=$(df -Pk /root/autodl-tmp | awk 'NR==2 {print $4}')
if [[ "$available_kb" -lt 2621440 ]]; then
    echo "V131 requires at least 2.5 GiB free on /root/autodl-tmp" >&2
    exit 75
fi
...
if ! screen -ls 2>/dev/null | grep -q "[.]$SESSION" \
        && [[ ! -e "$EXIT" ]]; then
    echo "V131 screen failed to remain alive and produced no exit receipt" >&2
    exit 75
fi
