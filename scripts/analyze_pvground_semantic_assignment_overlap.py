"""Attribute the completed D/G selected boxes to verified scene geometry on CPU."""
import argparse
import datetime
import gzip
import hashlib
...
def analyze(output):
    ...
    locations = {
        'G_initial': base/'mcln_pvground_scanrefer_finetune_20260918_semantic_assignment_v1/initial',
        'D_terminal': base/'mcln_pvground_scanrefer_finetune_20260917_task_observation_v1/terminal',
        'G_terminal': base/'mcln_pvground_scanrefer_finetune_20260918_semantic_assignment_v1/terminal',
    }
    pair_root = base/'mcln_pvground_semantic_assignment_comparison_20260918_v1'
    pair_result = json.loads((pair_root/'summary.json').read_bytes())['comparison']
    assert pair_result['status'] == 'complete' and pair_result['training_row_order_identical']
    inputs[str(pair_root/'summary.json')] = sha(pair_root/'summary.json')
    ...
    comparisons = {}
    for before, after in [('G_initial', 'G_terminal'),
                          ('D_terminal', 'G_terminal')]:
    ...
    for threshold, suffix in [('0.25', '025'), ('0.5', '050')]:
        assert metrics['G_terminal']['bbs'][suffix]['hits'] - metrics['D_terminal']['bbs'][suffix]['hits'] == pair_result['transitions']['bbs'][threshold]['net']
    ...
    result = dict(status='complete', time_cst=datetime.datetime.now(datetime.timezone(datetime.timedelta(hours=8))).isoformat(),
        rows_per_snapshot=6887, scenes=len({r['scan_id'] for r in source_rows['G_initial']}),
