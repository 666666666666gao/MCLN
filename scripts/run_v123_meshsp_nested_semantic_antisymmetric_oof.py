#!/usr/bin/env python
"""Nested scene-OOF screen of the V123 antisymmetric utility ranker."""
...
import models.rec_semantic_antisymmetric_utility as v123_model
...
V123_CRITIC_EPOCHS = 50
V123_CRITIC_BATCH_SIZE = 1024
V123_CRITIC_LEARNING_RATE = 1e-3
V123_CRITIC_WEIGHT_DECAY = 1e-3
...
def _pair_inputs(
    ...
    if count <= 0:
        raise ValueError("V123 pair population must be nonempty")
    for value in inputs.values():
        if (value.dtype != torch.float32 or value.device.type != "cpu"
                or value.shape[0] != count
                or not bool(torch.isfinite(value).all().item())):
            raise ValueError("V123 pair input layout changed")
...
def build_hard_pair_training_population(prediction, records):
    ...
    if (probabilities.dtype != torch.float32
        ...
        raise ValueError("V123 model probability layout changed")
    rows = torch.arange(count)
    ...
    if (tuple(ious.shape) != (count, QUERY_COUNT, VARIANT_COUNT)
            or not bool(torch.isfinite(ious).all().item())):
        raise ValueError("V123 candidate IoU layout changed")
    ...
    for column, suffix in enumerate(("025", "050")):
        ...
        if fixes <= 0 or breaks <= 0:
            raise ValueError("V123 hard pairs lack fix/break events")
        event_counts[suffix] = {
            "fix": fixes,
            "break": breaks,
...
def fit_semantic_pairwise_critic(prediction, records, device):
    ...
    if pair_count < 100:
        raise ValueError("V123 pairwise event population is too small")
    resolved = torch.device(device)
    _set_hierarchical_seed(resolved)
    model = v123_model.SemanticAntisymmetricUtilityRanker().to(resolved)
    optimizer = torch.optim.AdamW(
        model.parameters(),
        lr=V123_CRITIC_LEARNING_RATE,
        weight_decay=V123_CRITIC_WEIGHT_DECAY,
    )
    shuffle_generator = torch.Generator(device="cpu")
    shuffle_generator.manual_seed(0)
    epoch_receipts = []
    for epoch in range(V123_CRITIC_EPOCHS):
        model.train()
        order = torch.randperm(pair_count, generator=shuffle_generator)
        total_loss = 0.0
        total_events = 0
        for start in range(0, pair_count, V123_CRITIC_BATCH_SIZE):
            indices = order[start:start + V123_CRITIC_BATCH_SIZE]
            batch_inputs = {
    ...
    receipt.update({
        "epochs": V123_CRITIC_EPOCHS,
        "batch_size": V123_CRITIC_BATCH_SIZE,
        "epoch_receipts": epoch_receipts,
        "state_sha256": v99.tensor_sha256(*[
            value for _, value in sorted(model.state_dict().items())
        ]),
...
def predict_pairwise_utility(model, inputs, device):
    ...
    logits = []
    with torch.no_grad():
        for start in range(0, count, V123_CRITIC_BATCH_SIZE):
            stop = min(start + V123_CRITIC_BATCH_SIZE, count)
            logits.append(model(**{
                key: value[start:stop].to(resolved)
                for key, value in inputs.items()
            }).cpu().float())
    ...
    if (tuple(output.shape) != (count, 2)
        ...
        raise RuntimeError("V123 utility logits changed")
    return output
...
def pairwise_utility_accept_mask(base_accepted, utility_logits):
    if (base_accepted.dtype != torch.bool or base_accepted.dim() != 1
        ...
        raise ValueError("V123 utility gate tensors changed")
...
def main(argv=None):
    ...
    for held in range(5):
        ...
        if (set(records[index]["scan_id"] for index in inner_fit_indices)
            ...
            raise RuntimeError("scene leakage into V123 inner calibration")
        ...
        if not torch.equal(prediction["baselines"], baselines[held_indices]):
            raise RuntimeError("V123 OOF baseline identity changed")
    ...
    veto_reasons = {
        ...
        "nested_semantic_antisymmetric_vetoes": int(pairwise_veto.sum().item()),
    ...
    if protected_after != protected_before:
        raise RuntimeError("protected artifacts changed during V123")
    protected_metadata_after = {
        name: file_sha256(path)
        for name, path in protected_metadata_paths.items()
    }
    if protected_metadata_after != protected_metadata_before:
        raise RuntimeError("protected V123 cache metadata changed during OOF")
    script_sources = {
        "v123": str(Path(__file__).resolve()),
        ...
        "v123_model": str(Path(v123_model.__file__).resolve()),
        "v108": str(
    ...
    report = {
        "schema": "rec-v123-nested-semantic-antisymmetric-utility-scene-oof-v1",
        "version": 1,
        "validation_data_accessed": False,
        ...
        "protocol": {
            "architecture": (
                ...
                "candidate-minus-baseline antisymmetry"
            ),
            ...
            "acceptance": (
                ...
                "candidate-minus-baseline utility at REC@.25"
            ),
            "grid_search": False,
            "selection_data": (
            ...
            "critic_candidate_dim": v123_model.V123_CANDIDATE_DIM,
            "critic_pair_structure": "shared_score_difference",
            "critic_model_gain_scale": "learned_positive_per_threshold",
            "critic_epochs": V123_CRITIC_EPOCHS,
            "critic_batch_size": V123_CRITIC_BATCH_SIZE,
            "critic_learning_rate": V123_CRITIC_LEARNING_RATE,
            "critic_weight_decay": V123_CRITIC_WEIGHT_DECAY,
            "critic_class_weights": None,
            "critic_neutral_events_in_loss": False,
            "critic_acceptance": "antisymmetric_utility_logit025_nonnegative",
    ...
    try:
        ...
        while written < len(payload):
            ...
            if count <= 0:
                raise OSError("V123 output write made no progress")
            written += count
        os.fsync(descriptor)
    finally:
        os.close(descriptor)
