"""Fixed mask-query screen and Query-conditioned Mask diagnostics."""
...
def compare(reference, candidate, mask_field='mask_iou'):
    ...
    for a, b in zip(reference, candidate):
        ...
        old, new = a['native'][mask_field], b['memory'][mask_field]
...
def verify_terminal_run(directory):
    ...
    assert receipt['schema'] == 'mcln-nr3d-mask-query-pair-v1'
    ...
    assert receipt['text_mask_and_alpha_exactly_equal_to_start']
    ...
    point_batches = json.loads(point_raw)
    assert len(point_batches) == 1024
    for i, batch in enumerate(point_batches):
        assert batch['step'] == i + 1 and batch['row_ids'] == fit_order[i * 4:(i + 1) * 4]
    ...
    comparisons = {'memory_minus_terminal_native': compare(terminal, terminal),
                   'memory_minus_protected_start': compare(baseline, terminal)}
    ...
    for name, start, end, field in [
        ...
        diagnostics[name] = {
            ...
            'metrics': {stage: {arm: metrics(rows, arm, field) for arm in ['native', 'memory']}
                        for stage, rows in [('baseline', start), ('terminal', end)]},
            'comparisons': {'memory_minus_terminal_native': compare(end, end, field),
                            'memory_minus_protected_start': compare(start, end, field)}}
    ...
    return {'receipt_sha256': hashlib.sha256(receipt_raw).hexdigest(),
            ...
            'metrics': {stage: {arm: metrics(rows, arm) for arm in ['native', 'memory']}
