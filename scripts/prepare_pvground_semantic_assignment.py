"""Install G and run CPU tests plus a zero-update real-batch preflight only."""
import datetime
import hashlib
import json
import os
...
for suffix in ['task_observation', 'fixed_memory', 'rec_competition']:
...
assert out.channel.recv_exit_status() == 0 and free > 2*343_000_000 + 256*1024**2 + 768*1024**2, err.read().decode()
...
files = {'train.py': (repo / 'scripts/run_pvground_scanrefer_semantic_assignment.py').read_bytes(),
         'pvground_semantic_assignment.py': (repo / 'models/pvground_semantic_assignment.py').read_bytes(),
...
test = (repo / 'tests/test_pvground_semantic_assignment.py').read_text()
old = "Path(__file__).parents[1] / 'models/pvground_semantic_assignment.py'"
assert test.count(old) == 1
files['cpu_test.py'] = test.replace(old, "Path(__file__).parent / 'pvground_semantic_assignment.py'").encode()
spec.update(root=root, semantic_assignment=True, assignment_threshold=.5,
            assignment_module_sha256=sha(files['pvground_semantic_assignment.py']),
            comparison_control_root=json.loads((control / 'spec.json').read_bytes())['root'],
            comparison='G native unmatched-root token-label replacement versus same-budget D',
...
_, out, err = c.exec_command('screen -dmS mcln_pvg_semantic_assignment_preflight_v1 /root/miniconda3/envs/bdetr/bin/python -u ' + shlex.quote(root + '/preflight_controller.py'), timeout=30)
