...
s=c.open_sftp();assert not {'launch.json','run.log','training.exit','controller.exit'}.intersection(s.listdir(remote))
...
check="""import json,os,shutil,subprocess
gpu=subprocess.check_output(['nvidia-smi','--query-gpu=index,name,memory.used,memory.total','--format=csv,noheader,nounits'],universal_newlines=True).strip()
free=shutil.disk_usage('/root/autodl-tmp').free
assert int(gpu.split(',')[2])<500 and free>2*1024**3
print(json.dumps({'gpu':gpu,'free_bytes':free}))
"""
_,o,e=c.exec_command('/root/miniconda3/envs/bdetr/bin/python -c '+shlex.quote(check),timeout=30)
before=o.read().decode();assert o.channel.recv_exit_status()==0,e.read().decode();print(before,flush=True)
command='screen -dmS mcln_mask_geometry_pair_v1 bash -c '+shlex.quote('exec bash '+remote+'/controller.sh > '+remote+'/run.log 2>&1')
_,o,e=c.exec_command(command,timeout=30);assert o.channel.recv_exit_status()==0,e.read().decode()
_,o,e=c.exec_command("screen -ls\nps -eo pid,ppid,comm,etime,args | grep '[r]un_scanrefer_mask_geometry_pair.py'",timeout=30)
live=o.read().decode();assert 'mcln_mask_geometry_pair_v1' in live
assert any(len(line.split())>=5 and line.split()[2]=='python' and 'scripts/run_scanrefer_mask_geometry_pair.py --manifest '+remote in line for line in live.splitlines()),live
proof={'time_cst':datetime.datetime.now(datetime.timezone(datetime.timedelta(hours=8))).isoformat(),
    'remote_directory':remote,'screen':'mcln_mask_geometry_pair_v1','command':command,'published_source_commit':head,
    'manifest_sha256':hashlib.sha256((local/'input_manifest.json').read_bytes()).hexdigest(),
    'resource_precheck':json.loads(before),'live_processes':live,'steps_per_arm':2482,
    'candidate':'native_gt_mask_geometry','control':'native_gt','formal_rows':0,
    'estimated_baseline_minutes':40,'estimated_total_hours':4,'poll_interval_seconds':240,
    'terminal_cpu_audit_chained':True,'quality_result_available':False,'nr3d_sr3d_started':False}
raw=(json.dumps(proof,indent=2)+'\n').encode();(local/'launch.json').write_bytes(raw)
