...
for name in ['strict_load.json','strict_load.exit','strict_controller.exit','strict_load.log','strict_load.stderr','strict_execution.json','strict_queue.log']:
    sftp.get(remote+'/'+name,str(archive/name))
assert (archive/'strict_load.exit').read_text().strip()=='0'
assert (archive/'strict_controller.exit').read_text().strip()=='0'
