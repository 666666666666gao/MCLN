"""Launch one CPU-only real Sr3D batch preparation, without loading weights."""
import datetime
import hashlib
import json
import os
...
root='/root/autodl-tmp/mcln_pvground_sr_native_batch_20260917_v1'
archive=repo/'refine-logs/pvground_sr_native_batch_20260917_v1'
...
spec.update(env_spec_sha256=hashlib.sha256(json.dumps(env,sort_keys=True,separators=(',',':')).encode()).hexdigest(),
            batch_size=8,rows='existing fixed selection indices0,1,2,3,12,13,14,15 for Sr3D',augmentation=True,
            scope='CPU persist actual native input and labels; no model, optimizer, checkpoints, or formal rows')
files={'prepare.py':(repo/'scripts/prepare_pvground_sr_native_batch.py').read_bytes()}
...
assert state['free_bytes']>=1759741824+200*1024**2, 'preserve running Scan F checkpoint and export reserve'
...
argv=['screen','-dmS','mcln_pvg_sr_native_batch_v1','/root/miniconda3/envs/bdetr/bin/python',root+'/controller.py']
