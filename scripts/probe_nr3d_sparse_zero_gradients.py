"""Five first-batch backward comparisons; no optimizer or quality evaluation."""
...
def main():
    ...
    def differentiable_zero(module, incoming, outgoing):
        grouped, indices = outgoing
        return grouped + torch.zeros_like(grouped, requires_grad=True), indices

    handle = models['B'].super_grouper.register_forward_hook(differentiable_zero)
    capture('differentiable_zero_B', 'B')
    handle.remove()
    reference = 'plain_A_first'
    comparisons = {}
    for label in list(gradients)[1:]:
        per_parameter = {}
    ...
    result = {'schema': 'mcln-sparse-zero-gradient-probe-v1', 'status': 'complete',
        'manifest_sha256': sha(options.manifest), 'native_forwards': 5, 'backwards': 5,
