...
failed_root = '/root/autodl-tmp/mcln_scanrefer_local_visual_official_20260906_v2'
failed_local = repo / 'refine-logs/scanrefer_local_visual_official_20260906_v2'
for remote_name, local_name in [('controller.exit', 'controller.exit'), ('run.log', 'run.txt')]:
    with sftp.open(failed_root + '/' + remote_name, 'rb') as stream:
        (failed_local / local_name).write_bytes(stream.read())
assert (failed_local / 'controller.exit').read_text().strip() == '1'
assert 'DATA_ROOT_mcln_meshsproberta-base/' in (failed_local / 'run.txt').read_text()
failure = {'status': 'startup_failed', 'stage': 'native_dataset_tokenizer_before_model_construction',
           'reason': 'Native dataset concatenates root with roberta-base;trailing directory separator was missing.',
           'gpu_forwards': 0, 'optimizer_steps': 0, 'checkpoint_writes': 0}
failure_raw = (json.dumps(failure, indent=2) + '\n').encode()
(failed_local / 'startup_failure.json').write_bytes(failure_raw)
with sftp.open(failed_root + '/startup_failure.json', 'wx') as stream:
    stream.write(failure_raw)
...
tokenizer_path=str(root)+'/roberta-base/'
tokenizer=RobertaTokenizerFast.from_pretrained(tokenizer_path,local_files_only=True)
print(json.dumps({'data_root':str(root),'superpoint_files':superpoints,'non_superpoint_entries_share_original_inode':shared,
                  'native_concatenated_tokenizer_path':tokenizer_path,'tokenizer_vocab_size':tokenizer.vocab_size}))
'''
...
assert counts == {'tests': 30, 'failures': 0, 'errors': 0, 'skipped': 0}
...
command = 'screen -dmS mcln_scanrefer_local_visual_official_v3 bash -lc ' + shlex.quote('cd ' + shlex.quote(remote) + ' && bash controller.sh > run.log 2>&1')
...
assert output.channel.recv_exit_status() == 0 and 'mcln_scanrefer_local_visual_official_v3' in sessions
launch = {'schema': 'mcln-scanrefer-local-visual-official-launch-v2',
          'time_cst': datetime.datetime.now(datetime.timezone(datetime.timedelta(hours=8))).isoformat(),
          'screen_session': [line.strip() for line in sessions.splitlines() if 'mcln_scanrefer_local_visual_official_v3' in line],
