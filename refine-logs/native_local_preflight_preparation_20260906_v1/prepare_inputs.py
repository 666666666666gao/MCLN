...
local=repo/'refine-logs/native_local_preflight_preparation_20260906_v1'
remote='/root/autodl-tmp/mcln_native_local_preflight_preparation_20260906_v1'
...
files={'run_native_candidate_local_preflight.py':entry,
       'annotation_receipt.json':(repo/'refine-logs/native_joint_annotation_audit_20260906_v1/receipt.json').read_bytes(),
       'preflight_rows.json':(repo/'refine-logs/native_joint_annotation_audit_20260906_v1/preflight_rows.json').read_bytes(),
       'nr_contract.json':(repo/'refine-logs/candidate_local_native_preparation_20260906_v1/nr_contract.json').read_bytes()}
check=r'''import datetime, gc, hashlib, importlib.util, json, os, random, sys, time
from pathlib import Path
...
expected={'model_source':source,'source_manifest_sha256':'4af68cad46b52c9e250de17872a193485d75529378db983ad037779232e500fc',
...
for name,raw in files.items():
    ...
    with sftp.open(remote+'/'+name,'wx') as stream:
        stream.write(raw)
command="CUDA_VISIBLE_DEVICES='' OMP_NUM_THREADS=1 MKL_NUM_THREADS=1 TOKENIZERS_PARALLELISM=false PYTHONDONTWRITEBYTECODE=1 /root/miniconda3/envs/bdetr/bin/python -u "+shlex.quote(remote+'/check_inputs.py')+' '+shlex.quote(remote)
_,out,err=client.exec_command(command,timeout=900)
result=out.read().decode()
error=err.read().decode()
status=out.channel.recv_exit_status()
(local/'run.txt').write_text(result+error,encoding='utf-8')
with sftp.open(remote+'/run.txt','wx') as stream:
    stream.write((result+error).encode())
