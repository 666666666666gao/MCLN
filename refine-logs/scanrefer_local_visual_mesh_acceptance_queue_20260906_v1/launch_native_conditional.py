...
prep=repo/'refine-logs/native_local_preflight_preparation_20260906_v2'
prepared=json.loads((prep/'receipt.json').read_bytes())
assert prepared['status']=='actual_cpu_inputs_pass'
assert prepared['schema']=='mcln-native-local-real-input-cpu-preparation-v2'
assert prepared['old_root_point_samples_verified_equal']==64
assert prepared['unique_input_rows']==32 and prepared['point_samples_constructed']==64
assert prepared['gpu_forwards']==prepared['optimizer_steps']==prepared['checkpoint_writes']==0
entry=(repo/'scripts/run_native_candidate_local_preflight.py').read_bytes()
assert hashlib.sha256(entry).hexdigest()==prepared['files']['run_native_candidate_local_preflight.py']
...
for name,raw in files.items():
    assert hashlib.sha256(raw).hexdigest()==prepared['files'][name]
files['run_native_candidate_local_preflight.py']=entry
files['cpu_preparation_receipt.json']=(prep/'receipt.json').read_bytes()
formal_root='/root/autodl-tmp/mcln_scanrefer_local_visual_mesh_official_20260906_v1'
...
assert formal_audit['schema']=='mcln-scanrefer-local-visual-official-audit-v2'
...
local=repo/'refine-logs/native_local_preflight_20260906_v2'
remote='/root/autodl-tmp/mcln_native_local_preflight_20260906_v2'
local.mkdir()
sftp.mkdir(remote)
manifest={'schema':'mcln-native-local-gpu-preflight-input-v2',
          'model_source':'/root/autodl-tmp/mcln_candidate_local_native_preparation_20260906_v1/model_source',
...
cd '''+remote+'''
flock -x /root/autodl-tmp/mcln_v99_backbone_gpu0.lock /root/miniconda3/envs/bdetr/bin/python -u run_native_candidate_local_preflight.py --manifest '''+remote+'''/input_manifest.json
status=$?
...
command='screen -dmS mcln_native_local_preflight_v2 bash -lc '+shlex.quote('cd '+shlex.quote(remote)+' && bash controller.sh > run.log 2>&1')
...
assert out.channel.recv_exit_status()==0 and 'mcln_native_local_preflight_v2' in sessions
launch={'time_cst':datetime.datetime.now(datetime.timezone(datetime.timedelta(hours=8))).isoformat(),
        'screen_session':[line.strip() for line in sessions.splitlines() if 'mcln_native_local_preflight_v2' in line],
