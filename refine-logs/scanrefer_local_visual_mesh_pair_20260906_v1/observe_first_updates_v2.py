...
first_check = datetime.datetime(2026, 9, 6, 23, 2, tzinfo=zone)
...
schedule = {'first_check_cst': first_check.isoformat(), 'interval_seconds_after_first_check': 240,
    ...
    'remote_training_pid': 42648, 'local_observer_pid': os.getpid(),
    'script_sha256': hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),
    'eta_scope': 'resume after local observer prefix parsing error; original training process unchanged'}
with (local / 'first_update_observation_schedule_v2.json').open('x') as stream:
...
while True:
    ...
    _, stdout, stderr = client.exec_command('ps -p 42648 -o pid,stat,etime,args', timeout=30)
