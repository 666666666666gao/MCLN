"""Resume only the pending terminal stage of the existing CPU D/E comparison."""
import json
import os
...
remote=r'''import datetime,hashlib,json,os,signal,subprocess,time
from pathlib import Path
...
'''
compile(remote,'reschedule_terminal.py','exec')
client=paramiko.SSHClient();client.load_system_host_keys()
client.connect('region-9.autodl.pro',port=33476,username='root',password=os.environ['MCLN_SSH_PASSWORD'],timeout=30)
sftp=client.open_sftp()
with sftp.open(root+'/reschedule_terminal.py','wb') as stream:stream.write(remote.encode())
_,stdout,stderr=client.exec_command('/root/miniconda3/envs/bdetr/bin/python -u '+shlex.quote(root+'/reschedule_terminal.py'),timeout=30)
output=stdout.read().decode();assert stdout.channel.recv_exit_status()==0,stderr.read().decode()
(archive/'before_terminal_reschedule_20260917').mkdir()
for name in ['queue.py','controller.py','spec.json','launch.json','run.log','controller.pid']:
    sftp.get(root+'/before_terminal_reschedule_20260917/'+name,str(archive/'before_terminal_reschedule_20260917'/name))
for name in ['queue.py','controller.py','spec.json','launch.json','reschedule_receipt.json','reschedule_terminal.py']:
