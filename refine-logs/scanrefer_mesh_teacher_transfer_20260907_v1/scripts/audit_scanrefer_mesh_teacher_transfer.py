"""Bind V99 geometry-transfer targets to corrected mesh inputs, without updates.

The historical teacher audit inherited the mixed-superpoint DATA_ROOT. This
audit retains its fixed 512 fit rows but explicitly verifies the mesh version
and records current Query/Variant sets for a native-student training design.
"""
...
def transfer_rows(boxes, scores, default_indices, parent_indices, runtime, matches, roots):
    ...
    for index in range(len(boxes)):
        ...
        records.append({
            ...
            'native_boxes': boxes[index].tolist(),
            'native_scores': scores[index].tolist(),
            'parent_query_indices': parent_indices[index].tolist(),
            'variant_boxes': variant_boxes[index].reshape(16, 7, 6).tolist(),
            'variant_valid': variant_valid[index].reshape(16, 7).tolist(),
            'teacher_selected_flat_index': int(selected_flat[index]),
        })
    return records
...
def main():
    ...
    assert manifest['schema'] == 'mcln-scanrefer-mesh-teacher-transfer-input-v1'
    ...
    with torch.no_grad():
        for batch_index, raw in enumerate(loader):
            ...
            native_state = build_full_rec_query_state(outputs, inputs)
            ...
            assert batch['box_label_mask'][:, 0].bool().all() and torch.isfinite(boxes).all()
            captured = []
            ...
            rows = transfer_rows(native_state['boxes'], native_state['default_scores'],
                native_state['default_scores'].argsort(dim=1, descending=True)[:, 0],
                teacher['parent']['candidate_batch']['query_indices'], teacher['runtime'], captured[0], roots)
            for offset, row in enumerate(rows):
                position = len(records) + offset
                raw_invalid = (boxes[offset, :, 3:] < 0).any(dim=-1)
                row.update(row_id=protocol['selected_row_ids'][position], scan_id=batch['scan_ids'][offset],
                    point_sha256=hashlib.sha256(inputs['point_clouds'][offset].cpu().numpy().tobytes()).hexdigest(),
                    raw_negative_size_queries=int(raw_invalid.sum()),
                    raw_native_selected_negative_size=bool(raw_invalid[row['native_query_index']]),
                    raw_hungarian_root_negative_size=bool(raw_invalid[row['hungarian_root_query_index']]))
            records.extend(rows)
            evaluated = rec_evaluation_view(outputs)
            native_evaluator.evaluate_bbox_by_pos_align(evaluated, 'last_')
            ...
            del outputs, teacher, evaluated, inputs, batch, boxes, roots, native_loss, native_state
    ...
    receipt = {'schema': 'mcln-scanrefer-mesh-teacher-transfer-audit-v1', 'status': 'pass',
        ...
        'native_score_mode': 'raw_default_matching_authoritative_scanrefer_cli',
        'evaluation_extent_policy': 'existing rec_candidate_adapter floor at 1e-6;raw matcher inputs unchanged',
        'all_model_states_unchanged': True, 'batch_seconds': batch_seconds,
        'data_root': manifest['data_root'], 'superpoint_inputs': data_inputs,
        'same_512_fit_row_ids_as_historical_teacher_audit': True,
        'candidate_sets_are_training_diagnostics_not_deployment_oracles': True,
