...
readonly EXP="nr3d_mcln_joint_butdcls_v99_plateau_lr10_e58_e59_b16x1_w4p2"
readonly TRAIN_SCREEN_NAME="mcln_nr3d_plateau_lr10_train"
readonly GUARD_SCREEN_NAME="mcln_nr3d_plateau_lr10_guard"
readonly GUARD_SCRIPT="/root/mcln_nr3d_plateau_lr10_guard.py"
readonly GUARD_SCRIPT_SHA256="52a96a166228cea77e6f006df8f84e9e1b512b2568002efc71a782c429a6fa01"
readonly WATCHDOG_SCRIPT="/root/mcln_nr3d_plateau_lr10_watchdog.py"
readonly WATCHDOG_SCRIPT_SHA256="2bb87d64dabcc25ecfd5f05ae20e8ea1c191d0fd746f505e59a863cc833d41d4"
...
readonly PLATEAU_RUN_LEAF="${OUTPUT_ROOT}/backbone/nr3d_mcln_joint_butdcls_v99_effective_batch48_e58_e59_b16a3_w4p2_20260822_212541/nr3d/nr3d_mcln_joint_butdcls_v99_effective_batch48_e58_e59_b16a3_w4p2/1787405149"
readonly PLATEAU_E58_RECEIPT="${PLATEAU_RUN_LEAF}/eval_metrics_epoch_58.json"
readonly PLATEAU_E58_SHA256="b88af5359627937b373b5f3c5d68f2105258affdbb9e7d4ff8ebb971598b59a7"
readonly PLATEAU_E59_RECEIPT="${PLATEAU_RUN_LEAF}/eval_metrics_epoch_59.json"
readonly PLATEAU_E59_SHA256="b1600161b39a680e906c7b6a5d49c971e10e60c29227bb46c7ef59381109245f"
readonly PLATEAU_GUARD_STATE="${OUTPUT_ROOT}/control/effective_batch48/guard_state.json"
readonly PLATEAU_GUARD_STATE_SHA256="ef0338215acdca9b63e9d67b4a5ed0bfb0a4a1e1480d766573452fc962e64b20"
...
require_exact_sha256 "${PLATEAU_E58_RECEIPT}" "${PLATEAU_E58_SHA256}" "E58 plateau receipt"
require_exact_sha256 "${PLATEAU_E59_RECEIPT}" "${PLATEAU_E59_SHA256}" "E59 plateau receipt"
require_exact_sha256 "${PLATEAU_GUARD_STATE}" "${PLATEAU_GUARD_STATE_SHA256}" "plateau guard state"

PROOF_E58_RECEIPT="${PLATEAU_E58_RECEIPT}" PROOF_E59_RECEIPT="${PLATEAU_E59_RECEIPT}" PROOF_GUARD_STATE="${PLATEAU_GUARD_STATE}" "${PYTHON_BIN}" - <<'PY'
import json
import math
import os
from pathlib import Path

expected = ((58, 4400), (59, 4399))
baseline = 4463
sample_count = 7899
paths = (
...
for (epoch, expected_hits), path in zip(expected, paths):
    ...
    if observed_count != sample_count:
        raise SystemExit("plateau receipt sample_count mismatch")
    if hits025 != expected_hits or hits025 > baseline:
        raise SystemExit("plateau receipt does not prove non-improvement")
    if not math.isclose(
            acc025, hits025 / sample_count, rel_tol=0.0, abs_tol=1e-12):
        raise SystemExit("plateau receipt accuracy/hits mismatch")
state = json.loads(
...
if (
        ...
        or int(latest["hits025"]) != 4399
        or Path(latest["receipt"]).resolve() != paths[1].resolve()):
    raise SystemExit("formal guard state does not prove patience2 plateau")
print("plateau_proof=E58:4400,E59:4399,baseline:4463,patience:2")
PY
...
PY
  ...
  "${PYTHON_BIN}" "${WATCHDOG_SCRIPT}"     --guard-pid "${guard_pid}"     --guard-start-ticks "${guard_start_ticks}"     --guard-command-fragment "${GUARD_SCRIPT}"     --screen "${train_screen_id}"     --screen-start-ticks "${train_screen_start_ticks}"     --experiment "${EXP}"     --run-root "${run_root}"     --heartbeat "${watchdog_heartbeat}"     >>/root/mcln_nr3d_plateau_lr10_watchdog.log 2>&1 &
  watchdog_pid=$!
