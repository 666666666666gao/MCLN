"""Paired REC accounting for the fixed EG Nr3D adaptation and its actual transfer start."""
import argparse
import gzip
import hashlib
import json
...
def main():
    ...
    result = {'status': 'complete', 'rows': 7899, 'identical_inputs': True,
              'start_checkpoint_sha256': first['checkpoint_sha256'],
              'end_checkpoint_sha256': last['checkpoint_sha256'],
              'fit_optimizer_steps': 5614, 'analysis_model_forwards': 0, 'primary_mode': 'bbs', 'modes': {}}
    ...
    result['claim_boundary'] = 'Native adaptation plus train view-word repair; no independent causal attribution to that repair or any new module. Only bbs filtered is primary.'
    with (args.adaptation / 'paired_rec.json').open('x') as f:
        json.dump(result, f, indent=2)
    print('EG_NR_PAIRED_REC_COMPLETE ' + json.dumps({m: {t: {k: v for k, v in values.items() if not k.endswith('_ids')}
