...
train='/root/autodl-tmp/mcln_pvground_scanrefer_finetune_20260908_emptypool_v1'
...
free=int(out.read().decode().splitlines()[-1]);assert free>int(2.25*1024**3) and out.channel.recv_exit_status()==0 and not err.read()
...
files={'train.py':(repo/'scripts/run_pvground_scanrefer_empty_pool.py').read_bytes(),
       'plan.md':(repo/'docs/PVG_EMPTY_POOL_CONTROL_2026-09-08.md').read_bytes(),
...
training_launch=launch(train,'mcln_pvg_scan_emptypool_v1')
...
install(audit,audit_files);audit_launch=launch(audit,'mcln_pvg_emptypool_audit_v1')
