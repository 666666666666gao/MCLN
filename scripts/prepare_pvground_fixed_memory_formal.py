...
archive=repo/'refine-logs/pvground_scanrefer_formal_20260917_fixed_memory_v1'
manifest=json.loads((repo/'refine-logs/pvground_scanrefer_finetune_20260917_fixed_memory_v1/input_manifest.json').read_bytes())
spec=json.loads((old/'spec.json').read_bytes())
spec.update(training_root=training,training_audit_root=audit,training_audit_controller_pid=int(json.loads((repo/'refine-logs/pvground_scanrefer_endpoint_audit_20260917_fixed_memory_v1/launch.json').read_bytes())['process'].split()[0]),first_check_cst=(datetime.datetime.now(datetime.timezone(datetime.timedelta(hours=8)))+datetime.timedelta(minutes=180)).isoformat(),training_spec_sha256=hashlib.sha256((repo/'refine-logs/pvground_scanrefer_finetune_20260917_fixed_memory_v1/spec.json').read_bytes()).hexdigest())
files={'audit.py':(old/'audit.py').read_bytes()}
files['plan.md']=('''# Fixed pretrained visual memory formal continuation
...
3723-step endpoint passes integrity, actual frozen-backbone byte check and both bbs thresholds vs its fixed initial.
...
for name in ['formal_queue.py','controller.py','cpu_probe.py']:
    ...
    if name=='formal_queue.py':
        ...
        text=text.replace("assert audited['integrity_pass']","assert audited['integrity_pass'] and audited['fixed_visual_memory_verified']")
...
_,out,err=c.exec_command('screen -dmS mcln_pvg_fixed_memory_formal_v1 bash -c '+shlex.quote(inner),timeout=30)
