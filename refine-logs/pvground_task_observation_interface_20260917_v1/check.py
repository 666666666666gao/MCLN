...
def main():
    ...
    model_source = Path('/root/autodl-tmp/mcln_pvground_task_observation_source_20260917_v1/PV-Ground')
    ...
    assert sha(model_source.parent/'source_port.json')=='0375886f9df2b3ac18a63ce7571d672ceb433613b6defdfcd9f8d244a20bf855'
    ...
    for step, start in enumerate((0,2),1):
        ...
        record = {'step':step,'training_row_ids':[r['training_row_id'] for r in fixture_receipt['rows'][start:start+2]],
            ...
            'task_query_gradient_norms':task_norms,'all_present_gradients_finite':True,'seconds':time.time()-begin,
    ...
    receipt = {'status':'pass','time_cst':datetime.datetime.now(datetime.timezone(datetime.timedelta(hours=8))).isoformat(),
        ...
        'eval_forwards':4,'train_forwards':2,'trainable_tensors':len(trainable),
    ...
    print('PVG_TASK_OBSERVATION_INTERFACE_PASS '+json.dumps({k:v for k,v in receipt.items() if k not in ['steps','changed_parameters','changed_buffers']}),flush=True)
