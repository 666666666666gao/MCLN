#!/usr/bin/env python
"""Train-only OOF screen of the V130 hyperspherical semantic adapter."""
...
import models.rec_hyperspherical_semantic_adapter as v130_model
...
def fit_v130(records, device):
    """Fit V99 first, freeze it, then fit only the bounded V130 adapter."""
    anchor, statistics, anchor_epochs = v99.fit_v97(records, device)
    resolved = torch.device(device)
    _set_hierarchical_seed(resolved)
    model = v130_model.AnchoredHypersphericalSemanticResidualAdapter(
        anchor,
        normalization_statistics=statistics,
        hidden_dim=128,
        dropout=HIERARCHICAL_DROPOUT,
        residual_scale=v130_model.V130_RESIDUAL_SCALE,
    ).to(resolved)
    ...
    if not trainable or any(parameter.requires_grad
        ...
        raise RuntimeError("V130 anchor/trainable parameter contract changed")
    ...
    for epoch in range(HIERARCHICAL_EPOCHS):
        ...
        if model.anchor.training:
            raise RuntimeError("V130 frozen anchor entered training mode")
...
def _load_bound_receipt(path, expected_sha256, expected_schema):
    ...
    if not resolved.is_file() or resolved.is_symlink():
        raise ValueError("V130 receipt must be a regular non-symlink file")
    if file_sha256(resolved) != expected_sha256:
        raise ValueError("V130 receipt SHA-256 changed: {}".format(resolved))
    ...
    if (receipt.get("schema") != expected_schema
        ...
        raise ValueError("V130 receipt policy changed: {}".format(resolved))
    return receipt
...
def main(argv=None):
    ...
    for held in range(5):
        ...
        model, statistics, training = fit_v130(
        ...
        if not torch.equal(prediction["baselines"], baselines[held_indices]):
            raise RuntimeError("V130 OOF baseline identity changed")
    ...
    if protected_after != protected_before:
        raise RuntimeError("protected artifacts changed during V130")
    protected_metadata_after = {
        name: file_sha256(path)
        for name, path in protected_metadata_paths.items()
    }
    if protected_metadata_after != protected_metadata_before:
        raise RuntimeError("protected V130 cache metadata changed during OOF")
    script_sources = {
        "v130": str(Path(__file__).resolve()),
        "v130_model": str(Path(v130_model.__file__).resolve()),
        "v108": str(
    ...
    report = {
        "schema": "rec-v130-hyperspherical-semantic-scene-oof-v1",
        "version": 1,
        "validation_data_accessed": False,
        ...
        "protocol": {
            "architecture": (
                "frozen V99 contextual hierarchy plus a bounded explicit "
                "hyperspherical query-text semantic interaction adapter"
            ),
            ...
            "semantic_representation": (
                "fold-normalization inversion followed by unit query/text, "
                "product, absolute difference, cosine and component evidence"
            ),
            "set_competition": "one four-head permutation-equivariant layer",
            "language_condition": "frozen 64D target_text_proj mean",
            "anchor": "fold-local V99 frozen before adapter optimization",
            "semantic_heads": v130_model.V130_HEAD_COUNT,
            "semantic_input_dim": v130_model.V130_SEMANTIC_INPUT_DIM,
            "normalization_binding": "fold-local V99 query-feature mean/std",
            "residual_scale": v130_model.V130_RESIDUAL_SCALE,
    ...
    try:
        ...
        while written < len(payload):
            ...
            if count <= 0:
                raise OSError("V130 output write made no progress")
            written += count
        os.fsync(descriptor)
    finally:
        os.close(descriptor)
