#!/usr/bin/env python
"""Train-only OOF screen of the V129 text-conditioned box-relation adapter."""
...
import models.rec_box_relation_adapter as v129_model
...
def fit_v129(records, device):
    """Fit V99 first, freeze it, then fit only the bounded V129 adapter."""
    anchor, statistics, anchor_epochs = v99.fit_v97(records, device)
    resolved = torch.device(device)
    _set_hierarchical_seed(resolved)
    model = v129_model.AnchoredTextConditionedBoxRelationAdapter(
        anchor,
        hidden_dim=128,
        dropout=HIERARCHICAL_DROPOUT,
        residual_scale=v129_model.V129_RESIDUAL_SCALE,
    ).to(resolved)
    ...
    if not trainable or any(parameter.requires_grad
        ...
        raise RuntimeError("V129 anchor/trainable parameter contract changed")
    ...
    for epoch in range(HIERARCHICAL_EPOCHS):
        ...
        if model.anchor.training:
            raise RuntimeError("V129 frozen anchor entered training mode")
...
def _load_bound_receipt(path, expected_sha256, expected_schema):
    ...
    if not resolved.is_file() or resolved.is_symlink():
        raise ValueError("V129 receipt must be a regular non-symlink file")
    if file_sha256(resolved) != expected_sha256:
        raise ValueError("V129 receipt SHA-256 changed: {}".format(resolved))
    ...
    if (receipt.get("schema") != expected_schema
        ...
        raise ValueError("V129 receipt policy changed: {}".format(resolved))
    return receipt
...
def main(argv=None):
    ...
    for held in range(5):
        ...
        model, statistics, training = fit_v129(
        ...
        if not torch.equal(prediction["baselines"], baselines[held_indices]):
            raise RuntimeError("V129 OOF baseline identity changed")
    ...
    if protected_after != protected_before:
        raise RuntimeError("protected artifacts changed during V129")
    protected_metadata_after = {
        name: file_sha256(path)
        for name, path in protected_metadata_paths.items()
    }
    if protected_metadata_after != protected_metadata_before:
        raise RuntimeError("protected V129 cache metadata changed during OOF")
    script_sources = {
        "v129": str(Path(__file__).resolve()),
        "v129_model": str(Path(v129_model.__file__).resolve()),
        "v108": str(
    ...
    report = {
        "schema": "rec-v129-text-conditioned-box-relation-scene-oof-v1",
        "version": 1,
        "validation_data_accessed": False,
        ...
        "protocol": {
            "architecture": (
                "frozen V99 contextual hierarchy plus a bounded text-"
                "conditioned directed semantic/box-relation graph adapter"
            ),
            ...
            "relation_coordinates": (
                "directed center delta/absolute delta/size-normalized delta/"
                "log-size ratio/distances/box IoU from frozen normalized boxes"
            ),
            "relation_semantics": (
                "frozen query-projection cosine and target-text cosine pair"
            ),
            "language_condition": "frozen 64D target_text_proj mean",
            "anchor": "fold-local V99 frozen before adapter optimization",
            "relation_heads": v129_model.V129_HEAD_COUNT,
            "edge_feature_dim": v129_model.V129_EDGE_DIM,
            "residual_scale": v129_model.V129_RESIDUAL_SCALE,
    ...
    try:
        ...
        while written < len(payload):
            ...
            if count <= 0:
                raise OSError("V129 output write made no progress")
            written += count
        os.fsync(descriptor)
    finally:
        os.close(descriptor)
