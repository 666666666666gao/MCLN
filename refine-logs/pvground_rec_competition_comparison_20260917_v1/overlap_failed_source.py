...
def analyze(output):
    ...
    pair = json.loads((pair_root/'terminal_D_F.json').read_bytes())
    assert pair['status'] == 'complete' and pair['training_row_order_identical']
    ...
    for threshold, suffix in [('0.25', '025'), ('0.5', '050')]:
        assert metrics['F_terminal']['bbs'][suffix]['hits'] - metrics['D_terminal']['bbs'][suffix]['hits'] == pair['transitions']['bbs'][threshold]['net']
