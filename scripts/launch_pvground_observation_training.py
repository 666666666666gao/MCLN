...
train='/root/autodl-tmp/mcln_pvground_scanrefer_finetune_20260909_observation_v1'
audit='/root/autodl-tmp/mcln_pvground_scanrefer_endpoint_audit_20260909_observation_v1'
interface='/root/autodl-tmp/mcln_pvground_observation_interface_20260909_v2'
oldtrain=repo/'refine-logs/pvground_scanrefer_finetune_20260908_sourcequery_v1'
oldaudit=repo/'refine-logs/pvground_scanrefer_endpoint_audit_20260908_sourcequery_v1'
...
for dependency in ['finetune','endpoint_audit','formal']:
    with s.open('/root/autodl-tmp/mcln_pvground_scanrefer_'+dependency+'_20260908_sourcequery_v1/controller.exit','rb') as f:assert f.read().strip()==b'0'
...
assert checked['observation_state'] and checked['new_parameters']==757728
...
spec.update(root=train,model_source=model_source,source_port=source_port,
    ...
    observation_state=True,observation_module_sha256=module_sha,
    ...
    comparison='own initial and same-budget B/A; primary bbs; actual selected support state; no quality or generalization claim')
files={'train.py':(repo/'scripts/run_pvground_scanrefer_observation_query.py').read_bytes(),
       'pvground_observation_query.py':module,'pvground_source_query.py':(repo/'models/pvground_source_query.py').read_bytes(),'plan.md':(repo/'docs/PVG_OBSERVATION_QUERY_CONTROL_2026-09-09.md').read_bytes()}
...
training_launch=launch(train,'mcln_pvg_scan_observation_v1')
...
install(audit,audit_files);audit_launch=launch(audit,'mcln_pvg_observation_audit_v1')
