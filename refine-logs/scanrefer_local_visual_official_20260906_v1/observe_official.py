...
local=Path('C:/Users/gb/.codex_mcln_g0_20260905/refine-logs/scanrefer_local_visual_official_20260906_v1')
remote='/root/autodl-tmp/mcln_scanrefer_local_visual_official_20260906_v1'
first_terminal_check=datetime.datetime(2026,9,6,20,20,tzinfo=zone)
...
startup={'time_cst':datetime.datetime.now(zone).isoformat(),'process_live':True,'remote_pid':pid,
         ...
         'eta_basis':'Completed paired6887-row eval took1830.66s;9508 paired rows plus startup suggests around20:25. Startup checked once, then wait until20:20.'}
...
while True:
    ...
    if 'receipt.json' in result_names:
        targets.extend('result/'+name for name in ['receipt.json','rows.json','native_rows.json'])
    for name in targets:
