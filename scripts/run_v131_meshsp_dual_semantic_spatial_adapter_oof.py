#!/usr/bin/env python
"""Train-only OOF screen of the V131 dual semantic-spatial adapter."""
...
import models.rec_dual_semantic_spatial_adapter as v131_model
...
def fit_v131(records, device):
    """Fit V99 first, freeze it, then fit only the bounded V131 adapter."""
    anchor, statistics, anchor_epochs = v99.fit_v97(records, device)
    resolved = torch.device(device)
    _set_hierarchical_seed(resolved)
    model = v131_model.AnchoredDualSemanticSpatialResidualAdapter(
        anchor,
        normalization_statistics=statistics,
        hidden_dim=128,
        dropout=HIERARCHICAL_DROPOUT,
        residual_scale=v131_model.V131_RESIDUAL_SCALE,
    ).to(resolved)
    ...
    if not trainable or any(parameter.requires_grad
        ...
        raise RuntimeError("V131 anchor/trainable parameter contract changed")
    ...
    for epoch in range(HIERARCHICAL_EPOCHS):
        ...
        if model.anchor.training:
            raise RuntimeError("V131 frozen anchor entered training mode")
...
def _load_bound_receipt(path, expected_sha256, expected_schema):
    ...
    if not resolved.is_file() or resolved.is_symlink():
        raise ValueError("V131 receipt must be a regular non-symlink file")
    if file_sha256(resolved) != expected_sha256:
        raise ValueError("V131 receipt SHA-256 changed: {}".format(resolved))
    ...
    if (receipt.get("schema") != expected_schema
        ...
        raise ValueError("V131 receipt policy changed: {}".format(resolved))
    return receipt
...
def main(argv=None):
    ...
    for held in range(5):
        ...
        model, statistics, training = fit_v131(
        ...
        if not torch.equal(prediction["baselines"], baselines[held_indices]):
            raise RuntimeError("V131 OOF baseline identity changed")
    ...
    if protected_after != protected_before:
        raise RuntimeError("protected artifacts changed during V131")
    protected_metadata_after = {
        name: file_sha256(path)
        for name, path in protected_metadata_paths.items()
    }
    if protected_metadata_after != protected_metadata_before:
        raise RuntimeError("protected V131 cache metadata changed during OOF")
    script_sources = {
        "v131": str(Path(__file__).resolve()),
        "v131_model": str(Path(v131_model.__file__).resolve()),
        "v108": str(
    ...
    report = {
        "schema": "rec-v131-dual-semantic-spatial-scene-oof-v1",
        "version": 1,
        "validation_data_accessed": False,
        ...
        "protocol": {
            "architecture": (
                "frozen V99 contextual hierarchy plus parallel V115 center-"
                "relation and V130 hyperspherical semantic set branches"
            ),
            ...
            "spatial_branch": (
                "V115 four-head target-text-conditioned 5D center relation"
            ),
            "fusion": "two learned reliability gates and shared bounded heads",
            "language_condition": "frozen 64D target_text_proj mean",
            "anchor": "fold-local V99 frozen before adapter optimization",
            "branch_heads": v131_model.V131_HEAD_COUNT,
    ...
    try:
        ...
        while written < len(payload):
            ...
            if count <= 0:
                raise OSError("V131 output write made no progress")
            written += count
        os.fsync(descriptor)
    finally:
        os.close(descriptor)
