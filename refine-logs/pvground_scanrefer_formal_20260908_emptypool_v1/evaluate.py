...
def main():
    ...
    expected_delta = {name for name,p in model.named_parameters() if p.requires_grad}
    expected_delta |= {name for name,_ in model.named_buffers() if name in parent}
    assert set(delta['state_delta']) == expected_delta
    terminal = dict(parent);terminal.update(delta['state_delta'])
    assert set(terminal) == set(parent)
    assert all(torch.equal(terminal[name],parent[name]) for name,p in model.named_parameters() if not p.requires_grad)
    model.cuda().eval()
    ...
    write_json(root/'protocol.json',{'formal_rows':9508,'batch_size':8,'workers':2,'seed':2027,
        ...
        'empty_pool_mask_by_arm':{'published_parent':False,'fit_terminal':True},
    ...
    before,parent_result=evaluate('published_parent',parent)
