...
def native_mask_geometry_loss(end_points, inputs, root_boxes, matches, batch_indices=None):
    assert root_boxes.ndim == 2 and root_boxes.shape[1] == 6
    if batch_indices is None:
        batch_indices = list(range(len(root_boxes)))
    assert len(batch_indices) > 0
    query_indices = root_query_indices([matches[index] for index in batch_indices], root_boxes.device)
    assert len(query_indices) == len(batch_indices)
    superpoints, _ = _find_superpoint_map(end_points, inputs)
    boxes = []
    for position, index in enumerate(batch_indices):
        _, _, fused, _ = normalize_mcln_mask_logits(end_points, index, query_indices[position:position + 1])
    ...
    targets = root_boxes[batch_indices].detach()
