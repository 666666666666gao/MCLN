"""Queue fixed F training behind its live preflight and measured disk requirement."""
import datetime
import hashlib
import json
import os
...
training_launch = launch(root, archive, 'mcln_pvg_rec_competition_v1')
...
queue = queue.replace("verified['fixed_visual_memory_verified']", "verified['rec_competition_verified']")
files = {'audit.py': (repo / 'scripts/audit_pvground_rec_competition.py').read_bytes(),
...
for name, raw in files.items():
    ...
    with s.open(audit + '/' + name, 'wx') as stream:
        stream.write(raw)
audit_launch = launch(audit, audit_archive, 'mcln_pvg_rec_competition_audit_v1')
