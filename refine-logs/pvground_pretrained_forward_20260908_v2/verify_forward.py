...
def main():
    ...
    # Transformers4.40 omits this deterministic buffer from state; local4.17 persists it.
    # Use the public registration API to match the author's serialization, retaining strict learned-weight loading.
    position_key = 'text_encoder.embeddings.position_ids'
    assert set(model.state_dict()) - set(state) == {position_key}
    ...
    strict_receipt = {'status':'pass', 'checkpoint_sha256':checkpoint['sha256'], 'state_tensors':len(state),
        ...
        'position_ids_serialization_port':{'shape':list(position_ids.shape),'persistent':False,'values_changed':False}}
