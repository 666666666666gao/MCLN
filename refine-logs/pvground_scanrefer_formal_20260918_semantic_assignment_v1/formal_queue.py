...
root=Path('/root/autodl-tmp/mcln_pvground_scanrefer_formal_20260918_semantic_assignment_v1')
...
assert audited['integrity_pass'] and audited['semantic_assignment_verified']
...
restore=subprocess.run([str(runtime/'venv/bin/python'),'-u',str(root/'restore_probe.py'),'--spec',str(root/'spec.json'),'--terminal'],env=dict(os.environ,**dict(environment['env'],CUDA_VISIBLE_DEVICES='',OMP_NUM_THREADS='1')))
assert restore.returncode==0, 'actual terminal CPU restore failed'
assert shutil.disk_usage('/root/autodl-tmp').free>=1024*1024**2, 'formal needs 256MiB exports plus768MiB reserve'
