...
## Sr下载截断后的单次恢复（17:29记录）

原20372进程因下载仅397533346字节触发长度/SHA断言退出，未上传。新13440进程从确切偏移进行一次Range续传，206及Content-Range检查通过，完整829830168字节与固定SHA校验通过；17:28:26远端.part已有144474112字节，尚在传输。CPU inventory/strict-load均未完成，预计17:45—17:50再查；原错误日志保留，不重复下载已校验完整文件。Scan此次终态bbs净-28/-12，独立审计一致，正式已按预设跳过；资源准备不代表提前批准Nr/Sr训练。


## Sr完整父权重CPU准备完成（20.191）

Sr官方epoch31权重完整829830168字节、两端SHA通过；实际butd_cls协议与Nr一致，1235状态张量含persistent position_ids。CPU严格加载全部键/值一致，未做完整模型GPU前向、反向、训练或正式评估。17:58:24复验远端SHA后删除本地830MB传输副本，远端保留。启动器screen后的进程计数断言失败只作为观察错误保留，实际CPU任务exit0/receipt PASS，没有重启。
