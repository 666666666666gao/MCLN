...
OUTPUT=$BASE/v124_meshsp_nested_semantic_learned_pareto_oof.json
LOG=$BASE/v124_meshsp_nested_semantic_learned_pareto_oof.log
EXIT=$BASE/v124_meshsp_nested_semantic_learned_pareto_oof.exit
SESSION=v124_nested_semantic_learned_pareto_oof
SELF=$(readlink -f "$0")
...
if [[ "${1:-}" == "--inner" ]]; then
    ...
    env CUDA_VISIBLE_DEVICES=0 \
        ...
        "$PYTHON" scripts/run_v124_meshsp_nested_semantic_learned_pareto_oof.py \
...
cd "$REPO"
require_sha a9f18cf7ae164be991ca3e981ae3ca6cf71f23de1aa82549e5670a5998befc32 \
    scripts/run_v124_meshsp_nested_semantic_learned_pareto_oof.py
...
for path in "$OUTPUT" "$LOG" "$EXIT"; do
    if [[ -e "$path" ]]; then
        echo "Refusing to overwrite existing V124 output: $path" >&2
        exit 74
    fi
done
if [[ $(nvidia-smi -L | wc -l) -ne 1 ]]; then
    echo "V124 requires the frozen single-GPU topology" >&2
    exit 75
fi
...
if pgrep -af '[r]un_v124_meshsp_nested_semantic_learned_pareto_oof.py' >/dev/null; then
    echo "V124 process already exists" >&2
    exit 75
fi
if screen -ls 2>/dev/null | grep -q "[.]$SESSION"; then
    echo "V124 screen session already exists" >&2
    exit 75
fi
available_kb=$(df -Pk /root/autodl-tmp | awk 'NR==2 {print $4}')
if [[ "$available_kb" -lt 2621440 ]]; then
    echo "V124 requires at least 2.5 GiB free on /root/autodl-tmp" >&2
    exit 75
fi
...
if ! screen -ls 2>/dev/null | grep -q "[.]$SESSION" \
        && [[ ! -e "$EXIT" ]]; then
    echo "V124 screen failed to remain alive and produced no exit receipt" >&2
    exit 75
fi
