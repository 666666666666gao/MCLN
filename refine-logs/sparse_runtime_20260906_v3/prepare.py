...
with (root/'base_packages_before.json').open('x') as stream:json.dump(before,stream,sort_keys=True)
...
for package in metadata['packages']:
 ...
 cached=Path('/root/autodl-tmp/mcln_sparse_runtime_20260906_v2')/wheel['filename']
...
"""
result=subprocess.run([python,'-c',check,str(venv)],capture_output=True,text=True)
