...
readonly BASE_RUN="/root/autodl-tmp/DATA_ROOT/output/network_v99_baseline_gt/sr3d/backbone/sr3d_mcln_joint_butdcls_v99_official_e21_plateau_lr10_e22_e25_b12a2_w4p2_20260827_230027/sr3d/sr3d_mcln_joint_butdcls_v99_official_e21_plateau_lr10_e22_e25_b12a2_w4p2/1787842834"
readonly REQUIRED_RESUME_CHECKPOINT="${BASE_RUN}/ckpt_epoch_25.pth"
readonly REQUIRED_RESUME_SHA256="03891fe23fb2a359cc74d62a018feca486c0daf76e8264fb6487d098f214f998"
readonly REQUIRED_RESUME_EPOCH=25
readonly REQUIRED_E23_RECEIPT="${BASE_RUN}/eval_metrics_epoch_23.json"
readonly REQUIRED_E23_RECEIPT_SHA256="8a71bb980f413b6150dd0dd13005b7d200bfbd698d21978fb322fa4399aadc3e"
readonly REQUIRED_E24_RECEIPT="${BASE_RUN}/eval_metrics_epoch_24.json"
readonly REQUIRED_E24_RECEIPT_SHA256="1112c858728e15f02278a1b0466986d225d6d0c9b14db55aa4b921bab5ed7760"
readonly REQUIRED_E25_RECEIPT="${BASE_RUN}/eval_metrics_epoch_25.json"
readonly REQUIRED_E25_RECEIPT_SHA256="ecc9aa4f0123b342062e82dad56ca7ae533aa338179930341107ad899edbe83b"
...
readonly EXP="sr3d_mcln_joint_butdcls_v99_official_e25_plateau_lr100_e26_e29_b12a2_w4p2"
...
require_sha "${REQUIRED_E23_RECEIPT}" "${REQUIRED_E23_RECEIPT_SHA256}" "E23 receipt"
require_sha "${REQUIRED_E24_RECEIPT}" "${REQUIRED_E24_RECEIPT_SHA256}" "E24 receipt"
require_sha "${REQUIRED_E25_RECEIPT}" "${REQUIRED_E25_RECEIPT_SHA256}" "E25 receipt"
require_sha "${REQUIRED_RESUME_CHECKPOINT}" "${REQUIRED_RESUME_SHA256}" "E25 checkpoint"

"${PYTHON_BIN}" - \
  "${REQUIRED_E23_RECEIPT}" \
  "${REQUIRED_E24_RECEIPT}" \
  "${REQUIRED_E25_RECEIPT}" \
...
e23_path, e24_path, e25_path, checkpoint_path = sys.argv[1:]
expected_hits = {23: 12116, 24: 12090, 25: 12085}
observed_hits = {}
for epoch, path in ((23, e23_path), (24, e24_path), (25, e25_path)):
...
if not (observed_hits[24] <= observed_hits[23] and
        observed_hits[25] <= observed_hits[23]):
    raise SystemExit("E24/E25 do not prove two consecutive non-refreshes")
...
if current_lrs != expected_current_lrs:
    raise SystemExit("E25 current LR mismatch")
expected_initial_lrs = [1e-4, 1e-3, 1e-4, 1.25e-4]
if [float(group["initial_lr"]) for group in optimizer["param_groups"]] != expected_initial_lrs:
    raise SystemExit("E25 initial LR mismatch")
required = {
    "dataset": ["sr3d"],
    "test_dataset": "sr3d",
    "batch_size": 12,
...
if int(scheduler["last_epoch"]) != 25 * 3243:
    raise SystemExit("scheduler progress mismatch")
if int(scheduler["_step_count"]) != 25 * 3243 + 1:
...
if [float(value) for value in scheduler["base_lrs"]] != expected_initial_lrs:
...
target_lrs = [1e-6, 1e-5, 1e-6, 1.25e-6]
if any(not math.isclose(value * 0.1, target, rel_tol=0.0, abs_tol=1e-15)
       for value, target in zip(expected_current_lrs, target_lrs)):
    raise SystemExit("target LR proof mismatch")
print(
    "SR3D_OFFICIAL_E25_PLATEAU_LR100_PREFLIGHT=PASS epoch=25 current_lrs={} "
    "target_lrs={} scheduler_steps={} plateau_hits={}".format(
        current_lrs, target_lrs, scheduler["last_epoch"], observed_hits
    )
)
PY
...
flock -u 8
exec 8>&-
echo "SR3D_OFFICIAL_E25_PLATEAU_LR100_RESOURCE_PREFLIGHT=PASS gpu_mib=${gpu_used} free_gib=${free_gb}"
