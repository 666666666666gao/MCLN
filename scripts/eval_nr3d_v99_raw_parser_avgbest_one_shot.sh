...
readonly CHECKPOINT="${DATA_ROOT}/output/network_v99_baseline_gt/nr3d/control/official_rec_monitor/official_best_rec025_epoch_57_0p56652741.pth"
...
readonly RUN_ROOT="${DATA_ROOT}/output/network_v99_raw_parser_eval/nr3d_avgbest_conservative_syntax_v1_one_shot"
readonly EXP="nr3d_mcln_joint_butdcls_v99_avgbest_raw_parser_conservative_syntax_v1_one_shot"
...
readonly BASELINE_HITS025=4475
readonly BASELINE_HITS050=3759
...
readonly MAIN_SHA256="a2a355042b7af61e17576941dec1fe79d3820805f83461e60679e0e3e00182f8"
...
verify_fixed_inputs() {
  ...
  require_sha256 "${CHECKPOINT}" "${CHECKPOINT_SHA256}" "average-best checkpoint"
...
for key, expected in expected_selector.items():
    ...
    if isinstance(expected, float):
        if actual is None or not math.isclose(
            ...
            raise SystemExit("checkpoint selector contract changed: " + key)
    elif actual != expected:
        raise SystemExit("checkpoint selector contract changed: " + key)
for key in (
        "use_sacr_source", "use_sacr_score_refiner", "use_source_moe",
        "use_joint_query_quality_reranker", "use_decoder_query_adapter"):
    if config.get(key, False) is not False:
        raise SystemExit("checkpoint unexpectedly enables " + key)
if checkpoint.get("evaluation_only") is not True:
    raise SystemExit("average-best checkpoint is not marked evaluation-only")
if "optimizer" in checkpoint or "scheduler" in checkpoint:
    raise SystemExit("evaluation-only checkpoint unexpectedly contains training state")
provenance = checkpoint.get("weight_average_provenance")
if not isinstance(provenance, dict):
    raise SystemExit("weight-average provenance is missing")
expected_provenance = {
    ...
    "other_sha256":
        "1e01d162a58c903e7b469f6e51d23314753e40c1d33a4e3c0d06239b55173421",
    "base_weight": 0.75,
    "other_weight": 0.25,
...
for key, expected in expected_provenance.items():
    ...
    if isinstance(expected, float):
        if actual is None or not math.isclose(
                float(actual), expected, rel_tol=0.0, abs_tol=1e-12):
            raise SystemExit("weight-average provenance changed: " + key)
    elif actual != expected:
        raise SystemExit("weight-average provenance changed: " + key)
...
print("preflight_provenance=Nr3D_V99_average_best_bundle_exact_verified")
PY
...
readonly SNAPSHOT_CHECKPOINT="${INPUT_SNAPSHOT}/average_best_checkpoint.pth"
...
copy_verified_snapshot \
  "${CHECKPOINT}" "${SNAPSHOT_CHECKPOINT}" "${CHECKPOINT_SHA256}" "average-best checkpoint"
copy_verified_snapshot \
...
provenance = {
    "schema": "mcln-nr3d-avgbest-raw-parser-consumed-provenance-v1",
...
verify_fixed_inputs
verify_fixed_code_root "${CODE_SNAPSHOT}"
require_sha256 "${SNAPSHOT_CHECKPOINT}" "${CHECKPOINT_SHA256}" "consumed average-best checkpoint"
...
if provenance.get("schema") != "mcln-nr3d-avgbest-raw-parser-consumed-provenance-v1":
...
decision = {
    "schema": "mcln-nr3d-raw-parser-avgbest-one-shot-decision-v1",
