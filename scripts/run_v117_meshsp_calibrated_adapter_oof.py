#!/usr/bin/env python
"""Train-only OOF screen of the V117 calibrated anchored adapter."""
...
def threshold_calibration_loss(
        outputs, candidate_ious, query_valid, variant_valid):
    """Calibrate absolute REC hit probabilities at 0.25 and 0.50."""
    variant_target = torch.stack((
        candidate_ious.gt(0.25), candidate_ious.gt(0.50)
    ), dim=-1).to(candidate_ious.dtype)
    query_target = variant_target.masked_fill(
        ~variant_valid.unsqueeze(-1), 0.0
    ).max(dim=2).values
    query_probability = v99.monotone_hit_probabilities(
        outputs["query_logits"]
    ).clamp(1e-6, 1.0 - 1e-6)
    variant_probability = v99.monotone_hit_probabilities(
        outputs["variant_logits"]
    ).clamp(1e-6, 1.0 - 1e-6)
    query_terms = F.binary_cross_entropy(
        query_probability, query_target, reduction="none"
    )
    variant_terms = F.binary_cross_entropy(
        variant_probability, variant_target, reduction="none"
    )
    query_weighted = (
        2.0 * query_terms[..., 0] + query_terms[..., 1]
    ) / 3.0
    variant_weighted = (
        2.0 * variant_terms[..., 0] + variant_terms[..., 1]
    ) / 3.0
    query_loss = query_weighted[query_valid].mean()
    variant_loss = variant_weighted[variant_valid].mean()
    loss = query_loss + variant_loss
    return loss, {
        "calibration_query_loss": float(query_loss.detach().item()),
        "calibration_variant_loss": float(variant_loss.detach().item()),
    }


def fit_v117(records, device):
    """Fit V99, freeze it, then fit the calibrated bounded adapter."""
    ...
    if not trainable or any(parameter.requires_grad
        ...
        raise RuntimeError("V117 anchor/trainable parameter contract changed")
    ...
    for epoch in range(HIERARCHICAL_EPOCHS):
        ...
        if model.anchor.training:
            raise RuntimeError("V117 frozen anchor entered training mode")
        ...
        totals = {
            ...
            "calibration_query_loss": 0.0,
            "calibration_variant_loss": 0.0,
        }
        batches = 0
        for start in range(0, len(order), HIERARCHICAL_BATCH_SIZE):
            ...
            listwise_loss, stats = v95.graded_listwise_loss(
                outputs,
                candidate_ious,
                model_batch["query_valid"],
                model_batch["variant_valid"],
            )
            calibration_loss, calibration_stats = threshold_calibration_loss(
                outputs,
                candidate_ious,
                model_batch["query_valid"],
                model_batch["variant_valid"],
            )
            loss = listwise_loss + calibration_loss
            optimizer.zero_grad(set_to_none=True)
            loss.backward()
            clip_grad_norm_(trainable, HIERARCHICAL_GRAD_CLIP_NORM)
            optimizer.step()
            totals["loss"] += float(loss.detach().item())
            totals["listwise_loss"] += float(listwise_loss.detach().item())
            totals["query_loss"] += stats["query_loss"]
            totals["variant_loss"] += stats["variant_loss"]
            totals["calibration_loss"] += float(
                calibration_loss.detach().item()
            )
            totals["calibration_query_loss"] += calibration_stats[
                "calibration_query_loss"
            ]
            totals["calibration_variant_loss"] += calibration_stats[
                "calibration_variant_loss"
            ]
            batches += 1
        adapter_epochs.append({
            "epoch": epoch + 1,
            "loss": totals["loss"] / batches,
            "listwise_loss": totals["listwise_loss"] / batches,
            "query_loss": totals["query_loss"] / batches,
            "variant_loss": totals["variant_loss"] / batches,
            "calibration_loss": totals["calibration_loss"] / batches,
            "calibration_query_loss": (
                totals["calibration_query_loss"] / batches
            ),
            "calibration_variant_loss": (
                totals["calibration_variant_loss"] / batches
            ),
        })
...
def _load_bound_receipt(path, expected_sha256, expected_schema):
    ...
    if not resolved.is_file() or resolved.is_symlink():
        raise ValueError("V117 receipt must be a regular non-symlink file")
    if file_sha256(resolved) != expected_sha256:
        raise ValueError("V117 receipt SHA-256 changed: {}".format(resolved))
    ...
    if (receipt.get("schema") != expected_schema
        ...
        raise ValueError("V117 receipt policy changed: {}".format(resolved))
    return receipt
...
def main(argv=None):
    ...
    v115_source_path = Path(args.v115_report).expanduser().absolute()
    v116_source_path = Path(args.v116_report).expanduser().absolute()
    if file_sha256(v115_source_path) != V115_REPORT_SHA256:
        raise ValueError("V115 report SHA-256 changed")
    if file_sha256(v116_source_path) != V116_REPORT_SHA256:
        raise ValueError("V116 report SHA-256 changed")
    v115_source = json.loads(v115_source_path.read_text(encoding="ascii"))
    v116_source = json.loads(v116_source_path.read_text(encoding="ascii"))
    v115_oof = v115_source.get("oof", {})
    v116_oof = v116_source.get("oof", {})
    if (v115_source.get("schema")
            != "rec-v115-anchored-spatial-residual-scene-oof-v1"
            or v116_source.get("schema")
            != "rec-v116-primary025-policy-scene-oof-v1"
            or v115_source.get("validation_data_accessed") is not False
            or v116_source.get("validation_data_accessed") is not False
            or v115_oof.get("passed") is not False
            or v116_oof.get("passed") is not False
            or v115_oof.get("diagnostics", {}).get("delta_hits025") != 75
            or v115_oof.get("diagnostics", {}).get("delta_hits050") != 263
            or v116_oof.get("diagnostics", {}).get("delta_hits025") != 72
            or v116_oof.get("diagnostics", {}).get("delta_hits050") != 264):
        raise ValueError("V115/V116 protocol-design source contract changed")
    ...
    for held in range(5):
        ...
        model, statistics, training = fit_v117(
        ...
        if not torch.equal(prediction["baselines"], baselines[held_indices]):
            raise RuntimeError("V117 OOF baseline identity changed")
    ...
    if protected_after != protected_before:
        raise RuntimeError("protected artifacts changed during V117")
    protected_metadata_after = {
        name: file_sha256(path)
        for name, path in protected_metadata_paths.items()
    }
    if protected_metadata_after != protected_metadata_before:
        raise RuntimeError("protected V117 cache metadata changed during OOF")
    script_sources = {
        "v117": str(Path(__file__).resolve()),
    ...
    report = {
        "schema": "rec-v117-calibrated-anchored-adapter-scene-oof-v1",
        "version": 1,
        "validation_data_accessed": False,
        ...
        "source": {
            ...
            "protocol_design_v115": {
                "path": str(v115_source_path),
                "sha256": V115_REPORT_SHA256,
                "oof": copy.deepcopy(v115_oof),
            },
            "protocol_design_v116": {
                "path": str(v116_source_path),
                "sha256": V116_REPORT_SHA256,
                "oof": copy.deepcopy(v116_oof),
            },
        },
        "protocol": {
            "architecture": (
                "V115 frozen-anchor spatial residual adapter with absolute "
                "two-threshold probability calibration"
            ),
            "objective": (
                "V95 graded-listwise plus normalized 2:1 pointwise BCE at "
                "REC@0.25 and REC@0.50"
            ),
            "calibration_loss_coefficient": 1.0,
            "calibration_threshold_weights": [2.0, 1.0],
    ...
    try:
        ...
        while written < len(payload):
            ...
            if count <= 0:
                raise OSError("V117 output write made no progress")
            written += count
        os.fsync(descriptor)
    finally:
        os.close(descriptor)
