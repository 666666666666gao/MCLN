"""CPU inspection of the exact official Nr3D checkpoint, without a model."""
import datetime
import hashlib
import json
import os
...
shape_differences={name:{'scan':scan[name],'nr':inventory[name]} for name in sorted(shared) if scan[name]!=inventory[name]}
...
print('NR_CPU_INVENTORY_COMPLETE '+json.dumps(receipt),flush=True)
