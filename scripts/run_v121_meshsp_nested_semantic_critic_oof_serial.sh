...
OUTPUT=$BASE/v121_meshsp_nested_semantic_critic_oof.json
LOG=$BASE/v121_meshsp_nested_semantic_critic_oof.log
EXIT=$BASE/v121_meshsp_nested_semantic_critic_oof.exit
SESSION=v121_nested_semantic_critic_oof
SELF=$(readlink -f "$0")
...
if [[ "${1:-}" == "--inner" ]]; then
    ...
    env CUDA_VISIBLE_DEVICES=0 \
        ...
        "$PYTHON" scripts/run_v121_meshsp_nested_semantic_critic_oof.py \
...
cd "$REPO"
require_sha 1517e0170ac1a103fa86b3d6423076b1081e79fae8ddc87267e90e4372ee33dc \
    scripts/run_v121_meshsp_nested_semantic_critic_oof.py
...
for path in "$OUTPUT" "$LOG" "$EXIT"; do
    if [[ -e "$path" ]]; then
        echo "Refusing to overwrite existing V121 output: $path" >&2
        exit 74
    fi
done
if [[ $(nvidia-smi -L | wc -l) -ne 1 ]]; then
    echo "V121 requires the frozen single-GPU topology" >&2
    exit 75
fi
...
if pgrep -af '[r]un_v121_meshsp_nested_semantic_critic_oof.py' >/dev/null; then
    echo "V121 process already exists" >&2
    exit 75
fi
if screen -ls 2>/dev/null | grep -q "[.]$SESSION"; then
    echo "V121 screen session already exists" >&2
    exit 75
fi
available_kb=$(df -Pk /root/autodl-tmp | awk 'NR==2 {print $4}')
if [[ "$available_kb" -lt 2621440 ]]; then
    echo "V121 requires at least 2.5 GiB free on /root/autodl-tmp" >&2
    exit 75
fi
...
if ! screen -ls 2>/dev/null | grep -q "[.]$SESSION" \
        && [[ ! -e "$EXIT" ]]; then
    echo "V121 screen failed to remain alive and produced no exit receipt" >&2
    exit 75
fi
