...
for number,case in enumerate(manifest['cases'][:options.limit]):
    ...
    draw.text((24,H+540),'EG',font=font(40,True),fill=RED)
    draw.text((150,16),case['dataset']+' / '+name+' / '+case['target_name'],font=font(26,True),fill=INK)
    draw.text((150,H*2+145),'IoU {:.4f} | 256-box oracle {:.4f}'.format(case['recomputed_iou'],case['full256_native_oracle_iou']),font=font(30,True),fill=RED)
    last_y=draw_wrapped(draw,case['description'].rsplit(' . not mentioned',1)[0],(150,H*2+205),W-40,font(29),line_height=40)
    panel=panel.crop((0,0,panel.width,last_y+40))
    panel.save(out/(case['case_id']+'_GT_EG.png'),dpi=(300,300))
    gt_image.save(out/(case['case_id']+'_GT.png'))
    pred_image.save(out/(case['case_id']+'_EG.png'))
...
if options.limit==6:
    ...
    for dataset in ('ScanRefer','Nr3D'):
        ...
        draw.text((24,H+610),'EG',font=font(39,True),fill=RED)
        for i,case in enumerate(selected):
            ...
            canvas.paste(Image.open(out/(case['case_id']+'_EG.png')),(x,H+170))
            y=H*2+190
            draw.text((x+35,y),'({}) {} | IoU {:.4f}'.format('abc'[i],case['target_name'],case['recomputed_iou']),font=font(30,True),fill=RED)
            draw.text((x+35,y+45),case['category']+' | oracle {:.3f}'.format(case['full256_native_oracle_iou']),font=font(23),fill=INK)
            end=draw_wrapped(draw,case['description'].rsplit(' . not mentioned',1)[0],(x+35,y+85),W-75,font(29),line_height=38)
            assert end<canvas.height-80,(case['case_id'],end)
        draw.text((155,canvas.height-62),'Green: GT | Red: EG prediction | Author ScanRefer epoch69 | Zero-update evaluation',font=font(25),fill=(75,75,75))
