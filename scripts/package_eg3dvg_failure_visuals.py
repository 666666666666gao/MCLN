...
assert len(render['cases']) == len(manifest['cases']) == 6
...
<p><strong>模型来源：</strong>作者 ScanRefer epoch69 完整预训练权重，零更新评估，原生 last/bbs 输出。Nr3D 是该 ScanRefer 权重的跨数据集起点，不是 Nr3D 训练终点；这些图片也不是历史 V99/E57 的预测。</p>
<p>Sr3D 正式预测尚未完成，本页暂不提供 Sr3D 图片。案例用于解释失败类型，不代表随机样本或整体准确率。Oracle 只用于离线诊断。</p>''']
verified = []
...
parts.append('</div><p><a href="paper_style_dense/ScanRefer_Nr3D_GT_EG_dense.pdf">下载两页合并 PDF</a> · <a href="cases.json">预测与输入来源记录</a></p><small>图中文字仅去掉模型追加的末尾 “. not mentioned”；完整实际输入保存在 cases.json。房间水平剖切仅用于显示，原始网格、GT 和预测框保持不变。</small></html>')
...
result = {'status': 'complete', 'cases': 6, 'datasets': ['ScanRefer', 'Nr3D'],
          'sr3d_complete': False, 'images': verified, 'new_model_forwards': 0,
...
打开 index.html 查看 ScanRefer、Nr3D 各三个不同场景。使用原始彩色三角网格，GT/EG 两排共享相机与裁切，绿色 GT、红色真实预测，白色描边便于区分。PNG 和合并 PDF 位于 paper_style_dense。

这是作者 ScanRefer epoch69 原权重的完整零更新评估输出；Nr3D 属于跨数据集起点，不是 Nr3D 适配终点。Sr3D 尚无本轮输出，没有替代或虚构案例。旧 V99/E57 可视化目录未修改。

六例全部核对实际模型输入点云 SHA、场景实例 GT、预测框和 IoU；mesh 按作者坐标对齐后与输入采样点最大最近距离小于 2e-6 米。完整来源、选择规则、框、哈希见 cases.json；渲染相机、剖切高度和像素位置见 paper_style_dense/dense_render_verification.json。候选上界只作 GT 离线诊断。案例经过显示筛选，不是总体指标估计。
