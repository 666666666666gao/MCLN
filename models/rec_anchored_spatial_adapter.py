"""Bounded language-spatial residual adapter over a frozen V99 anchor."""
...
from models.rec_hierarchical_reranker import VARIANT_COUNT
...
V115_HIDDEN_DIM = 128
V115_DROPOUT = 0.1
V115_RESIDUAL_SCALE = 0.25
V115_TARGET_TEXT_START = 64
V115_TARGET_TEXT_END = 128
V115_CENTER_START = 128
V115_CENTER_END = 131
...
class AnchoredLanguageSpatialResidualAdapter(nn.Module):
    """Keep a frozen contextual anchor and learn only bounded corrections."""

    def __init__(
            self, anchor, hidden_dim=V115_HIDDEN_DIM,
            dropout=V115_DROPOUT, residual_scale=V115_RESIDUAL_SCALE):
        super().__init__()
        if (not isinstance(anchor, HierarchicalQueryVariantReranker)
            ...
            raise TypeError(
                "V115 anchor must be the one-layer contextual hierarchy "
                "returned by the V99 factory"
            )
        if hidden_dim != V115_HIDDEN_DIM or anchor.hidden_dim != hidden_dim:
            raise ValueError("V115 requires hidden_dim=128")
        if float(residual_scale) != V115_RESIDUAL_SCALE:
            raise ValueError("V115 residual scale is frozen at 0.25")
    ...
    def forward(
        ...
        gate = torch.sigmoid(self.reliability_gate(anchor_context))
        fused_context = self.fusion_norm(
            anchor_context + gate * (spatial_context - anchor_context)
        )
