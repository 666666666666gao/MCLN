...
local = repo / 'refine-logs/scanrefer_probability_geometry_probe_20260907_v1'
...
files = {}
for name in ['scripts/probe_scanrefer_probability_geometry.py', 'scripts/scanrefer_data_contract.py', 'scripts/scanrefer_joint_readout.py', 'scripts/prototype_probability_geometry.py', 'tests/test_mask_geometry_gradient_contract.py', 'tests/test_probability_geometry_prototype.py']:
...
manifest.update(schema='mcln-probability-geometry-probe-v1', files=files, real_train_rows=16, batch_size=4,
    optimizer_steps=0, checkpoint_writes=0, formal_rows=0, new_network_modules=0,
    purpose='Check fixed sigmoid-mass inverse CDF boxes against hard-mask geometry before any training or integration.')
...
cd {remote}
flock -n /root/autodl-tmp/mcln_v99_backbone_gpu0.lock /root/miniconda3/envs/bdetr/bin/python -u scripts/probe_scanrefer_probability_geometry.py --manifest {remote}/input_manifest.json
status=$?
...
syntax = "import ast,pathlib; p=pathlib.Path(" + repr(remote + '/scripts/probe_scanrefer_probability_geometry.py') + "); ast.parse(p.read_text()); print('Python3.7 syntax PASS')"
...
assert gpu < 500
command = 'screen -L -Logfile ' + shlex.quote(remote + '/run.log') + ' -dmS mcln_probability_geometry_probe_v1 bash ' + shlex.quote(remote + '/controller.sh')
...
assert 'scripts/probe_scanrefer_probability_geometry.py' in live
proof = {'time_cst': datetime.datetime.now(datetime.timezone(datetime.timedelta(hours=8))).isoformat(),
    'screen': 'mcln_probability_geometry_probe_v1', 'remote_directory': remote, 'live': live,
