...
probe="import json,subprocess,shutil;print(json.dumps({'free_bytes':shutil.disk_usage('/root/autodl-tmp').free,'processes':[v for v in subprocess.check_output(['ps','-eo','pid,ppid,etimes,args']).decode().splitlines() if 'mcln_pvground_sr_native_batch_20260917_v1/' in v and ' -c ' not in v]}))"
