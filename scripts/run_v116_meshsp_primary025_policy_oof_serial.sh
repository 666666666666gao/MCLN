...
OUTPUT=$BASE/v116_meshsp_primary025_policy_oof.json
LOG=$BASE/v116_meshsp_primary025_policy_oof.log
EXIT=$BASE/v116_meshsp_primary025_policy_oof.exit
SESSION=v116_primary025_policy_oof
SELF=$(readlink -f "$0")
...
if [[ "${1:-}" == "--inner" ]]; then
    ...
    env CUDA_VISIBLE_DEVICES=0 \
        ...
        "$PYTHON" scripts/run_v116_meshsp_primary025_policy_oof.py \
...
cd "$REPO"
require_sha 340d214b80f4616c8ee3fd7c3c04071b11472389e84abb3ffd1db6a7ed671a3b \
    scripts/run_v116_meshsp_primary025_policy_oof.py
...
for path in "$OUTPUT" "$LOG" "$EXIT"; do
    if [[ -e "$path" ]]; then
        echo "Refusing to overwrite existing V116 output: $path" >&2
        exit 74
    fi
done
if [[ $(nvidia-smi -L | wc -l) -ne 1 ]]; then
    echo "V116 requires the frozen single-GPU topology" >&2
    exit 75
fi
...
if pgrep -af '[r]un_v116_meshsp_primary025_policy_oof.py' >/dev/null; then
    echo "V116 process already exists" >&2
    exit 75
fi
if screen -ls 2>/dev/null | grep -q "[.]$SESSION"; then
    echo "V116 screen session already exists" >&2
    exit 75
fi
available_kb=$(df -Pk /root/autodl-tmp | awk 'NR==2 {print $4}')
if [[ "$available_kb" -lt 2621440 ]]; then
    echo "V116 requires at least 2.5 GiB free on /root/autodl-tmp" >&2
    exit 75
fi
...
if ! screen -ls 2>/dev/null | grep -q "[.]$SESSION" \
        && [[ ! -e "$EXIT" ]]; then
    echo "V116 screen failed to remain alive and produced no exit receipt" >&2
    exit 75
fi
