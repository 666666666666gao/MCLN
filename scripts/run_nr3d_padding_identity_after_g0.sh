#!/usr/bin/env bash
# Read-only four-fit-row diagnostics, serialized after the complete G0 pair.
set -euo pipefail
...
export PYTHONDONTWRITEBYTECODE=1 PYTHONPATH="$PAIR_ROOT/padding_identity:$PWD"
exec "$PYTHON" -u "$PAIR_ROOT/padding_identity/scripts/audit_nr3d_padding_scene.py" \
  --checkpoint "$CHECKPOINT" \
  --masked-layer-source "$PAIR_ROOT/padding_encoder_decoder_layers.py" \
  --output "$PAIR_ROOT/padding_identity_receipt.json"
