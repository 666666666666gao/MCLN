#!/usr/bin/env python
"""Nested scene-OOF screen of the V119 midpoint break veto."""
...
V118_BREAK_COST = 4.0
V118_EVENT_WEIGHT = 4.0
V119_BREAK_VETO_THRESHOLD = -V118_BREAK_COST / 2.0
...
def signed_switch_targets(records, proposals, baselines):
    """Build training-only signed utilities: +1 fix, -4 break, 0 neutral."""
    candidate_ious = torch.stack([
    ...
    for suffix, threshold in (("025", 0.25), ("050", 0.50)):
        ...
        target = fixes.float() - V118_BREAK_COST * breaks.float()
...
def fit_pairwise_risk_head(features, targets, device):
    """Fit one fixed small signed-risk head on inner-held scene examples."""
    if (features.dtype != torch.float32
        ...
        raise ValueError("V118 risk training tensors changed")
    for column in range(2):
        values = set(targets[:, column].unique().tolist())
        if not {-V118_BREAK_COST, 0.0, 1.0}.issubset(values):
            raise ValueError("V118 inner calibration lacks fix/break classes")
    mean = features.mean(dim=0)
    std = features.std(dim=0, unbiased=False).clamp_min(
        v118_model.V118_MIN_STD
    )
    resolved = torch.device(device)
    _set_hierarchical_seed(resolved)
    model = v118_model.PairwiseSwitchRiskHead(mean, std).to(resolved)
    optimizer = torch.optim.AdamW(
        model.parameters(),
    ...
    event_weight = torch.where(
        train_targets.ne(0.0),
        torch.full_like(train_targets, V118_EVENT_WEIGHT),
        torch.ones_like(train_targets),
    )
    final_loss = None
    for _ in range(V118_RISK_EPOCHS):
        model.train()
        prediction = model(train_features)
        difference = (prediction - train_targets).abs()
        huber = torch.where(
            difference < 1.0,
            0.5 * difference.square(),
            difference - 0.5,
        )
        loss = (huber * event_weight).sum() / event_weight.sum()
        optimizer.zero_grad(set_to_none=True)
        loss.backward()
...
def predict_pairwise_risk(model, features, device):
    resolved = torch.device(device)
    model.to(resolved).eval()
    with torch.no_grad():
        scores = model(features.to(resolved)).cpu().float()
    if (tuple(scores.shape) != (features.shape[0], 2)
            or not bool(torch.isfinite(scores).all().item())):
        raise RuntimeError("V118 risk prediction contract changed")
    return scores
...
def main(argv=None):
    ...
    risk_scores = torch.zeros(EXPECTED_SAMPLE_COUNT, 2, dtype=torch.float32)
    folds = []
    for held in range(5):
        ...
        if (set(records[index]["scan_id"] for index in inner_fit_indices)
            ...
            raise RuntimeError("scene leakage into V119 inner calibration")
        ...
        inner_targets, _ = signed_switch_targets(
            inner_records,
        ...
        for column, suffix in enumerate(("025", "050")):
            ...
            target_counts[suffix] = {
                "fix": int(values.eq(1.0).sum().item()),
                "break": int(values.eq(-V118_BREAK_COST).sum().item()),
                "neutral": int(values.eq(0.0).sum().item()),
            }
        risk_model, risk_training = fit_pairwise_risk_head(
        ...
        if not torch.equal(prediction["baselines"], baselines[held_indices]):
            raise RuntimeError("V119 OOF baseline identity changed")
        fold_risk_scores = predict_pairwise_risk(
            risk_model, prediction["features"], args.device
        )
        fold_accepted = (
            prediction["base_accepted"]
            & fold_risk_scores[:, 0].gt(V119_BREAK_VETO_THRESHOLD)
            & fold_risk_scores[:, 1].gt(V119_BREAK_VETO_THRESHOLD)
        )
        ...
        risk_scores[held_indices] = fold_risk_scores
        fold_record = {
            "held_fold": held,
    ...
    risk_veto = base_accepted & ~accepted
    veto_reasons = {
        ...
        "nested_pairwise_risk_vetoes": int(risk_veto.sum().item()),
    ...
    if protected_after != protected_before:
        raise RuntimeError("protected artifacts changed during V119")
    protected_metadata_after = {
        name: file_sha256(path)
        for name, path in protected_metadata_paths.items()
    }
    if protected_metadata_after != protected_metadata_before:
        raise RuntimeError("protected V119 cache metadata changed during OOF")
    script_sources = {
        "v119": str(Path(__file__).resolve()),
    ...
    report = {
        "schema": "rec-v119-nested-midpoint-break-veto-scene-oof-v1",
        "version": 1,
        "validation_data_accessed": False,
        ...
        "protocol": {
            "architecture": (
                ...
                "general proposal-vs-baseline signed-risk MLP"
            ),
            "adapter_objective": "frozen V95 bounded threshold-aligned listwise",
            "risk_objective": (
                "event-weighted Huber regression to signed fix(+1), "
                "break(-4), neutral(0) utilities at both REC thresholds"
            ),
            "proposal_margin": v99.V97_MARGIN,
            "acceptance": (
                "V115 Pareto acceptance and signed risk above the fixed "
                "break(-4)/neutral(0) midpoint at both thresholds"
            ),
            "grid_search": False,
            "selection_data": (
            ...
            "risk_feature_dim": v118_model.V118_FEATURE_DIM,
            "risk_hidden_dim": v118_model.V118_HIDDEN_DIM,
            "risk_epochs": V118_RISK_EPOCHS,
            "risk_learning_rate": V118_RISK_LEARNING_RATE,
            "risk_weight_decay": V118_RISK_WEIGHT_DECAY,
            "risk_break_cost": V118_BREAK_COST,
            "risk_event_weight": V118_EVENT_WEIGHT,
            "risk_acceptance_threshold": V119_BREAK_VETO_THRESHOLD,
        ...
        "prediction_sha256": v99.tensor_sha256(
            ...
            base_accepted, risk_scores, accepted
        ),
        "oof": {
    ...
    try:
        ...
        while written < len(payload):
            ...
            if count <= 0:
                raise OSError("V119 output write made no progress")
            written += count
        os.fsync(descriptor)
    finally:
        os.close(descriptor)
