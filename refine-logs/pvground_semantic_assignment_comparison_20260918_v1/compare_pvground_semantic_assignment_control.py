"""Compare fixed D/G outputs to isolate the native semantic assignment; no model execution."""
import argparse
import datetime
import hashlib
import json
...
def compare(spec, stage):
    ...
    assert bool(configs[0].get('observation_state',False))==spec['control_observation_state']
    assert configs[1]['task_read'] and configs[0]['task_read']
    assert configs[1]['semantic_assignment'] and not configs[0].get('semantic_assignment',False)
    assert all(not cfg.get('fixed_visual_memory',False) for cfg in configs)
    assert configs[0]['task_module_sha256']==configs[1]['task_module_sha256']
    assert configs[0]['observation_module_sha256']==configs[1]['observation_module_sha256']
    assert configs[0]['source_query_module_sha256']==configs[1]['source_query_module_sha256']
    assert configs[0]['source_port_sha256']==configs[1]['source_port_sha256']
    assert sha(roots[1]/'pvground_task_observation_query.py')==configs[1]['task_module_sha256']
    if spec['control_observation_state']:
        assert sha(roots[0]/'pvground_observation_query.py')==configs[0]['observation_module_sha256']
