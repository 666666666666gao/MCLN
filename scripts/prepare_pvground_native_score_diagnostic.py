...
root = '/root/autodl-tmp/mcln_pvground_native_score_diagnostic_20260917_v2'
archive = repo/'refine-logs/pvground_native_score_diagnostic_20260917_v2'
...
assert read('/root/autodl-tmp/mcln_pvground_native_score_diagnostic_20260917_v1/controller.exit').strip() == b'143'
...
files = {'diagnose.py':(repo/'scripts/diagnose_pvground_native_score.py').read_bytes(),
         ...
         'plan.md':(repo/'docs/PVG_NATIVE_SCORE_CONTRACT_DIAGNOSTIC_2026-09-17.md').read_bytes()}
...
spec = dict(training_root=training,training_spec_sha256=sha(train_spec_raw),terminal_sha256=receipt['terminal_sha256'],
    ...
    training_rows=128,batch_size=8,model_forwards=16,optimizer_steps=0,formal_rows=0,
    scope='fixed trained D native loss versus bbs diagnostic; no model updates or formal evaluation',
...
command = 'screen -dmS mcln_pvg_native_score_diagnostic_v2 '+shlex.quote('/root/miniconda3/envs/bdetr/bin/python')+' -u '+shlex.quote(root+'/controller.py')
...
record = dict(time_cst=datetime.datetime.now(datetime.timezone(datetime.timedelta(hours=8))).isoformat(),
    root=root,process=process,expected_seconds=1200,first_check_after_seconds=300,
