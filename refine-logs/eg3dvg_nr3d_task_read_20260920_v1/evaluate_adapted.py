"""Fixed ScanRefer-to-Nr3D transfer with author object-input/filter protocol and same-forward diagnostics."""
import argparse
import datetime
import gzip
import hashlib
...
def main():
    ...
    model.cuda().eval()
    dataset = Joint3DDataset(dataset_dict={'nr3d': 1, 'scannet': 10}, test_dataset='nr3d',
                            split='val', data_path=spec['data_root'], use_color=True,
                            detect_intermediate=True, butd_cls=True)
    assert len(dataset) == 7899 and not dataset.augment
    assert not dataset.butd_gt and dataset.butd_cls and not dataset.butd
    expected_rows = 8 if args.stage == 'preflight' else 7899
        ...
        for batch in loader:
            for k, v in batch.items():
                if torch.is_tensor(v):
                    batch[k] = v.cuda()
            # Author Nr3D inputs include all scene GT instance boxes and predicted classes.
    ...
    receipt = {'status': 'complete', 'time_cst': now(), 'stage': args.stage, 'rows': count,
               ...
               'dataset': 'nr3d', 'initialization_dataset': 'scanrefer', 'experiment_type': 'single_epoch_adaptation', 'checkpoint_optimizer_steps': spec['checkpoint_optimizer_steps'],
