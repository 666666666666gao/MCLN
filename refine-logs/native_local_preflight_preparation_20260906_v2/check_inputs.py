...
for dset in ['nr3d','sr3d']:
    argv=entry.build_probe_argv(contract['eval_argv'],dset,annotation['protocols'][dset]['val']['total_rows'],data_root)
    ...
    for current,previous in zip(records,old_cpu['protocols'][dset]['records']):
...
result={'schema':'mcln-native-local-real-input-cpu-preparation-v2','status':'actual_cpu_inputs_pass',
