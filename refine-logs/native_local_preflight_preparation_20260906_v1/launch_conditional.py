...
prep=repo/'refine-logs/native_local_preflight_preparation_20260906_v1'
...
files={name:(prep/name).read_bytes() for name in ['annotation_receipt.json','preflight_rows.json','nr_contract.json']}
...
formal_root='/root/autodl-tmp/mcln_scanrefer_local_visual_official_20260906_v1'
...
assert formal['schema']=='mcln-scanrefer-local-visual-official-v1'
...
local=repo/'refine-logs/native_local_preflight_20260906_v1'
remote='/root/autodl-tmp/mcln_native_local_preflight_20260906_v1'
local.mkdir()
sftp.mkdir(remote)
manifest={'schema':'mcln-native-local-gpu-preflight-input-v1',
...
command='screen -dmS mcln_native_local_preflight_v1 bash -lc '+shlex.quote('cd '+shlex.quote(remote)+' && bash controller.sh > run.log 2>&1')
...
assert out.channel.recv_exit_status()==0 and 'mcln_native_local_preflight_v1' in sessions
launch={'time_cst':datetime.datetime.now(datetime.timezone(datetime.timedelta(hours=8))).isoformat(),
        'screen_session':[line.strip() for line in sessions.splitlines() if 'mcln_native_local_preflight_v1' in line],
