#!/usr/bin/env python
"""Train-only OOF screen of V114 language-conditioned spatial context."""
...
import models.rec_language_spatial_context as v114_model
...
MIN_DELTA_025 = 110
MIN_DELTA_050 = 220
MIN_BOOTSTRAP_LOWER_025 = 65
MIN_BOOTSTRAP_LOWER_050 = 165
...
def fit_v114(records, device):
    """Fit the fixed V114 architecture with the frozen V95 objective."""
    ...
    v95.HierarchicalQueryVariantReranker = (
        v114_model.LanguageConditionedSpatialHierarchicalReranker
    )
    try:
...
def _load_bound_receipt(path, expected_sha256, expected_schema):
    ...
    if not resolved.is_file() or resolved.is_symlink():
        raise ValueError("V114 receipt must be a regular non-symlink file")
    if file_sha256(resolved) != expected_sha256:
        raise ValueError("V114 receipt SHA-256 changed: {}".format(resolved))
    ...
    if (receipt.get("schema") != expected_schema
        ...
        raise ValueError("V114 receipt policy changed: {}".format(resolved))
    return receipt
...
def main(argv=None):
    ...
    for held in range(5):
        ...
        model, statistics, epochs = fit_v114(
        ...
        if not torch.equal(prediction["baselines"], baselines[held_indices]):
            raise RuntimeError("V114 OOF baseline identity changed")
    ...
    if protected_after != protected_before:
        raise RuntimeError("protected artifacts changed during V114")
    protected_metadata_after = {
        name: file_sha256(path)
        for name, path in protected_metadata_paths.items()
    }
    if protected_metadata_after != protected_metadata_before:
        raise RuntimeError("protected V114 cache metadata changed during OOF")
    script_sources = {
        "v114": str(Path(__file__).resolve()),
        "v114_model": str(Path(v114_model.__file__).resolve()),
        "v108": str(
    ...
    report = {
        "schema": "rec-v114-language-spatial-context-scene-oof-v1",
        "version": 1,
        "validation_data_accessed": False,
        ...
        "protocol": {
            "architecture": (
                "language-conditioned spatial multi-head attention over "
                "the 16 REC query candidates"
            ),
    ...
    try:
        ...
        while written < len(payload):
            ...
            if count <= 0:
                raise OSError("V114 output write made no progress")
            written += count
        os.fsync(descriptor)
    finally:
        os.close(descriptor)
