...
root='/root/autodl-tmp/mcln_pvground_source_query_checkpoint_audit_20260908_v2'
archive=repo/'refine-logs/pvground_source_query_checkpoint_audit_20260908_v2'
training='/root/autodl-tmp/mcln_pvground_scanrefer_finetune_20260908_sourcequery_v1'
train_spec=json.loads((repo/'refine-logs/pvground_scanrefer_finetune_20260908_sourcequery_v1/spec.json').read_bytes())
files={'check.py':(repo/'scripts/check_pvground_source_query_latest.py').read_bytes()}
...
record=[json.loads(line.split(' ',1)[1]) for line in lines if line.startswith('PVG_SOURCE_QUERY_LATEST_CPU_PASS ')]
...
controller=(repo/'refine-logs/pvground_scanrefer_formal_20260908_sourcequery_v1/controller.py').read_text()
controller=controller.replace('/root/autodl-tmp/mcln_pvground_scanrefer_formal_20260908_sourcequery_v1',root).replace("root/'formal_queue.py'","root/'checkpoint_queue.py'")
files['controller.py']=controller.encode()
files['plan.md']=b'One read-only CPU restore of first observed periodic checkpoint, after 22:52 CST; no training changes, new weights, scene inference or formal score. Uses the existing formal restore helpers. Original training PID25348; poll300s if snapshot not ready. Record actual observed step, not assumed step512.\n'
spec=dict(training_root=training,training_pid=25348,runtime=train_spec['runtime'],
    env_spec_sha256=train_spec['env_spec_sha256'],first_check_cst='2026-09-08T22:52:00+08:00',
...
_,out,err=c.exec_command('ps -p 25348 -o pid=,args=',timeout=30)
...
_,out,err=c.exec_command('screen -dmS mcln_pvg_sourcequery_checkpoint_audit_v2 bash -c '+shlex.quote(inner),timeout=30)
