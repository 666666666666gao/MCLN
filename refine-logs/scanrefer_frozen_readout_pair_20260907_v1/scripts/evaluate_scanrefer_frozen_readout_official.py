"""One fixed9508-row evaluation of protected V99, native-only control and frozen-readout-GT candidate."""
...
def verify_training_endpoint(directory):
    ...
    assert baseline['frozen_gt'] == baseline['native_only']
    ...
    for reference in [receipt['baseline_metrics']['frozen_gt'], receipt['terminal_metrics']['native_only']]:
        assert all(receipt['terminal_metrics']['frozen_gt']['rec_hits' + suffix] >= reference['rec_hits' + suffix]
...
def main():
    ...
    assert manifest['schema'] == 'mcln-scanrefer-frozen-readout-official-input-v1'
    ...
    assert audit['integrity_pass'] and audit['eligible_for_fixed_terminal_formal_evaluation']
    assert audit['receipt_sha256'] == manifest['training_receipt_sha256']
    assert manifest['candidate_predeclared'] == 'frozen_gt_v99'
    assert manifest['arms'] == ['protected_v99', 'native_only_v99', 'frozen_gt_v99']
    ...
    write_json(result_directory / 'protocol.json', {'authoritative_base_command': command,
        ...
        'native_loader_and_worker_seeding': True, 'formal_checkpoint_arm': 'frozen_gt', 'formal_control_arm': 'native_only', 'candidate_predeclared': 'frozen_gt',
    ...
    for arm, label in [('native_only', 'native_only_v99'), ('frozen_gt', 'frozen_gt_v99')]:
        trained = torch.load(receipt['checkpoints'][arm]['path'], map_location='cpu')
        assert trained['schema'] == 'mcln-scanrefer-frozen-readout-trained-state-v1'
        assert trained['arm'] == arm and trained['steps'] == 2482
        assert trained['manifest_sha256'] == receipt['manifest_sha256']
        assert_metadata_equal(trained['readout'], old_artifacts)
        model = copy.deepcopy(protected)
        model.load_state_dict(trained['model'], strict=True)
        models[label], artifacts[label], states[label] = model, trained['readout'], trained['model']
        del trained
    ...
    with torch.no_grad():
        for batch_index, raw in enumerate(test_loader):
            ...
            if batch_index == 0 or (batch_index + 1) % 128 == 0:
                ...
                count = len(records['frozen_gt_v99'])
                print('SCANREFER FROZEN READOUT OFFICIAL', json.dumps({'rows': count, 'total': 9508,
    ...
    for candidate_arm in ['native_only_v99', 'frozen_gt_v99']:
    ...
    result = {'schema': 'mcln-scanrefer-frozen-readout-official-v1', 'status': 'complete',
        ...
        'promotion': promotion_check(metrics['protected_v99'], metrics['frozen_gt_v99']),
        'manifest_sha256': file_sha(option.manifest), 'trained_checkpoints': receipt['checkpoints'],
        'candidate_predeclared': 'frozen_gt_v99', 'native_only_is_mechanism_control_not_validation_selected_candidate': True,
        'data_root': args.data_root, 'superpoint_inputs': data_inputs,
    ...
    print('SCANREFER FROZEN READOUT OFFICIAL COMPLETE', json.dumps(result), flush=True)
