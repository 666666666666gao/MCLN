...
root='/root/autodl-tmp/mcln_pvground_scanrefer_formal_20260909_observation_v1'
...
archive=repo/'refine-logs/pvground_scanrefer_formal_20260909_observation_v1'
manifest=json.loads((repo/'refine-logs/pvground_scanrefer_finetune_20260909_observation_v1/input_manifest.json').read_bytes())
spec=json.loads((old/'spec.json').read_bytes())
spec.update(training_root=training,training_audit_root=audit,training_audit_controller_pid=int(json.loads((repo/'refine-logs/pvground_scanrefer_endpoint_audit_20260909_observation_v1/launch.json').read_bytes())['process'].split()[0]),first_check_cst=(datetime.datetime.now(datetime.timezone(datetime.timedelta(hours=8)))+datetime.timedelta(minutes=165)).isoformat(),training_spec_sha256=hashlib.sha256((repo/'refine-logs/pvground_scanrefer_finetune_20260909_observation_v1/spec.json').read_bytes()).hexdigest())
...
require all 36 new state tensors. Recount preparation uses previous completed6887
...
files['restore_probe.py']=(repo/'scripts/check_pvground_observation_restore.py').read_bytes()
...
_,out,err=c.exec_command('screen -dmS mcln_pvg_observation_formal_v1 bash -c '+shlex.quote(inner),timeout=30)
