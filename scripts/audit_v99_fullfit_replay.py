#!/usr/bin/env python
"""Train-only replay audit for the frozen V99 full-fit artifact."""
...
from scripts.build_v99_pareto_contextual_artifact import load_v99_artifact
...
def _readonly_identity(path):
    ...
    if stat.S_ISLNK(entry.st_mode) or not stat.S_ISREG(entry.st_mode):
        raise ValueError("V99 protected artifact must be a regular file")
    mode = stat.S_IMODE(entry.st_mode)
    if mode != 0o444:
        raise ValueError("V99 protected artifact must have mode 0444")
    return {
...
def main(argv=None):
    ...
    parser.add_argument("--v99-artifact", required=True)
    parser.add_argument("--v99-oof", required=True)
    ...
    if _sha256(args.v99_artifact) != V99_ARTIFACT_SHA256:
        raise ValueError("V99 artifact SHA changed")
    if _sha256(args.v99_oof) != V99_OOF_SHA256:
        raise ValueError("V99 OOF SHA changed")
    ...
    protected_before["v99"] = _readonly_identity(args.v99_artifact)
    ...
    model, artifact = load_v99_artifact(
        args.v99_artifact, device=args.device,
        expected_artifact_sha256=V99_ARTIFACT_SHA256,
    ...
    if artifact["input_sha256"] != loaded["input_sha256"]:
        raise ValueError("V99 artifact input binding differs from live caches")
    if len(records) != artifact["fit"]["row_count"]:
        raise ValueError("V99 replay row count changed")
    ...
    if (row_sha != artifact["fit"]["deployable_rows_sha256"]
        ...
        raise ValueError("V99 replay rows differ from artifact fit evidence")
    ...
    if fold_sha != artifact["fit"]["scene_fold_sha256"]:
        raise ValueError("V99 replay scene fold identity changed")
    protected_after = capture_immutable_artifact_identities(protected_paths)
    protected_after["v99"] = _readonly_identity(args.v99_artifact)
    if protected_after != protected_before:
        raise RuntimeError("protected artifacts changed during V99 replay")

    oof = json.loads(Path(args.v99_oof).read_text(encoding="utf-8"))
    report = {
        "schema": "rec-v99-fullfit-train-replay-audit-v1",
        "version": 1,
        "validation_data_accessed": False,
        "contaminated_calibration_accessed": False,
        "weights_modified": False,
        "input_sha256": copy.deepcopy(loaded["input_sha256"]),
        "artifact_sha256": V99_ARTIFACT_SHA256,
        "oof_sha256": V99_OOF_SHA256,
