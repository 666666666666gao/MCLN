"""Observe missing raw Mask-branch evidence in one native protected evaluation.
...
def main():
    ...
    provenance = json.loads((addon / "data_provenance.json").read_text())
    for path, expected in provenance["files"].items():
        assert file_sha(Path(path)) == expected["sha256"], path
    ...
    from scripts.nr3d_mask_branch_diagnostic import diagnose_root_candidates
        ...
        def _main_eval_branch(self, *branch_args):
            ...
            for batch_row, row in enumerate(rows):
                ...
                row.update(id=index, scan_id=anno["scan_id"], target_id=anno["target_id"],
                           ...
                           normalized_token_count=len(anno["utterance"].split()),
    ...
    write_json(output / "preflight.json", {
        "schema": "mcln-nr3d-mask-branch-diagnostic-v1",
        ...
        "source_identity": "immutable_current_source_raw_mask_branch_observation",
    ...
    write_json(output / "receipt.json", {
        "schema": "mcln-nr3d-mask-branch-diagnostic-v1",
        "data_file_count": len(provenance["files"]), "data_hashes_before_after_match": True,
    ...
    print("MASK_BRANCH_DIAGNOSTIC_COMPLETE", json.dumps(summary), flush=True)
