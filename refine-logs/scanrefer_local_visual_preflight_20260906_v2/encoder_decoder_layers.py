...
class BiDecoderLayer(nn.Module):
    ...
    def forward(self, query, vis_feats, lang_feats, query_pos,
                ...
                detected_feats=None, detected_mask=None, local_visual_inputs=None):
        """
        Forward pass.
        Args:
        ...
        if self.local_visual is not None:
            local = self.local_visual((query + query_pos).transpose(0, 1),
                                      candidate_boxes, *local_visual_inputs)
            query2 = query2 + local.transpose(0, 1)
