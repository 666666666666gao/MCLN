...
local = repo / 'refine-logs/native_range_preparation_20260907_v1'
remote = '/root/autodl-tmp/mcln_native_range_preparation_20260907_v1'
...
for name, raw in files.items():
    ...
    with sftp.open(remote + '/' + name, 'wx') as stream:
        stream.write(raw)
command = 'screen -dmS mcln_native_range_cpu_prep_v1 bash -c ' + shlex.quote('exec bash ' + remote + '/controller.sh > ' + remote + '/run.log 2>&1')
...
assert output.channel.recv_exit_status() == 0, error.read().decode()
matches = [line.strip() for line in sessions.splitlines() if '.mcln_native_range_cpu_prep_v1' in line]
