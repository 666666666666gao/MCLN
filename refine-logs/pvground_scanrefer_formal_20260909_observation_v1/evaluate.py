...
def expanded_parent_state(model, parent):
    ...
    assert len(added) == 36 and all(name.startswith('decoder.5.source_query_read.') for name in added)
    assert sum(p.numel() for p in model.decoder[-1].source_query_read.parameters()) == 757728
    state = dict(parent)
