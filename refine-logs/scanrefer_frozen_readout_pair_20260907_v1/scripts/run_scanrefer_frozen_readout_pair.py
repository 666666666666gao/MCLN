"""Fixed native-GT versus frozen-readout-GT coupling from protected E71."""
...
def main():
    ...
    assert manifest['schema'] == 'mcln-scanrefer-frozen-readout-training-input-v1'
    ...
    assert native_probe['readout_frozen_and_unchanged'] and native_probe['formal_rows'] == 0
    ...
    readouts = {'native_only': first_readout, 'frozen_gt': first_readout}
    ...
    class FitDataset(Joint3DDataset):
        def _scene_graph_parse(self, annos):
            ...
            for index, row in enumerate(annos):
                row['_compatibility_training_id'] = index
        ...
        def __getitem__(self, index):
            ...
            result['joint_training_id'] = self.annos[index]['_compatibility_training_id']
            return result
    ...
    assert [row['_compatibility_training_id'] for row in dataset.annos] == list(range(36665))
    ...
    write_json(directory / 'protocol.json', {'row_ids': partitions, 'physical_spaces': spaces,
        ...
        'readout_trainable_tensors': [], 'data_root': manifest['data_root'], 'superpoint_inputs': data_inputs})
    ...
    def evaluate(stage):
        ...
        with torch.no_grad():
            for batch_index, raw in enumerate(loader('holdout', 1000, False)):
                ...
                if batch_index == 0 or (batch_index + 1) % 128 == 0:
                    print('SCANREFER FROZEN READOUT EVAL', json.dumps({'stage': stage,
                        'rows': len(records['frozen_gt']), 'total': len(partitions['holdout']),
        ...
        print('SCANREFER FROZEN READOUT EVAL COMPLETE', json.dumps({'stage': stage, 'metrics': metrics,
    ...
    assert baseline['native_only'] == baseline['frozen_gt']
    assert baseline_metrics['native_only'] == baseline_metrics['frozen_gt']
    ...
    for raw in loader('fit', 0, True):
        ...
        for arm, model in models.items():
            ...
            outputs = model(inputs)
            readout = readouts[arm](outputs, inputs, detach_visual=(arm == 'native_only'))
        ...
        if step == 1 or step % 64 == 0 or step == manifest['steps_per_arm']:
            ...
            print('SCANREFER FROZEN READOUT TRAIN', json.dumps({'step': step, 'total': manifest['steps_per_arm'],
    ...
    for arm, model in models.items():
        ...
        checkpoint = directory / (arm + '_frozen_readout_state.pt')
        assert not checkpoint.exists()
        # Preserve the completed fit endpoint before the potentially long terminal evaluation.
        torch.save({'schema': 'mcln-scanrefer-frozen-readout-trained-state-v1', 'arm': arm, 'steps': step,
            ...
            'decision_rule': 'updated core; unchanged pretrained Parent/Geometry/V99 and fixed Pareto'}, checkpoint)
    ...
    effects = {reference: {str(t): paired_effect(
        baseline['frozen_gt'] if reference == 'baseline' else terminal['native_only'],
        terminal['frozen_gt'], 'rec_iou', t) for t in (.25, .5)} for reference in ('baseline', 'native_only')}
    eligible = all(effect['net'] >= 0 for comparison in effects.values() for effect in comparison.values())
    receipt = {'schema': 'mcln-scanrefer-frozen-readout-pair-v1', 'status': 'complete',
        ...
        'frozen_readout_rec_effects': effects, 'eligible_for_fixed_terminal_formal_evaluation': eligible,
    ...
    print('SCANREFER FROZEN READOUT PAIR COMPLETE', json.dumps(receipt), flush=True)
