...
readonly PARENT_LEAF="${OUTPUT_ROOT}/backbone/nr3d_mcln_joint_butdcls_v99_e59_lr10_e60_e62_b16a1_20260829_201052/nr3d/nr3d_mcln_joint_butdcls_v99_e59_lr10_e60_e62_b16a1/1788005461"
readonly REQUIRED_RESUME_CHECKPOINT="${PARENT_LEAF}/ckpt_epoch_62.pth"
readonly REQUIRED_RESUME_SHA256="b0378ff8c7c69d902884130ac5af1578fc6d60b105617eeac2c2c57f7a605d2e"
readonly REQUIRED_RESUME_EPOCH=62
readonly E60_RECEIPT="${PARENT_LEAF}/eval_metrics_epoch_60.json"
readonly E60_RECEIPT_SHA256="600ebbd196cec2d42274ed204bde672f9d32e11042491185d4fecadf4468e43f"
readonly E61_RECEIPT="${PARENT_LEAF}/eval_metrics_epoch_61.json"
readonly E61_RECEIPT_SHA256="5019a985a57546c770e93c4e373b12b43c4e3cb6b80978f02a4d084d35734060"
readonly E62_RECEIPT="${PARENT_LEAF}/eval_metrics_epoch_62.json"
readonly E62_RECEIPT_SHA256="6ebdd06443df3fe8d395c9a99910908fb1cb851d841291eb0a94cfba785f6c44"
...
readonly EXP="nr3d_mcln_joint_butdcls_v99_e62_continue_e63_e65_b16a1"
readonly TRAIN_SCREEN_NAME="mcln_nr3d_e62_continue"
readonly BATCH_SIZE=16
readonly MAX_EPOCH=65
readonly EXPECTED_EVAL_SAMPLE_COUNT=7899
readonly MASTER_PORT=5324
...
require_sha "${E60_RECEIPT}" "${E60_RECEIPT_SHA256}" "E60 formal receipt"
require_sha "${E61_RECEIPT}" "${E61_RECEIPT_SHA256}" "E61 formal receipt"
require_sha "${E62_RECEIPT}" "${E62_RECEIPT_SHA256}" "E62 formal receipt"
require_sha "${REQUIRED_RESUME_CHECKPOINT}" "${REQUIRED_RESUME_SHA256}" "E62 resume checkpoint"

PROOF_E60_RECEIPT="${E60_RECEIPT}" \
PROOF_E61_RECEIPT="${E61_RECEIPT}" \
PROOF_E62_RECEIPT="${E62_RECEIPT}" \
...
protected_hits = 4463
target_hits = 4724
expected = ((60, 4342, 3643), (61, 4321, 3616), (62, 4363, 3669))
for (epoch, expected_hits025, expected_hits050), env_name in zip(
        expected,
        ("PROOF_E60_RECEIPT", "PROOF_E61_RECEIPT", "PROOF_E62_RECEIPT")):
...
if expected[-1][1] <= max(row[1] for row in expected[:-1]):
    raise SystemExit("E62 did not refresh the low-LR branch")
...
with open(checkpoint_path, "rb") as handle:
    ...
    if digest.hexdigest() != os.environ["PROOF_CHECKPOINT_SHA256"]:
        raise SystemExit("E62 checkpoint SHA mismatch")
    handle.seek(0)
    ...
    if digest_after.hexdigest() != digest.hexdigest():
        raise SystemExit("E62 checkpoint changed while loading")
...
if int(checkpoint.get("epoch", -1)) != 62:
    raise SystemExit("resume checkpoint is not E62")
...
if len(groups) != 4 or len(optimizer.get("state", {})) != 716:
    raise SystemExit("E62 optimizer topology mismatch")
if [float(group["lr"]) for group in groups] != expected_current_lrs:
    raise SystemExit("E62 current LR mismatch")
if [float(group["initial_lr"]) for group in groups] != expected_initial_lrs:
    raise SystemExit("E62 initial LR mismatch")
if scheduler.get("base_lrs") != expected_initial_lrs:
    raise SystemExit("E62 scheduler base LR mismatch")
if scheduler.get("_last_lr") != expected_current_lrs:
    raise SystemExit("E62 scheduler current LR mismatch")
if int(scheduler.get("last_epoch", -1)) != 173972:
    raise SystemExit("E62 scheduler progress mismatch")
if int(scheduler.get("_step_count", -1)) != 173973:
    raise SystemExit("E62 scheduler step-count mismatch")
if dict(scheduler.get("milestones", {})) != {423706: 1}:
    raise SystemExit("E62 scheduler milestone mismatch")
print("extension_proof=E60:4342,E61:4321,E62:4363,protected:4463,target:4724 resume=E62 same_lr=E63-E65")
PY
