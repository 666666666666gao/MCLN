...
root='/root/autodl-tmp/mcln_pvground_scanrefer_formal_20260917_rec_competition_v1'
training='/root/autodl-tmp/mcln_pvground_scanrefer_finetune_20260917_rec_competition_v1'
audit='/root/autodl-tmp/mcln_pvground_scanrefer_endpoint_audit_20260917_rec_competition_v1'
archive=repo/'refine-logs/pvground_scanrefer_formal_20260917_rec_competition_v1'
manifest=json.loads((repo/'refine-logs/pvground_scanrefer_finetune_20260917_rec_competition_v1/input_manifest.json').read_bytes())
spec=json.loads((old/'spec.json').read_bytes())
spec.update(training_root=training,training_audit_root=audit,training_audit_controller_pid=int(json.loads((repo/'refine-logs/pvground_scanrefer_endpoint_audit_20260917_rec_competition_v1/launch.json').read_bytes())['process'].split()[0]),first_check_cst=(datetime.datetime.now(datetime.timezone(datetime.timedelta(hours=8)))+datetime.timedelta(minutes=180)).isoformat(),training_spec_sha256=hashlib.sha256((repo/'refine-logs/pvground_scanrefer_finetune_20260917_rec_competition_v1/spec.json').read_bytes()).hexdigest())
files={'audit.py':(old/'audit.py').read_bytes()}
files['plan.md']=('''# Native bbs competition formal continuation
...
3723-step endpoint passes integrity, actual native bbs competition audit and both bbs thresholds vs its fixed initial.
Before GPU inference, restore the actual terminal delta into the full model on CPU;
require all 37 new state tensors. Recount preparation uses previous completed6887
...
files['evaluate.py']=(repo/'scripts/evaluate_pvground_scanrefer_task_observation.py').read_bytes()
files['pvground_source_query.py']=(repo/'models/pvground_source_query.py').read_bytes()
files['pvground_observation_query.py']=(repo/'models/pvground_observation_query.py').read_bytes()
files['pvground_task_observation_query.py']=(repo/'models/pvground_task_observation_query.py').read_bytes()
files['restore_probe.py']=(repo/'scripts/check_pvground_task_observation_restore.py').read_bytes()
...
for name in ['formal_queue.py','controller.py','cpu_probe.py']:
    ...
    if name=='formal_queue.py':
        text=text.replace('import datetime,hashlib,json,os,subprocess,time','import datetime,hashlib,json,os,shutil,subprocess,time')
        text=text.replace("assert audited['integrity_pass']","assert audited['integrity_pass'] and audited['rec_competition_verified']")
        ...
        check+="assert shutil.disk_usage('/root/autodl-tmp').free>=1024*1024**2, 'formal needs 256MiB exports plus768MiB reserve'\n"
...
_,out,err=c.exec_command('screen -dmS mcln_pvg_rec_competition_formal_v1 bash -c '+shlex.quote(inner),timeout=30)
...
record=dict(time_cst=datetime.datetime.now(datetime.timezone(datetime.timedelta(hours=8))).isoformat(),process=process,screen='mcln_pvg_rec_competition_formal_v1',first_check_cst=spec['first_check_cst'],poll_seconds=300,formal_rows=0,cpu_probe_source='previous completed6887 recount plus actual CPU model restore with in-memory fixture; no terminal claim',training_changes=False)
