"""Compare saved native-control and empty-pool outputs; never run the model."""
import argparse
import datetime
import hashlib
import json
...
def compare(spec, stage):
    ...
    for key in ['checkpoint_sha256','seed','batch_size','fit_passes','lr','lr_backbone',
                'input_manifest','env_spec_sha256','model_source','source_port','primary_mode']:
        assert configs[0][key]==configs[1][key],key
    assert [sha(p/'spec.json') for p in roots]==spec['training_spec_sha256']
    assert 'empty_pool_mask' not in configs[0] and configs[1]['empty_pool_mask'] is True
    assert sha(roots[1]/'pvground_empty_pool_mask.py')==configs[1]['empty_pool_module_sha256']
    receipts=[];sets=[]
    for root in roots:
