...
def main():
    ...
    # Local zero is valid in both batches; only BallQuery's mask identifies empty.
    global_indices, valid = stack_indices_to_global(
        np.array([[0, 1], [0, 0], [0, 2], [0, 0]]),
        np.array([False, True, False, True]), np.array([2, 3]), np.array([2, 2]))
    assert np.array_equal(global_indices, [[0, 1], [-1, -1], [2, 4], [-1, -1]])
    assert np.array_equal(valid, [True, False, True, False])
    print(json.dumps({'status': 'pass', 'scope': 'five synthetic queries; not real scene coverage',
                      'batch_local_conversion_queries': 4,
                      'converted_global_indices': global_indices.tolist(),
