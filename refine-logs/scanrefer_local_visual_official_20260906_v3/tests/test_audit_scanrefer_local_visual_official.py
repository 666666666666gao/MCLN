...
def formal_fixture(directory, local_iou):
    ...
    manifest = {'formal_rows': 9508, 'trained_checkpoint': {'fixture': True},
                'data_root': '/synthetic/mesh',
                'val_superpoint_files': {str(i): 'unit fixture' for i in range(312)},
    ...
    data_inputs = {'split': 'val', 'files_verified': 312, 'resolved_directory': '/synthetic/mesh/superpoints/val'}
    save(result / 'protocol.json', {'rows': 9508, 'native_loader_and_worker_seeding': True,
                                   ...
                                   'authoritative_base_command': ['--data_root', manifest['data_root']],
    ...
    receipt = {'schema': 'mcln-scanrefer-local-visual-official-v2', 'status': 'complete',
               'data_root': manifest['data_root'], 'superpoint_inputs': data_inputs,
...
def test_historical_command_root_cannot_override_pinned_mesh_inputs(tmp_path):
    formal_fixture(tmp_path, .8)
    path = tmp_path / 'result/protocol.json'
    protocol = json.loads(path.read_text())
    protocol['authoritative_base_command'][1] = '/old/mixed'
    save(path, protocol)
    with pytest.raises(AssertionError):
        audit_rows(tmp_path)
