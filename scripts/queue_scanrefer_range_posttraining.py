"""Complete the fixed range experiment without choosing epochs or validation arms."""
...
def main():
    ...
    while not (training / 'controller.exit').exists():
        ...
        assert process.returncode == 0 and b'mcln_scanrefer_range_pair_v1' in process.stdout, process.stdout.decode()
        ...
        progress = {}
        for line in lines:
            for label in ['SCANREFER RANGE EVAL ', 'SCANREFER RANGE EVAL COMPLETE ', 'SCANREFER RANGE TRAIN ']:
    ...
    with (directory / 'training_audit.txt').open('xb') as stream:
        process = subprocess.run([PYTHON, '-m', 'scripts.audit_scanrefer_range_pair', str(training), str(audit_path)],
    ...
    formal_manifest = {
        'schema': 'mcln-scanrefer-range-official-input-v1', 'training_directory': str(training),
        ...
        'data_root': training_manifest['data_root'], 'val_superpoint_files': training_manifest['superpoint_files']['val'],
        ...
        'candidate_predeclared': 'local_v99', 'arms': ['protected_v99', 'center_v99', 'local_v99'],
    ...
    command = ['flock', '-x', '/root/autodl-tmp/mcln_v99_backbone_gpu0.lock', PYTHON, '-u',
               'scripts/evaluate_scanrefer_range_official.py', '--manifest', str(formal / 'input_manifest.json')]
    ...
    with (directory / 'formal_audit.txt').open('xb') as stream:
        process = subprocess.run([PYTHON, '-m', 'scripts.audit_scanrefer_range_official', str(formal), str(final_audit)],
    ...
    decision = {'time_cst': datetime.datetime.now(ZONE).isoformat(), 'training_audit_sha256': sha(audit_path),
        ...
        'native_range_preflight_required': final['promotion']['advance_to_nr3d_sr3d_rec'],
