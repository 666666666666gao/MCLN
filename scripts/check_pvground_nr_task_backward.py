"""One gated Nr3D D-task forward/backward, with no optimizer update or saved model."""
import argparse
...
def main():
    ...
    assert formal==Path('/root/autodl-tmp/mcln_pvground_scanrefer_formal_20260917_task_observation_v1')
    ...
    record=dict(status='pass',time_cst=datetime.datetime.now(datetime.timezone(datetime.timedelta(hours=8))).isoformat(),
        ...
        scope='one fixed augmented eight-row D backward; not training, formal accuracy or full-dataset capacity')
    (args.output/'receipt.json').write_text(json.dumps(record,indent=2,allow_nan=False)+'\n')
    print('PVG_NR_TASK_BACKWARD_PASS '+json.dumps(record),flush=True)
