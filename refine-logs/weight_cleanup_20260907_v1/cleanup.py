...
for arm,item in plan['delete'].items():
 path=Path(item['path'])
 assert path.resolve()==path and path.parent==old and path.name==arm+'_range_visual_state.pt'
...
for path,digest in plan['protected_files'].items():assert sha(path)==digest,path
receipt={'schema':'mcln-sealed-failed-range-weight-cleanup-v1','status':'complete',
