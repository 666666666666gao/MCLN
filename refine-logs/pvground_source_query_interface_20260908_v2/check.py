...
def main():
    ...
    replay_output=paired_replay(replay_inputs,'train_before_update')
    del replay_output,replay_inputs,replay_batch
    ...
    receipt.update(source_query_read=True,module_sha256=sha(Path(__file__).parent/'pvground_source_query.py'),model_source=str(model_source),source_port_sha256=sha(model_source.parent/'source_port.json'),zero_residual_rec_replay_exact=True,replay_records=replay_records,mask_bitwise_equality_claim=False,new_parameters=sum(p.numel() for p in reader.parameters()))
