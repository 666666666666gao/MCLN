...
OUTPUT=$BASE/v122_meshsp_nested_semantic_pairwise_oof.json
LOG=$BASE/v122_meshsp_nested_semantic_pairwise_oof.log
EXIT=$BASE/v122_meshsp_nested_semantic_pairwise_oof.exit
SESSION=v122_nested_semantic_pairwise_oof
SELF=$(readlink -f "$0")
...
if [[ "${1:-}" == "--inner" ]]; then
    ...
    env CUDA_VISIBLE_DEVICES=0 \
        ...
        "$PYTHON" scripts/run_v122_meshsp_nested_semantic_pairwise_oof.py \
...
cd "$REPO"
require_sha 429ba348623f966ab1c5813bec37bd2a0f52191b508a979e9883bcbec6b49a84 \
    scripts/run_v122_meshsp_nested_semantic_pairwise_oof.py
require_sha 181f19f041c9a652550442710c010a01d6065da03c3cfaa726172235a8a6676c \
    models/rec_semantic_pairwise_utility.py
...
for path in "$OUTPUT" "$LOG" "$EXIT"; do
    if [[ -e "$path" ]]; then
        echo "Refusing to overwrite existing V122 output: $path" >&2
        exit 74
    fi
done
if [[ $(nvidia-smi -L | wc -l) -ne 1 ]]; then
    echo "V122 requires the frozen single-GPU topology" >&2
    exit 75
fi
...
if pgrep -af '[r]un_v122_meshsp_nested_semantic_pairwise_oof.py' >/dev/null; then
    echo "V122 process already exists" >&2
    exit 75
fi
if screen -ls 2>/dev/null | grep -q "[.]$SESSION"; then
    echo "V122 screen session already exists" >&2
    exit 75
fi
available_kb=$(df -Pk /root/autodl-tmp | awk 'NR==2 {print $4}')
if [[ "$available_kb" -lt 2621440 ]]; then
    echo "V122 requires at least 2.5 GiB free on /root/autodl-tmp" >&2
    exit 75
fi
...
if ! screen -ls 2>/dev/null | grep -q "[.]$SESSION" \
        && [[ ! -e "$EXIT" ]]; then
    echo "V122 screen failed to remain alive and produced no exit receipt" >&2
    exit 75
fi
