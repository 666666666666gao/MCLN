"""CPU-only source and retained-resource checks for the gated Nr E entry."""
import json
import os
import shlex
...
root='/root/autodl-tmp/mcln_pvground_nr_fixed_memory_preparation_20260917_v1'
...
'''
compile(probe,'prepare.py','exec')
client=paramiko.SSHClient();client.load_system_host_keys()
client.connect('region-9.autodl.pro',port=33476,username='root',password=os.environ['MCLN_SSH_PASSWORD'],timeout=30)
sftp=client.open_sftp();sftp.mkdir(root)
files={'check.py':(repo/'scripts/check_pvground_nr_fixed_memory_backward.py').read_bytes(),'prepare.py':probe.encode()}
for name,raw in files.items():
    (archive/name).write_bytes(raw)
    with sftp.open(root+'/'+name,'wb') as stream:stream.write(raw)
_,stdout,stderr=client.exec_command('/root/autodl-tmp/mcln_pvground_runtime_20260908_v1/venv/bin/python -u '+shlex.quote(root+'/prepare.py'),timeout=60)
output=stdout.read().decode();errors=stderr.read().decode()
code=stdout.channel.recv_exit_status()
(archive/'prepare.log').write_text(output+errors,encoding='utf-8')
(archive/'prepare.exit').write_text(str(code)+'\n',encoding='utf-8')
assert code==0,errors
for name in ['receipt.json','gate_rejection.log']:sftp.get(root+'/'+name,str(archive/name))
