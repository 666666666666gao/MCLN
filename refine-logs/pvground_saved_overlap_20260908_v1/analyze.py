...
def analyze(output):
    ...
    comparisons = {}
    for after in ['native_terminal', 'empty_pool_initial']:
        before = 'native_initial'
        ...
        for mode in ['bbs', 'bbf']:
            ...
            comparisons[after][mode] = transitions(old, new)
            comparisons[after][mode]['both_root_unique_max'] = dict(
    ...
    result = dict(status='complete', time_cst=datetime.datetime.now(datetime.timezone(datetime.timedelta(hours=8))).isoformat(),
        ...
        snapshots=metrics, comparisons_from_native_initial=comparisons,
        limitations='Maximum annotated box overlap is a geometric proxy, not semantic identity. Includes structural and nested objects; same-label means exact raw annotation label. Both-root comparisons do not separate reranking from regression changes. Initial empty-pool output is not its training endpoint. Backbone-seen module holdout, not unseen-scene generalization.')
