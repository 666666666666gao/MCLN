...
def assert_metadata_equal(current, original):
    """The three real artifacts contain nested normalization tensors and lists."""
    import torch

    assert type(current) is type(original)
    if torch.is_tensor(original):
        assert current.dtype == original.dtype and torch.equal(current, original)
    elif isinstance(original, dict):
        assert set(current) == set(original)
        for key in original:
            assert_metadata_equal(current[key], original[key])
