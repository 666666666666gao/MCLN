#!/usr/bin/env python
"""Launch and seal the one frozen official REC selective residual evaluation."""
...
OFFICIAL_EXPERIMENT = "epoch71_selective_residual_official"
...
OFFICIAL_RESULT_SCHEMA = "rec-selective-residual-official-validation-result"
...
OFFICIAL_GOAL_NAME = "epoch71_rec_selective_residual_official_validation_once"
OFFICIAL_SELECTION_RECORD_PATH = Path(
    "/root/autodl-tmp/DATA_ROOT/output/rec_reranker/e71_top16/"
    "residual_artifacts/selection.json"
)
OFFICIAL_SELECTED_RESIDUAL_ARTIFACT_PATH = Path(
    "/root/autodl-tmp/DATA_ROOT/output/rec_reranker/e71_top16/"
    "residual_artifacts/selected_residual_deployable.pth"
)
OFFICIAL_RESIDUAL_RECEIPT_PATH = Path(
    "/root/autodl-tmp/DATA_ROOT/output/rec_reranker/e71_top16/"
    "residual_artifacts/promotion-receipt.json"
)
OFFICIAL_GEOMETRY_ARTIFACT_PATH = Path(
    "/root/autodl-tmp/DATA_ROOT/output/rec_reranker/e71_top16/"
    "geometry_artifacts/selected_geometry_reranker.pth"
)
# Backward-compatible spelling used by a few shared validation helpers.
OFFICIAL_SELECTED_ARTIFACT_PATH = OFFICIAL_SELECTED_RESIDUAL_ARTIFACT_PATH
...
OFFICIAL_SELECTED_ARTIFACT_SHA256 = OFFICIAL_SELECTED_RESIDUAL_ARTIFACT_SHA256
...
def acceptance_gate_pass(hits025, hits050, sample_count,
                         inference_uses_ground_truth):
    """Return the fixed paper-goal decision without rounding ambiguity."""
    return (
        type(hits025) is int
...
def build_authoritative_command():
    ...
    residual = Path(
        OFFICIAL_SELECTED_RESIDUAL_ARTIFACT_PATH
    ).expanduser().resolve()
    geometry = Path(OFFICIAL_GEOMETRY_ARTIFACT_PATH).expanduser().resolve()
...
def _freeze_regular_file(path, label):
    """Fsync and chmod one produced evidence file without following links."""
    resolved = _path_without_following_final(path)
    flags = os.O_RDONLY | getattr(os, "O_NOFOLLOW", 0)
    try:
        descriptor = os.open(str(resolved), flags)
    except OSError as error:
        raise ValueError("could not freeze {}: {}".format(label, error))
    try:
        ...
        os.close(descriptor)
    live = os.lstat(str(resolved))
    if (stat.S_ISLNK(live.st_mode)
            or _file_identity(before)[:2] != _file_identity(after)[:2]
            or _file_identity(after) != _file_identity(live)
            or stat.S_IMODE(live.st_mode) != 0o444):
...
def _validate_selection_bindings(selection, receipt, residual, geometry,
                                 parent, checkpoint):
    """Bind deployment evidence to all four immutable model artifacts."""
    artifact = selection.get("artifact")
    inputs = selection.get("input_sha256")
    online = selection.get("online_calibration")
    receipt_artifact = receipt.get("artifact")
    receipt_inputs = receipt.get("input_sha256")
    receipt_online = receipt.get("online_calibration")
    expected_inputs = {
        "backbone": checkpoint["sha256"],
        "parent": parent["sha256"],
        "geometry": geometry["sha256"],
    }
    if (selection.get("schema")
            != "rec-selective-residual-deployment-selection-v1"
            or selection.get("version") != 1
            or selection.get("selected") != "deployable_residual"
            or selection.get("deployable") is not True
            or selection.get("validation_data_accessed") is not False
            or not isinstance(artifact, dict)
            or artifact.get("name") != Path(residual["path"]).name
            or artifact.get("sha256") != residual["sha256"]
            or inputs != expected_inputs
            or selection.get("promotion_receipt_sha256")
            != receipt["_snapshot_sha256"]):
        raise ValueError("selection record does not bind frozen artifacts")
    if (receipt.get("schema")
            != "rec-selective-residual-promotion-receipt-v1"
            or receipt.get("version") != 1
            or receipt.get("selected") != "deployable_residual"
            or receipt.get("deployable") is not True
            or receipt.get("validation_data_accessed") is not False
            or receipt.get("inference_uses_ground_truth") is not False
            or receipt_artifact != artifact
            or receipt_inputs != expected_inputs):
        raise ValueError("residual promotion receipt is invalid")
    for label, value in (("selection", online), ("receipt", receipt_online)):
        if (not isinstance(value, dict)
                or type(value.get("sample_count")) is not int
                or value["sample_count"] <= 0
                or type(value.get("baseline_hits025")) is not int
                or type(value.get("baseline_hits050")) is not int
                or type(value.get("candidate_hits025")) is not int
                or type(value.get("candidate_hits050")) is not int
                or value["baseline_hits025"] != 3461
                or value["baseline_hits050"] != 3316
                or value["candidate_hits025"] < 3524
                or value["candidate_hits050"] < 3316):
            raise ValueError("{} online calibration gate is invalid".format(label))
    if online != receipt_online:
        raise ValueError("online calibration evidence differs across receipts")
...
def _input_snapshots(selection_record, residual_artifact, residual_receipt,
                     geometry_artifact, parent_artifact, checkpoint):
    snapshots = {
        "selection_record": _stable_snapshot(
            selection_record, "selection record"
        ),
        "residual_artifact": _stable_snapshot(
            residual_artifact, "selected residual artifact"
        ),
        "residual_receipt": _stable_snapshot(
            residual_receipt, "residual promotion receipt"
        ),
        "geometry_artifact": _stable_snapshot(
    ...
    if any(snapshot["mode"] != 0o444 for snapshot in snapshots.values()):
        raise ValueError("every official frozen input must have mode 0444")
    selection = _read_selection(snapshots["selection_record"])
    receipt = _read_selection(snapshots["residual_receipt"])
    receipt["_snapshot_sha256"] = snapshots["residual_receipt"]["sha256"]
    _validate_selection_bindings(
        selection,
        receipt,
        snapshots["residual_artifact"],
        snapshots["geometry_artifact"],
...
def _authoritative_input_snapshots():
    paths = {
        ...
        "residual_artifact": OFFICIAL_SELECTED_RESIDUAL_ARTIFACT_PATH,
        "residual_receipt": OFFICIAL_RESIDUAL_RECEIPT_PATH,
        "geometry_artifact": OFFICIAL_GEOMETRY_ARTIFACT_PATH,
    ...
    expected_shas = {
        ...
        "residual_artifact": OFFICIAL_SELECTED_RESIDUAL_ARTIFACT_SHA256,
        "residual_receipt": OFFICIAL_RESIDUAL_RECEIPT_SHA256,
        "geometry_artifact": OFFICIAL_GEOMETRY_ARTIFACT_SHA256,
    ...
    snapshots = _input_snapshots(
        ...
        paths["residual_artifact"],
        paths["residual_receipt"],
        paths["geometry_artifact"],
...
def run_official_launch():
    ...
    claim_payload = {
        "schema": "rec-selective-residual-official-launch-claim",
        "version": 1,
    ...
    receipt = {
        "schema": "rec-selective-residual-official-launch-receipt",
        "version": 1,
        "created_at_utc": _utc_now(),
        "cwd": code["root"],
...
def _validate_claim(claim):
    if (not isinstance(claim, dict) or set(claim) != _CLAIM_FIELDS
            or claim.get("schema") != "rec-selective-residual-official-launch-claim"
    ...
    expected_inputs = {
        ...
        "residual_receipt": (
            OFFICIAL_RESIDUAL_RECEIPT_PATH,
            OFFICIAL_RESIDUAL_RECEIPT_SHA256,
        ),
        "geometry_artifact": (
            OFFICIAL_GEOMETRY_ARTIFACT_PATH,
            OFFICIAL_GEOMETRY_ARTIFACT_SHA256,
        ),
        "parent_artifact": (
...
def _validate_receipt_schema(receipt):
    if (not isinstance(receipt, dict) or set(receipt) != _RECEIPT_FIELDS
            ...
            != "rec-selective-residual-official-launch-receipt"
            or receipt.get("version") != 1
...
def _validate_config(config, claim, run_path):
    ...
    paths = {
        ...
        "rec_selective_residual_checkpoint": claim["inputs"][
            "residual_artifact"
        ]["path"],
    }
...
def validate_official_result(record):
    ...
    expected_booleans = {
        ...
        "acceptance_gate_pass": acceptance_gate_pass(
            record["hits025"], record["hits050"], record["sample_count"],
            False,
        ),
        "inference_uses_ground_truth": False,
    }
...
def _load_launch_evidence():
    ...
    _freeze_regular_file(run_path / "config.json", "official config")
    _freeze_regular_file(run_path / "log.txt", "official log")
...
def _build_argument_parser():
    parser = argparse.ArgumentParser(
        description="Launch and seal the frozen official REC selective residual run."
    )
