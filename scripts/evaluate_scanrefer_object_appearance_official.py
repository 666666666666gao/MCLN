"""Evaluate one fixed object-appearance endpoint against protected ScanRefer V99."""
...
def verify_training_endpoint(directory):
    ...
    assert receipt['fixed_endpoint_ready_for_official_evaluation'] and receipt['readouts_unchanged']
    assert receipt['development_dual_rec_nonregression']
    assert (directory / 'audit_queue.exit').read_text().strip() == '0'
    audit = json.loads((directory / 'audit.json').read_text())
    assert audit['integrity_pass'] and audit['module_rec_screen_pass']
    assert (directory / 'native_queue.exit').read_text().strip() == '0'
    native = json.loads((directory / 'native_evaluation/receipt.json').read_text())
    assert native['status'] == 'complete' and native['rows_per_arm'] == 6887
    assert native['training_receipt_sha256'] == file_sha(directory / 'receipt.json')
    assert native['rows_sha256'] == file_sha(directory / 'native_evaluation/rows.json')
    checkpoint = receipt['checkpoints']['appearance']
...
def main():
    ...
    assert manifest['schema'] == 'mcln-scanrefer-object-appearance-official-input-v1'
    ...
    assert file_sha(training / 'audit.json') == manifest['training_audit_sha256']
    assert file_sha(training / 'native_evaluation/receipt.json') == manifest['native_receipt_sha256']
    assert manifest['formal_rows'] == 9508
    for name, digest in manifest['files'].items():
        assert file_sha(directory / name) == digest, name
    source = Path(train_manifest['model_source'])
    assert file_sha(source / 'appearance_source_manifest.json') == train_manifest['source_manifest_sha256']
    ...
    tester.logger = logging.getLogger('scanrefer-object-appearance-official')
    ...
    write_json(result_directory / 'protocol.json', {'authoritative_base_command': command,
        ...
        'native_loader_and_worker_seeding': True, 'formal_checkpoint_arm': 'appearance',
    ...
    models = {'protected_v99': protected, 'appearance_v99': copy.deepcopy(protected)}
    trained = torch.load(receipt['checkpoints']['appearance']['path'], map_location='cpu')
    assert trained['schema'] == 'mcln-scanrefer-object-appearance-trained-state-v1'
    assert trained['arm'] == 'appearance' and trained['steps'] == 2482
    assert trained['manifest_sha256'] == receipt['manifest_sha256']
    assert trained['model_config'] == {'use_pretrained_object_appearance': True}
    models['appearance_v99'].object_appearance = PretrainedObjectAppearance().cuda().eval()
    models['appearance_v99'].load_state_dict(trained['model'], strict=True)
    old_artifacts = {name: torch.load(item['path'], map_location='cpu')
                     for name, item in train_manifest['artifacts'].items() if name != 'backbone'}
    artifacts = {'protected_v99': old_artifacts, 'appearance_v99': trained['readout']}
    readouts = {arm: JointRecReadout(value).cuda().eval() for arm, value in artifacts.items()}
    states = {'protected_v99': protected_state, 'appearance_v99': trained['model']}
    ...
    assert cache_receipt['scene_list_split'] == 'ScanRefer val' and not cache_receipt['gt_used_for_crop']
    ...
    assert {v[0] for v in identity} == set(cache_records)
    feature_cache = {}
    ...
    with torch.no_grad():
        for batch_index, raw in enumerate(test_loader):
            size = raw['all_detected_bbox_label_mask'].shape[1]
            features = np.zeros((len(raw['scan_ids']), size, 1280), dtype=np.float32)
            available = np.zeros((len(raw['scan_ids']), size), dtype=np.bool_)
            for i, scene_id in enumerate(raw['scan_ids']):
                stored, record = feature_cache[scene_id], cache_records[scene_id]
                count = len(stored['boxes'])
                assert int(raw['all_detected_bbox_label_mask'][i].sum()) == count
                assert np.array_equal(raw['all_detected_boxes'][i, :count].numpy(), stored['boxes'])
                assert hashlib.sha256(raw['point_clouds'][i].numpy().tobytes()).hexdigest() == record['native_point_sha256']
                features[i, :count], available[i, :count] = stored['features'], stored['available']
            raw['object_visual_features'] = torch.from_numpy(features)
            raw['object_visual_available'] = torch.from_numpy(available)
            ...
            if batch_index == 0 or (batch_index + 1) % 128 == 0:
                ...
                count = len(records['appearance_v99'])
                print('SCANREFER OBJECT APPEARANCE OFFICIAL', json.dumps({'rows': count, 'total': 9508,
    ...
    for before, after in zip(records['protected_v99'], records['appearance_v99']):
        for key in ['row_id', 'scan_id', 'physical_space', 'point_sha256']:
            assert before[key] == after[key]
    ...
    result = {'schema': 'mcln-scanrefer-object-appearance-official-v1', 'status': 'complete',
        ...
        'promotion': promotion_check(metrics['protected_v99'], metrics['appearance_v99']),
        'manifest_sha256': file_sha(option.manifest), 'trained_checkpoint': receipt['checkpoints']['appearance'],
        'data_root': args.data_root, 'superpoint_inputs': data_inputs,
    ...
    result['cache_receipt_sha256'] = manifest['cache_receipt_sha256']
    result['training_audit_sha256'] = manifest['training_audit_sha256']
    result['native_module_receipt_sha256'] = manifest['native_receipt_sha256']
    for item in list(train_manifest['artifacts'].values()) + [receipt['checkpoints']['appearance']]:
        assert file_sha(item['path']) == item['sha256']
    verify_scanrefer_superpoints(manifest['data_root'], 'val', manifest['val_superpoint_files'])
    write_json(result_directory / 'receipt.json', result)
    print('SCANREFER OBJECT APPEARANCE OFFICIAL COMPLETE', json.dumps(result), flush=True)
