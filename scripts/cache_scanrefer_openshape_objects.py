"""Frozen training-scene visual preprocessing, with no annotation-based cropping."""
import argparse
import hashlib
import json
import os
...
def main():
    ...
    assert len(scene_ids) == spec['scene_count'] == 562
    ...
    scene_file = data_root / 'train_v3scans.pkl'
    ...
    for scene_id in scene_ids:
        ...
        detector_path = data_root / 'group_free_pred_bboxes/group_free_pred_bboxes_train' / (scene_id + '.npy')
    ...
    receipt = dict(status='complete', scene_count=len(records),
                   ...
                   mcln_forwards=0, formal_rows=0, scene_list_split='ScanRefer train',
