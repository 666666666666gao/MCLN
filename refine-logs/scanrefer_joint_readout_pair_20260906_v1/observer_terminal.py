...
first = datetime.datetime.fromisoformat('2026-09-06T15:27:30.268616+08:00')
print(json.dumps({'first_observation_cst': first.isoformat(), 'observer_started_cst': datetime.datetime.now(zone).isoformat(), 'subsequent_interval_seconds': 240,
                  'stop_after': 'controller exit; no job restart'}), flush=True)
...
while True:
    ...
    for name in ['protocol.json', 'baseline_metrics.json', 'baseline_rows.json', 'fit_complete.json',
                 'receipt.json', 'terminal_metrics.json', 'terminal_rows.json', 'fit_point_batches.json', 'controller.exit']:
