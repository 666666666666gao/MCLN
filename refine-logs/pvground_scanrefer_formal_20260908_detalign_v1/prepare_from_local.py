...
old=repo/'refine-logs/pvground_scanrefer_formal_20260908_v1'
root='/root/autodl-tmp/mcln_pvground_scanrefer_formal_20260908_detalign_v1'
training='/root/autodl-tmp/mcln_pvground_scanrefer_finetune_20260908_detalign_v1'
audit='/root/autodl-tmp/mcln_pvground_scanrefer_endpoint_audit_20260908_detalign_v1'
archive=repo/'refine-logs/pvground_scanrefer_formal_20260908_detalign_v1'
manifest=json.loads((repo/'refine-logs/pvground_scanrefer_finetune_20260908_detalign_v1/input_manifest.json').read_bytes())
spec=json.loads((old/'spec.json').read_bytes())
spec.update(training_root=training,training_audit_root=audit,training_audit_controller_pid=9522,first_check_cst='2026-09-08T13:45:00+08:00',training_spec_sha256=hashlib.sha256((repo/'refine-logs/pvground_scanrefer_finetune_20260908_detalign_v1/spec.json').read_bytes()).hexdigest())
files={name:(old/name).read_bytes() for name in ['evaluate.py','audit.py','plan.md']}
contract=json.loads((old/'formal_input_contract.json').read_bytes())
contract.update(dataset_source=manifest['model_source'],source_manifest_sha256=manifest['source_manifest_sha256'],source_adaptation='only training augmentation detected-box operation order; augmentation-off output unchanged in32 real samples',prior_data_contract_sha256=hashlib.sha256((old/'formal_input_contract.json').read_bytes()).hexdigest())
files['formal_input_contract.json']=(json.dumps(contract,indent=2)+'\n').encode()
for name in ['queue.py','controller.py','cpu_probe.py']:
    text=(old/name).read_text().replace('/root/autodl-tmp/mcln_pvground_scanrefer_formal_20260908_v1',root)
...
_,out,err=c.exec_command('screen -dmS mcln_pvg_detalign_formal_v1 bash -c '+shlex.quote(inner),timeout=30)
...
record=dict(time_cst=datetime.datetime.now(datetime.timezone(datetime.timedelta(hours=8))).isoformat(),process=process,screen='mcln_pvg_detalign_formal_v1',first_check_cst=spec['first_check_cst'],poll_seconds=300,formal_rows=0,cpu_probe_source='previous completed6887 initial; unchanged evaluator/recount code',training_changes=False)
