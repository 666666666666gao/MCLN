"""Evaluate one fixed local-visual endpoint against protected ScanRefer V99."""
...
def verify_training_endpoint(directory):
    ...
    checkpoint = receipt['checkpoints']['local']
...
def main():
    ...
    tester.logger = logging.getLogger('scanrefer-local-visual-official')
    ...
    with torch.no_grad():
        for batch_index, raw in enumerate(test_loader):
            ...
            if batch_index == 0 or (batch_index + 1) % 128 == 0:
                ...
                print('SCANREFER LOCAL VISUAL OFFICIAL', json.dumps({'rows': count, 'total': 9508,
    ...
    print('SCANREFER LOCAL VISUAL OFFICIAL COMPLETE', json.dumps(result), flush=True)
