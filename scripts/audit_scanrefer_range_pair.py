...
Run as a module: CUDA_VISIBLE_DEVICES='' python -m scripts.audit_scanrefer_range_pair RUN OUTPUT
"""
...
def audit_rows(directory):
    ...
    assert receipt['schema'] == 'mcln-scanrefer-range-pair-v1'
    ...
    superpoint_sha = hashlib.sha256(json.dumps(manifest['superpoint_files'], sort_keys=True).encode()).hexdigest()
    assert receipt['data_root'] == protocol['data_root'] == manifest['data_root']
    assert receipt['superpoint_files_sha256'] == protocol['superpoint_files_sha256'] == superpoint_sha
...
def reconstruct_initial_states(manifest):
    ...
    import scripts
    import models
    scripts.__path__ = [str(Path(manifest['run_directory']) / 'scripts'), str(source / 'scripts')]
    models.__path__ = [str(Path(manifest['run_directory']) / 'models'), str(source / 'models')]
    ...
    local = CandidateRangeVisual('center')
...
def audit_checkpoints(directory):
    ...
    assert probe['status'] == 'pass' and probe['disposable_optimizer_steps_per_arm'] == 2
    assert probe['data_root'] == manifest['data_root']
    assert file_sha(manifest['native_probe_manifest']) == probe['manifest_sha256']
    ...
    assert probe['frozen_parameters_buffers_readouts_unchanged']
    ...
    assert core_names == protocol['core_trainable_tensors']
    assert list(local_initial) == protocol['local_trainable_tensors']
    ...
    for arm in ['control', 'local']:
        ...
        assert checkpoint['model_config'] == {'candidate_range_visual': 'extent' if arm == 'local' else 'center'}
        assert checkpoint['core_trainable_tensors'] == core_names
        local_names = list(local_initial)
...
if __name__ == '__main__':
    ...
    result.update(schema='mcln-scanrefer-range-independent-audit-v1',
