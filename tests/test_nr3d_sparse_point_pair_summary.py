...
from scripts.summarize_nr3d_sparse_point_pair import (
...
def test_sparse_must_improve_both_native_endpoint_and_protected_start():
    end = rows([.62] * 4, 'sparse')
...
def test_good_box_cohort_is_fixed_by_start_boxes_and_strict_threshold():
    ...
    after = [{'id': i, 'sparse': {'legal_box_oracle_iou': .9}} for i in range(4)]
...
def test_mask_improvement_does_not_authorize_changed_rec_or_inputs():
    ...
    before = [{'id': 1, 'scan_id': 'scene1', 'grounding_sha256': 'g',
               'input_point_sha256': 'p', 'native': copy.deepcopy(value), 'sparse': copy.deepcopy(value)}]
    after = copy.deepcopy(before)
    after[0]['sparse']['mask_iou'] = .7
    require_identity(before, after)
    after[0]['sparse']['rec_query'] = 5
    with pytest.raises(AssertionError):
        require_identity(before, after)
    after = copy.deepcopy(before)
