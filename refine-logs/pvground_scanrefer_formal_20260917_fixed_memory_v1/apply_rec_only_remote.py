...
assert parent==10116 and (str(root)+'/controller.py').encode() in command(parent)
...
for name in ['controller.exit','decision.json','evaluation.exit','terminal_restore.json','rec_only_policy_receipt.json']:
    assert not (root/name).exists(),name
assert (root/'run.log').stat().st_size==0
spec=json.loads((root/'spec.json').read_bytes())
assert spec['first_check_cst']=='2026-09-17T19:15:00+08:00'
...
before=root/'before_rec_only_policy_20260917'
before.mkdir()
for name in ['launch.json','spec.json','run.log','evaluate.py','audit.py','plan.md']:(before/name).write_bytes((root/name).read_bytes())
...
new_digests={'evaluate.py': '49c185dcf9d774318c8df8483feed87599c464c4bf22925788da5da38fc2379c', 'audit.py': 'b33ba826e43fe25de52f43365b40c52e622da15be3d3377ac068c649985e4da2', 'plan.md': '8419e66bdaf08c701557d7d2c909c62bbdc7b4ba58825f4ee5ecb34eb097bfbc'}
...
receipt=dict(time_cst=now,old_controller_pid=parent,old_queue_pid=child,new_process=process,new_queue_pid=int(new_children[0]),
    old_first_check_cst='2026-09-17T19:15:00+08:00',first_check_cst=spec['first_check_cst'],poll_seconds=300,
    old_spec_sha256=old_spec_sha,new_spec_sha256=sha(root/'spec.json'),training_spec_sha256=train_sha,
    training_controller_pid=8666,audit_controller_pid=8673,training_unchanged=True,formal_code_unchanged=False,promotion_policy_only=True,scanrefer_mask_gate=False,
    formal_rows=0,reason='Explicit user instruction2026-09-17: ScanRefer promotion uses only Acc0.25 and Acc0.50; retain Mask diagnostics and training loss')
(root/'rec_only_policy_receipt.json').write_text(json.dumps(receipt,indent=2)+'\n')
