#!/usr/bin/env python
"""Train-only OOF screen of the V116 REC@0.25-primary policy."""
...
V116_THRESHOLD_WEIGHTS = (2.4, 0.6)
...
def select_v116_proposal(
        query_logits, variant_logits, query_valid, variant_valid):
    """Select hierarchically with a normalized 4:1 REC@0.25 priority."""
    v99.select_hierarchical_proposal(
        query_logits, variant_logits, query_valid, variant_valid
    )
    query_probability = v99.monotone_hit_probabilities(query_logits)
    variant_probability = v99.monotone_hit_probabilities(variant_logits)
    weight025, weight050 = V116_THRESHOLD_WEIGHTS
    query_utility = (
        weight025 * query_probability[..., 0]
        + weight050 * query_probability[..., 1]
    )
    variant_utility = (
        weight025 * variant_probability[..., 0]
        + weight050 * variant_probability[..., 1]
    )
    ...
    return {
        ...
        "flat_indices": selected_query * VARIANT_COUNT + selected_variant,
        "query_utility": query_utility,
        "variant_utility": variant_utility,
    }


def predict_v116_pareto(model, records, statistics, device):
    """Apply only the frozen V116 selector and the existing Pareto veto."""
    ...
    with torch.no_grad():
        for start in range(0, len(records), HIERARCHICAL_BATCH_SIZE):
            ...
            head_gain = (
                probabilities[rows, proposal]
                - probabilities[rows, baselines]
            )
            weight025, weight050 = V116_THRESHOLD_WEIGHTS
            aggregate_gain = (
                weight025 * head_gain[..., 0]
                + weight050 * head_gain[..., 1]
            )
            all_proposals.append(proposal.cpu().long())
...
def fit_v116(records, device):
    """Fit V99 first, freeze it, then fit only the bounded adapter."""
    ...
    if not trainable or any(parameter.requires_grad
        ...
        raise RuntimeError("V116 anchor/trainable parameter contract changed")
    ...
    for epoch in range(HIERARCHICAL_EPOCHS):
        ...
        if model.anchor.training:
            raise RuntimeError("V116 frozen anchor entered training mode")
...
def _load_bound_receipt(path, expected_sha256, expected_schema):
    ...
    if not resolved.is_file() or resolved.is_symlink():
        raise ValueError("V116 receipt must be a regular non-symlink file")
    if file_sha256(resolved) != expected_sha256:
        raise ValueError("V116 receipt SHA-256 changed: {}".format(resolved))
    ...
    if (receipt.get("schema") != expected_schema
        ...
        raise ValueError("V116 receipt policy changed: {}".format(resolved))
    return receipt
...
def main(argv=None):
    ...
    v115_diagnostics = v115_oof.get("diagnostics", {})
    if (v115_source.get("schema")
            ...
            or v115_diagnostics.get("delta_hits025") != 75
            or v115_diagnostics.get("delta_hits050") != 263):
        raise ValueError("V115 protocol-design source contract changed")
    ...
    for held in range(5):
        ...
        model, statistics, training = fit_v116(
            [records[index] for index in train_indices], args.device
        )
        prediction = predict_v116_pareto(
            model,
        ...
        if not torch.equal(prediction["baselines"], baselines[held_indices]):
            raise RuntimeError("V116 OOF baseline identity changed")
    ...
    if protected_after != protected_before:
        raise RuntimeError("protected artifacts changed during V116")
    protected_metadata_after = {
        name: file_sha256(path)
        for name, path in protected_metadata_paths.items()
    }
    if protected_metadata_after != protected_metadata_before:
        raise RuntimeError("protected V116 cache metadata changed during OOF")
    script_sources = {
        "v116": str(Path(__file__).resolve()),
    ...
    report = {
        "schema": "rec-v116-primary025-policy-scene-oof-v1",
        "version": 1,
        "validation_data_accessed": False,
        ...
        "protocol": {
            "architecture": (
                "unchanged V115 frozen-anchor spatial residual adapter plus "
                "a normalized 4:1 REC@0.25-primary selector"
            ),
            "objective": "frozen V95 bounded threshold-aligned listwise",
            "proposal_margin": v99.V97_MARGIN,
            "acceptance": (
                "normalized_4_to_1_aggregate_margin_and_positive_"
                "delta025_and_delta050"
            ),
            "threshold_weights": list(V116_THRESHOLD_WEIGHTS),
            "threshold_weight_sum": sum(V116_THRESHOLD_WEIGHTS),
            "grid_search": False,
    ...
    try:
        ...
        while written < len(payload):
            ...
            if count <= 0:
                raise OSError("V116 output write made no progress")
            written += count
        os.fsync(descriptor)
    finally:
        os.close(descriptor)
