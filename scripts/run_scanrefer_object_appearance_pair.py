"""Paired native ScanRefer training with frozen pretrained object appearance.

Reuses the established same-input evaluation/endpoint recording machinery.
The failed candidate-local module is not loaded or trained.
"""
...
def main():
    ...
    for name, parameter in first_model.named_parameters():
        parameter.requires_grad_(name.startswith(('cross_encoder.', 'decoder.', 'prediction_heads.')))
    models = {'control': first_model, 'appearance': copy.deepcopy(first_model)}
    models['appearance'].object_appearance = PretrainedObjectAppearance().cuda().eval()
    appearance_initial = {name: value.detach().cpu().clone()
                     for name, value in models['appearance'].state_dict().items()
                     if name.startswith('object_appearance.')}
    ...
    for arm, model in models.items():
        ...
        if arm == 'appearance':
            groups.append({'params': list(model.object_appearance.parameters()),
                           'lr': manifest['appearance_learning_rate']})
    ...
    write_json(directory / 'protocol.json', {'row_ids': partitions, 'physical_spaces': spaces,
        'data_root': manifest['data_root'], 'superpoint_files_sha256': superpoint_files_sha256,
        ...
        'appearance_trainable_tensors': list(appearance_initial),
    ...
    def evaluate(stage):
        ...
        with torch.no_grad():
            for batch_index, raw in enumerate(loader('holdout', 1000, False)):
                ...
                if batch_index == 0 or (batch_index + 1) % 128 == 0:
                    print('SCANREFER OBJECT APPEARANCE EVAL', json.dumps({'stage': stage,
                        'rows': len(records['appearance']), 'total': len(partitions['holdout']),
        ...
        print('SCANREFER OBJECT APPEARANCE EVAL COMPLETE', json.dumps({'stage': stage, 'metrics': metrics,
              'elapsed_seconds': time.time() - begin}), flush=True)
        return records, metrics

    # Check full chosen trainable scope at the actual training batch size.
    # No update is made and eval-mode buffers stay fixed before the baseline.
    raw = next(iter(loader('fit', 0, True)))
    batch = TrainTester._to_gpu(raw)
    inputs = make_inputs(batch)
    inputs['train'] = False
    capacity = {}
    for arm, model in models.items():
        ...
        outputs = model(inputs)
        outputs.update(batch)
        objective, outputs = TrainTester._compute_loss(outputs, criterion, set_criterion, args)
        assert torch.isfinite(objective)
        objective.backward()
        gradients = {name: float(parameter.grad.norm()) for name, parameter in model.named_parameters() if parameter.grad is not None}
        assert gradients and all(math.isfinite(v) for v in gradients.values())
        if arm == 'appearance':
            assert gradients['object_appearance.projection.weight'] > 0
        capacity[arm] = dict(loss=float(objective), gradient_tensors=len(gradients),
                             gradient_norms=gradients, parameters=sum(p.numel() for p in trainable[arm]))
        model.zero_grad(set_to_none=True)
        del outputs, objective
    write_json(directory / 'capacity.json', dict(status='pass', batch_size=12, arms=capacity,
        max_gpu_mib=torch.cuda.max_memory_allocated()/1024**2, optimizer_steps=0))
    del raw, batch, inputs
    baseline, baseline_metrics = evaluate('baseline')
    assert baseline['control'] == baseline['appearance']
    assert baseline_metrics['control'] == baseline_metrics['appearance']
    for model in models.values():
        for name, value in model.state_dict().items():
            expected = appearance_initial[name] if name in appearance_initial else initial[name]
    ...
    for raw in loader('fit', 0, True):
        ...
        if step == 1 or step % 64 == 0 or step == manifest['steps_per_arm']:
            ...
            print('SCANREFER OBJECT APPEARANCE TRAIN', json.dumps({'step': step, 'total': manifest['steps_per_arm'],
    ...
    for arm, model in models.items():
        ...
        for name, value in model.state_dict().items():
            ...
            if name not in core_names and name not in appearance_initial:
        ...
        checkpoint = directory / (arm + '_object_appearance_state.pt')
        assert not checkpoint.exists()
        # Preserve the completed fit endpoint before the potentially long terminal evaluation.
        torch.save({'schema': 'mcln-scanrefer-object-appearance-trained-state-v1', 'arm': arm, 'steps': step,
            'model': {name: value.detach().cpu() for name, value in model.state_dict().items()},
            'readout': readouts[arm].export_artifacts(), 'optimizer': optimizer.state_dict(),
            'core_trainable_tensors': core_names, 'appearance_trainable_tensors': list(appearance_initial) if arm == 'appearance' else [],
            'model_config': {'use_pretrained_object_appearance': arm == 'appearance'},
    ...
    effects = {reference: {str(t): paired_effect(
        baseline['appearance'] if reference == 'baseline' else terminal['control'],
        terminal['appearance'], 'rec_iou', t) for t in (.25, .5)} for reference in ('baseline', 'control')}
    ...
    receipt = {'schema': 'mcln-scanrefer-object-appearance-pair-v1', 'status': 'complete',
        'data_root': manifest['data_root'], 'superpoint_files_sha256': superpoint_files_sha256,
        ...
        'appearance_rec_effects': effects, 'development_dual_rec_nonregression': development_nonregression,
    ...
    print('SCANREFER OBJECT APPEARANCE PAIR COMPLETE', json.dumps(receipt), flush=True)
