"""Read the original observation-query trial handles and bounded result files, without launching."""
import datetime,json,os,shlex
from pathlib import Path
import paramiko

repo=Path(__file__).resolve().parents[1]
roots=['/root/autodl-tmp/mcln_pvground_scanrefer_finetune_20260918_semantic_assignment_v1',
       '/root/autodl-tmp/mcln_pvground_scanrefer_endpoint_audit_20260918_semantic_assignment_v1',
       '/root/autodl-tmp/mcln_pvground_scanrefer_formal_20260918_semantic_assignment_v1']
...
for root in roots:
    ...
    for name in ['preflight.log','preflight.exit','preflight_receipt.json','preflight_launch.json','training_start.json','cleanup_receipt.json','run.log','controller.exit','capacity.json','receipt.json','audit.json','audit.exit','initial_audit.json','train.jsonl','decision.json','evaluation.exit','terminal_restore.json']:
        if name in names:
    ...
    if 'preflight.log' in names:
        entry['last_preflight_line']=(archive/'preflight.log').read_text().splitlines()[-1:]
        if 'preflight.exit' not in names:
            preflight=json.loads((archive/'preflight_launch.json').read_bytes())
            preflight_pid=int(preflight['process'].split()[0])
            _,out,err=c.exec_command('ps -p '+str(preflight_pid)+' -o pid=,args=',timeout=30)
            entry['actual_preflight_process']=out.read().decode().strip()
            assert out.channel.recv_exit_status()==0,err.read().decode()
            assert root+'/preflight_controller.py' in entry['actual_preflight_process']
...
s.close();c.close()
archive=repo/'refine-logs/pvground_scanrefer_finetune_20260918_semantic_assignment_v1'
raw=(json.dumps(record,indent=2)+'\n').encode()
(archive/'observation_latest.json').write_bytes(raw)
(archive/('observation_'+datetime.datetime.fromisoformat(record['time_cst']).strftime('%Y%m%d_%H%M%S')+'.json')).write_bytes(raw)
