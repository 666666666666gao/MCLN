...
V127_REPORT=$BASE/v127_meshsp_nested_semantic_prior_corrected_oof.json
...
OUTPUT=$BASE/v128_meshsp_nested_semantic_protected_rescue_oof.json
LOG=$BASE/v128_meshsp_nested_semantic_protected_rescue_oof.log
EXIT=$BASE/v128_meshsp_nested_semantic_protected_rescue_oof.exit
SESSION=v128_nested_semantic_protected_rescue_oof
SELF=$(readlink -f "$0")
...
if [[ "${1:-}" == "--inner" ]]; then
    ...
    env CUDA_VISIBLE_DEVICES=0 \
        ...
        "$PYTHON" scripts/run_v128_meshsp_nested_semantic_protected_rescue_oof.py \
...
cd "$REPO"
require_sha 3464f6fed7e3e174915f8296ff43145f7ffaa6168230bdd222589544b961be7c \
    scripts/run_v128_meshsp_nested_semantic_protected_rescue_oof.py
...
require_sha 899a884cdd362e13f9d1772e53bd8cca01af9bf849d66417eee33df6fd7e8c53 \
    "$V126_REPORT"
require_sha ad1fe06606eb696462a08877f314050e0a0bf6bb9780df1d149897fcfcb1904a \
    "$V127_REPORT"
...
for path in "$OUTPUT" "$LOG" "$EXIT"; do
    if [[ -e "$path" ]]; then
        echo "Refusing to overwrite existing V128 output: $path" >&2
        exit 74
    fi
done
if [[ $(nvidia-smi -L | wc -l) -ne 1 ]]; then
    echo "V128 requires the frozen single-GPU topology" >&2
    exit 75
fi
...
if pgrep -af '[r]un_v128_meshsp_nested_semantic_protected_rescue_oof.py' >/dev/null; then
    echo "V128 process already exists" >&2
    exit 75
fi
if screen -ls 2>/dev/null | grep -q "[.]$SESSION"; then
    echo "V128 screen session already exists" >&2
    exit 75
fi
available_kb=$(df -Pk /root/autodl-tmp | awk 'NR==2 {print $4}')
if [[ "$available_kb" -lt 2621440 ]]; then
    echo "V128 requires at least 2.5 GiB free on /root/autodl-tmp" >&2
    exit 75
fi
...
if ! screen -ls 2>/dev/null | grep -q "[.]$SESSION" \
        && [[ ! -e "$EXIT" ]]; then
    echo "V128 screen failed to remain alive and produced no exit receipt" >&2
    exit 75
fi
