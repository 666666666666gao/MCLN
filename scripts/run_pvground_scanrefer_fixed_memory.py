...
def main():
    ...
    assert spec['fixed_visual_memory'] is True
    checkpoint_parameter_names={n for n,p in model.named_parameters() if p.requires_grad}
    assert len(checkpoint_parameter_names)==interface['trainable_tensors']
    assert sum(p.numel() for n,p in model.named_parameters() if n in checkpoint_parameter_names)==interface['trainable_parameters']
    visual_names={n for n in model.state_dict() if n.startswith('backbone_net.')}
    assert visual_names
    for p in model.backbone_net.parameters():p.requires_grad_(False)
    def training_mode():
        model.train()
        model.backbone_net.eval()
    def check_visual_memory():
        assert not any(m.training for m in model.backbone_net.modules())
        assert all(not p.requires_grad and p.grad is None for p in model.backbone_net.parameters())
        assert all(torch.equal(v.detach().cpu(),initial[n]) for n,v in model.state_dict().items() if n in visual_names)
    model.cuda()
    ...
    assert set(trainable)=={n for n in checkpoint_parameter_names if not n.startswith('backbone_net.')}
    assert all(n.startswith(('text_encoder.','backbone_net.')) for n in frozen)
    memory_policy=dict(fixed_visual_memory=True,visual_state_tensors=len(visual_names),
        trainable_tensors=len(trainable),trainable_parameters=sum(p.numel() for p in trainable.values()),
        frozen_tensors=len(frozen),frozen_parameters=sum(p.numel() for p in frozen.values()))
    write_json(output/'memory_policy.json',memory_policy)
    ...
    def step(batch,optimizer,update):
        ...
        assert not any(m.training for m in model.backbone_net.modules())
        assert all(p.grad is None for p in model.backbone_net.parameters())
    ...
    check_visual_memory()
    capacity.update(visual_state_unchanged=True,**memory_policy)
    ...
    assert all(torch.equal(v.detach().cpu(),initial[k]) for k,v in model.state_dict().items())
    reset_rng(spec['seed']);dataset.augment=True;dataset.augment_det=True;training_mode()
    optimizer=BaseTrainTester.get_optimizer(training,model)
    selected_names=checkpoint_parameter_names|{n for n,_ in model.named_buffers() if n in initial}
    ...
    def save_checkpoint(name,step_number,seen_rows):
        ...
        data.update(fixed_visual_memory=True,memory_policy=memory_policy)
    ...
    check_visual_memory()
    receipt.update(fixed_visual_memory=True,visual_state_unchanged=True,memory_policy=memory_policy)
