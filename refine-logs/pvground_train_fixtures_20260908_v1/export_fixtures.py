...
def main():
    ...
    for row_id in partitions['fit']:
        physical = dataset.annos[row_id]['scan_id'].split('_')[0]
    ...
    for row_id in selected:
        ...
        inputs = {
            ...
            'superpoint': torch.from_numpy(sample['superpoint']).unsqueeze(0),
        }
    ...
    receipt = {'status':'pass','time_cst':datetime.datetime.now(datetime.timezone(datetime.timedelta(hours=8))).isoformat(),
        ...
        'gt_model_inputs':False,'augmentation':False,'script_sha256':sha(__file__),'elapsed_seconds':time.time()-started}
