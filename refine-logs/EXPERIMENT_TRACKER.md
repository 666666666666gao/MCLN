...
## 2026-09-17 D shared-parameter native-loss direction diagnostic

Completed at16:53:24,228.83s,exit0. Same128 fit scenes/16 eval forwards;820 trainable tensors,28883227 parameters;32 loss-to-parameter and72 nonzero-index margin autograd calls. Covered errors14/21: last-CE logit positive14/21; weighted last-CE parameter positive11/13; full native loss parameter positive12/17 and negative2/4. CE-positive to full-negative only1/1. Full native loss has more positive margin directions than weighted last-layer CE in this fixed diagnostic (17 vs 13 at 0.5), so this fixture does not justify reducing Mask loss or asserting dominant inter-task gradient conflict. Batch8 loss direction, fixed realized Gumbel/eval/native stop-gradient graph; not train-mode, AdamW, finite-step or accuracy evidence. Total loss includes real-GT and native prediction-derived Mask self-consistency. State unchanged, all .grad None,0 optimizer/new weights/formal rows. Point/text/order and selected/root-match agree with prior; scores/IoU have small numeric changes, not all-input/GT byte identity. Local export recount plus fresh same-family provisional review; see master§20.216 and pvground_parameter_direction_20260917_v1. Health16:57:06:GPU empty,controller gone,1804066816 bytes free;0 new deletions. No new training mechanism launched.
...
## 2026-09-17 ReferIt full annotation partition preparation
...
## 2026-09-17 F score-evidence diagnostic completed
- Original controller17519 exit0 at23:46:13;128.048s,32 paired initial/F eval forwards over the previously fixed128 fit rooms.0 backward/optimizer/new checkpoints/formal rows; exact F state retained. Point/input hashes and independent65536-box score/IoU/loss recount passed.
- Selected114/107→115/106; root matched and raw256128/126 unchanged. All terminal13/20 covered errors have eligible root matches; strict20 errors include16 where highest-native-score good box is not matched. No disabled-loss explanation in this sample.
- Only1/20 strict errors has a good root-token component above chosen;19/20 good candidates have higher null probability. Score-per-nonnull advantage13/19→3/20; on14 persistent errors9→2. Descriptive factorization, no reranking or causal claim. Neither removing null nor reweighting terms is established as a fix.
- New semantic supervision for GT-qualified unmatched queries is a candidate design, not implemented or launched. Preserve Hungarian regression, fixed budget/seed and REC-only gates. Array3,844,187 bytes SHA c2cd9aa9d78cc9a3d04b0ff9f9d8f00addb00acc6fdf926096734204fa56f00e retained remotely and in D; not committed as binary. See master20.238 and PVG_REC_SCORE_EVIDENCE_RESULT_2026-09-17.md.

## 2026-09-18 G semantic-assignment trial prepared
- Last-layer CE replaces eos targets only for unmatched queries whose detached root IoU exceeds0.5; native root token map, eos weight0.1 and0.5/7 coefficient retained. All matched root/other-target queries and regression assignments unchanged; no new model/inference head or F margin.
- CPU four-case behavior pass. Actual batch8 zero-update preflight18178 live00:17:56; training18224, audit18228, conditional formal18404 waiting on its result. No new formal metrics or G endpoint yet. SameD architecture,parent,seed2027,3723-step budget.
- Formal CPU historical-output recount and real-model synthetic serialization probe pass; neither is actual G terminal performance. Scan gates REC only5572/4797, no Mask gate. Same9508 two-arm formal expected35–45min after eligibility; not20–30min for the whole paired procedure. See master20.239.
- Update00:21:42: real batch8 preflight exited0 at00:19:31, exact native/replaced CE values, gradient error2.33e-10, geometry detached, all states restored.259 reassigned slots across7/8 rows;0 optimizer/checkpoints. Training controller18224 alive waiting its300s dependency check; not yet an optimization-progress claim.
- Update00:24:59: controller18224 started full driver00:23:23 after preflight success; actual GPU863497/2426MiB loading data/text, no optimizer-progress or initial-score claim yet. Conditional formal ETA03:55–04:15 CST; next useful check about00:45.


## 2026-09-20 G completed results recovered and EG pretrained acceptance started
- G completed 2026-09-18: fixed3723 steps; module6176/5602 (+29/+53 own, +40/+55 D), formal9508 bbs5615/4495=59.0555/47.2760, paired published parent5579/4381 (+36/+114). All audits passed; strictREC302 hits short of protectedV99. No Mask gate, no G Nr/Sr launch. Useful terminal retained. See master20.240 and PVG_SEMANTIC_ASSIGNMENT_RESULT_2026-09-20.md.
- User authorizes complete pretrained alternative. EG3DVG official174e348... source and875723586-byte author ScanRefer checkpoint downloaded; SHA785f46ca...24f7e. Remote isolated staging, native annotation9508/312 and CUDA FPS passed. Exact environment command independently witnessed. Weight transfer/strict restoration/full model forward/formal still pending; no EG accuracy claim. Preserve native averaged regression+Mask box rule. See master20.241 and acceptance plan.
...
## 2026-09-20 §20.245 完整EG作者预训练权重9508正式完成：58.2878/52.0825；ReferIt输入准备通过（2026-09-20 17:45 CST）

17:38:20正式推理完成，17:38:26独立CPU审计通过。固定主输出last/bbs为5542/4952，即58.2878/52.0825%；同次bbf诊断5552/4966，即58.3929/52.2297%。同一9508条、作者epoch69完整权重、1276状态不变、0训练、不接旧V99/C/D/G。formal/audit/controller全部exit0。推理1741.79秒；所选候选、框、分数与完整导出匹配，复算两阈值一致。详见docs/EG3DVG_PRETRAINED_RESULT_2026-09-20.md及formal/receipt.json、audit.json。

相对受保护V99 5572/4797，主输出−30/+155命中，即−0.3155/+1.6302个百分点。完整EG可保留为严格定位更强的原生研究起点，但未同时满足V99两阈值不退化；V99保护不变，不将bbf改成主输出，不拼单项。作者58.54/52.36与本机结果分开，cu116/种子等已知复现差异如实保留。Mask无验收门槛，但作者预测Mask-derived平均框仍是原生REC组成。

完整候选CPU分析已完成exit0：raw256未过滤GT oracle9079/8069，作者平均框9083/8354。同一bbs已选Query原框5536/4732，平均框5542/4952；严格阈值修复342、破坏122，净+220。严格失败中3402条仍有合格平均框，1154条没有。仅供离线诊断，不等于合法候选召回/可部署精度，不据此宣布语义或回归的全部根因。
...
## 20.251 EG Nr3D零更新候选覆盖与排序分解完成；固定终点同口径分析已接续（2026-09-20 19:21 CST）

上一目标轮为实时进程核验后的等待；本轮利用已完成7899条预测做CPU离线分析，0模型forward/0优化步，不改活动训练。输入receipt/audit及rows/candidates SHA核对通过，重新计算的bbs命中仍3508/2909。原256回归框GT oracle6702/5403，作者平均框6793/5789；平均框按已有场景对象支持位过滤后的oracle同为6793/5789。由于作者输入包含目标在内的GT场景实例框，支持oracle相同有协议依赖，不称无GT候选召回或部署精度。

实际失败分解：@.25有1106条无合格平均框，3285条有合格框未选中；@.50分别2110/2880。后者的最高分合格候选在实际bbs分数中的排名，@.25按第2/3—16/17—64/65—256名为513/573/1211/988；@.50为513/599/1033/735。同时计算等分排名上下界，所有这些失败的最佳合格分数均无并列。只是GT离线诊断，不据此把第2名等规则用于推理，不把IoU合格直接等同语义身份正确。

同一已选bbs Query，原框3391/2661→作者平均框3508/2909；@.25修复130破坏13，@.50修复300破坏52。已有对象支持过滤相对同forward未过滤bbs修复268/131、破坏0/0；乘0并非硬删除，11条实际选中框仍无对象支持。未过滤分支仍输入相同GT对象框，不称单阶段。这些计数表明跨数据集起点同时存在覆盖与选择缺口，不能全部归因scorer，也不能外推为完整Nr训练后的瓶颈。
...
## 20.257 EG任务条件读取已接入独立源码并通过CPU行为检查；真实GPU配对训练已排队（2026-09-21 00:04 CST）

回应用户“自己的创新接入EG后是否涨点”：截至§256只有原生Nr续训3970/3204，尚无本机制性能。现已实际新增最后层语义/几何请求矩阵，两组288×288共165888参数，零初始化；两种请求分别访问作者GMA全局点和SWA superpoint，共享原注意力及FFN。语义输出供soft-token/对比投影，几何输出供center/size/Query Mask及superpoint refinement。候选身份、原匹配、原生全部损失、bbs及平均框保持；未加入PV六源VSA/C状态/G标签替换，也未接V99。此前PV D的-3/+1负结果保留，本次只检验该分工在完整EG中的独立作用，不声称已经有效。

独立root /root/autodl-tmp/mcln_eg3dvg_nr3d_task_read_20260920_v1，由已完成Nr v3源码复制，未修改正在Sr评估的源码或保护权重。CPU使用真实作者decoder/head权重、256Query夹具：train/eval零初始化输出、refined super_features与RNG精确一致；两支独立干预、实际梯度及严格序列化恢复通过。此处0实际场景/0优化步，不是精度。接入预检曾由精确替换保护阻止误改proposal投影；CPU夹具从16改成作者固定256Query，并用新构造器验证恢复，未通过修改模型回避问题。相关失败边界和最终证据保留。

固定计划docs/EG3DVG_TASK_READ_PLAN_2026-09-20.md：同作者Scan epoch69父权重、同44909行训练顺序、batch8/5614步/seed2027/原LR与损失，从父权重重载，不沿用Nr终点。预先固定控制为已完成Nr原生v3；首先用实际8条训练输入比较完整原生/任务模型正负表达前向、原损失、BN缓冲和CPU/CUDA RNG，随后两批真实优化预检，丢弃预检权重再开始完整fit。唯一终点评估7899条，不挑checkpoint；Mask只诊断但保留模型计算。新增两个请求的训练随机掩码相同，RNG仅推进一次原生读取，避免无关随机流变化。

训练spec SHA f0e2a209899105c612e48fd154f19be23ed548feeca7c6ae777b63612ae7de54，完整记录refine-logs/eg3dvg_nr3d_task_read_20260920_v1。00:01:34控制器12201启动，每300秒等待Sr零更新正式controller终态0/独立audit通过，随后获取原GPU锁。00:02:11实查12201活着，尚无真实场景GPU预检或优化步；Sr5662/GPU137192活着，已7168/17726、1494.16秒，预计00:40左右完成。Task训练预计约5小时加约27分钟正式评估，须以实际预检和吞吐更新，不能将排队说成开始训练。

原环境spec 81a835e8144f7070d66feb0afa460911dec610052ced6b96c98097352cd022fd warm reuse，无新依赖。系统盘可用4.636GB、数据盘1.644GB，新增恢复权重写系统盘，数据/历史权重不删除。ScanRefer V99保护5572/4797与EG零更新5542/4952保持独立；三数据集目标尚未达成。后续先取得本机制相对原生的终态差异，再决定跨数据集推进，不能把CPU通过或路径分开当作创新收益。


## 20.258 EG任务读取终态的三方配对与候选分析已接续（2026-09-21 00:12 CST）

新增scripts/analyze_eg3dvg_task_read.py，固定比较作者迁移起点、已完成原生Nr控制与未来任务读取终点。先核对共同父权重、seed/输入hash/行顺序/44909行5614步/优化配方，再核对7899条正式表达、点SHA、GT及输入对象框逐行相同；两阈值、主bbs及诊断分数分别记录修复/破坏/净变化。训练日志检查只证明配方与场景顺序一致，不伪称逐步点SHA均已记录。复用作者平均框/原框/支持过滤的候选上界分析，不改任何推理规则或训练设置。

真实历史7899行重放复现原生+462/+295，自身比较所有修复/破坏/净值为0；0模型forward/0优化步。分析spec及回执位于refine-logs/eg3dvg_nr3d_task_read_20260920_v1，训练spec仍f0e2a209899105c612e48fd154f19be23ed548feeca7c6ae777b63612ae7de54。00:10:18独立CPU等待器12456启动，每300秒等待原12201 controller终态0，然后执行两项分析；它没有启动或修改GPU工作。仅训练前准备和历史重计通过，未来新模型指标仍未知。

00:11:05实查Sr正式9728/17726、2032.38秒，5662/GPU137192存活；Sr失败图等待7083、任务训练等待12201、配对分析12456均真实存活。Sr预计约00:40完成，随后按§257做实际GPU起点检查及两步预检，再开始固定任务读取训练。当前0任务优化步，未出现新正式REC；不重复启动任何原任务。三数据集超过baseline并保留Scan保护的目标未完成。
...
## 20.261 ScanRefer EG任务读取已进入真实训练；实际GPU接入检查与两臂优化预检完成（2026-09-21 01:10 CST）

当前继续遵循§260的ScanRefer优先顺序，不采用用户补充旧综述中的Nr优先安排。PV-G的正式5615/4495（59.0555/47.2760%）作为已有正结果保留，但本轮未加入G标签策略。当前唯一新增机制是EG最后层任务条件读取；尚无其正式REC增益。

完整数据准备耗时411.446秒，Scan训练表达36665条，加1199条ScanNet检测prompt各重复10次，共48655条，每epoch batch8共6082更新。输入与GroupFree文件已核验。独立检查16个实际训练场景、518个检测框，修正后的坐标变换最大分量误差1.77636e-15米，旧增强顺序最大框中心偏差5.23706米；两臂使用同一修正。固定作者epoch69父权重，seed2027，三组LR为2e-5/1e-6/1e-6。

首次完整模型逐位相等预检失败，控制器13771在0优化步停止，证据归档failed_whole_model_exact_check。进一步同权重、同输入、同RNG重复原生EG本身也有非零输出差，尚未隔离到具体算子，不能将其全部归因于新增模块，也不能宣称全模型零更新输出一致。对捕获的真实GPU最后层输入单独重放，原生与零初始化任务读取的geometry、semantic、refined_super三项最大误差均为0，RNG精确一致。检查因此明确改为真实接入层的精确等价，并单独保留全模型原生重放差异，不通过增加数值容差或修改模型来掩盖。源网络、LR、训练预算与训练spec均未改变。

重启控制器14125。当前initial_equality.json记录实际6次完整forward与2次最后层重放、0优化；原生自重放center/size最大差约0.006653/0.013985米，Mask logits最大差0.499527，属于执行一致性边界而非新精度结果。task全模型也并非逐位相同；只对真实被插入层作精确等价结论。

01:04:55 task两批真实优化预检完成，01:06:37 native两批预检完成，均16条/2更新、冻结的199项参数保持不变；两臂预检点SHA和行顺序相同。两组预检权重均已丢弃，从共同作者权重重新开始fit。01:10:54实查task epoch1完成56/6082步、448条，训练累计165.279秒；语义/几何新矩阵均有真实非零梯度，峰值已分配显存31,778,636,800 bytes。正式native同预算训练将在task首轮正式评估后开始，不能把两步native预检算成完整控制。

按当前约2.9秒/步，创新组首轮剩余训练约4.9小时，加预加载及9508条正式评估约5.5—6小时，预计9月21日06:40—07:10 CST左右获得首个创新组正式结果；这是吞吐估算，不是保证。完整首轮配对还需接续native训练评估，预计约中午完成，按实际吞吐更新。每个完整epoch保留REC两阈值及修复/破坏；同一task权重至少5543/4953才超过实际原生EG的5542/4952。Mask不设晋级门槛，保留原生计算。没有启动Nr/Sr训练。

当前campaign SHA 4b9029ddf9801d383332f542a4b43c4ee805919bee47ad1db446c43a93fca5a8；task/native训练spec仍分别abc505429465da5c746813a7799db20f9eba6f65ef0fb1db2dd8e9715df6ddf1与1cb0589f455cdd49b6a405ca4cabc5fbe979eec6cbda81f7740c0c4449063858。轻量执行证据在refine-logs/eg3dvg_scan_task_20260921_v1，读取观察脚本scripts/observe_eg3dvg_scan_task.py。控制器将自动完成训练、正式评估、同预算控制和逐行比较；当前启动成功不等于科学目标达成，ScanRefer超过原生EG仍待实际正式结果。


## 20.262 ScanRefer任务读取首个512步恢复文件已通过CPU检查，训练继续（2026-09-21 01:35 CST）

01:33:59，独立CPU检查完成首次epoch1/512步/4096条恢复文件。latest.pth大小877636156字节，SHA cac7e0b85c8797f8cbb504161f35c5d5765b2a31c74e5bad8331957d59f9f754；1277项模型状态准确对应作者父模型加task_queries，形状/类型及全部模型和842项优化器状态数值检查通过，三组优化器和Python/NumPy/Torch/CUDA随机状态齐全。新增语义/几何矩阵范数为0.12599191/0.10946181。检查耗时5.29秒，0模型forward、0额外优化、0精度评估、未选择checkpoint；它仅证明CPU反序列化及保存内容完整，尚未执行从该快照恢复的训练步。

01:35:14核对/proc：controller14125及其fit子进程14301仍存活，训练已继续到554/6082步、4432条，累计1618.87秒；恢复检查进程正常结束并留下task/recovery_checkpoint_512.json。系统盘尚有3.756GB，数据盘1.407GB，未删除历史数据或权重。现有512步文件只用于恢复保障，不替代预定完整epoch终点。正式9508评估尚未开始，无新增REC结果；仍按约2.9秒/步估算首个task结果06:40—07:10，后接同预算原生控制。Nr/Sr不启动，当前配置与§261的训练spec不变。

本轮同时检查了正式评估接续：先安装新增任务参数再严格加载训练状态，保持作者原生平均框/last-bbs/9508条，不改变GT用途、输出选择或阈值。此处是代码检查，实际GPU正式评估仍待完整训练终点。新证据位于refine-logs/eg3dvg_scan_task_20260921_v1/task/recovery_checkpoint_512.json及runtime_20260921_0135.json。


## 20.263 首轮Scan正式候选排名分析已接续；作者预测缓存复算通过（2026-09-21 02:04 CST）

复用§245已经审计通过的9508条作者ScanRefer预测，新增scripts/analyze_eg3dvg_scan_candidates.py作CPU离线排名分析，未重跑模型、未改分数或指标。已有候选上界和平均框收益仍以§245为准，不重复作为新成果。新增结果是：@0.25有合格平均框却未选中的3541条，最佳合格排名第2/3—16/17—64/65—256分别520/771/1018/1232；@0.50对应3402条分别515/833/963/1091。最高合格分数与不合格框并列导致的排名歧义均为0；脚本保留按实际分数计算的排名区间，不伪造GPU并列顺序。当前证据仍不能将IoU合格等同语义身份正确，不能把候选覆盖与选择的计数关系解释成单个评分器的因果贡献。

新分析先校验formal receipt/audit及预测SHA，重新验证实际bbs/bbf命中，再统计全部256框覆盖、同Query原框与平均框修复/破坏、合格候选的排名。bbs仍是唯一主输出，bbf仅诊断。作者缓存回执pretrained_candidate_rank_analysis.json与既有5542/4952完整一致，0模型forward/0优化步。新文件candidate_rank_analysis.json不覆盖原candidate_analysis.json。

独立CPU等待器15960已启动，每300秒等待本轮task/evaluation_01与native/evaluation_01的正式audit，逐臂完成同样的离线分析。脚本与spec哈希固定于candidate_analysis_spec.json，运行前完成Python3.7语法检查和真实9508行作者缓存重计。尚未取得新模型formal，不将等待器启动写成候选性能改善。当前训练controller、模型、优化器、数据、训练预算、正式推理规则均未改动；没有新增GPU任务或Nr/Sr训练。

实查时间2026-09-21T02:04:26.670130+08:00：controller14125、训练14301及分析等待15960均可读取到匹配的/proc命令；task epoch1为1143/6082更新、9144条，尚在训练。检查记录rank_queue_live.json。完整Scan目标与三数据集目标均未完成。


## 20.264 ScanRefer EG任务读取首轮训练完成，9508条正式评估已启动（2026-09-21 06:11 CST）

06:06:59，task epoch1按固定预算完成48655条、6082次更新，训练耗时17926.479秒，退出码0。199项冻结参数保持不变，训练未读取验证行、未按中间loss选择checkpoint。终点latest.pth的SHA为10e07ac58b301534963c744033bca804754d4b4261ead7f2629e13dfdf0f3ac6，父权重和训练spec与§261一致；峰值已分配显存34599586816字节。完整6082步日志已保存，包含每批实际点SHA、行身份与两组任务矩阵梯度。

06:07:49，8条评估预检完成，严格加载上述6082步终点；评估期间模型状态未变化，0优化、未创建优化器。预检的3/2命中只对应8条，不是正式准确率。正式评估spec SHA为3ab2523ff251456f548cd69169d7ae88ccb2e99a497a3ed05eead009635a68a7。06:11:34直接读取/proc确认controller14125、正式评估子进程25078和离线分析等待器15960均存活；评估进程命令明确为task/evaluation_01/spec.json的formal阶段。06:10观察日志已完成512/9508条，累计97.14秒，尚无完整formal receipt/audit。

当前阶段已经由训练切换到完整正式评估，尚不能判断REC涨点。按早期约0.19秒/行估计正式推理还需约半小时，连同终态导出与审计，首份结果仍预计06:40—07:10 CST；只按真实9508条终态报告，不将预检或部分行比例外推为正式成绩。评估后接续同预算native首轮，完成逐行和训练点SHA配对后区分普通续训与模块增量。Nr/Sr训练继续暂缓，Mask不设晋级门槛，作者平均框与bbs主输出不变。

新证据：refine-logs/eg3dvg_scan_task_20260921_v1/task/epoch_01/receipt.json、updates.jsonl、task/evaluation_01/preflight/receipt.json及evaluation_transition_20260921_0611.json。当前训练完成与预检通过属于执行证据；ScanRefer双Acc超过原生EG及三数据集总体目标仍未证明。


## 20.265 ScanRefer任务读取首轮正式REC为58.0353/51.5355%，未超过作者父模型；同预算原生控制继续（2026-09-21 06:45 CST）

06:37:56，完整9508条正式评估结束，主输出last/bbs为5518/4900，即Acc@0.25=58.03533866%、Acc@0.50=51.53554901%。相对本机作者EG父模型5542/4952，少24/52个命中，下降0.252419/0.546908个百分点；未达到同一checkpoint至少5543/4953的当前目标。相对历史V99的5572/4797为-54/+103，不能将两系统各自较好的单项拼成新结果。bbf诊断5521/4888，不替换bbs主口径。Mask没有晋级门槛，作者原生平均框与Mask计算保持。

本轮checkpoint SHA仍为10e07ac58b301534963c744033bca804754d4b4261ead7f2629e13dfdf0f3ac6，实际经过6082步训练；formal评估本身0优化、全部模型状态未改变。正式rows SHA为664abd76ce4d1bd5f22614614555773b06b6373a420872c9458d39a87baf8357，candidates SHA为786981c75dd3e1b0bd952fa4d01990554d8ffd6c8935579c664dca4d350d09cd。06:38:01审计通过：完整9508条、所选Query与导出候选一致，独立IoU重算最大误差3.86402e-6。已额外对父模型与新模型逐行核对row/scene/target/表达、点SHA及GT框一致，@0.25修复257、破坏281；@0.50修复273、破坏325。这里只能评价第一轮相对父模型的变化；同预算native尚无终态，不能将下降完全归因于新增读取。

06:42:51离线候选分析完成，0模型forward/0优化，不改变推理。全部256个未过滤原框oracle为9029/7993，作者平均框oracle为9035/8252，后者相对父模型9083/8354下降48/102。无合格平均框的失败473/1256条，有合格平均框却未选中3517/3352条。两阈值中最佳合格候选第2/3—16/17—64/65—256名分别559/731/1118/1109和560/796/1053/943，没有首个合格排名的分数并列歧义。候选覆盖亦下降，不能套用“候选更好、仅选择更差”的统一解释；oracle及排名是GT离线诊断，不能部署，也不能证明语义身份或回归/评分的因果贡献。

固定同一本轮已选Query，原框5508/4693，平均框5518/4900；平均框@0.25修复71、破坏61，@0.50修复312、破坏105。严格阈值净收益207仍说明Mask-derived几何在原生REC中发挥作用，取消Mask验收不等于应删除Mask计算或loss。未基于这些结果改输出或添加新Gate。

controller14125按原计划已启动native epoch1子进程26414，从同一作者父权重独立开始同预算原生续训。06:44:59实际日志112/6082步、896条、累计332.326秒，GPU进程持续存在；预计约中午得到native首轮完整结果。之后核对全部训练点SHA与正式逐行输入、计算同预算修复/破坏。当前首轮task未超过父模型，控制器在完成配对后按已冻结三epoch上限接续epoch2，不中途修改LR/损失/结构、不增加G标签、不挑中间权重，也不启动Nr/Sr。首轮负结果完整保留，尚未宣告整个三epoch实验终态失败。

执行证据：refine-logs/eg3dvg_scan_task_20260921_v1/task/evaluation_01/formal/{receipt,audit,candidate_rank_analysis}.json、task_epoch1_vs_pretrained.json、native_started_20260921_0644.json。首轮task正式结果已知，同预算方法增量、后续epoch和三数据集目标仍未完成。


## 20.266 ScanRefer EG 首轮同预算配对完成（2026-09-21 12:10 CST）

原生控制于11:36完成48655条训练、6082次更新，耗时17791.6525秒；199项冻结参数保持不变，训练期间验证输入为0。正式9508条评估于12:06:29完成，耗时1735.1054秒，固定last/bbs结果5537/4934（58.2352%/51.8931%）；bbf为5527/4932，仅作诊断。独立导出审计通过，全部选中候选与导出对应，最大IoU重算差3.96535e-6。

| 同一9508条、last/bbs | Acc@0.25命中 | Acc@0.50命中 |
|---|---:|---:|
| 作者预训练父模型 | 5542 | 4952 |
| 原生续训epoch1 | 5537 | 4934 |
| 任务读取epoch1 | 5518 | 4900 |

原生续训相对父模型为-5/-18；任务读取相对同预算原生控制为-19/-34（-0.1998/-0.3576个百分点），相对父模型仍为-24/-52。任务相对原生的逐行修复/破坏分别156/175、181/215。控制器确认两臂训练点输入一致、正式评估输入一致。因此本轮未证明任务读取独立增益，不能继续归为“同预算控制尚未完成”；也不能将首轮结果扩大为预设最多三轮实验的终态。

保持既定网络、seed2027、训练预算、损失与输出口径，按原计划进入第二轮完整配对。首轮没有达到同一任务模型至少5543/4953的双阈值目标。Nr3D/Sr3D训练继续暂停，G策略不插入当前实验。Mask仍不设指标门槛，原生训练与平均框计算保留。候选几何分析待其独立回执收齐后追加，不用其他模型数据代填。

原生终点checkpoint SHA256：1ba000598046cee1367f6b04ea6657d9a9d7e463e983d4cd1a8e8d10141d5091；正式rows SHA256：740e5d4e17144b0d14d7e5b94bd51428004a0d04c74dfaad0c0cd2e4f43c5901；candidates SHA256：0f2383de84fd8f422881f892d22d9d3707d4179089a172e1f3dcafc7ec9ef1ab。证据位于refine-logs/eg3dvg_scan_task_20260921_v1/epoch_results.json及native/epoch_01、native/evaluation_01/formal，保留完整6082步日志。


## 20.267 首轮候选对照与第二轮实际接续（2026-09-21 12:13 CST）

原生epoch1候选分析于12:08完成，绑定正式回执及候选SHA，9508条，0模型前向/0优化，不改变推理。与任务epoch1相同的未过滤256框口径如下：

| 候选诊断 | 原生epoch1 @0.25/@0.50 | 任务epoch1 @0.25/@0.50 | 任务减原生 |
|---|---|---|---|
| 原始回归框oracle | 9019/7990 | 9029/7993 | +10/+3 |
| 作者平均框oracle | 9037/8289 | 9035/8252 | -2/-37 |
| 有合格平均框但未选中 | 3500/3355 | 3517/3352 | +17/-3 |
| 实际bbs命中 | 5537/4934 | 5518/4900 | -19/-34 |

计数恒等式为@0.25：-19=-2-17；@0.50：-34=-37+3。不能将该算术分解当作评分器/回归器的因果贡献，也不能由未选中数量减少3条就认定任务评分改善。原始框与平均框上界方向不同，说明必须继续区分几何输出形式；当前证据不足以确定Mask头或特定读取路径是单独原因。

原生同一已选Query的原始框命中5520/4705，平均框5537/4934；平均框修复/破坏72/55、345/116，净+17/+229。严格阈值有框未选中的3355条，第一合格候选排名2、3—16、17—64、65—256分别513、750、1144、948。所有oracle均为离线GT上界，不是部署结果或语义身份真值。

12:13实时确认控制器PID14125存活，子进程PID40024正在执行task epoch2，已有97/6082步、776条输入、elapsed298.55秒。两组任务矩阵梯度范数均非零；这仅确认更新执行，不是精度收益。没有重启或插入新模块，继续原定最多三轮完整配对。Nr/Sr仍暂停。新证据：native/evaluation_01/formal/candidate_rank_analysis.json及task_epoch2_live_20260921_1213.json。


## 20.268 用户切回MCLN/V99体系，Nr3D优先；EG训练已停止（2026-09-21 12:22 CST）

用户明确指令：先不管强baseline，继续自己的MCLN/V99最好体系，保留已接近目标的ScanRefer/Sr3D，重点解决Nr3D比原生MCLN低很多的问题。本条覆盖此前ScanRefer EG优先、最多三轮的继续执行顺序；不是依据中间验证挑选EG权重。

已核对PID命令后停止EG控制器14125及其第二轮训练子进程40024，并确认两者不再运行。停止时task epoch2为278步、2224条输入、839.968秒；保留现有检查点、首轮正式结果及运行日志。第二轮部分更新不得记为终态或独立配对负结果，未启动第二轮native。记录user_cancellation_20260921.json，没有伪造正常完成回执。第一次停止调用因远端没有python命令而未执行任何信号；改为已存在的隔离环境Python后执行成功。

研究基础重新绑定：ScanRefer保护E71+Parent+Geometry+V99完整系统5572/4797（58.6033/50.4523）；Nr3D保护其自身MCLN E57权重4475/3759（56.6527/47.5883）；Sr3D保护12139/10335（68.4813/58.3042）。V99是ScanRefer完整系统，不是三个数据集共用的单个检查点。当前引用原MCLN Nr参照59.82/51.38，对应严格超过至少4726/4059；距离历史保护需净增251/300。Sr原参照68.43/57.30，目前已略超，先保住现有结果，Nr取得有效增量后再研究小幅提升。Mask继续不设晋级指标门。

已实查Nr E57文件599090977字节，SHA256=76aa6cd49ca20a34e78509465f1185b1b9040e60807ad327d6b0876aeb6edba1，与保护契约一致。本次仅文件验证，0模型前向/0更新；该文件为历史权重平均产物，不能当作具备完整优化器状态的无缝续训检查点。新适配必须明确新优化器、同起点配对和固定单seed，不覆盖保护文件。

下一步优先复用Nr原生完整候选诊断及已有训练入口，核对当前可复用源码/环境/文本目标，再设计一项针对候选语义学习的最小配对；不恢复长期baseline重训，不搬入EG/PV骨干，不重复普通MoE/Gate扫描。Nr历史完整诊断在同一权重的当前源码上为4478/3763，不能冒充新方法提升；@0.25的3421失败里2995条有合法合格Full256框，426条无合格框，说明候选判别应优先，但不能声称全部都是排序问题。已有G标签替换仅在PV上有正证据，是否适合Nr MCLN须核对原生文本/其他实例/联合检测prompt及对比损失，尚未实现或启动新Nr训练。ScanRefer/Sr3D保护输出不随新Nr分支改动。


## 20.269 Nr3D语义标签替换实现及真实零更新预检完成（2026-09-21 12:40 CST）

沿用户指定MCLN/Nr3D主线新增models/nr3d_semantic_assignment.py，不新增网络参数或推理规则。仅在最后层、真实sample_dataset为nr3d的样本，对原始预测框IoU>0.5且未匹配任何GT的Query，将no-object目标替换为原生root文本目标0.6/0.2/0.2/0.1；保留非单位目标质量、熵项、eos=0.1和criterion实际归一化。全部已匹配Query、其他层、几何/Mask损失、对比目标及联合ScanNet prompt不变。IoU仅是几何资格代理，不能称为可靠实例身份真值。

替换差分直接进入原生loss_pos_align，原生聚合负责最后系数：Nr3D为1/7，ScanRefer为0.5/7，不能照搬PV G的系数。合成测试从实际losses.py提取原生CE方法，检查显式重标记值/梯度、严格0.5边界、其他GT/检测样本保护、空集合、末层绑定和几何detach；通过。初始合成夹具因float64损失除以float32一维分母产生2.38e-7精度差，修正夹具分母dtype；预检初稿系数也已在运行前纠正。模块本身未因此改动。新鲜同模型家族审查通过此限定预检，不冒充跨家族验证。

实际在bdetr环境、既有fixed_source、受保护E57严格加载后完成16条既定fit训练样本前向，seed2027，4个batch。14条样本共69个未匹配Query符合资格。完整原生总损失差与1/7标签差分一致；显式标签重写最大CE误差2.3841858e-7，最大logit梯度误差3.7252903e-9；非选中Query不受差分梯度影响，资格框无梯度，全部模型state逐项未改变。数据加载后耗时4.0844秒，峰值分配1955633664字节。

这是eval模式冻结模型的真实数据零更新执行检查，0优化步、0正式验证行，不证明训练模式、模型参数反传、优化器恢复或REC收益。当前未启动完整Nr训练，既有4475/3759及ScanRefer/Sr3D结果不变。下一步明确训练更新范围与同预算控制，先完成真实优化器预检，再进行原生/标签替换配对。计划见docs/NR3D_SEMANTIC_ASSIGNMENT_2026-09-21.md；代码审查、合成/实际回执及日志见refine-logs/nr3d_semantic_assignment_20260921_v1。EG仍停止，不恢复旧多源Gate扫描或多seed。


## 20.270 Nr3D配对优化器预检通过，完整适配入口启动（2026-09-21）

继续MCLN/V99受保护路线，Nr3D优先；EG停止，ScanRefer/Sr3D权重不改。新增scripts/train_nr3d_semantic_assignment_pair.py：原生和末层标签替换两组均严格加载E57，新AdamW统一1e-5、wd0.0005、clip0.1、seed2027，32919条Nr训练表达各一次，batch4、每组8230步。两组共用点输入、顺序和逐步随机种子。本次共同适配不增强、不混联合检测行，保留原生中间对象监督、Mask、对比及source-selector损失；不是历史完整训练的复现或优化器无缝恢复。

第一次两组各两步更新成功，但第二份临时检查点保存因数据盘ENOSPC失败，退出134；不算预检通过。临时归档传输未完成，未获得SHA验证归档。保留日志后只删除本次两个可再生临时文件（794108937及323944448字节），历史保护和EG检查点均未动；temporary_cleanup.json记录路径与空间。改用系统盘/root/mcln_nr_semantic_states_20260921_v1存两组恢复文件，日志留数据盘。

独立optimizer_preflight_v2通过：两组各2步、716参数张量有梯度，梯度范数有限且非零；首步训练预测一致；保存后实际严格重载模型与优化器状态相等；峰值GPU分配8987907072字节，0正式评估行。预检权重不进入完整训练。跨进程第二步数值有变化，不声称逐位执行确定性。

完整入口已启动，控制器PID43284，输出/root/autodl-tmp/mcln_nr_semantic_assignment_20260921_v1/pair。先执行完整7899条父模型评估，再完成同预算配对训练及两个终点完整评估；不挑中间权重。每512步保存恢复状态，终点严格重载保存模型后评估。当前尚无新正式准确率或终态；保护Nr4475/3759不变。具体进度以live observation为准，启动不能等同训练完成。


结果分析补充：scripts/analyze_nr3d_semantic_assignment_pair.py只接受完整8230步/32919训练行、7899评估行及终点重载回执；独立重算父模型/原生/标签替换的两阈值命中、三组配对修复破坏、合法与未过滤候选上界、候选缺失和未选中分解，并与训练回执交叉核对。当前仅完成语法检查，真实终态尚不存在，未声称结果分析已执行。不会修改训练或据中间指标选择权重。


## 20.271 Nr3D完整起点复核结束，配对已进入实际更新（2026-09-21 13:39 CST）

本轮受保护E57在固定输入/既有源码下完整7899条起点为4477/3759，即56.6781%/47.5883%。独立重算逐行IoU命中与summary一致，行ID完整有序；逐行文件12912270字节，SHA256=c404d3c072d0efa98450aab14ba52c5a198958f35c53a347e914ff9e453b8af7，保存在远端pair/baseline_rows.json，不将大文件加入Git。相对历史4475/3759为+2/0，这是零更新执行差异，不能计为标签策略收益或覆盖历史保护结果；后续同时报告自身真实起点与同预算控制。

13:39控制器43284仍存活，两组已实际完成49/8230步。此步原生loss17.172098、梯度范数35.416447；标签替换loss17.935604、梯度范数84.288139；两组均716项参数有梯度，资格数量[2,1,2,3]。截至该步未触发有限性/初始一致性检查错误。更新耗时93.9893秒，约1.92秒/配对步，剩余训练约4.4小时，之后还有两组完整终点评估；时间为初期估计，不是完成承诺。尚无训练终态REC。

结果分析脚本以7899条构造数据补测配对修复/破坏、错误分解和目标差距，均通过；故意改变点SHA后拒绝比较。属于统计代码测试，formal_model_rows=0，不是模型准确率证据。回执与实际起点审核、运行观察一并归档。现有训练/推理/保护权重均不变，继续Nr3D优先。


## 20.272 Nr3D两组第512步恢复文件通过只读检查（2026-09-21 13:56 CST）

同一配对任务13:55已实际完成558/8230步，持续运行，无终态。两组第512步恢复文件均794164425字节，包含1144项模型状态和716项优化器状态；torch.load成功，浮点模型状态及AdamW一阶/二阶矩均有限，所有优化器step均为512，读取前后文件尺寸/mtime一致。native SHA256=46c74325444edcddba62d0c1ee44ea9b825661f9742e2c1e2251975074090272；replacement SHA256=ee4cf14119882e75740e15a37cd30d2a2145fb90a9f8f59e160ade8cb99302a1。它们是当前恢复文件快照，后续按计划覆盖，不能当成固定终点权重。

该检查只证明保存张量和优化器状态完整，不是重新实例化完整模型并恢复训练的测试，不用于挑权重。558步用时1080.735秒；某些批次裁剪前梯度较大（该步native1066.275、replacement52.575），均有限，仍执行既定clip0.1；没有据此改变学习率或中断。尚无训练后REC结果。恢复审核与实际运行观察已归档；原实验、保护权重、seed和预算不变。
