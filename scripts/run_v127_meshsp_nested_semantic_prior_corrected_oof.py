#!/usr/bin/env python
"""Nested OOF screen of V127 prior-corrected listwise abstention."""
...
import models.rec_semantic_antisymmetric_utility as v127_model
...
V127_CRITIC_EPOCHS = 50
V127_ROW_BATCH_SIZE = 512
V127_INFERENCE_BATCH_SIZE = 1024
V127_CRITIC_LEARNING_RATE = 1e-3
V127_CRITIC_WEIGHT_DECAY = 1e-3
V127_FALSE_POSITIVE_COST = 4.0
...
def _pair_inputs(
    ...
    if count <= 0:
        raise ValueError("V127 pair population must be nonempty")
    for value in inputs.values():
        if (value.dtype != torch.float32 or value.device.type != "cpu"
                or value.shape[0] != count
                or not bool(torch.isfinite(value).all().item())):
            raise ValueError("V127 pair input layout changed")
...
def build_listwise_training_population(prediction, records):
    ...
    if (probabilities.dtype != torch.float32
        ...
        raise ValueError("V127 model probability layout changed")
    rows = torch.arange(count)
    ...
    if (tuple(ious.shape) != (count, QUERY_COUNT, VARIANT_COUNT)
            or not bool(torch.isfinite(ious).all().item())):
        raise ValueError("V127 candidate IoU layout changed")
    ...
    if (not bool(positive_rows.any(dim=0).all().item())
        ...
        raise ValueError("V127 listwise population lacks action diversity")
...
def prior_corrected_positive_row_weight(population):
    """Preserve a fixed 4:1 baseline-to-fix total loss budget."""
    values = []
    for suffix in ("025", "050"):
        positive = population["candidate_positive_rows"][suffix]
        baseline = population["baseline_target_rows"][suffix]
        if (not isinstance(positive, int) or not isinstance(baseline, int)
                or positive <= 0 or baseline <= 0
                or positive + baseline != population["row_count"]):
            raise ValueError("V127 prior-correction counts changed")
        values.append(
            baseline / (V127_FALSE_POSITIVE_COST * positive)
        )
    weights = torch.tensor(values, dtype=torch.float32)
    if not bool(torch.isfinite(weights).all().item()):
        raise RuntimeError("V127 prior-correction weights are non-finite")
    return weights


def listwise_abstain_loss(
        candidate_logits, candidate_positive, positive_row_weight):
    """Prior-corrected acceptable-action loss with zero-score abstention."""
    if (candidate_logits.dtype != torch.float32
            ...
            or positive_row_weight.dtype != torch.float32
            or tuple(positive_row_weight.shape) != (2,)
            or positive_row_weight.device != candidate_logits.device
            or not bool(torch.isfinite(positive_row_weight).all().item())
            or not bool(positive_row_weight.gt(0.0).all().item())
        ...
        raise ValueError("V127 listwise loss tensors changed")
    ...
    row_weight = torch.where(
        ...
        positive_row_weight[None, :].expand_as(denominator),
    )
    loss = ((denominator - numerator) * row_weight).sum() / row_weight.sum()
    if not bool(torch.isfinite(loss).item()):
        raise RuntimeError("V127 listwise abstain loss became non-finite")
    return loss, baseline_target, row_weight
...
def fit_semantic_listwise_critic(prediction, records, device):
    ...
    if pair_count < 100:
        raise ValueError("V127 listwise candidate population is too small")
    row_count = len(records)
    positive_row_weight = prior_corrected_positive_row_weight(population)
    resolved = torch.device(device)
    _set_hierarchical_seed(resolved)
    model = v127_model.SemanticAntisymmetricUtilityRanker().to(resolved)
    optimizer = torch.optim.AdamW(
        model.parameters(),
        lr=V127_CRITIC_LEARNING_RATE,
        weight_decay=V127_CRITIC_WEIGHT_DECAY,
    )
    shuffle_generator = torch.Generator(device="cpu")
    shuffle_generator.manual_seed(0)
    epoch_receipts = []
    for epoch in range(V127_CRITIC_EPOCHS):
        model.train()
        order = torch.randperm(row_count, generator=shuffle_generator)
        total_loss = 0.0
        total_weight = 0.0
        for start in range(0, row_count, V127_ROW_BATCH_SIZE):
            batch_rows = order[start:start + V127_ROW_BATCH_SIZE]
            ...
            loss, _, row_weight = listwise_abstain_loss(
                dense_logits, dense_positive,
                positive_row_weight.to(resolved),
            )
            optimizer.zero_grad(set_to_none=True)
            loss.backward()
            clip_grad_norm_(model.parameters(), 1.0)
            optimizer.step()
            batch_weight = float(row_weight.sum().detach().item())
    ...
    receipt.update({
        "epochs": V127_CRITIC_EPOCHS,
        "row_batch_size": V127_ROW_BATCH_SIZE,
        "baseline_score": 0.0,
        "false_positive_cost": V127_FALSE_POSITIVE_COST,
        "positive_row_weight": {
            "025": float(positive_row_weight[0].item()),
            "050": float(positive_row_weight[1].item()),
        },
        "epoch_receipts": epoch_receipts,
        "state_sha256": v99.tensor_sha256(*[
            value for _, value in sorted(model.state_dict().items())
...
def all_hard_candidate_inputs(prediction):
    ...
    if (probabilities.dtype != torch.float32
            ...
            or tuple(baselines.shape) != (count,)
            or not bool(torch.isfinite(probabilities).all().item())):
        raise ValueError("V127 all-candidate probability layout changed")
    ...
    if pair_rows.numel() <= 0:
        raise ValueError("V127 all-candidate population must be nonempty")
    pair_baselines = baselines[pair_rows]
    baseline_query = pair_baselines.div(
        VARIANT_COUNT, rounding_mode="floor"
    )
...
def predict_pairwise_utility(model, inputs, device):
    ...
    logits = []
    with torch.no_grad():
        for start in range(0, count, V127_INFERENCE_BATCH_SIZE):
            stop = min(start + V127_INFERENCE_BATCH_SIZE, count)
            logits.append(model(**{
                key: value[start:stop].to(resolved)
                for key, value in inputs.items()
            }).cpu().float())
    ...
    if (tuple(output.shape) != (count, 2)
        ...
        raise RuntimeError("V127 utility logits changed")
    return output
...
def select_all_hard_candidates(model, prediction, device):
    ...
    if (proposals.dtype != torch.long
        ...
        raise RuntimeError("V127 all-candidate selection contract changed")
    if bool((accepted & proposals.eq(baselines)).any().item()):
        raise RuntimeError("V127 accepted a baseline as a candidate")
    return {
        "proposals": proposals,
        "selected": selected,
        "accepted": accepted,
...
def pairwise_utility_accept_mask(base_accepted, utility_logits):
    if (base_accepted.dtype != torch.bool or base_accepted.dim() != 1
        ...
        raise ValueError("V127 utility gate tensors changed")
...
def main(argv=None):
    ...
    for held in range(5):
        ...
        if (set(records[index]["scan_id"] for index in inner_fit_indices)
            ...
            raise RuntimeError("scene leakage into V127 inner calibration")
        ...
        if not torch.equal(prediction["baselines"], baselines[held_indices]):
            raise RuntimeError("V127 OOF baseline identity changed")
        fold_selection = select_all_hard_candidates(
    ...
    if (veto_reasons["selected_margin_violations"] != 0
        ...
        raise RuntimeError("V127 all-candidate Pareto contract changed")
    protected_after = capture_immutable_artifact_identities(protected_paths)
    if protected_after != protected_before:
        raise RuntimeError("protected artifacts changed during V127")
    protected_metadata_after = {
        name: file_sha256(path)
        for name, path in protected_metadata_paths.items()
    }
    if protected_metadata_after != protected_metadata_before:
        raise RuntimeError("protected V127 cache metadata changed during OOF")
    script_sources = {
        "v127": str(Path(__file__).resolve()),
        ...
        "v127_model": str(Path(v127_model.__file__).resolve()),
        "v108": str(
    ...
    report = {
        ...
        "protocol": {
            "architecture": (
                ...
                "zero-score baseline abstain action"
            ),
            ...
            "critic_objective": (
                ...
                "fix-target rows receive closed-form prior correction"
            ),
            "proposal_margin": v99.V97_MARGIN,
            "acceptance": (
                "argmax REC@.25 antisymmetric utility among all V115-Pareto "
                "hard candidates, then accept iff the maximum is nonnegative"
            ),
            "candidate_scope": (
            ...
            "listwise_row_weight": (
                "baseline-target=1; fix-target=N_baseline/(4*N_fix) "
                "computed independently per threshold and inner fold"
            ),
            "false_positive_cost": V127_FALSE_POSITIVE_COST,
            "grid_search": False,
            "selection_data": (
            ...
            "critic_candidate_dim": v127_model.V123_CANDIDATE_DIM,
            "critic_pair_structure": "shared_score_difference",
            "critic_model_gain_scale": "learned_positive_per_threshold",
            "critic_epochs": V127_CRITIC_EPOCHS,
            "critic_row_batch_size": V127_ROW_BATCH_SIZE,
            "critic_inference_batch_size": V127_INFERENCE_BATCH_SIZE,
            "critic_learning_rate": V127_CRITIC_LEARNING_RATE,
            "critic_weight_decay": V127_CRITIC_WEIGHT_DECAY,
            "critic_class_weights": (
                "closed_form_prior_correction_with_false_positive_cost_4"
            ),
            "critic_neutral_candidates_are_negative_actions": True,
            "critic_acceptance": (
                "all_candidate_argmax_then_utility_logit025_nonnegative"
            ),
        ...
        "prediction_sha256": v99.tensor_sha256(
            ...
            v115_proposals, utility_logits, accepted
        ),
        "oof": {
    ...
    try:
        ...
        while written < len(payload):
            ...
            if count <= 0:
                raise OSError("V127 output write made no progress")
            written += count
        os.fsync(descriptor)
    finally:
        os.close(descriptor)
