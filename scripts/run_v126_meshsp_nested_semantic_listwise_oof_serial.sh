...
OUTPUT=$BASE/v126_meshsp_nested_semantic_listwise_oof.json
LOG=$BASE/v126_meshsp_nested_semantic_listwise_oof.log
EXIT=$BASE/v126_meshsp_nested_semantic_listwise_oof.exit
SESSION=v126_nested_semantic_listwise_oof
SELF=$(readlink -f "$0")
...
if [[ "${1:-}" == "--inner" ]]; then
    ...
    env CUDA_VISIBLE_DEVICES=0 \
        ...
        "$PYTHON" scripts/run_v126_meshsp_nested_semantic_listwise_oof.py \
...
cd "$REPO"
require_sha 97b17c9bbb56b2654a10c12e7cb72db507528a5a21d0e7b39b928d8d0b750ded \
    scripts/run_v126_meshsp_nested_semantic_listwise_oof.py
...
for path in "$OUTPUT" "$LOG" "$EXIT"; do
    if [[ -e "$path" ]]; then
        echo "Refusing to overwrite existing V126 output: $path" >&2
        exit 74
    fi
done
if [[ $(nvidia-smi -L | wc -l) -ne 1 ]]; then
    echo "V126 requires the frozen single-GPU topology" >&2
    exit 75
fi
...
if pgrep -af '[r]un_v126_meshsp_nested_semantic_listwise_oof.py' >/dev/null; then
    echo "V126 process already exists" >&2
    exit 75
fi
if screen -ls 2>/dev/null | grep -q "[.]$SESSION"; then
    echo "V126 screen session already exists" >&2
    exit 75
fi
available_kb=$(df -Pk /root/autodl-tmp | awk 'NR==2 {print $4}')
if [[ "$available_kb" -lt 2621440 ]]; then
    echo "V126 requires at least 2.5 GiB free on /root/autodl-tmp" >&2
    exit 75
fi
...
if ! screen -ls 2>/dev/null | grep -q "[.]$SESSION" \
        && [[ ! -e "$EXIT" ]]; then
    echo "V126 screen failed to remain alive and produced no exit receipt" >&2
    exit 75
fi
