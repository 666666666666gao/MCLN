"""One preregistered P2 epoch: shared frozen inputs, two isolated readout heads."""
...
def main():
    ...
    from scripts.nr3d_pair_readout_contract import CONTRACT, covered_ranking_loss, decide, split_rows
    ...
    backbone.load_state_dict({key[7:]: value for key, value in checkpoint["model"].items()}, strict=True)
    ...
    class ReadoutDataset(Joint3DDataset):
        def __getitem__(self, index):
            ...
            result["p2_id"] = np.int64(self.p2_ids[index])
            return result
    ...
    for mode in ["global", "pair"]:
        ...
        head = CandidateEdgeDirectScorer(mode).cuda()
        head.spatial.load_state_dict(backbone.decoder[-1].self_attn.state_dict(), strict=True)
        heads[mode] = head
    for name, value in heads["global"].state_dict().items():
        assert torch.equal(value, heads["pair"].state_dict()[name]), name
    ...
    def forward_batch(batch):
        ...
        return batch, outputs, shared, ious
    ...
    print("P2 PREFLIGHT", json.dumps(preflight), flush=True)
    started = time.time()
    ...
    for batch_index, raw_batch in enumerate(loaders["fit"]):
        batch, outputs, shared, ious = forward_batch(raw_batch)
        seen.extend(batch["p2_id"].tolist())
        ...
        if count:
            ...
            skipped += 1
        del batch, outputs, shared, ious, target_ious, target_valid
        if opt.preflight_only:
            ...
            print("P2 FIT-ONLY ZERO-STEP SMOKE PASS", json.dumps(smoke), flush=True)
            hook.remove()
            verify_inputs()
            return
        if (batch_index + 1) % 100 == 0 or batch_index + 1 == len(loaders["fit"]):
            elapsed = time.time() - started
            print("P2 FIT", json.dumps({"batch": batch_index + 1, "total": len(loaders["fit"]),
    ...
    assert steps["global"] == steps["pair"] == len(loaders["fit"]) - skipped
    assert all(p.grad is None for p in backbone.parameters())
    ...
    with (opt.output / "holdout_rows.jsonl").open("x") as row_stream, torch.no_grad():
        for batch_index, raw_batch in enumerate(loaders["holdout"]):
            ...
            for mode, head in heads.items():
                scores[mode] = head(**shared)["query_scores"]
            ...
            for bid, source_id in enumerate(batch["p2_id"].tolist()):
                ...
                mask_ious = ((predicted & gt_mask.unsqueeze(0)).sum(-1).float()
                             / (predicted | gt_mask.unsqueeze(0)).sum(-1).float())
                ...
                for name, values in scores.items():
                    query = int(values[bid].masked_fill(~valid, -float("inf")).argmax()) if legal_ids.numel() else None
                ...
                for label, ids in [("before_filter", order), ("after_filter", order[valid[order]])]:
            ...
            if (batch_index + 1) % 100 == 0 or batch_index + 1 == len(loaders["holdout"]):
                row_stream.flush()
                print("P2 HOLDOUT", json.dumps({"batch": batch_index + 1, "total": len(loaders["holdout"]),
    ...
    print("P2 COMPLETE", json.dumps(receipt, sort_keys=True), flush=True)
