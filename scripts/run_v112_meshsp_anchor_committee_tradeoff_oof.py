#!/usr/bin/env python
"""Train-only nested OOF audit of a threshold-tradeoff anchor committee."""
...
MIN_BOOTSTRAP_LOWER = {"025": 34, "050": 160}
MIN_SUBGROUP_BOOTSTRAP_LOWER = {
    "corrected": {"025": 20, "050": 120},
    "regular": {"025": 0, "050": 20},
}
...
def fit_seeded_model(records, statistics, device, seed):
    if seed not in ENSEMBLE_SEEDS:
        raise ValueError("V112 seed is outside the frozen committee")
...
def _validate_policy_inputs(
    ...
    if (proposals.dtype != torch.long or baselines.dtype != torch.long
        ...
        raise ValueError("V112 proposals and baselines must align as int64")
    ...
    if (anchor_head_gain.dtype != torch.float32
        ...
        raise ValueError("V112 anchor-risk tensors have invalid shape or dtype")
    if (not bool(torch.isfinite(anchor_head_gain).all().item())
        ...
        raise ValueError("V112 anchor-risk tensors are invalid")


def policy_accept_mask(
        proposals, baselines, anchor_head_gain, head_risk, policy):
    if policy not in POLICY_GRID:
        raise ValueError("V112 policy is outside the frozen grid")
...
def select_nested_policy(
    ...
    return {
        "selection_rule": (
            "strictly_improve_v109_anchor_delta025_while_retaining_at_least_"
            "95_percent_of_anchor_delta050;require_nonnegative_source_folds;"
            "maximize_min_fold025_then_"
...
def predict_anchor_committee(models, records, statistics, device):
    if len(models) != len(ENSEMBLE_SEEDS):
        raise ValueError("V112 requires exactly three committee members")
    ...
    if (not bool(torch.isfinite(result["anchor_agreement"]).all().item())
        ...
        raise ValueError("V112 anchor-agreement tensor is invalid")
    return result


def policy_risk_family(policy):
    return {
        key: value for key, value in policy.items()
        if key != "aggregate_lcb_margin"
    }


def derive_deployment_policy(meta_folds):
    policies = [row["selected_policy"] for row in meta_folds]
    families = [policy_risk_family(policy) for policy in policies]
    family_consistent = all(family == families[0] for family in families[1:])
    margins = [policy["aggregate_lcb_margin"] for policy in policies]
    counts = {
    ...
    policy["aggregate_lcb_margin"] = selected_margin
    return {
        "rule": "meta_fold_majority_margin_ties_choose_higher_margin",
        "risk_family_consistent": family_consistent,
...
def acceptance_gate(
    ...
    return {
        ...
        "delta050_at_least_tradeoff_floor": (
        ...
        "meta_risk_family_consistent": (
            deployment_policy["risk_family_consistent"]
        ),
...
def write_exclusive(path, payload, label):
    ...
    try:
        ...
        while written < len(payload):
            ...
            if count <= 0:
                raise OSError("{} write made no progress".format(label))
            written += count
        os.fsync(descriptor)
    finally:
        os.close(descriptor)
...
def main(argv=None):
    ...
    parser.add_argument("--v111-report", required=True)
    ...
    prediction_cache = Path(args.prediction_cache).expanduser().absolute()
    if output == prediction_cache:
        raise ValueError("V112 report and prediction cache must differ")
    for path in (output, prediction_cache):
        if path.exists():
    ...
    v111_path = Path(args.v111_report).expanduser().resolve(strict=True)
    if file_sha256(v111_path) != V111_REPORT_SHA256:
        raise ValueError("V111 source report SHA-256 changed")
    v111_source = json.loads(v111_path.read_text(encoding="ascii"))
    v111_oof = v111_source.get("oof", {})
    v111_anchor = v111_source.get("anchor_contract", {})
    if (v111_source.get("validation_data_accessed") is not False
            or v111_oof.get("passed") is not False
            or v111_oof.get("diagnostics", {}).get("delta_hits025") != 72
            or v111_oof.get("diagnostics", {}).get("delta_hits050") != 246
            or v111_anchor.get("raw_prediction_sha256_match") is not True
            or v111_anchor.get("base_policy_delta_match") is not True):
        raise ValueError("V111 source exact-fallback contract changed")
    ...
    if (len(joined_rows) != EXPECTED_SAMPLE_COUNT
        ...
        raise ValueError("V112 full-train identity changed")
    ...
    if (len(corrected_scenes) != EXPECTED_CORRECTED_SCENE_COUNT
        ...
        raise ValueError("V112 subgroup partition changed")
    ...
    if torch.cat([fold_indices[fold] for fold in range(5)]).sort().values.tolist(
        ...
        raise RuntimeError("V112 folds do not partition all rows")
    ...
    for held in range(5):
        ...
        if train_scenes & held_scenes:
            raise RuntimeError("scene leakage in V112 model fold")
        ...
        if not torch.equal(prediction["baselines"], baselines[held_rows]):
            raise RuntimeError("V112 OOF baseline identity changed")
    ...
    if (not anchor_contract["raw_prediction_sha256_match"]
        ...
        raise RuntimeError("V112 seed-0 anchor does not exactly reproduce V109")
    ...
    deployment_policy = derive_deployment_policy(meta_folds)
    ...
    if protected_after != protected_before or metadata_after != metadata_before:
        raise RuntimeError("V112 protected inputs changed during OOF")
    cache_document = {
        "schema": "rec-v112-anchor-committee-train-oof-prediction-cache-v1",
        "version": 1,
        "validation_data_accessed": False,
        "train_labels_only": True,
        "v109_report_sha256": V109_REPORT_SHA256,
        "v111_report_sha256": V111_REPORT_SHA256,
        "anchor_raw_prediction_sha256": anchor_raw_sha256,
        ...
        "anchor_head_gain": anchor_head_gain.tolist(),
        "head_risk": head_risk.tolist(),
        "anchor_agreement": anchor_agreement.tolist(),
        "baseline_ious": baseline_ious.tolist(),
        "proposal_ious": proposal_ious.tolist(),
        "nested_selected": selected.tolist(),
    }
    cache_json = json.dumps(
        cache_document, sort_keys=True, ensure_ascii=True,
        separators=(",", ":"), allow_nan=False,
    ).encode("ascii")
    cache_buffer = io.BytesIO()
    with gzip.GzipFile(
            fileobj=cache_buffer, mode="wb", compresslevel=9,
            mtime=0, filename="") as stream:
        stream.write(cache_json)
    cache_payload = cache_buffer.getvalue()
    write_exclusive(prediction_cache, cache_payload, "V112 prediction cache")
    cache_identity = {
        "path": str(prediction_cache),
        "sha256": hashlib.sha256(cache_payload).hexdigest(),
        "compressed_size": len(cache_payload),
        "uncompressed_size": len(cache_json),
        "schema": cache_document["schema"],
    }
    report = {
        "schema": "rec-v112-meshsp-anchor-committee-tradeoff-scene-oof-v1",
        "version": 1,
        "validation_data_accessed": False,
        ...
        "source": {
            ...
            "v111": {
                "path": str(v111_path),
                "sha256": V111_REPORT_SHA256,
                "exact_fallback_gate": copy.deepcopy(v111_oof),
            },
        },
        "protocol": {
            "architecture": (
            ...
            "selection_objective": (
                "strictly_improve_anchor_delta025_while_retaining_95_percent_"
                "of_anchor_delta050;"
        ...
        "oof": {
            ...
            "required_bootstrap_lower": copy.deepcopy(
                MIN_BOOTSTRAP_LOWER
            ),
        ...
        "prediction_cache": cache_identity,
        "source_sha256": {
            "v112": file_sha256(Path(__file__).resolve()),
            "v111": file_sha256(
                Path(__file__).resolve().parent
                / "run_v111_meshsp_anchor_committee_oof.py"
            ),
            "v110": file_sha256(
                Path(__file__).resolve().parent
    ...
    write_exclusive(output, payload, "V112 output")
    print(json.dumps({
        "output": str(output),
