...
OUTPUT=$BASE/v129_meshsp_text_box_relation_adapter_oof.json
LOG=$BASE/v129_meshsp_text_box_relation_adapter_oof.log
EXIT=$BASE/v129_meshsp_text_box_relation_adapter_oof.exit
SESSION=v129_text_box_relation_adapter_oof
SELF=$(readlink -f "$0")
...
if [[ "${1:-}" == "--inner" ]]; then
    ...
    env CUDA_VISIBLE_DEVICES=0 \
        ...
        "$PYTHON" scripts/run_v129_meshsp_box_relation_adapter_oof.py \
...
cd "$REPO"
require_sha bf0bff490760daa227671c5e38b365cfb6d3aeebe3c5b645981a539f572627e0 \
    scripts/run_v129_meshsp_box_relation_adapter_oof.py
require_sha b20bfd625f0f9ba437efe3e152bdc134d7b3f3426c9c4397e5f1cbb332fe72da \
    models/rec_box_relation_adapter.py
require_sha eff523e27f8bec84d75eee3490b1df9a10d6084900dd4852c266463da65c1b15 \
    tests/test_rec_box_relation_adapter.py
...
for path in "$OUTPUT" "$LOG" "$EXIT"; do
    if [[ -e "$path" ]]; then
        echo "Refusing to overwrite existing V129 output: $path" >&2
        exit 74
    fi
done
if [[ $(nvidia-smi -L | wc -l) -ne 1 ]]; then
    echo "V129 requires the frozen single-GPU topology" >&2
    exit 75
fi
...
if pgrep -af '[r]un_v129_meshsp_box_relation_adapter_oof.py' >/dev/null; then
    echo "V129 process already exists" >&2
    exit 75
fi
if screen -ls 2>/dev/null | grep -q "[.]$SESSION"; then
    echo "V129 screen session already exists" >&2
    exit 75
fi
available_kb=$(df -Pk /root/autodl-tmp | awk 'NR==2 {print $4}')
if [[ "$available_kb" -lt 2621440 ]]; then
    echo "V129 requires at least 2.5 GiB free on /root/autodl-tmp" >&2
    exit 75
fi
...
if ! screen -ls 2>/dev/null | grep -q "[.]$SESSION" \
        && [[ ! -e "$EXIT" ]]; then
    echo "V129 screen failed to remain alive and produced no exit receipt" >&2
    exit 75
fi
