"""Queue the single Nr F batch probe after the actual REC-only Scan formal gate."""
...
if decision['status']=='skipped_primary_rec_regression':
 ...
 if not audit['advance_to_nr3d_sr3d_rec']:
  ...
  assert run.returncode==0,'actual Nr F probe failed; inspect original log'
  result=dict(status='nr_real_batch_probe_complete',receipt=str(root/'actual_probe/receipt.json'),training_jobs_launched=0)
...
_,out,err=c.exec_command('screen -dmS mcln_pvg_nr_rec_competition_probe_v1 bash -c '+shlex.quote(inner),timeout=30);assert out.channel.recv_exit_status()==0,err.read().decode()
...
record=dict(time_cst=datetime.datetime.now(datetime.timezone(datetime.timedelta(hours=8))).isoformat(),process=process,poll_seconds=300,controller_sha256=hashlib.sha256(raw).hexdigest(),scope='one gated real Nr batch forward/backward; not Nr training',sr_gpu_job_queued=False)
