#!/usr/bin/env python
"""Exact train-cache parity audit for V99 materialization versus runtime."""
...
from train_dist_mod import _build_rec_hierarchical_runtime_batch
...
def _readonly_identity(path):
    ...
    if stat.S_ISLNK(entry.st_mode) or not stat.S_ISREG(entry.st_mode):
        raise ValueError("V99 artifact must be a regular file")
    mode = stat.S_IMODE(entry.st_mode)
    if mode != 0o444:
        raise ValueError("V99 artifact must have mode 0444")
    return {
...
def _comparison(left, right):
    ...
    if left.dtype.is_floating_point:
        ...
        result["max_abs"] = float(difference.max().item())
        result["different_elements"] = int(difference.ne(0).sum().item())
    else:
...
def main(argv=None):
    ...
    report = {
        "schema": "rec-v99-train-runtime-parity-audit-v1", "version": 1,
    ...
    if not all_equal:
        raise RuntimeError("V99 offline/runtime train parity failed")
