...
def main():
    ...
    for index,name in enumerate(['native_disabled','empty_mask_enabled','disabled_repeat']):
        ...
        for reader in [model.backbone_net.vsa.SA_rawpoints]+list(model.backbone_net.vsa.SA_layers):reader.mask_empty=(index==1)
        seed()
        trace={}
    ...
    result=dict(status='complete',time_cst=datetime.datetime.now(datetime.timezone(datetime.timedelta(hours=8))).isoformat(),
        ...
        scope='same frozen nonempty eight-row batch; native, empty-mask enabled, disabled repeat; no training or population equivalence claim',
        repeated_seed_exact=all(v['exact'] for v in results[1]['compared_to_first'].values()))
    assert all(v['exact'] for case in results[1:] for v in case['compared_to_first'].values())
