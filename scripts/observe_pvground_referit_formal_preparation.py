"""Observe the original CPU validation-input controller and verify downloaded contracts."""
import datetime
import hashlib
import json
import os
...
for name in ['controller.pid','controller.exit','tests.log','tests.exit','run.log','receipt.json']:
    if name in names:
        with s.open(root+'/'+name,'rb') as f:raw=f.read()
        (archive/name).write_bytes(raw)
        result[name]=json.loads(raw) if name.endswith('.json') else raw.decode()[-2400:]
if 'receipt.json' in result:
    for dataset,info in result['receipt.json']['datasets'].items():
        name=dataset+'_formal_input_contract.json';s.get(root+'/'+name,str(archive/name))
        assert hashlib.sha256((archive/name).read_bytes()).hexdigest()==info['contract_sha256']
...
result['original_process']=out.read().decode()
