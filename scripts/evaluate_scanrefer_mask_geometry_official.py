"""One fixed9508-row evaluation of protected V99, native-GT control and Mask-geometry candidate."""
...
def verify_training_endpoint(directory):
    ...
    assert baseline['native_gt_mask_geometry'] == baseline['native_gt']
    ...
    for reference in [receipt['baseline_metrics']['native_gt_mask_geometry'], receipt['terminal_metrics']['native_gt']]:
        assert all(receipt['terminal_metrics']['native_gt_mask_geometry']['rec_hits' + suffix] >= reference['rec_hits' + suffix]
...
def main():
    ...
    assert manifest['schema'] == 'mcln-scanrefer-mask-geometry-gt-official-input-v1'
    ...
    assert manifest['candidate_predeclared'] == 'native_gt_mask_geometry_v99'
    assert manifest['arms'] == ['protected_v99', 'native_gt_v99', 'native_gt_mask_geometry_v99']
    ...
    write_json(result_directory / 'protocol.json', {'authoritative_base_command': command,
        ...
        'native_loader_and_worker_seeding': True, 'formal_checkpoint_arm': 'native_gt_mask_geometry', 'formal_control_arm': 'native_gt', 'candidate_predeclared': 'native_gt_mask_geometry',
    ...
    for arm, label in [('native_gt', 'native_gt_v99'), ('native_gt_mask_geometry', 'native_gt_mask_geometry_v99')]:
        trained = torch.load(receipt['checkpoints'][arm]['path'], map_location='cpu')
        assert trained['schema'] == 'mcln-scanrefer-mask-geometry-core-state-v1'
        ...
        actual_names = [name for name, _ in protected.named_parameters() if name.startswith(('decoder.5.', 'prediction_heads.5.', 'x_query.', 'x_mask.', 'rel_encoder.'))]
        assert names == actual_names
        assert len(names) == 84 and sorted(trained['core_parameters']) == sorted(names)
        assert trained['pretrained_artifacts'] == train_manifest['artifacts']
        restored = dict(protected_state)
        for name, value in trained['core_parameters'].items():
            assert name.startswith(('decoder.5.', 'prediction_heads.5.', 'x_query.', 'x_mask.', 'rel_encoder.'))
    ...
    with torch.no_grad():
        for batch_index, raw in enumerate(test_loader):
            ...
            if batch_index == 0 or (batch_index + 1) % 128 == 0:
                ...
                count = len(records['native_gt_mask_geometry_v99'])
                print('SCANREFER MASK GEOMETRY GT OFFICIAL', json.dumps({'rows': count, 'total': 9508,
    ...
    for candidate_arm in ['native_gt_v99', 'native_gt_mask_geometry_v99']:
    ...
    result = {'schema': 'mcln-scanrefer-mask-geometry-gt-official-v1', 'status': 'complete',
        ...
        'promotion': promotion_check(metrics['protected_v99'], metrics['native_gt_mask_geometry_v99']),
        'manifest_sha256': file_sha(option.manifest), 'trained_checkpoints': receipt['checkpoints'],
        'candidate_predeclared': 'native_gt_mask_geometry_v99', 'native_gt_is_mechanism_control_not_validation_selected_candidate': True,
        'data_root': args.data_root, 'superpoint_inputs': data_inputs,
    ...
    print('SCANREFER MASK GEOMETRY GT OFFICIAL COMPLETE', json.dumps(result), flush=True)
