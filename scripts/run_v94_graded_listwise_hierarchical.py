#!/usr/bin/env python
"""Fixed train-only V94 graded listwise hierarchical experiment."""
...
def graded_quality(candidate_ious):
    ...
    return (
        candidate_ious
        + candidate_ious.gt(0.25).to(candidate_ious.dtype)
        + 2.0 * candidate_ious.gt(0.50).to(candidate_ious.dtype)
    )
...
def main(argv=None):
    ...
    if bool(proposals.lt(0).any().item()):
        raise RuntimeError("V94 OOF predictions are incomplete")
    margin = nearest_rank_hierarchical_margin(gain, MARGIN_PERCENTILE)
    if margin is None:
        raise RuntimeError("V94 has no positive OOF proposal gain")
    config = {
    ...
    if protected_after != protected_before:
        raise RuntimeError("protected artifacts changed during V94")
    report = {
        "schema": "rec-graded-listwise-hierarchical-train-only-v1",
        "version": 1,
