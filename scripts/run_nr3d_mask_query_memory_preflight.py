...
Run in isolation after the B endpoint. No training or quality screen.
"""
...
def main():
    ...
    from scripts.nr3d_mask_query_memory import (
        MaskQueryMemoryReadout, MaskQueryMemoryIntervention)
    ...
    del checkpoint
    addon = MaskQueryMemoryReadout().cuda()
    ...
    for index, raw in enumerate(loader):
        ...
        untouched = rec_keys + ['query'] + [name for name in original
        ...
        print('MASK QUERY MEMORY PREFLIGHT', json.dumps(results[-1]), flush=True)
    ...
    verify_inputs()
    receipt = {'schema': 'mcln-mask-query-memory-preflight-v1', 'status': 'complete',
               ...
               'formal_rows': 0, 'holdout_rows': 0, 'parameters': 74880,
    ...
    with (directory / 'receipt.json').open('x') as stream:
        ...
        stream.write('\n')
    print('MASK QUERY MEMORY PREFLIGHT COMPLETE', json.dumps(receipt), flush=True)
