...
root=Path('/root/autodl-tmp/mcln_scanrefer_native_box_transfer_pair_20260907_v1')
old=Path('/root/autodl-tmp/mcln_scanrefer_frozen_readout_pair_20260907_v1')
...
assert rows['gt_only']==rows['gt_teacher_box'] and recorded['gt_only']==recorded['gt_teacher_box']
previous=read(old/'baseline_rows.json')['native_only']
actual=rows['gt_only']
assert len(actual)==len(previous)==6887
keys=['row_id','scan_id','physical_space','point_sha256']
identity_differences=[]
value_differences={field:[] for field in ['rec_iou','native_rec_iou','native_query_index','mask_iou','selected_variant_position']}
...
assert metrics==recorded['gt_only']
...
assert native==native_recorded['gt_only']==native_recorded['gt_teacher_box']
result={'schema':'mcln-native-box-transfer-baseline-cross-run-audit-v1','status':'complete',
 ...
 'prior_frozen_readout_baseline_rows_sha256':sha(old/'baseline_rows.json'),
 ...
 'retired_frozen_readout_weights_not_read':True}
