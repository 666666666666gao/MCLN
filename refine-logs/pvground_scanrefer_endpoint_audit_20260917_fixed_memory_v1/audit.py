...
def audit(root):
    ...
    assert spec['fixed_visual_memory'] and receipt['fixed_visual_memory'] and receipt['visual_state_unchanged']
    import torch
    env=json.loads((Path(spec['runtime'])/'env_spec.json').read_bytes())
    parent_path=Path(env['weight_dirs']['scanrefer']['path'])
    assert sha(parent_path)==spec['checkpoint_sha256']
    parent={k[7:]:v for k,v in torch.load(str(parent_path),map_location='cpu')['model'].items()}
    payload=torch.load(str(root/'terminal.pth'),map_location='cpu')
    assert payload['fixed_visual_memory'] and payload['memory_policy']==receipt['memory_policy']
    keys={n for n in parent if n.startswith('backbone_net.')}
    assert len(keys)==receipt['memory_policy']['visual_state_tensors']
    assert all(n in payload['state_delta'] and torch.equal(parent[n],payload['state_delta'][n]) for n in keys)
    ...
    return {'integrity_pass': True, 'fixed_visual_memory_verified':True, 'time_cst': datetime.datetime.now(datetime.timezone(datetime.timedelta(hours=8))).isoformat(),
