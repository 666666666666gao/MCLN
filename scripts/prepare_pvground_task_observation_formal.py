...
root='/root/autodl-tmp/mcln_pvground_scanrefer_formal_20260917_task_observation_v1'
training='/root/autodl-tmp/mcln_pvground_scanrefer_finetune_20260917_task_observation_v1'
audit='/root/autodl-tmp/mcln_pvground_scanrefer_endpoint_audit_20260917_task_observation_v1'
archive=repo/'refine-logs/pvground_scanrefer_formal_20260917_task_observation_v1'
manifest=json.loads((repo/'refine-logs/pvground_scanrefer_finetune_20260917_task_observation_v1/input_manifest.json').read_bytes())
spec=json.loads((old/'spec.json').read_bytes())
spec.update(training_root=training,training_audit_root=audit,training_audit_controller_pid=int(json.loads((repo/'refine-logs/pvground_scanrefer_endpoint_audit_20260917_task_observation_v1/launch.json').read_bytes())['process'].split()[0]),first_check_cst=(datetime.datetime.now(datetime.timezone(datetime.timedelta(hours=8)))+datetime.timedelta(minutes=180)).isoformat(),training_spec_sha256=hashlib.sha256((repo/'refine-logs/pvground_scanrefer_finetune_20260917_task_observation_v1/spec.json').read_bytes()).hexdigest())
files={'audit.py':(old/'audit.py').read_bytes()}
files['plan.md']=('''# Task-specific observed-memory formal continuation
...
_,out,err=c.exec_command('screen -dmS mcln_pvg_task_observation_formal_v1 bash -c '+shlex.quote(inner),timeout=30)
...
record=dict(time_cst=datetime.datetime.now(datetime.timezone(datetime.timedelta(hours=8))).isoformat(),process=process,screen='mcln_pvg_task_observation_formal_v1',first_check_cst=spec['first_check_cst'],poll_seconds=300,formal_rows=0,cpu_probe_source='previous completed6887 recount plus actual CPU model restore with in-memory fixture; no terminal claim',training_changes=False)
