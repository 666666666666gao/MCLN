...
from scripts.evaluate_scanrefer_joint_readout_official import (
    file_sha, promotion_check, row_metrics, verify_training_screen,
)
...
def screen_fixture(directory, joint_iou):
    ...
    baseline = {'detached': rows, 'joint': rows}
    terminal = {'detached': rows, 'joint': changed}
    save('input_manifest.json', {'steps_per_arm': 2482, 'epochs': 1})
    ...
    save('receipt.json', {
        ...
        'eligible_for_fixed_terminal_formal_evaluation': True,
        'checkpoints': {'joint': {'path': str(directory / 'synthetic_checkpoint'),
...
def test_eligible_endpoint_is_recomputed_from_rows(tmp_path):
    ...
    receipt, _ = verify_training_screen(tmp_path)
    assert receipt['steps_per_arm'] == 2482


def test_claimed_pass_does_not_override_strict_threshold_damage(tmp_path):
    screen_fixture(tmp_path, .4)
    with pytest.raises(AssertionError):
        verify_training_screen(tmp_path)
