...
OUTPUT=$BASE/v115_meshsp_anchored_spatial_adapter_oof.json
LOG=$BASE/v115_meshsp_anchored_spatial_adapter_oof.log
EXIT=$BASE/v115_meshsp_anchored_spatial_adapter_oof.exit
SESSION=v115_anchored_spatial_adapter_oof
SELF=$(readlink -f "$0")
...
if [[ "${1:-}" == "--inner" ]]; then
    ...
    env CUDA_VISIBLE_DEVICES=0 \
        ...
        "$PYTHON" scripts/run_v115_meshsp_anchored_spatial_adapter_oof.py \
...
cd "$REPO"
require_sha dfbe142a44339137c977375cca77afce17fd8b515dcdde2184d7f3126d02ca4b \
    scripts/run_v115_meshsp_anchored_spatial_adapter_oof.py
require_sha 2ca6cc3657661401aae708192a0d8c5c2d157abc198d2324c296073e691a3b4e \
...
for path in "$OUTPUT" "$LOG" "$EXIT"; do
    if [[ -e "$path" ]]; then
        echo "Refusing to overwrite existing V115 output: $path" >&2
        exit 74
    fi
done
if [[ $(nvidia-smi -L | wc -l) -ne 1 ]]; then
    echo "V115 requires the frozen single-GPU topology" >&2
    exit 75
fi
...
if pgrep -af '[r]un_v115_meshsp_anchored_spatial_adapter_oof.py' >/dev/null; then
    echo "V115 process already exists" >&2
    exit 75
fi
if screen -ls 2>/dev/null | grep -q "[.]$SESSION"; then
    echo "V115 screen session already exists" >&2
    exit 75
fi
available_kb=$(df -Pk /root/autodl-tmp | awk 'NR==2 {print $4}')
if [[ "$available_kb" -lt 2621440 ]]; then
    echo "V115 requires at least 2.5 GiB free on /root/autodl-tmp" >&2
    exit 75
fi
...
if ! screen -ls 2>/dev/null | grep -q "[.]$SESSION" \
        && [[ ! -e "$EXIT" ]]; then
    echo "V115 screen failed to remain alive and produced no exit receipt" >&2
    exit 75
fi
