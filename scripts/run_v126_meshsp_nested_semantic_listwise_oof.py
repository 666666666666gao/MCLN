#!/usr/bin/env python
"""Nested OOF screen of V126 listwise utility selection with abstention."""
...
import models.rec_semantic_antisymmetric_utility as v126_model
...
V126_CRITIC_EPOCHS = 50
V126_ROW_BATCH_SIZE = 512
V126_INFERENCE_BATCH_SIZE = 1024
V126_CRITIC_LEARNING_RATE = 1e-3
V126_CRITIC_WEIGHT_DECAY = 1e-3
...
def _pair_inputs(
    ...
    if count <= 0:
        raise ValueError("V126 pair population must be nonempty")
    for value in inputs.values():
        if (value.dtype != torch.float32 or value.device.type != "cpu"
                or value.shape[0] != count
                or not bool(torch.isfinite(value).all().item())):
            raise ValueError("V126 pair input layout changed")
...
def build_listwise_training_population(prediction, records):
    ...
    if (probabilities.dtype != torch.float32
        ...
        raise ValueError("V126 model probability layout changed")
    rows = torch.arange(count)
    ...
    if (tuple(ious.shape) != (count, QUERY_COUNT, VARIANT_COUNT)
            or not bool(torch.isfinite(ious).all().item())):
        raise ValueError("V126 candidate IoU layout changed")
    ...
    if (not bool(positive_rows.any(dim=0).all().item())
        ...
        raise ValueError("V126 listwise population lacks action diversity")
...
def listwise_abstain_loss(candidate_logits, candidate_positive):
    ...
    if (candidate_logits.dtype != torch.float32
        ...
        raise ValueError("V126 listwise loss tensors changed")
    ...
    if not bool(torch.isfinite(loss).item()):
        raise RuntimeError("V126 listwise abstain loss became non-finite")
...
def fit_semantic_listwise_critic(prediction, records, device):
    ...
    if pair_count < 100:
        raise ValueError("V126 listwise candidate population is too small")
    row_count = len(records)
    resolved = torch.device(device)
    _set_hierarchical_seed(resolved)
    model = v126_model.SemanticAntisymmetricUtilityRanker().to(resolved)
    optimizer = torch.optim.AdamW(
        model.parameters(),
        lr=V126_CRITIC_LEARNING_RATE,
        weight_decay=V126_CRITIC_WEIGHT_DECAY,
    )
    shuffle_generator = torch.Generator(device="cpu")
    shuffle_generator.manual_seed(0)
    epoch_receipts = []
    for epoch in range(V126_CRITIC_EPOCHS):
        model.train()
        order = torch.randperm(row_count, generator=shuffle_generator)
        total_loss = 0.0
        total_rows = 0
        for start in range(0, row_count, V126_ROW_BATCH_SIZE):
            batch_rows = order[start:start + V126_ROW_BATCH_SIZE]
    ...
    receipt.update({
        "epochs": V126_CRITIC_EPOCHS,
        "row_batch_size": V126_ROW_BATCH_SIZE,
        "baseline_score": 0.0,
        "epoch_receipts": epoch_receipts,
        "state_sha256": v99.tensor_sha256(*[
            value for _, value in sorted(model.state_dict().items())
...
def all_hard_candidate_inputs(prediction):
    ...
    if (probabilities.dtype != torch.float32
            ...
            or tuple(baselines.shape) != (count,)
            or not bool(torch.isfinite(probabilities).all().item())):
        raise ValueError("V126 all-candidate probability layout changed")
    ...
    if pair_rows.numel() <= 0:
        raise ValueError("V126 all-candidate population must be nonempty")
    pair_baselines = baselines[pair_rows]
    baseline_query = pair_baselines.div(
        VARIANT_COUNT, rounding_mode="floor"
    )
...
def predict_pairwise_utility(model, inputs, device):
    ...
    logits = []
    with torch.no_grad():
        for start in range(0, count, V126_INFERENCE_BATCH_SIZE):
            stop = min(start + V126_INFERENCE_BATCH_SIZE, count)
            logits.append(model(**{
                key: value[start:stop].to(resolved)
                for key, value in inputs.items()
            }).cpu().float())
    ...
    if (tuple(output.shape) != (count, 2)
        ...
        raise RuntimeError("V126 utility logits changed")
    return output
...
def select_all_hard_candidates(model, prediction, device):
    ...
    if (proposals.dtype != torch.long
        ...
        raise RuntimeError("V126 all-candidate selection contract changed")
    if bool((accepted & proposals.eq(baselines)).any().item()):
        raise RuntimeError("V126 accepted a baseline as a candidate")
    return {
        "proposals": proposals,
        "selected": selected,
        "accepted": accepted,
...
def pairwise_utility_accept_mask(base_accepted, utility_logits):
    if (base_accepted.dtype != torch.bool or base_accepted.dim() != 1
        ...
        raise ValueError("V126 utility gate tensors changed")
...
def main(argv=None):
    ...
    for held in range(5):
        ...
        if (set(records[index]["scan_id"] for index in inner_fit_indices)
            ...
            raise RuntimeError("scene leakage into V126 inner calibration")
        ...
        if not torch.equal(prediction["baselines"], baselines[held_indices]):
            raise RuntimeError("V126 OOF baseline identity changed")
    ...
    if (veto_reasons["selected_margin_violations"] != 0
        ...
        raise RuntimeError("V126 all-candidate Pareto contract changed")
    protected_after = capture_immutable_artifact_identities(protected_paths)
    if protected_after != protected_before:
        raise RuntimeError("protected artifacts changed during V126")
    protected_metadata_after = {
        name: file_sha256(path)
        for name, path in protected_metadata_paths.items()
    }
    if protected_metadata_after != protected_metadata_before:
        raise RuntimeError("protected V126 cache metadata changed during OOF")
    script_sources = {
        "v126": str(Path(__file__).resolve()),
        ...
        "v126_model": str(Path(v126_model.__file__).resolve()),
        "v108": str(
    ...
    report = {
        "schema": "rec-v126-nested-semantic-listwise-abstain-scene-oof-v1",
        "version": 1,
        "validation_data_accessed": False,
        ...
        "protocol": {
            ...
            "critic_candidate_dim": v126_model.V123_CANDIDATE_DIM,
            "critic_pair_structure": "shared_score_difference",
            "critic_model_gain_scale": "learned_positive_per_threshold",
            "critic_epochs": V126_CRITIC_EPOCHS,
            "critic_row_batch_size": V126_ROW_BATCH_SIZE,
            "critic_inference_batch_size": V126_INFERENCE_BATCH_SIZE,
            "critic_learning_rate": V126_CRITIC_LEARNING_RATE,
            "critic_weight_decay": V126_CRITIC_WEIGHT_DECAY,
    ...
    try:
        ...
        while written < len(payload):
            ...
            if count <= 0:
                raise OSError("V126 output write made no progress")
            written += count
        os.fsync(descriptor)
    finally:
        os.close(descriptor)
