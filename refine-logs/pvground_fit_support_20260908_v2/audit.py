    ...
    try:
        for augmented in [False, True]:
            ...
            for start in range(0, spec['scene_count'], 8):
                ...
                raw_sizes = output['last_pred_size'].detach().cpu().numpy()
                assert np.isfinite(centers).all() and np.isfinite(raw_sizes).all()
                # Match native_root_rec_readout and the independent evaluator audit.
                sizes = np.maximum(raw_sizes, np.float32(1e-6))
                for b, case in enumerate(cases):
                    ...
                    case.update(raw_nonpositive_size_queries=int((raw_sizes[b] <= 0).any(axis=1).sum()),
                                query_indices=selected_queries.tolist(), sampled_root_iou=iou[selected_queries].tolist(),
