...
assert train_spec['task_read']
assert sha(root/'pvground_task_observation_query.py') == train_spec['task_module_sha256']
assert train_spec['observation_state']
assert sha(root/'pvground_observation_query.py') == train_spec['observation_module_sha256']
...
if args.terminal:
    ...
    scope = 'in-memory serialization fixture changing all 37 reader tensors; not trained weights'
...
result = dict(status='pass',time_cst=now(),scope=scope,parent_tensors=len(parent),
    ...
    module_sha256=train_spec['task_module_sha256'],evaluator_sha256=sha(root/'evaluate.py'),
