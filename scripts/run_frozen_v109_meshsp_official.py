#!/usr/bin/env python
"""Run and seal the sole frozen V109 ScanRefer mesh-superpoint validation."""
...
SAMPLE_COUNT = 9508
EXPERIMENT = "epoch71_v109_meshsp_nested_policy_full9508"
MASTER_PORT = 29791
V109_BASE = Path(
    "/root/autodl-tmp/DATA_ROOT/output/rec_reranker/e71_top16_meshsp"
)
PARENT_ARTIFACT_PATH = V109_BASE / "v108_artifacts/parent_h256_seed0.pth"
GEOMETRY_ARTIFACT_PATH = V109_BASE / "v108_artifacts/geometry_h256_seed0.pth"
ARTIFACT_PATH = V109_BASE / (
    "v109_artifacts/nested_policy_h128_seed0_fullfit.pth"
)
PARITY_PATH = V109_BASE / "v109_train_runtime_parity.json"
...
PARITY_SHA256 = (
    "5bc46bbf1146a34ee834b4241f934b244f1d8b2287fb931ec963c720350a9c46"
)
CLAIM_PATH = ARTIFACT_PATH.parent / (
    "v109_meshsp_official_once_after_train_runtime_parity.claim.json"
)
RESULT_SCHEMA = "rec-v109-meshsp-official-validation-result-v1"
...
def validate_authoritative_command(command, output_dir):
    ...
    if list(command) != expected:
        raise ValueError("V109 meshsp official command changed")
    required = {
    ...
    if not required.issubset(command) or FORBIDDEN.intersection(command):
        raise ValueError("V109 meshsp official command policy is invalid")
    if "--eval_use_rec_selective_residual_scores" in command:
        raise ValueError("V109 cannot combine the selective residual policy")
    return list(command)
...
def _protected_snapshots():
    result = {
        ...
        "v109": _snapshot(ARTIFACT_PATH, "V109 artifact", True),
        "parity": _snapshot(PARITY_PATH, "V109 parity report", True),
    }
    expected = {
        "backbone": geometry_official.OFFICIAL_CHECKPOINT_SHA256,
        "parent": PARENT_ARTIFACT_SHA256,
        "geometry": GEOMETRY_ARTIFACT_SHA256,
        "v109": ARTIFACT_SHA256,
    ...
    if (parity.get("schema") != "rec-v109-train-runtime-parity-audit-v1"
        ...
        raise ValueError("V109 train/runtime parity contract is invalid")
    model, artifact = load_v109_artifact(
        ARTIFACT_PATH,
        device="cpu",
    ...
    if (artifact.get("deployable") is not True
        ...
        raise ValueError("V109 deployable artifact contract is invalid")
    return result
...
def run_official(output_dir, dry_run=False):
    ...
    if not dry_run and (CLAIM_PATH.exists() or CLAIM_PATH.is_symlink()):
        raise FileExistsError("V109 meshsp validation claim already exists")
    ...
    if protected_after != protected_before:
        raise RuntimeError("protected artifact changed during V109 official run")
    if code_after != code_before:
        raise RuntimeError("Python source tree changed during V109 official run")
    text = stdout_path.read_text(encoding="utf-8", errors="replace")
    if returncode != 0:
        raise RuntimeError("V109 official subprocess failed with code {}".format(returncode))
    populations = re.findall(r"length of testing dataset:\s*(\d+)", text)
    if len(populations) != 1 or populations[0] != str(SAMPLE_COUNT):
        raise ValueError("V109 official validation population is not 9508")
    ...
    if len(timestamp_runs) != 1:
        raise ValueError("V109 official run directory is ambiguous")
