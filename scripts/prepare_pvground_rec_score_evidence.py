...
root = '/root/autodl-tmp/mcln_pvground_rec_score_evidence_20260917_v1'
archive = repo/'refine-logs/pvground_rec_score_evidence_20260917_v1'
...
for dependency in ['finetune','endpoint_audit','formal']:
    assert read('/root/autodl-tmp/mcln_pvground_scanrefer_'+dependency+'_20260917_rec_competition_v1/controller.exit').strip() == b'0'
assert read('/root/autodl-tmp/mcln_pvground_rec_competition_comparison_20260917_v1/comparison.exit').strip() == b'0'
...
assert receipt['terminal_sha256'] == '9762fb3e91ed1203f085328b4388ed45e74a5e8b920b9aac7c3a499cd298f4ca'
...
files = {'diagnose.py':(repo/'scripts/diagnose_pvground_rec_score_evidence.py').read_bytes(),
         ...
         'plan.md':(repo/'docs/PVG_REC_SCORE_EVIDENCE_PLAN_2026-09-17.md').read_bytes()}
...
for name,key in [('pvground_source_query.py','source_query_module_sha256'),
                 ...
                 ('pvground_rec_competition.py','competition_module_sha256')]:
...
spec = dict(training_root=training,training_spec_sha256=sha(train_spec_raw),terminal_sha256=receipt['terminal_sha256'],
    ...
    training_rows=128,batch_size=8,model_forwards=32,optimizer_steps=0,formal_rows=0,
    scope='fixed initial/F token components and current matching; two eval arms; no optimizer or formal evaluation',
...
command = 'screen -dmS mcln_pvg_rec_score_evidence_v1 '+shlex.quote('/root/miniconda3/envs/bdetr/bin/python')+' -u '+shlex.quote(root+'/controller.py')
