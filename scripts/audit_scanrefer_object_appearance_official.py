...
ARMS = ['protected_v99', 'appearance_v99']
...
def audit_rows(directory):
    ...
    assert receipt['schema'] == 'mcln-scanrefer-object-appearance-official-v1'
    ...
    promotion = promotion_check(actual['protected_v99'], actual['appearance_v99'])
    ...
    return {'integrity_pass': True, 'formal_rows': 9508, 'metrics': actual,
            ...
            'system_appearance_minus_protected': compare(records['protected_v99'], records['appearance_v99']),
            'native_appearance_minus_protected': rec_compare(native['protected_v99'], native['appearance_v99']),
...
def audit_inputs(directory):
    ...
    for filename, field in [('receipt.json', 'training_receipt_sha256'),
                            ('audit.json', 'training_audit_sha256')]:
    ...
    assert manifest['trained_checkpoint'] == training_receipt['checkpoints']['appearance']
    ...
    artifacts = dict(train_manifest['artifacts'], trained_appearance=manifest['trained_checkpoint'])
    ...
    cache = Path(manifest['cache_root'])
    assert file_sha(cache / 'receipt.json') == manifest['cache_receipt_sha256']
    ...
    assert cache_receipt['scenes_sha256'] == file_sha(cache / 'scenes.jsonl')
    cache_rows = {v['scene_id']: v for v in map(json.loads, (cache / 'scenes.jsonl').read_text().splitlines())}
    for v in cache_rows.values():
        assert file_sha(cache / v['file']) == v['file_sha256']
    formal = json.loads((directory / 'result/rows.json').read_text())
    for rows in formal.values():
        for row in rows:
            assert row['point_sha256'] == cache_rows[row['scan_id']]['native_point_sha256']
...
def main():
    ...
    result['schema'] = 'mcln-scanrefer-object-appearance-official-audit-v1'
