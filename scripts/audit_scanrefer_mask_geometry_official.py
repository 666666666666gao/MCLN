...
from scripts.evaluate_scanrefer_mask_geometry_official import promotion_check
from scripts.scanrefer_data_contract import verify_scanrefer_superpoints


ARMS = ['protected_v99', 'native_gt_v99', 'native_gt_mask_geometry_v99']
...
def audit_rows(directory):
    ...
    assert receipt['schema'] == 'mcln-scanrefer-mask-geometry-gt-official-v1'
    ...
    assert receipt['candidate_predeclared'] == 'native_gt_mask_geometry_v99'
    ...
    promotion = promotion_check(actual['protected_v99'], actual['native_gt_mask_geometry_v99'])
    ...
    return {'integrity_pass': True, 'formal_rows': 9508, 'metrics': actual,
            ...
            'system_native_gt_mask_geometry_minus_protected': compare(records['protected_v99'], records['native_gt_mask_geometry_v99']),
            'native_native_gt_mask_geometry_minus_protected': rec_compare(native['protected_v99'], native['native_gt_mask_geometry_v99']),
            'system_native_gt_mask_geometry_minus_native_gt': compare(records['native_gt_v99'], records['native_gt_mask_geometry_v99']),
            'native_native_gt_mask_geometry_minus_native_gt': rec_compare(native['native_gt_v99'], native['native_gt_mask_geometry_v99']),
            'system_native_gt_minus_protected': compare(records['protected_v99'], records['native_gt_v99']),
...
def audit_inputs(directory):
    ...
    assert manifest['trained_checkpoints'] == training_receipt['checkpoints']
    ...
    artifacts = dict(train_manifest['artifacts'], **{'trained_' + name: item for name, item in manifest['trained_checkpoints'].items()})
...
def main():
    ...
    result['schema'] = 'mcln-scanrefer-mask-geometry-gt-official-audit-v1'
