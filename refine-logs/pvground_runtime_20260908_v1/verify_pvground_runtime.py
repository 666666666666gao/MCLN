...
def main():
    ...
    for points in source.cpu().numpy():
        ...
        for voxel, coord, count in zip(value['voxels'], value['voxel_coords'], value['voxel_num_points']):
            expected = np.floor((voxel[0, :3] - np.array(cfg.DATA_CONFIG.POINT_CLOUD_RANGE[:3])) / .02).astype(np.int32)[::-1]
    ...
    receipt = {
        ...
        'voxel_coordinate_order': 'zyx', 'raw_point_order_preserved': True,
