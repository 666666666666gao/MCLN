...
training=Path('/root/autodl-tmp/mcln_pvground_scanrefer_finetune_20260908_sourcequery_v1')
...
assert len(parameter_ids)==807 and len(set(parameter_ids))==807
...
result=dict(status='pass',time_cst=now(),step=data['step'],fit_rows=len(data['row_ids']),
    ...
    elapsed_seconds=time.time()-begin,module_sha256=data['source_query_module_sha256'],
...
print('PVG_SOURCE_QUERY_LATEST_CPU_PASS '+json.dumps(result),flush=True)
