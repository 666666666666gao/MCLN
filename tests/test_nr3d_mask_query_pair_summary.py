...
from scripts.summarize_nr3d_mask_query_pair import (
...
def test_memory_must_improve_both_native_endpoint_and_protected_start():
    end = rows([.62] * 4, 'memory')
...
def test_good_box_cohort_is_fixed_by_start_boxes_and_strict_threshold():
    ...
    after = [{'id': i, 'memory': {'legal_box_oracle_iou': .9}} for i in range(4)]
...
def test_mask_improvement_does_not_authorize_changed_rec_or_inputs():
    ...
    before = [{'id': 1, 'scan_id': 'scene1', 'grounding_sha256': 'g',
               'input_point_sha256': 'p', 'frozen_text_mask_alpha_sha256': 'text_alpha', 'native': copy.deepcopy(value), 'memory': copy.deepcopy(value)}]
    after = copy.deepcopy(before)
    after[0]['memory']['mask_iou'] = .7
    require_identity(before, after)
    after[0]['memory']['rec_query'] = 5
    with pytest.raises(AssertionError):
        require_identity(before, after)
    after = copy.deepcopy(before)
...
def test_candidate_mask_gain_does_not_allow_text_mask_or_alpha_changes():
    value = {'rec_query': 2, 'rec_box_iou': .8, 'mask_query': 3,
             'legal_box_oracle_query': 4, 'legal_box_oracle_iou': .9, 'mask_iou': .4}
    before = [{'id': 1, 'scan_id': 'scene1', 'grounding_sha256': 'g',
               'input_point_sha256': 'p', 'frozen_text_mask_alpha_sha256': 'unchanged',
               'native': copy.deepcopy(value), 'memory': copy.deepcopy(value)}]
    after = copy.deepcopy(before)
    after[0]['memory']['mask_iou'] = .7
    require_identity(before, after)
    after[0]['frozen_text_mask_alpha_sha256'] = 'changed'
    with pytest.raises(AssertionError):
        require_identity(before, after)
