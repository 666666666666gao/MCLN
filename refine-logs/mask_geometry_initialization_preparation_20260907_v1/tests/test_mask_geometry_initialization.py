...
from scripts.export_mask_geometry_initialization import build_initialization
...
def fixture():
    ...
    names += ['decoder.5.fixture.' + str(index) + '.weight' for index in range(66)]
    names += ['decoder.5.cross_l.in_proj_weight', 'decoder.5.cross_l.in_proj_bias']
    ...
    endpoint = {'schema': 'mcln-scanrefer-mask-geometry-core-state-v1', 'arm': 'native_gt_mask_geometry',
                'steps': 2482, 'pretrained_artifacts': {'backbone': {'sha256': 'base'}},
                'core_parameters': {name: torch.ones(2) for name in names},
...
def test_native_prefix_preserves_frozen_state_and_discards_both_optimizers():
    ...
    for name, value in original['model'].items():
        if name[7:] not in endpoint['core_parameters']:
    ...
    assert len(output['initialization_provenance']['replaced_tensors']) == 84
    assert torch.equal(output['model']['module.decoder.5.cross_l.in_proj_weight'], torch.ones(2))
...
def test_incompatible_delta_is_not_exported(failure):
    base, endpoint = fixture()
    name = next(iter(endpoint['core_parameters']))
    if failure == 'wrong_base':
        endpoint['pretrained_artifacts']['backbone']['sha256'] = 'other'
    elif failure == 'missing_head':
        del endpoint['core_parameters'][name]
    elif failure == 'extra_head':
        endpoint['core_parameters']['backbone_net.frozen'] = torch.ones(3)
    elif failure == 'bn_buffer':
        endpoint['core_parameters']['prediction_heads.5.center_residual_head.net.1.running_mean'] = torch.ones(2)
    elif failure == 'wrong_shape':
        endpoint['core_parameters'][name] = torch.ones(3)
    elif failure == 'wrong_dtype':
        endpoint['core_parameters'][name] = torch.ones(2, dtype=torch.float64)
    elif failure == 'control_arm':
        endpoint['arm'] = 'native_gt'
