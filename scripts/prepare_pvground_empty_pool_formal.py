...
root='/root/autodl-tmp/mcln_pvground_scanrefer_formal_20260908_emptypool_v1'
training='/root/autodl-tmp/mcln_pvground_scanrefer_finetune_20260908_emptypool_v1'
audit='/root/autodl-tmp/mcln_pvground_scanrefer_endpoint_audit_20260908_emptypool_v1'
archive=repo/'refine-logs/pvground_scanrefer_formal_20260908_emptypool_v1'
manifest=json.loads((repo/'refine-logs/pvground_scanrefer_finetune_20260908_emptypool_v1/input_manifest.json').read_bytes())
spec=json.loads((old/'spec.json').read_bytes())
spec.update(training_root=training,training_audit_root=audit,training_audit_controller_pid=int(json.loads((repo/'refine-logs/pvground_scanrefer_endpoint_audit_20260908_emptypool_v1/launch.json').read_bytes())['process'].split()[0]),first_check_cst=(datetime.datetime.now(datetime.timezone(datetime.timedelta(hours=8)))+datetime.timedelta(minutes=165)).isoformat(),training_spec_sha256=hashlib.sha256((repo/'refine-logs/pvground_scanrefer_finetune_20260908_emptypool_v1/spec.json').read_bytes()).hexdigest())
files={name:(old/name).read_bytes() for name in ['audit.py','plan.md']}
files['evaluate.py']=(repo/'scripts/evaluate_pvground_scanrefer_empty_pool.py').read_bytes()
...
_,out,err=c.exec_command('screen -dmS mcln_pvg_emptypool_formal_v1 bash -c '+shlex.quote(inner),timeout=30)
...
record=dict(time_cst=datetime.datetime.now(datetime.timezone(datetime.timedelta(hours=8))).isoformat(),process=process,screen='mcln_pvg_emptypool_formal_v1',first_check_cst=spec['first_check_cst'],poll_seconds=300,formal_rows=0,cpu_probe_source='previous completed6887 initial; unchanged evaluator/recount code',training_changes=False)
