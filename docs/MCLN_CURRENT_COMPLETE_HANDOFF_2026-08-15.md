...
### 20.20 machine35608 E15 完整回执与 E20 稀疏监控（2026-09-02 01:50 CST）

`12_sacr_no_pairwise_geometry` 的 E15 于 `2026-09-02 01:45:17 CST` 完成，训练进程随后进入 E16。
`eval_epoch_15.log` 与 `best_primary.json` 同时存在，后者把 E15 登记为该消融当前最好。三次同口径 Top-1：

| Epoch | BBS @0.25 | BBS @0.50 | BBF @0.25 | BBF @0.50 |
|---:|---:|---:|---:|---:|
| 5 | `29.0177%` | `12.5368%` | `31.8679%` | `14.0724%` |
| 10 | `38.7463%` | `20.7299%` | `39.5141%` | `21.8973%` |
| 15 | `41.9541%` | `25.3576%` | `43.0690%` | `26.6512%` |

E10→E15 的 BBS @0.25/@0.50 分别继续增长 `+3.2078pp/+4.6277pp`，BBF @0.25/@0.50 增长
`+3.5549pp/+4.7539pp`；best-primary 精确值为 `0.41954143878838873`。E5→E10 与 E10→E15 的四项
主指标均为明显正增益，当前没有平台证据，因此保持配置 `lr=1e-4` 与 `lr_decay_epochs=55,60`，没有提前衰减。

原 01:15 Windows 任务在 E15 仍处于 `200/397` batch 时启动。任务运行 10 分钟后由其固定 `PT10M`
执行时限终止，返回 `0x41306`，父 PowerShell 消失但留下 PID 33068 的孤儿 `ssh.exe`；核对创建时间、父 PID
与进程名后已只终止该孤儿采集进程，未接触远端 trainer。随后一次带 `ConnectTimeout=20`、
`ServerAliveInterval=10`、`ServerAliveCountMax=2` 的只读 SSH 成功，确认服务器可达且 E15 当时尚未完成。
这些连接/保活上限已写入本地 collector，当前 SHA256 为
`97b3a4e61c1b592fe2e77e91393092a4e26bbdc82e89d0d682f166cb6bb85a12`；它们针对本轮真实发生的孤儿
进程风险，不是为假设性 edge case 增加兼容层。

本地证据：

```text
C:\Users\gb\.codex\tmp\mcln_35608_e15_interim_receipt_20260902.txt
C:\Users\gb\.codex\tmp\mcln_35608_e15_receipt_20260902.txt
```

按真实 E10→E15 完成时间计算，machine50630 用时约 `3h12m`，machine35608 用时约 `3h59m`。下一次只读
采集安排为 machine50630 E20 `2026-09-02 02:45 CST`、machine35608 E20 `2026-09-02 05:55 CST`；对应
Windows task 分别为 `Codex_MCLN_50630_E20_20260902` 与 `Codex_MCLN_35608_E20_20260902`。E20 前不轮询，
只有完整 E20 与此前完整节点形成平台证据时才考虑提前衰减。两条 ScanRefer 消融仍不是 CEGD 或正式
ScanRefer/Nr3D/Sr3D 指标，受保护正式最好和严格目标保持 §20.1/§20.18 的定义不变。


### 20.21 machine50630 E20 刷新最好、LR 继续保持（2026-09-02 02:50 CST）

`05_rapf_no_query_quality` 的 E20 于 `2026-09-02 02:39:20 CST` 完整结束，`best_primary.json` 已把最好
epoch 从 15 更新为 20，精确 BBS @0.25 为 `0.47854438367690366`。完整同口径轨迹：
...
| 10 | `38.7148%` | `21.2558%` | `40.0084%` | `22.3812%` |
| 15 | `46.0034%` | `27.7030%` | `46.3715%` | `28.7127%` |
| 20 | `47.8544%` | `31.8258%` | `48.3067%` | `32.5515%` |

E15→E20 的 BBS @0.25/@0.50 增长 `+1.8511pp/+4.1228pp`，BBF @0.25/@0.50 增长
`+1.9352pp/+3.8389pp`。@0.25 的增长速度相对 E10→E15 有所下降，但四项主指标全部继续明显提高，且 E20
刷新 best-primary，不能据此判定平台。训练采集时已经进入 E21，保持 `lr=1e-4`，没有提前衰减。

02:45 Windows 任务在 4 秒内返回 1，原始输出仅为 `Connection closed by UNKNOWN port 65535`；它没有留下
孤儿进程。随后同一端口的一次受限只读 SSH 返回 0，确认 trainer 存活、E20 完整以及 E21 正在训练，因此该
任务错误只作为一次瞬时采集失败记录，不推断服务器或训练失败，也不重启任何进程。结构化回执：

```text
C:\Users\gb\.codex\tmp\mcln_50630_e20_receipt_20260902.txt
```

完成的 E20 task 已删除。按 E15→E20 实测约 `3h17m`，machine50630 E25 采集固定为
`2026-09-02 06:10 CST`（task `Codex_MCLN_50630_E25_20260902`）；machine35608 E20 仍为
`2026-09-02 05:55 CST`。在这两个节点前不增加远端轮询。


### 20.22 machine35608 E20 刷新最好、LR 继续保持（2026-09-02 06:05 CST）

`12_sacr_no_pairwise_geometry` 的 E20 于 `2026-09-02 05:44:45 CST` 完整结束，`best_primary.json` 已把最好
epoch 从 15 更新为 20，精确 BBS @0.25 为 `0.45319730753050064`。完整同口径轨迹：
...
| 10 | `38.7463%` | `20.7299%` | `39.5141%` | `21.8973%` |
| 15 | `41.9541%` | `25.3576%` | `43.0690%` | `26.6512%` |
| 20 | `45.3197%` | `29.3753%` | `45.6878%` | `30.0273%` |

E15→E20 的 BBS @0.25/@0.50 增长 `+3.3656pp/+4.0177pp`，BBF @0.25/@0.50 增长
`+2.6188pp/+3.3761pp`。四项主指标继续同时明显提高，且 E20 刷新 best-primary，因此没有平台证据；训练进程
在回执采集时仍然存活，保持 `lr=1e-4` 与原定 `lr_decay_epochs=55,60`，没有提前衰减，也没有重启训练。

05:55 Windows 定时任务实际已启动，但停留在本地 SSH 只读收集阶段，没有生成输出文件。核对定时任务状态、
父 PowerShell、子 `ssh.exe` 的 PID、创建时间和完整命令行后，使用一次远端 `timeout 40s`、本地
`ConnectTimeout=20`/`ServerAliveInterval=10`/`ServerAliveCountMax=2` 的定点只读命令，直接读取固定的
`eval_epoch_20.log` 与 `best_primary.json`，7.9 秒成功返回。确认指标落盘后，只停止并删除这一个本地采集任务及
其已核验的孤儿 SSH 进程，未接触远端 trainer。该事件只说明通用递归收集命令仍可能卡住，不构成训练异常。

结构化回执保存在：

```text
C:\Users\gb\.codex\tmp\mcln_35608_e20_receipt_20260902.txt
```

下一次已预注册检查仍是 machine50630 E25，于 `2026-09-02 06:10 CST` 执行；在完整 E25 回执出现前不根据
单批次或训练 loss 调整 LR。两条运行均为独立 ScanRefer 消融，不替代受保护的 ScanRefer/Nr3D/Sr3D 正式
指标，也不构成 CEGD 或 G0 证据。


### 20.23 machine50630 E25 继续刷新、尚不足以判定平台（2026-09-02 06:20 CST）

`05_rapf_no_query_quality` 的 E25 于 `2026-09-02 05:50:03 CST` 完整结束，`best_primary.json` 已把最好
epoch 从 20 更新为 25，精确 BBS @0.25 为 `0.48664282709297435`。完整同口径轨迹：
...
| 15 | `46.0034%` | `27.7030%` | `46.3715%` | `28.7127%` |
| 20 | `47.8544%` | `31.8258%` | `48.3067%` | `32.5515%` |
| 25 | `48.6643%` | `32.3622%` | `48.9903%` | `32.8986%` |

E20→E25 的 BBS @0.25/@0.50 增长 `+0.8098pp/+0.5364pp`，BBF @0.25/@0.50 增长
`+0.6836pp/+0.3471pp`。增长速度相对 E15→E20 明显放缓，但四项仍全部为正且 E25 再次刷新 best-primary；
单个较慢节点尚不足以构成连续平台证据，因此本轮仍保持 `lr=1e-4`，不提前衰减、不重启训练。下一节点 E30
用于判断减速是否持续；若后续多节点不再刷新或四项变化接近零，再按证据决定是否提前衰减。

06:10 旧 collector 再次卡在本地 SSH 阶段且没有生成输出。一次固定 E25 文件名、远端 `timeout 40s` 的只读
命令在 9.0 秒内成功，确认 trainer 存活、E25 完整和 `best_primary.json` 已更新。随后只清理该本地 task 与经
PID、父 PID、创建时间和完整命令行核验的孤儿 SSH，没有触碰远端训练进程。由于同一递归扫描命令已在两台
机器连续复现卡住，collector 已做最小修改：新增必填 `-Epoch`，直接读取
`eval_epoch_<Epoch>.log`/`eval_metrics_epoch_<Epoch>.json`，完全删除三次递归 `find`；PowerShell AST 解析
通过，新 SHA256 为 `7867de9468b1b9d57c51932fa6fde881bcaa1c2a45667dd581e114576f839e3d`。

结构化回执：

```text
C:\Users\gb\.codex\tmp\mcln_50630_e25_receipt_20260902.txt
```

按 E20→E25 实测约 `3h11m`，下一轮稀疏检查已经固定为：

- machine50630 E30：`2026-09-02 09:10 CST`，task `Codex_MCLN_50630_E30_20260902`；
- machine35608 E25：`2026-09-02 09:55 CST`，task `Codex_MCLN_35608_E25_20260902`。

两个新 task 都使用固定 epoch collector，执行上限 5 分钟；此前不追加远端轮询。


### 20.24 machine50630 E30：@0.25 已近平台、@0.50 仍增长（2026-09-02 09:20 CST）

固定 epoch collector 首次正式运行成功：Windows task 于 `09:10:00 CST` 启动，`09:10:07` 写完回执，
`LastTaskResult=0`，没有再发生递归扫描卡住。E30 日志实际于 `2026-09-02 08:57:29 CST` 落盘，四项为：

| Epoch | BBS @0.25 | BBS @0.50 | BBF @0.25 | BBF @0.50 |
|---:|---:|---:|---:|---:|
| 20 | `47.8544%` | `31.8258%` | `48.3067%` | `32.5515%` |
| 25 | `48.6643%` | `32.3622%` | `48.9903%` | `32.8986%` |
| 30 | `48.6958%` | `32.9512%` | `48.9693%` | `33.2878%` |

E25→E30 的 BBS @0.25/@0.50 分别为 `+0.0316pp/+0.5890pp`，BBF @0.25/@0.50 为
`-0.0210pp/+0.3891pp`。`best_primary.json` 仍把 E30 登记为新最好，但 BBS @0.25 只提高约 3 个万分点；结合
E20→E25 已减速至 `+0.8098pp`，主指标 @0.25 已形成连续减速并接近零增益的真实平台证据。与此同时，两个
@0.50 指标仍持续提高，所以不能把整个训练判成全面停滞。

代码审计确认现有 plain `MultiStepLR` 没有运行时控制入口；仓库已有的 `resume_lr_scale` 只能在停止现进程、
加载完整 optimizer/scheduler checkpoint 后生效。当前约束是不重启这两条独立 ScanRefer 消融，且 @0.50 仍
有增益，因此没有通过杀进程/重启来强制提前降 LR，原进程与 `lr_decay_epochs=55,60` 保持不变。下一次 E35
继续确认 @0.25 是否稳定平台；已注册固定读取 task `Codex_MCLN_50630_E35_20260902`，执行时间
`2026-09-02 12:15 CST`。E30 task 在回执验证后已删除。

结构化回执：

```text
C:\Users\gb\.codex\tmp\mcln_50630_e30_receipt_20260902.txt
```


### 20.25 machine35608 E25 仍明显增长、LR 保持（2026-09-02 10:05 CST）

`12_sacr_no_pairwise_geometry` 的固定 epoch collector 于 `09:55:00 CST` 启动、`09:55:07` 写完，Windows
task result 为 0。E25 日志实际于 `2026-09-02 09:31:54 CST` 落盘，完整轨迹：

| Epoch | BBS @0.25 | BBS @0.50 | BBF @0.25 | BBF @0.50 |
|---:|---:|---:|---:|---:|
| 15 | `41.9541%` | `25.3576%` | `43.0690%` | `26.6512%` |
| 20 | `45.3197%` | `29.3753%` | `45.6878%` | `30.0273%` |
| 25 | `47.9070%` | `30.3955%` | `48.3698%` | `31.5524%` |

E20→E25 的 BBS @0.25/@0.50 增长 `+2.5873pp/+1.0202pp`，BBF @0.25/@0.50 增长
`+2.6820pp/+1.5250pp`；`best_primary.json` 把 E25 登记为新最好，精确 BBS @0.25 为
`0.47907025662599917`。该轨迹与 machine50630 不同，@0.25 仍有明显增益，没有平台证据，所以保持
`lr=1e-4`，不提前衰减、不重启训练。

按 E20→E25 实测约 `3h47m`，下一次固定读取为 E30，task
`Codex_MCLN_35608_E30_20260902` 已注册在 `2026-09-02 13:30 CST`；此前不轮询。完成的 E25 task 已在
回执验证后删除。

结构化回执：

```text
C:\Users\gb\.codex\tmp\mcln_35608_e25_receipt_20260902.txt
```


### 20.26 machine50630 E35 明显反弹，E30 平台未持续（2026-09-02 12:25 CST）

E35 固定读取 task 于 `12:15:01 CST` 启动、`12:15:07` 成功写完，task result 为 0；E35 日志实际于
`2026-09-02 12:12:28 CST` 落盘。最近三节点：

| Epoch | BBS @0.25 | BBS @0.50 | BBF @0.25 | BBF @0.50 |
|---:|---:|---:|---:|---:|
| 25 | `48.6643%` | `32.3622%` | `48.9903%` | `32.8986%` |
| 30 | `48.6958%` | `32.9512%` | `48.9693%` | `33.2878%` |
| 35 | `49.6529%` | `33.5717%` | `50.3576%` | `34.6866%` |

E30→E35 的 BBS @0.25/@0.50 增长 `+0.9571pp/+0.6205pp`，BBF @0.25/@0.50 增长
`+1.3883pp/+1.3988pp`；`best_primary.json` 将 E35 登记为新最好，精确 BBS @0.25 为
`0.4965292385359697`。因此 E30 的近零 @0.25 增益只是单节点局部停顿，没有形成持续平台；E35 的四项同时
反弹也证明当时不通过停止/重启强制降 LR 是正确的。当前继续保持 `lr=1e-4` 与原训练进程。

按 E30→E35 实测约 `3h15m`，E40 固定读取 task `Codex_MCLN_50630_E40_20260902` 已安排在
`2026-09-02 15:40 CST`；E35 task 已在回执验证后删除。

结构化回执：

```text
C:\Users\gb\.codex\tmp\mcln_50630_e35_receipt_20260902.txt
```


### 20.27 machine35608 E30 继续刷新，首查时间偏早（2026-09-02 14:20 CST）

预估的 `13:30 CST` 首次固定读取成功连接且 trainer 存活，但 E30 日志尚未落盘；没有用缺失日志推断指标或
修改 LR。延后到 `14:10` 的一次固定重试成功，确认 E30 日志实际于 `2026-09-02 13:43:05 CST` 落盘。
最近三节点：
...
| 20 | `45.3197%` | `29.3753%` | `45.6878%` | `30.0273%` |
| 25 | `47.9070%` | `30.3955%` | `48.3698%` | `31.5524%` |
| 30 | `49.1376%` | `31.8784%` | `48.8641%` | `32.0993%` |

E25→E30 的 BBS @0.25/@0.50 增长 `+1.2305pp/+1.4830pp`，BBF @0.25/@0.50 增长
`+0.4943pp/+0.5469pp`；`best_primary.json` 将 E30 登记为新最好，精确 BBS @0.25 为
`0.4913756836348338`。四项仍全部为正，没有连续平台证据，所以保持 `lr=1e-4`，不提前衰减、不重启。

E25→E30 实际用时约 `4h11m`。下一次 E35 固定读取 task `Codex_MCLN_35608_E35_20260902` 已安排在
`2026-09-02 18:10 CST`；完成的 E30 重试 task 已删除。

结构化回执：

```text
C:\Users\gb\.codex\tmp\mcln_35608_e30_receipt_20260902.txt
```


### 20.28 machine50630 E40 继续刷新主指标（2026-09-02 15:50 CST）

E40 固定读取 task 于 `15:40:00 CST` 启动、`15:40:09` 成功完成，task result 为 0；E40 日志实际于
`2026-09-02 15:39:45 CST` 落盘。最近三节点：

| Epoch | BBS @0.25 | BBS @0.50 | BBF @0.25 | BBF @0.50 |
|---:|---:|---:|---:|---:|
| 30 | `48.6958%` | `32.9512%` | `48.9693%` | `33.2878%` |
| 35 | `49.6529%` | `33.5717%` | `50.3576%` | `34.6866%` |
| 40 | `50.0947%` | `34.7602%` | `50.2419%` | `35.3071%` |

E35→E40 的 BBS @0.25/@0.50 增长 `+0.4417pp/+1.1885pp`，BBF @0.25/@0.50 为
`-0.1157pp/+0.6205pp`；`best_primary.json` 把 E40 登记为新最好，精确 BBS @0.25 为
`0.500946571308372`。主指标仍在刷新、两个 @0.50 指标继续明显提高；单个 BBF @0.25 小幅波动不能构成连续
平台。因此保持原进程和 `lr=1e-4`，不提前衰减、不重启。

按 E35→E40 实测约 `3h27m`，下一次 E45 固定读取 task `Codex_MCLN_50630_E45_20260902` 已安排在
`2026-09-02 19:20 CST`；完成的 E40 task 已删除。
...
#### 20.38.1 P2首次吞吐检查与完成窗口（2026-09-05 14:40:30 CST）

Python9430仍运行；两头已共同完成1,000更新、4,000行fit，其中3,956行有合法Top32>.25候选。
训练计时357.476 s，global/pair累计监督行加权loss为1.86121/1.84901；这不是准确率或改善证据。
GPU现场3,767 MiB/35%。此时距完整fit预计2,032.97 s，约15:14结束训练；预计评估再需5–10分钟。
因此新完成窗口约15:20–15:25，而非把单批检查的4秒直接外推整轮。

已创建独立只读定时采集器，15:18 CST采集训练日志末尾、PID9430状态、GPU与完成回执是否存在，
写入`PAIR/pair_readout_train_v1/completion_window_1518.json`。
...
### 20.41 P1正式路径全量诊断完成：Full256缺框426，Mask预测也有独立短板（2026-09-05 16:56 CST）

覆盖§20.40运行状态。7899表达/130场景、494批原生评估完成，用时648.1149秒。
controller.exit=0，11977/11988及workers已结束，16:46:31 GPU 1MiB/0%。
独立CSV身份顺序、源文件/checkpoint SHA、主REC与Mask原生算术全部PASS；
所有模型参数/缓冲区在前后均与保护checkpoint逐项相同，零训练/零权重写入。

**精确历史复现未通过，但本次逐行输出口径已闭合。**
历史REC4475/3759、Mask4192/3479、mIoU37.433749%；
本次REC4478/3763（56.690720%/47.638942%）、Mask4194/3482、mIoU37.459280%。
五项历史相等检查均False。核心历史源码未恢复，不能把+3/+4解释成方法收益；
保护正式最好仍保持4475/3759，不以本次微小变化替换。
本次unfiltered Source诊断4274/3708，而主REC过滤后4478/3763；
与历史同次4271/3705对4475/3759的路径差异一致。

过滤前Top16/32/64/256 oracle@.25为6325/6882/7208/7473；
过滤后为6564/7086/7349/7473，@.50为6017/6491/6719/6823。
Full256 oracle@.25=94.6069%，不是可部署准确率。
3421条当前REC错误中：合法Top16内可重选2086，合格框只在更大合法集合909，
过滤丢失全部合格框0，Full256真正缺框426。按Top32边界则2608/387/0/426，
按Top64则2871/124/0/426。2995/3421=87.5475%错误有合法合格框；
旧Top16“proposal failure”不能再当作Full256生成失败。物理实例身份与边界差仍未完全分离。
有1行无合法Query，按原生流程计miss，无新增fallback。

426条Full256缺框中，322条目标输入点数<=227，其中89条<=32；
90条fp2无目标采样中心，149条KPS无目标采样中心。
全部7899行中SA1/SA2/SA3/SA4零目标中心行75/156/539/1165，fp2/KPS为156/221。
这些是实测FPS索引中心，不是感受野覆盖或已证明Query实例身份。
<=32点组131行，REC28/3，Full256 oracle42；小目标生成问题真实且集中。
mouse34行仅5个Full256命中，soap dish56行25个；book84行71个、door428行416个，
后两类大量Top16缺框实际属于更大候选集合中的排序/截断问题。

REC与native Mask选择不同Query共534行，含1行无合法REC。
共同7898行若只读取REC Query原有Mask，@.25修复64/破坏17净+47，
@.50修复36/破坏9净+27，mIoU净+0.288417百分点。
这只是离线反事实，实际Mask路径未改，也不足以解释全部分割差距。
Full Query Mask oracle7126/5973、mIoU64.6422%。
6823条有合法box IoU>.5，其中1105（16.1952%）所有现有Query Mask都不超过.5。
故这些严格Mask错误无法只靠选择现有Mask修复。

两条Mask分支最终同样回映superpoint。额外CPU纯GT oracle覆盖1213个独立scene-target/
7899表达，计算当前superpoint任意二值组合的精确IoU上界：7843/7476、mIoU83.37498%。
最优前缀算法与255个非空小例目标的全部组合穷举吻合；所有Query Mask oracle均不超过该上界，
目标点数也逐行吻合。在上述1105条错误中，154条连superpoint上界也<=.5，
951条当前superpoint仍能表达合格Mask。因此不能把这951条都归因于superpoint划分。
这是表达能力与网络预测的区分，不是某一网络层的唯一因果证明。

完整Query对象槽覆盖102067/311948=32.7192%，目标Top32仅50520=16.1950%。
按表达重复计数，不是文本Anchor GT召回。下一步先在训练输入核验现有butd_cls对象分支的
完整对象特征接口与参照物可用性，不默认“完整Query集合”含所有参照物。
P2 v1仍失败封存，不因P1统计变好而追认通过，不直接进入P3或启动另一个盲目scorer。

**字段说明更正：**封存rows.raw_token_count实际上是Scene_graph_parse之后的空白分词数。
analysis.json的text_length同用此数。补充enrichment.json保留原始预测/证据不变，
提供真正CSV tokens长度；四组行数1639/1576/2389/2295，与旧缓存分组一致。
对应本次REC hits@.25为1001/949/1341/1187，Full256 oracle1556/1501/2267/2149。
以后与旧表对照必须用enrichment的CSV长度组，不能使用误命名字段。

完整报告：`docs/NR3D_OFFICIAL_CANDIDATE_AUDIT_RESULT_2026-09-05.md`；
全部原生日志、历史/当前回执、130场景/类别分组、原始行、SP上界、CSV补充在
`refine-logs/official_candidate_audit_20260905_v1/`。
raw JSONL23614096bytes，SHA `d99a504828876eb18d8fe77de3b901dc3ce374e58363ba662e317871fe6d5767`，
lossless gzip1468026bytes，SHA `3d63076f4de2aff1cfb37bd589a20cc2cac433ef477c90896bc01c5abba435ba`。
...
### 20.49 对象外观输入CPU审计完成，M5已完成两臂1024更新；新L1机制已定义（2026-09-05 20:43 CST运行观测）

本轮取得了新的具体证据和可执行代码，属于progress。完整三数据集目标仍未达成：
保护Nr3D4475/3759、Sr3D12139/10335以及ScanRefer受保护工件均未被替换。
M5两臂在20:43:04观测已完成各1024/1024步，原进程17187仍在；终态评估尚未完成。
step896/960/1024的elapsed分别2227.546/2304.198/2380.815秒，最近64步76.617秒。
按原始baseline吞吐估计完整6172行终态约21:03结束，接近21:00–21:03再查看。
不读取中途质量来更改门槛或步数，不将训练loss当作质量结论。

独立CPU对象输入审计已exit0：相同M3/M4的16个fit表达/16场景，683有效对象框，
原始点裁剪为空0，1024 seed框内为空8，其中4个仍有至少32原始点；
非正尺寸框0。原始裁剪中真实实例点占比中位数.7429577465，123框低于.5。
前4行重现R1的190对象槽/155正确预测类别。输入点SHA、M4 seed映射及
612源码/36数据文件前后校验全通过，0 GPU forward、0更新、0 holdout/formal行。
butd_cls当前实际使用既有实例框加预测类别，不能描述成纯检测框输入。
GT点实例身份只用于上述纯度审计，不能用来清理新模型的物体裁剪输入。
这仅证明已有输入含外观点，同时存在真实背景混入；不是REC收益或文本Anchor recall。
完整报告docs/NR3D_OBJECT_POINT_INPUT_AUDIT_RESULT_2026-09-05.md；
receipt648400字节，SHA723a49de580d7770de2bd7523473a84bf428c71e84e218d12239e96c28fba11b。

在此前§20.18.10文献记录上补查EG3DVG公开源码174e34894aea6513442da6b5dfa9b3e2bf8a1efa。
PECA确有每head文本到点的注意力，将对应点位置投影聚合到文本key。
GMA静态代码仍读取该类未初始化的spatial_n_head，hard_sigmoid未在forward调用，
PECA.kp_linear定义后未使用；未进行其完整运行/论文复现实验，不能据此评论论文指标。
原仓库许可CC BY-NC-SA4.0，未复制其源文件。参考地址与边界写入L1计划。

新L1只检验位置证据进入现有最后层Query→文本注意力的效果：
每臂新增零初始化无bias的288×288矩阵（82944参数），比较普通文本key更新和
按文本→点兼容性聚合点位置的key更新。保持原注意力投影/value、原主网参数/buffer、
原损失/候选过滤/最终决策路径；原Q/Box/Mask可以通过该注意力改变。
它不是P2/R1失败头的晋级，也不是新的独立文献创新或“已经解耦”的结论。
新脚本/测试已写，先CPU及16行原生loss梯度/零初始化身份预检，再决定是否启动已定义训练。
计划固定26747fit/413场景、6172模块留出/98 backbone已见场景、同batch4、
...
### 20.64 C1配对短训封存：完整性PASS，固定质量FAIL（2026-09-06 03:38 CST）
...
### 20.65 真实对象点外观原生预检PASS，准备独立配对学习（2026-09-06 03:45 CST）
...
### 20.66 对象外观原生配对学习启动，首批与源数据检查PASS（2026-09-06 04:06 CST）
...
### 20.67 Sr3D协议与全训练裁剪完成，真实单点对象修复验证PASS（2026-09-06 04:56 CST）
...
同次Nr3D对象外观配对进度：完整6172起点两臂与受保护C1起点逐行一致，经本地独立
复算REC6005/5306、Mask5767/5057、mIoU68.881519715%。baseline_rows SHA
398865c958df8d970546f305040cf2883728d6add2ddeca19da64fafed3d7765；仍是主干见过的
98个训练场景模块留出，不是正式验证。
04:53:15 observer确认两臂均1024/1024更新，PID24914继续终态评估，尚无终态receipt。
最近64步91.306秒；预计完整终态约05:17–05:22，继续既有240秒observer，不重启任务。
正在训练的是冻结旧模块4b3dad...，仓库中2a5600...零轴修复未混入该实验。
原质量门槛、完整预算、输出协议不变，暂不推断外观有效，也没有Sr3D学习或新正式指标。
三数据集受保护结果保持原值，完整目标继续active。


### 20.68 对象外观原生配对终态：完整性PASS，固定质量FAIL（2026-09-06 05:19 CST）
...
### 20.69 稀疏局部记忆环境与GPU算子验证PASS（2026-09-06 05:28 CST）
...
### 20.70 真实点—体素局部记忆原生预检PASS（2026-09-06 05:59 CST）
...
### 20.71 稀疏局部记忆全fit配对已启动（2026-09-06 06:13 CST）
...
### 20.72 稀疏局部记忆首批梯度诊断与V2启动（2026-09-06 06:42 CST）
...
### 20.73 V2完整启动仍失败，回到完整初始化梯度诊断（2026-09-06 07:08 CST）

06:47:33真实检查：V2 controller1、PID27756消失、GPU1MiB；仍在
run_nr3d_sparse_point_pair.py第261行共享GPU梯度范数精确相等断言退出。
已经完成1次原生预热和2次首批检查反向，但0优化器更新、0留出评估，没有科学终态。
warmup.json中4条实际输入点hash与缩小回归一致，loss=16.18368148803711。
这否定了“缩小回归通过即可认为完整启动已修好”；§20.72的启动待确认状态已终结。
V1/V2两个原目录、源码、计划、controller和错误栈都保留，没有自动续跑或覆盖。

06:54:01另开完整范围诊断，PID28034，目录
/root/autodl-tmp/mcln_sparse_full_start_probe_20260906_v1，manifest
6c58447ef971e564c07516441e8b5c93fe0a3b09068bee80842f9b2962dc12e5。
原Python3.7全部入口编译PASS；启动前GPU1MiB。逐行保留V2全部模型/优化器/
evaluator及32919条数据初始化；原生预热、原生检查、稀疏检查、原生重复、稀疏重复
共5次反向，0更新。保存实际梯度/范数，比较完整前向及状态/输入，不查看留出质量。
原atol1e-6/rtol1e-5和范数精确检查均报告，不在此诊断中放宽。预计07:04首查，
未结束则240秒观察；明确区分标量归约、原生数值波动与跨臂实质梯度变化。

独立终态AdamW/权重校验器只完成编译，未有V2终态实物可验。校验参数索引使用
实际模型注册顺序；平均parent文件按键排序，不能用该顺序推定优化器顺序。
当前无V3训练、无新正式指标；三个基准保护结果保持，完整目标仍active。


### 20.74 完整反向数值诊断、相对梯度回归与V3启动（2026-09-06 07:19 CST）
...
### 20.75 V3完整首批检查通过，进入起点全量复核（2026-09-06 07:29 CST）

07:27:31真实观察PID28416仍在运行，GPU10815MiB/25%；原始完整启动首批检查通过，
已进入baseline评估，50个batch/800行，累计197.234秒（记录范围含首批检查）。
当前训练更新数0；完整6172行起点身份还未终结，不能称为已完成起点复现或有新质量。
按该段数据约4行/秒估算，baseline还需20多分钟；原observer07:37再按实际进度估时，
后续接近预计完成时240秒观察，不用中间质量改变设置。

真实两臂loss均16.18368148803711；Box/score精确相同，所有冻结参数/buffer不变。
16个共享梯度最大每张量相对L2差1.938917148696763e-5，最大绝对差3.594160079956055e-5，
均在预注册相对L2<=1e-4内；GPU范数精确性仍为false、原逐元素检查有false，未隐瞒。
这在原始完整流程中复现了原生数值波动并验证新的数值检查可以继续；没有预热和优化器更新。
新输出矩阵梯度非零，其余16个新张量初始零梯度符合零输出投影；17张量梯度连通性仍由
原16扫描固定扰动预检提供证据。preflight及first_batch_gradient_comparison原JSON均归档。

GitHubmain已验证f7eb50849d1179d8517868b8f3ab6853e5e7c7de。逐字节发布检查发现runner、
summarizer及summary测试三份文件经Git CRLF/LF转换，换行归一化后内容相同。
补充source_snapshot.zip保留manifest全部11份源码/计划/测试原始字节，逐项解包SHA通过。
159463-byte归档SHA为5a4c3b96430d4f35aec4b81d8f26c42a45b2b20563e2a09c536b6ef5de41823f。
该归档只恢复精确源码；数据、权重、稀疏环境继续由原清单独立约束，不称为自包含训练包。
终态实物/逐行质量检查入口已准备且编译，但终态尚无，不冒充已执行审计。

当前正式Nr3D/Sr3D/ScanRefer保护结果不变，整体目标未完成；继续等待起点完整复核及
两臂6687步终态，科学验收和后续正式验证条件保持原计划。


### 20.76 V3全量起点复核通过，实际配对更新与终态估时（2026-09-06 07:58 CST）
...
### 20.78 稀疏终态的条件式正式比较入口与原生CPU验证（2026-09-06 08:39 CST）

终态汇总已有预注册修复/破坏及固定条件Mask诊断，运行中V3文件未改。独立实现
run_nr3d_sparse_native_formal.py及nr3d_sparse_formal_state.py，准备在完整训练质量
和实际AdamW/权重审计通过后，对正式7899行执行protected/native/sparse三个状态比较。
两个参照用于区分原生Mask更新与新局部信息；目前没有合格manifest、训练终态加载或正式GPU运行。

入口从已哈希完整训练行重新计算双参照Mask质量门，不能只读取保存的PASS字段；保留
原生TrainTester数据/eval路径、B16/4worker和各自evaluator，逐批临时切换16个Mask
张量及稀疏hook，再恢复受保护参数。全部REC/候选选择保持相同，逐行指标核对各臂原生
evaluator；终态输出还包括两组修复/破坏、scene区间及固定条件Mask，不自动升级保护结果。

原Python3.7.11/Torch1.10.2环境v2的19项CPU检查PASS；11项新加载/切换/三臂统计检查，
8项原evaluator和质量规则回归。fixture均为合成数据；真实稀疏模块只CPU构造并核对
17张量267936参数，0原生模型/GPU forward、0优化器更新。612源码hash一致，CLI编译/
--help通过。v1的额外tests包标记依赖已移除，v2与仓库目录一致，两份证据保留。

完整DDP/native epoch接口仍待实际fit输入复核，正式性能及运行开销未验证；文档见
...
### 20.86 Scan联合读出固定终点：完整审计PASS、REC筛选FAIL（2026-09-06 15:59 CST）

原任务于15:30:35完成，15:31:34观察到controller.exit=0且PID32521已结束。两臂均
完成29778条fit表达的一次遍历、2482次更新；终态评估覆盖全部6887条表达。独立CPU
审计重新读取实际checkpoint、优化器和逐行记录：108个参与更新的参数张量均2482步，
梯度矩有限，冻结参数/缓冲区及三份读出元数据不变，fit遍历及两臂/起终点采样身份一致。

| 6887-row训练场景模块留出 | REC hits25/50 | Mask hits25/50 | Mask mIoU% |
|---|---:|---:|---:|
| 初始保护V99 | 6688/6323 | 6515/5829 | 70.450467571 |
| detached终点 | 6686/6351 | 6512/5823 | 70.427802972 |
| joint终点 | 6681/6343 | 6514/5824 | 70.445763291 |

joint相对起点：REC25修复12/破坏19/净-7；REC50修复83/破坏63/净+20。
joint相对detached：REC25修复4/破坏9/净-5；REC50修复19/破坏27/净-8。
detached相对起点为-2/+28，亦未满足双阈值不退化。joint相对起点的106物理场景
聚类95%区间分别[-0.24937,+0.04282]、[-0.09568,+0.67944]个百分点；区间仅诊断，
不改变事先固定的REC筛选。该模块留出已被原预训练看过，不能用约97%的数字表示
正式新场景成绩。记录不能区分实例身份变化与同实例框边界变化。

固定筛选FAIL，当前配置封存，未启动9508-row正式验证，也未以它启动Nr/Sr训练。
这说明本次预算/参数范围的联合读出没有优于控制，不能推广为所有联合训练无效。
当前Scan受保护5572/4797保持原模型/原结果；三个数据集没有新的正式成绩。
下一独立结构工作定位最后层cross_v：保留全局路径，候选在框回归之前读取原始RGB、
早层点特征及候选相对位置。先做Scan原生输入/零初始化/梯度预检，再定义固定对照；
此时尚未实现或启动该结构，不称作体素模块，不同时加入新的质量损失。

两份终点原SHA：detached 8e3919958693fc95abc591d78f2f7833bdd5b4a5cf0aa3ffbb3097b526f4ded7；
joint de78c59e86630f3bd7a2b119f81b3044ee79957f202cfdcb8024bc1ff9dde25a。
各622982227 bytes，已先完成真实权重独立审计，随后按§20.87用户要求删除；不是仍可
读取的checkpoint。完整逐行结果、fit采样hash、源代码、receipt及审计证据保留于
refine-logs/scanrefer_joint_readout_pair_20260906_v1/。整体goal仍active。

### 20.87 用户要求的磁盘权重清理（2026-09-06 15:59 CST）

15:52:55完成12项明确路径操作。数据盘50GiB，清理前空闲1978150912 bytes
（1.8423GiB），清理后10971111424 bytes（10.2176GiB），实际增加8992960512 bytes
（8.37535GiB）。启动前GPU无计算进程，并独占现有GPU任务锁。

1. 9份旧E57独立快照与受保护full-state E57的SHA完全一致，均为只读0444。将这些
   重复物理副本替换为指向已有原文件的硬链接；原路径、内容、大小及只读属性保留。
2. 1份旧raw-parser审计中的平均权重独立副本同样合并到当前Nr受保护平均权重。
3. 删除§20.86已完成审计且未晋级的detached/joint两份完整权重。未删除它们的
   逐行结果、训练/评估记录或源代码。

清理后再次验证Scan E71、Parent、Geometry、V99、Nr平均最好及原E57共6个指定
保护路径SHA全部不变。其他权重未删除；数据集和初始化文件保留。两份失败终点的
删除状态另写回原run的weights_removed_after_audit.json，避免旧receipt被误读为
当前仍有可加载的终点。删除及去重清单、原/后磁盘读数与校验记录位于
...
### 20.89 Scan局部视觉原生预检PASS，固定配对已启动（2026-09-06 16:44 CST）

V3于16:34:28完成真实16条fit预检（B12+4），controller.exit=0。启用145008参数
局部分支后，零初始化的Box、token、原Query/Text Mask及完整V99 runtime均与原模型
逐位一致。原生GT loss传入局部输出投影；两步一次性内存更新后，point/Q/K/V权重
梯度均有限非零。固定参数、buffers和读出保持，0正式行、0checkpoint写入。
峰值4323.57MiB；B12与B4新模型前向分别0.4412/0.2726秒，仅该预检批次测量。
这些是接口和训练连通证据，不代表REC提升。

重新加载E71并新建零输出分支后，16:36:40启动配对screen36962，16:37:30实际
验证Python PID36968存活且GPU已分配，处于数据加载；当时尚未观测优化器更新。
固定2482步/臂，源/数据/GT loss/冻结读出及正式规则均遵守§20.88计划。manifest SHA
d3cfd5497c934b55730f121846d8c302438489f70cc42e5d01e9e2ccd07b58a9。
目录/root/autodl-tmp/mcln_scanrefer_local_visual_pair_20260906_v1；本地同名refine-logs。
没有沿用预检2步参数，没有复制完整预训练权重；数据盘可用10923843584 bytes，约
10.17GiB。源仍为§20.88的614文件隔离版本，保护权重路径保持只读。

首段观察定17:20 CST；本地观察器18496之后按240秒检查，取得>=64步或实际终态
即返回，以测得步速再安排预计终点前五分钟观察。原观察器42996在第一次查询前因
日志前缀解析修正而停止并替换，仅变更观察器，未重启训练进程36968。
整轮暂估3–4小时，包含解析、6887起点评估、训练和6887终点评估；正式9508评估
在固定终点后单独进行，不将本次估时当作实际完成时间。当前仍没有新正式成绩。
下一步：独立复核终点参数/逐行结果，执行已准备的固定正式比较；Scan底线通过后
尽快Nr/Sr REC。所有旧失败结论与用户磁盘清理范围保持，整体goal仍active。


### 20.90 Scan局部视觉独立终点审计入口准备（2026-09-06 17:10 CST）

16:55:33在原Python3.7/Torch1.10.2环境完成6项CPU检查与真实预训练读出检查。
脚本scripts/audit_scanrefer_local_visual_pair.py检查同一固定2482步终点：完整fit
遍历、6887行输入身份、阈值命中/修复破坏、106物理场景聚类区间、614源文件和四份
预训练输入SHA，并逐张量核验冻结核心、buffers及三份V99读出内容/元数据。
优化器实际状态预期control66/local76个参数张量，各2482步；原norm1两张量在forward中
未使用，必须保持。局部分支另10张量、145008参数，不把两臂称为等容量对照。

本次准备在CPU按训练器相同seed0模型构造顺序重复初始化两次，10个新张量逐位一致，
1144个原模型键和68个允许更新的核心张量一致。新分支初值属于同构造顺序重建，
不是训练器独立保存的起点tensor dump。三份真实读出export与预训练内容一致；
分别人为改变Parent/Geometry/V99权重的检查均能拒绝。6项检查还覆盖冻结buffer、
未使用参数、嵌套归一化元数据及优化器步数/动量形状错误。

以上仅为审计入口准备PASS；没有审计尚未生成的训练终点，没有GPU前向、优化器更新
或正式评估行。准备证据位于refine-logs/scanrefer_local_visual_audit_preparation_
20260906_v1，服务器同名mcln目录；审计脚本SHA
dcb6866d77e596cf451ff3aecc0e9ce70beb6cb2a53ce84b7c8f64995401b593。

终点收集和固定正式启动辅助入口已编译，尚未执行：前者要求训练正常完成并直接读取
服务器实际权重与优化器；后者要求本轮完成记录及独立审计匹配，再启动固定9508行
protected/local配对。只同步日志、逐行指标和SHA，不把大权重下载或复制到新run。
仍沿§20.88预先固定的规则，开发集质量仅诊断，不据其选择epoch；正式Scan REC/Mask
底线通过才进入Nr/Sr REC。本轮训练源码/优化器/进程未修改，下一次进度观察仍为
17:20 CST。§20.87磁盘清理已完成；当前没有新正式成绩，整体goal继续active。


### 20.91 Scan局部视觉128步实测与6887起点逐行一致（2026-09-06 17:25 CST）

17:20:07按原预约观察到训练PID36968仍为Rl+，两臂日志各128/2482步，原生GT loss
与梯度有限。首段平均2.58434秒/配对步，最近64步2.58549秒/配对步；不是单臂步速。
完整6887起点评估已结束，用时1790.91秒。17:23:53独立下载逐行数据后验证：新
control/local两臂完全一致，并在行ID、场景、点云SHA、REC/Mask IoU及所选变体位置
七个字段上与此前E71/V99起点逐行一致。仍为106个主干见过的开发物理场景，0正式行。
证据：refine-logs/scanrefer_local_visual_pair_20260906_v1/baseline_verification.json；
baseline_rows SHA348847d90aa4a685a9412cb738febec80b89378892615a1da8c9d45f64378c2a。

按当前步速及本次起点评估耗时推算，训练约19:00结束，完整终点约19:30完成；估时
依赖后续速度稳定，并非实际终态。首段观察器18496已正常返回，未重启GPU训练。
新观察器41352预约19:24 CST开始，此后240秒检查至终态，同时记录数据盘可用字节。
...
### 20.95 Scan局部读取训练终态与独立核验，固定正式评估启动（2026-09-06 19:46 CST）

本节是实际终态，不是预检。19:32:41训练器完成两臂各2482次更新和6887-row最终
留出评估；19:36观察确认controller退出0、主进程结束。19:37完成实际权重和逐行
独立审计，integrity PASS：fit遍历/成对点输入一致，control/local分别66/76个
优化器参数state均为2482步，允许范围外的核心/缓冲区及三组V99评分器参数、metadata
保持不变。终态统计如下，全部来自主干曾见过的106个物理空间留出，不是正式验证：

| 状态 | REC hits@0.25 / @0.50 | Mask hits@0.25 / @0.50 | Mask mIoU |
|---|---:|---:|---:|
| 共同起点 | 6688 / 6323 | 6515 / 5829 | 70.45046757% |
| 原cross_v控制终态 | 6680 / 6343 | 6512 / 5822 | 70.42949279% |
| 候选局部读取终态 | 6668 / 6294 | 6513 / 5826 | 70.44170041% |

local相对起点REC净-20/-29：@0.25修复12、破坏32；@0.50修复87、破坏116。
local相对控制净-12/-49：修复/破坏分别17/29、58/107，未显示REC收益。
相对控制的107个严格阈值破坏中，93个新输出仍在(0.25,0.50]；本实验Box允许
更新且没有完整实例身份标签，因此不能仅据此归因为“选了较差现有框”。
106物理空间、2000次seed0 bootstrap中，local-control的REC@0.50差异95%区间
为[-1.17175,-0.31766]pp；这些区间为诊断，不作为本轮新晋级规则。

实际权重已保留待正式评估：control618602343字节，SHA
1ebf8cd5df3b889de3a626a74723ce43b06957e5779da97e77c93dd11b1d40ec；
local620352201字节，SHA
4c6d41c49a21d73e3174828c20abec2703efb18df285a27594d21f423741b986。
审计SHA40b8019ecd3b80490d221adf6657954f0fa94a41f1dcffe5655fc3155b90d545，
训练receipt SHA94c8c4c728f7d91278d0d2004bcf6473dc0ce39362d31edc05d58687d2ccceb5。
19:36数据盘可用9627742208字节，约8.97GiB；未下载或删除这两个待评完整权重。

按20.88在结果前固定的规则，开发留出不选择epoch，实际完整性通过后仍评估唯一
固定local端点。19:39:15已启动9508-row正式Scan配对：protected_v99与local_v99；
同时从同一模型forward记录各自原生REC，完整系统/原生网络分开报告。
正式run=/root/autodl-tmp/mcln_scanrefer_local_visual_official_20260906_v1，
screen39696.mcln_scanrefer_local_visual_official_v1，Python39702，flock39701。
19:42:25已核对真实进程与首批12/9508输出；没有正式终态/晋级结果。
正式manifest SHA53bc72f64be73c92e8e4b74ba7a1430b063262dcd28c9825fd03ecceb8053a3f。

以本次6887-row成对评估1830.66秒估计，正式终态约20:25；首批有初始化开销，
后续以实际稳定进度修正。Windows观察器28336、会话98782已验证启动，20:20开始
查询，之后240秒；前一个训练观察会话39610已正常结束，不再使用。
...
### 20.97 正确 mesh 数据正式复核仍未晋级；从受保护起点重复训练配对（2026-09-06 22:24 CST）

§20.96 的修正正式评估 v3 已于 `2026-09-06T21:05:13.396012+08:00` 实际完成，controller 退出0。此次使用 DATA_ROOT_mcln_meshsp/，完整9508条、两臂相同采样点；训练更新与权重写入均为0。

| 系统 | REC hits@0.25 / @0.50 | REC Acc@0.25 / @0.50 | Mask hits@0.25 / @0.50 | Mask mIoU |
|---|---:|---:|---:|---:|
| 受保护 V99 控制 | 5570 / 4797 | 58.582247% / 50.452251% | 5689 / 4975 | 45.92202560% |
| 旧数据训练的局部视觉终点 | 5549 / 4744 | 58.361380% / 49.894825% | 5690 / 4961 | 45.85679961% |

局部模型相对同数据控制：REC@0.25修复42/破坏63，净−21；REC@0.50修复76/破坏129，净−53；Mask净+1/−14。三个Scan Mask论文底线通过，两个REC相对历史和相对控制的四项检查均失败，**不能晋级Nr3D/Sr3D训练**。受保护控制5570/4797较历史5572/4797仍少2/0；没有精确恢复历史源码，不将这2个命中归因于新方法，也不在缺少证据时归因于浮点噪声。

同一forward在V99之前的原生REC为控制5514/4409、local5512/4433，净−2/+24。完整系统的严格阈值退化与原生变化不同，表明需继续研究表征更新和冻结读出的关系；但当前未完整区分实例身份错误与同实例低质量框，不把IoU转移直接写成实例消歧结论。

实际独立CPU审计重新计算全量指标、修复/破坏和晋级条件，核对逐行/采样点身份、614文件冻结源码、评估后312个mesh验证superpoint及4个受保护系统artifact和local终点的文件SHA。评估器自身同时检查内存模型状态未变；CPU审计不是第二次GPU复算。

- 正式receipt SHA：`119db3618456afa9c42eb146ca860dfde76517859040ed9a3368c02bcd515e59`。
- 独立审计 SHA：`8645d692e54bcc3f57ab0d68373a9e8cc3feca19d7b33bc7ded98970c33f80e9`。
- 全量结果：`refine-logs/scanrefer_local_visual_official_20260906_v3/result/`。

**下一步已固定并开始执行：只修正训练输入，重复原配对。** 计划为`docs/SCANREFER_LOCAL_VISUAL_MESH_REPEAT_2026-09-06.md`，启动前SHA为`cc911fd52154cce1103f705b6a73864aff5457a405b4e3707efd5705a5a48dd0`。仍采用相同614文件模型源码、E71预训练、原局部读取结构、GT原生损失和冻结V99；两臂各一遍29778条fit/2482更新，batch12、末批6；6887条模块holdout只作固定对照，不选择epoch。没有同步加入新的评分、匹配或损失改动。

修正数据的16条真实GPU预检于`2026-09-06T22:17:57.888056+08:00`通过：零初始化原生/V99输出一致，2次一次性更新后局部点编码及Q/K/V梯度有效，冻结参数/buffer/读出不变，完整权重写入0。预检receipt SHA：`e4ad0603bba390425f7d9e5a74a36426ea6a77732e4a1517634735db70bd26ec`。正式训练重新实例化受保护权重，没有继承预检的两次更新。

配对训练启动时间`2026-09-06T22:19:50.607427+08:00`，screen `42642.mcln_scanrefer_local_visual_mesh_pair_v1	(09/06/26 22:19:49)	(Detached)`；目录`/root/autodl-tmp/mcln_scanrefer_local_visual_mesh_pair_20260906_v1`。此处只记录已核实的启动，不是训练终点或新指标；首批更新与ETA由独立进度观察补充。训练/预检receipt明确记录data_root与superpoint清单SHA，终态审计检查命令中的实际data_root并重查1201/312文件。更新后的审计与数据契约在原环境完成12项CPU检查。

最近实际进程观察`2026-09-06T22:23:52.683885+08:00`：训练PID`42648`存活，正在原生文本解析，尚未观察到优化器更新。一次性观察器本地PID`28200`已启动，首次观察`2026-09-06T22:54:00+08:00`，此后240秒，达到64步或进程实际终止后退出。该时间依据上一轮baseline1790.91秒和本轮约7分钟输入初始化估计，不是完成承诺；不得因观察超时重启训练。

**Nr/Sr准备保留，训练尚未启动。** 修正数据的CPU输入预检于`2026-09-06T20:57:34.271182+08:00`完成：32条固定记录、两种增强状态合计64份真实点样本；64份点SHA及已记录字段与旧预检一致。原始表达/目标身份由原生构造器核对；没有宣称逐元素比较全部token map。新的superpoint身份另外记录。GPU前向/优化器更新/权重写入均0。CPU回执SHA：`5ce8796009e8e425114985bde51a7b5b4560f2593d13fe73fe919a5c74929e0e`。对应GPU条件启动器已经准备，但正确Scan v3未通过，因此没有执行，也没有新Nr/Sr REC结果。

**磁盘与保留：** 22:10:21启动前实查服务器剩余9606447104 bytes（约8.95GiB），GPU仅1MiB、无计算进程。此前已释放8.375GiB的清理记录继续有效；本轮复用原预训练文件，不新增完整副本，训练只写两臂各一个终点。保留日志、逐行结果和受保护最好；没有删除正在被使用的权重。Scan达到历史/配对REC不退化及Scan Mask底线即转Nr/Sr，不等待59/51，Nr/Sr Mask不设门。三数据集目标仍未完成，goal保持active。
随后于`2026-09-06T22:21:58.954729+08:00`按用户磁盘清理要求，删除旧配对的`control_local_visual_state.pt`与`local_local_visual_state.pt`：两者训练审计已通过、固定终点结果已封存，正确数据的formal v3确认local不达标，旧进程已终止，新轮artifact输入不依赖这两个文件。删除前复核大小、SHA和单链接身份，删除后复核六个受保护权重SHA一致；日志、逐行结果及旧审计保留。本次释放1238966272 allocated bytes（约1.154GiB），实查剩余10843734016 bytes（约10.10GiB）。证据`refine-logs/weight_cleanup_20260906_v2/receipt.json`，SHA`828cdd051462366f940f4d1c2c35ab966690522c2ae855b4b68fd532ba03ee2d`。旧权重已按此记录退休，以后不可再声称其文件仍可重载；历史审计是删除前实际完成的证据。


### 20.98 mesh训练初始6887条核对通过、两臂64步与终态接续队列（2026-09-06 23:12 CST）

§20.97 的原训练PID42648继续运行。23:02:05的实际观察确认两臂各完成64/2482次更新；control/local原生GT loss分别10.266443/10.223102，梯度范数为有限值。此处是早期训练状态，不是训练终点、留出增益或正式9508成绩。累计172.08秒，平均每对更新2.68881秒。

**观察器错误已隔离。** 原本地观察器在22:58处理`EVAL COMPLETE`时，较短的`EVAL`前缀也匹配该行，引发JSONDecodeError并退出1。22:59:39重新核对同一训练PID仍存活、训练controller没有退出；只将观察器匹配改为`label + "{"`，复用先前已修复的读取方式。实际捕获的完成/更新日志解析通过，v2观察器于23:02读到64步后正常退出。训练没有被重启或修改。

**完整初始评估的独立核对：** 两臂6887条所有已保存字段完全一致；与旧数据运行相比，row_id、scan_id、physical_space、point SHA逐条一致。106个holdout物理场景与456个fit物理场景无交集，但冻结预训练主干以前见过这些训练场景，因此不能把本表当新场景泛化成绩。

| 相同E71＋V99起点、模块holdout | REC hits@0.25 / @0.50 | Mask hits@0.25 / @0.50 | Mask mIoU |
|---|---:|---:|---:|
| 旧mixed superpoint数据 | 6688 / 6323 | 6515 / 5829 | 70.45046757% |
| 正确mesh superpoint数据 | 6684 / 6426 | 6511 / 6097 | 77.81086079% |

数据修正的初始REC修复/破坏为24/28与179/76，净−4/+103；Mask为45/49与368/100，净−4/+268。它说明输入版本确实影响输出，尤其严格阈值和Mask；尚无新训练终点，不能计作局部模块收益。训练仍执行已固定的一遍更新预算，不据此重新选择超参数或正式验证阈值。

初始核对回执：`refine-logs/scanrefer_local_visual_mesh_pair_20260906_v1/baseline_verification.json`，SHA`a83c85e9b2e10bb771c9d864c1e3eb44dbc410333618a5f8168d18e3d60d0b47`；完整baseline_rows SHA`415d8b321702f693fb8597fbcf5f23d1bd61e935a656e406b2c656c48f4dd455`。

**实测时间与已启动接续任务：** 初始6887评估耗时1833.90秒；据64步的实测速度，训练及终态6887评估预计于`2026-09-07T01:18:08.483258+08:00`至`2026-09-07T01:21:00.567069+08:00`结束。此估计假设训练速度和终态评估耗时保持接近，不是完成承诺。

- 服务端接续任务：screen `43353.mcln_scanrefer_mesh_posttraining_v1	(09/06/26 23:04:46)	(Detached)`，实际等待worker PID`43358`。
- 首次检查：`2026-09-07T01:13:08.483258+08:00`，之后240秒；等待原训练screen42642真正退出，随后要求controller退出0。
...
- 本地观察器PID`50584`，首次收集`2026-09-07T01:14:08.483258+08:00`，以后240秒，遇到实际formal启动或接续进程真实终止后退出。

实际接续启动回执：`refine-logs/scanrefer_local_visual_mesh_posttraining_20260906_v1/queue_launch.json`，SHA`847d902669a098d91cba9f5c891fa1da4097d4493fc5082d4f6ee51039cbff89`。原环境已编译10个封存入口，生成的Bash控制器语法和精确退出码输出检查通过。正式评估的6个源码文件与已完成v3逐字节一致，复用其实际30项测试证据；训练审计入口复用§20.97实际12项测试。准备、编译和排队都不是实际终态审计或正式评估完成。

接续任务已排队，后续不要再单独启动第二份终态审计/正式评估。若观察会话超时，先检查同一任务句柄和回执，不能据此重启训练。新formal完成后仍需读取实际独立正式审计和Scan验收结果；Nr/Sr条件入口须绑定这次新的通过回执，目前绑定旧v3负结果的入口继续不执行。历史最好、Scan REC/Mask门槛及Nr/Sr Mask豁免保持§20.79。

本次文档同步时实查磁盘剩余10839334912 bytes（约10.095GiB）。仍只计划两臂各一个终点；本节没有新增权重清理。整体三数据集目标未达成，goal保持active。


### 20.99 本轮Scan正式评估后独立审计及Nr/Sr条件接续（2026-09-06 23:40 CST）

§20.98的训练、服务端终态审计/正式评估队列及本地观察器均沿用原设置。本节没有新的训练终点或正式指标；最近训练进度证据仍为23:02两臂64/2482步。没有据此改变主干、损失、学习率、更新预算、候选规则或晋级阈值。

**实际衔接更新：** 旧Nr/Sr GPU预检入口绑定了已经失败的Scan formal v3。新归档入口只将这一个formal_root改为本轮`mcln_scanrefer_local_visual_mesh_official_20260906_v1`；与旧入口比较，其他源码完全相同。其完整9508行身份/点SHA、REC与Mask复算、实际正式回执SHA、独立审计SHA和GPU空闲检查继续生效。旧失败入口仍未执行。

新本地接续进程PID50808于23:38:35确认存活，子进程PID37596实际执行`Wait-Process -Id 50584`；目标观察器50584也同时确认存活。执行会话10701。它等待现有收集进程自然结束，不另开一份训练观察或正式评估。服务端只保存此Windows入口的归档；实际接续worker运行在本地。

后续顺序固定为：原观察器获得本轮真实formal启动回执并退出；新worker核验该回执与输入manifest；于实际formal启动后24分钟首次查询formal screen，此后240秒；screen真实退出且controller为0后，执行既有独立CPU正式审计；审计通过后按既定Scan条件决定是否调用Nr/Sr原生GPU预检。24分钟来自上一轮同源码9508配对评估实际1537.14秒加初始化的时间证据，只用于查询调度。

正式审计入口绑定§20.98封存preparation及本轮实际launch，继续核对训练终点、源码、权重、mesh superpoints、9508条原生/系统输出与修复破坏。它尚未执行，因为本轮正式结果尚未生成。若Scan未达标，接续worker只保存负结果decision并停止；若达标，先启动既已准备的Nr/Sr真实输入GPU预检，不把预检或运行状态写成Nr/Sr训练完成。后续完整训练仍须读取实际预检结果。

**本次已执行检查：** 三个入口完成语法编译；Windows原生等待在真实短生命周期子进程上完成；旧v3真实9508行重新计算仍为REC四项未过、ScanMask三项通过，晋级false；新native入口除formal_root外与原入口逐字一致。没有新增GPU forward、optimizer更新或权重。原正式评估及审计源码仍使用§20.98封存并已经测试的版本。
...
### 20.102 正确mesh训练及正式评估负结果；逐级诊断实测启动（2026-09-07 02:07 CST）

**本轮训练已经完整结束。** 两臂各2482次更新，29778条fit、6887条模块holdout；训练于01:16:33完成，退出0，独立CPU审计通过。该holdout仍是预训练主干见过的训练场景。相同正确mesh起点REC为6684/6426，终点control为6679/6449，local为6670/6405；local相对起点−14/−21，相对control−9/−44。固定终点仍按原计划进行唯一正式评估，未据开发集选择epoch。

6887条完整终态和fit点身份记录已归档；命中数本地复算完全一致。Windows本地mIoU复算与原环境记录最大差4.55e−13pp，采用既有evaluator的1e−8浮点比较精度；未修改模型、损失或指标阈值。训练回执SHA`c0248a1928306ae73565a5d54c8eb1b56d68de01ca77b3aad1998138bdd075ef`；独立训练审计SHA`bb270b9ec04259ec20a587a0c9833f2c657abce74fc8f52bf9eeb5469fdc55c3`。

**9508条正式评估于01:44:34结束，独立审计及条件队列于01:45:41完成，均退出0。** 正确mesh的312个val文件、输入身份、权重及冻结状态核验通过，但质量晋级失败：

| 同次正式评估 | REC hits@0.25 / @0.50 | REC百分比@0.25 / @0.50 | Mask hits@0.25 / @0.50 | Mask mIoU |
|---|---:|---:|---:|---:|
| 受保护V99控制 | 5570/4797 | 58.58225% / 50.45225% | 5689/4975 | 45.92202560% |
| 正确mesh局部视觉终点 | 5543/4722 | 58.29828% / 49.66344% | 5695/4976 | 45.93195287% |

local相对同次保护控制：REC@0.25修复37、破坏64，净−27；@0.50修复73、破坏148，净−75。Mask两阈值净+6/+1，mIoU+0.009927pp，三项ScanMask底线通过；REC相对历史5572/4797及同次5570/4797均未过。历史受保护版本没有替换，也未将Mask单项拼入历史最好行。

同次原生REC：保护5514/4409，local5510/4426，净−4/+17。原生严格阈值略升、完整系统下降的现象在正确数据重训后仍出现；这支持继续逐级定位，但尚不能证明冻结读出不兼容是唯一原因。正式审计SHA`df5d90d3f3eb10320eea6aba1e358a35983a033f0438d52434166a0f6b3d6ce5`；正式回执SHA`0ecd12be9c1377c0efe11d471be65f31cef91297781f032aee92a3f44e888a64`，位于`refine-logs/scanrefer_local_visual_mesh_official_20260906_v1/result/`。按§20.79，Nr/Sr预检和训练均未启动。原观察/正式审计队列已正常完成，不要重启。

**独立逐级诊断已实际启动。** 01:51:47，screen`45890.mcln_scanrefer_stage_diagnostic_v1`，绑定本次正式结果、两个固定权重、同一正确mesh和原始输入配置；无训练、无新增权重，`formal_rows=0`、`used_for_promotion=false`。输入manifest SHA`0d8391d25894ccaf41d405a6cbbb11d5c35da9b97757c16f45cad95b5628a5d2`。

02:01:26实际观察到1536/9508条，两臂逐级选择已通过每个forward的最终分数完全一致校验；Parent/Geometry的归一化特征读取也已在真实输入上执行。完整阶段汇总、参考结果一致性和独立终态审计仍待完成，不能将1536条进度当正式新指标。首批12条6.21秒含启动影响，没有用其82分钟外推安排后续；按1536条260.54秒的记录，保守剩余约1352秒。

本地观察器PID49108、会话6419已启动；首查`2026-09-07T02:20:58.548174+08:00`，之后240秒，结束后只收集诊断输出，不重复训练或启动Nr/Sr。目录`refine-logs/scanrefer_stage_diagnostic_20260907_v1/`。下一步先读取实际逐级损失和归一化统计，再决定范围读取或兼容约束，不恢复旧Gate/Pair扫参。

01:45实查磁盘剩余9584189440 bytes（约8.926GiB），6个受保护文件均在；本轮只生成两臂终点共1238954544 bytes，未下载权重。local终点正在用于诊断，继续保留；既有累计约9.529GiB清理记录不变。三数据集目标未完成，goal保持active。


### 20.103 逐级诊断完成：Parent宽松阈值退化与Geometry严格阈值收益减少（2026-09-07 02:27 CST）

**实际诊断已于02:20:37结束，9508条、两固定权重、controller=0。** 观察器6419/PID49108于02:21:18完成收集并退出0；当前没有本项目训练/诊断GPU任务运行，不要重启旧观察器。这是既定正式评估之后的独立诊断：formal_rows=0，optimizer_updates=0，checkpoint_writes=0，不能晋级或替代§20.102正式数值。

**独立CPU审计通过。** 两臂共114096个阶段框及304256个Top-16框相对root GT的IoU用NumPy独立重算，与保存值最大差均为0；所有阶段命中、修复/破坏、Query/变体映射、输入身份、152/179维有效特征计数及文件SHA核对通过。运行时还逐forward核对最终分数完全一致，模型和读出状态未变化。

| 本次诊断阶段 | 保护REC hits@0.25/@0.50 | local REC hits@0.25/@0.50 | local−保护 |
|---|---:|---:|---:|
| native | 5514/4409 | 5511/4428 | -3/19 |
| parent | 5511/4420 | 5460/4426 | -51/6 |
| parent_after_geometry_validity | 5511/4420 | 5460/4426 | -51/6 |
| geometry | 5556/4816 | 5509/4727 | -47/-89 |
| v99_proposal | 5568/4786 | 5566/4753 | -2/-33 |
| v99_final | 5570/4795 | 5543/4723 | -27/-72 |

本次合法Top-16 oracle保护6764/5968，local6806/6050，增加42/82；没有证据将此次完整系统下降归因为Top-16覆盖减少。Parent有效性转接没有改变任何选择。@0.25的差距在Parent扩大，@0.50在Geometry由+6转成−89。未加Pareto限制的V99 proposal只是诊断中间态，不能据正式验证集结果直接删规则或改部署。

**Geometry损失进一步分解。** 在各自已经选定的同一个Query内，以原始框替代所选变体作为诊断：保护4484→4816（+332），local4485→4727（+242），因此89个严格阈值差距可算术分解为所选Query原框+1、该Query变体相对原框收益−90。原始Parent到Geometry所选Query原框的收益为+64/+59。最终V99同样是保护4388→4795（+407），local4435→4723（+288）。这说明原框的改进没有补回几何变体收益；尚不能进一步区分候选变体质量、变体选择和跨臂样本组成的因果贡献，也不能把Query槽变化说成实例变化。

**真实归一化特征变化。** 两臂Parent有效输入各152128个，Geometry为1044873/1040832。Parent152维均值差的RMS为0.28590；最大均值变化位于query_proj_42（+1.08186个既有归一化单位）。Geometry的source_mask_to_regressed_volume_ratio均值差+0.70632、RMS差+19.30311。这些是各臂实际有效候选总体的描述统计，不是相同候选上的因果对照，不能直接据此调归一化、裁剪或阈值。

**重跑并非逐行完全一致。** 相同点SHA和固定权重下，保护native有66个Query索引变化、最终89个变体位置变化；local为63/107。保护最终REC相对既定正式结果0/−2，local为0/+1；本次原生为5514/4409和5511/4428。具体漂移原因尚未定位，不能自动归因于浮点误差或将原正式回执改写。本次诊断最终−27/−72，§20.102正式仍是−27/−75；两者必须分开报告。

**证据归档。** 原始stage_rows.json为104832441 bytes，GitHub存储无损gzip 22566787 bytes，解压SHA与原件完全一致，原件留在远程和本地工作目录；完整收集与压缩回执位于`refine-logs/scanrefer_stage_diagnostic_20260907_v1/`。复查时先将stage_rows.json.gz解压为同目录stage_rows.json，再用Python+NumPy运行`scripts/audit_scanrefer_stage_diagnostic.py --directory <诊断目录> --reference <§20.102正式目录>`；输出采用排他创建，复核应在独立副本中保留既有审计文件。模型推理没有引入新依赖。

本次诊断回执SHA`78376986d4adc0817165c03ab7b09b9cafcf025f0aed1046a5680755ad9e673c`，独立审计SHA`c8ddb192d9671241bb671aa3192ca4517e241bbdedf8c0ecdd40ea5908037d3e`。新脚本只是只读诊断，不是新模型贡献。接下来从同一E71预训练起点准备固定64点的中心读取/范围读取最小对照；先在训练数据上核对区域覆盖、空间布局和梯度，再锁定ScanRefer训练预算。不得根据本次9508逐场景结果调规则，暂不同时加入质量分布、兼容损失或新Gate。范围读取需要证明本身有效；旧读出与几何变体接口仍须在终态单列检查，不能用原生提升替代完整系统门槛。此新对照尚未启动。

磁盘本次实查剩余9470029824 bytes（8.820GiB），6个受保护文件大小一致。已失败旧权重的累计清理仍约9.529GiB；本轮两臂终点暂保留作配对复核，只保留必要终点，不增加逐步权重。历史ScanRefer V99保持5572/4797，Nr/Sr未启动；三数据集目标尚未完成，goal保持active。


### 20.104 范围读取实现、真实预检通过，ScanRefer配对任务实际启动（2026-09-07 03:06 CST）

**上一轮判定为实质进展，目标仍未完成。** §20.102正式负结果及§20.103逐级诊断维持原结论。本轮新增范围读取对照，没有替换历史V99或启动Nr/Sr。

**具体新增结构。** `models/candidate_range_visual.py`在最后一层`cross_v`沿用既有局部残差接口；点输入仍为128维预训练SA1特征、RGB、相对坐标及轴缩放坐标，映射144维，4头读取后输出288维零初始化残差。两臂均145008个新增参数，共用区域内点注意力和Query条件的区域注意力。center在共同窗口内取中心最近64点再按八象限分组；extent在每个象限分别取最多8个不同点，支撑位置为各轴±0.5半尺寸，合计64槽位。共同读取窗口为各轴1.5倍半尺寸，半尺寸下限沿用0.05m。空槽有明确valid mask，零观测不贡献占位point0，也不复制点补满。保留全局视觉路径、原生损失和冻结Parent/Geometry/V99，训练限decoder.5及prediction_heads.5和局部读取参数。这是空间分组读取的最小结构对照，六面边界分布、质量头和BCT损失均未加入；不能提前声称提高精度或形成新颖性结论。

**原环境CPU3项测试及16条真实训练表达预检通过。** 输入来自既定fit中的16个固定ID；两臂共用相同点SHA，零初始化下Box、语义、Query投影、两路Mask、融合权重及完整V99运行输出均与原起点完全一致。每臂两次临时更新后，point/query/key/value读取权重梯度均非零且有限；未训练参数、buffer和旧读出未变。预检权重已随进程结束丢弃，checkpoint_writes=0、formal_rows=0。实际前向/梯度检查阶段约10.75秒，峰值4825.35MiB；从screen启动到完成约7分钟，主要时间是原生数据加载/文本解析，不能把10.75秒说成包含全部启动耗时。
...
全4096个Query的共同窗口中，两臂各有526个空Query。这是实际观察到的空域，已被valid mask处理。上述仅证明范围组读取更多区域和更大的空间范围，不是正式REC提升；GT IoU只用于预检统计，未进入采样或模型前向。

**一次启动观察超时已核实。** 首个预检启动器使用`screen -DmS`并等待其退出，SSH30秒观察超时；随后确认原screen46874/Python46877继续运行，未重启。原任务于02:48:53正常结束、controller=0，回执与coverage已收集。后续启动器使用`screen -dmS`，训练和队列均已实际返回启动回执。

**配对训练任务已启动，当前仍处于零更新基线。** 02:54:11创建screen`47112.mcln_scanrefer_range_pair_v1`，03:03:12实际观察baseline12/6887条。两臂都从受保护E71及完全相同的零输出区域读取初值开始，采用既定正确mesh1201train/312val文件、29778fit/6887模块holdout、batch12、每臂2482更新、1epoch；core LR1e−6、reader LR1e−4、AdamW decay0.0005、clip0.1。holdout仍是预训练主干见过的训练场景。尚未观察到参数更新或终态指标，不能将任务启动写成训练完成。只保存两个固定终点，不根据holdout选epoch。

**终态接续已经排队。** screen`47242.mcln_scanrefer_range_posttraining_v1`在03:01:44启动，训练/审计/正式评估/正式审计4个实际环境入口导入通过。队列首查03:04:11，之后240秒检查原screen47112；训练完整结束且独立CPU审计通过后，执行一次固定9508条的保护V99、center控制、extent候选三组评估，同时记录原生REC和完整系统REC/Mask。extent是预先指定的晋级候选，不根据validation临时挑组。晋级仍须历史5572/4797、同次保护不退化、ScanMask58.70/50.70/44.72；不等待59/51，不恢复Nr/Sr Mask门。通过后记录应立即进行新范围模块Nr/Sr原生预检；队列本身未冒充已经启动Nr/Sr训练。

训练manifest SHA`2baa2e3cfbf95d558277267ca8489dcba4afbbc9541ad87458790558be3e4f73`；计划SHA`93e95372ae71cd32961d828dc9240c4c47961f1dd3e0faad99c2fc5a3e2551c6`；预检回执SHA`66be800428c618e57ce3d571724da6390df9c5cfa6a5f23238f8b19a38995608`。运行目录分别为`/root/autodl-tmp/mcln_scanrefer_range_preflight_20260907_v1`、`mcln_scanrefer_range_pair_20260907_v1`和`mcln_scanrefer_range_posttraining_20260907_v1`。

本次磁盘实查剩余9466589184 bytes（8.816GiB）；新预检无权重文件，当前配对尚未写终点。原有受保护权重及需复核的两个旧终点继续保留，之前累计清理约9.529GiB不变。目标保持active。


### 20.105 原生范围读取接入准备完成；ScanRefer原配对继续（2026-09-07 03:29 CST）

**本轮有实质准备进展，尚无新性能结果，目标保持active。** 原screen47112及接续screen47242已在本次写入时重新确认存活，没有重复启动ScanRefer。最近一条远端240秒观察为`{"progress": {"SCANREFER RANGE EVAL": {"elapsed_seconds": 1292.2311198711395, "rows": 4608, "stage": "baseline", "total": 6887}}, "screen_live": true, "screen_pid": 47112, "time_cst": "2026-09-07T03:28:12.222665+08:00"}`。零更新基线与更新训练、模块holdout与正式9508评估继续分开报告；现有正式最好没有替换。

**新增原生入口。** `--use_candidate_local_visual --candidate_local_visual_variant extent`显式构造与当前ScanRefer候选相同的`CandidateRangeVisual(extent)`；可选center作为对应控制，默认local保留已有局部读取实现。原有独立reader优化器分组继续使用，无额外优化器分支或新损失。`save_checkpoint`原有config字段记录该参数；恢复含reader权重的checkpoint时核对其计算类型。这是必要的实际接口约束：三种reader有完全相同的10个参数名称/形状，形状检查不能区分三种计算。既存local checkpoint没有该新增字段时保持其已知local语义；无reader的旧完整主干可用于新分支的model-only初始化。源码更新发生在独立原生快照和项目仓库，运行中的ScanRefer冻结源码及manifest没有改动。

**原服务器CPU检查通过。** 62项检查覆盖读取器的有效点/空区域、已有模型加载与优化器分组、同形状异类型恢复拒绝以及正确类型的优化器/调度器恢复。另外实际构造Nr3D和Sr3D的MCLN，均加载已锁定Nr3D权重：1144个原有state tensor逐项完全一致，新增10个reader tensor保持零输出初值，五个优化器分组无重复/缺失、optimizer/scheduler保持全新。两个协议均有1154个state tensor，reader145008参数。使用原Py3.7/Torch1.10.2环境、CUDA_VISIBLE_DEVICES为空，真实GPU前向=0，真实数据采样=0，原生模型更新=0，持久checkpoint写入=0。Sr3D历史受保护权重仍未恢复；Nr权重在Sr配置上的可加载性不是Sr性能或直接迁移结果。

**首轮CPU测试的失败口径。** preparation_v1的62项通过、1项失败；失败是新增toy模型只有2个backbone tensor，不满足既有model-only加载器至少90个backbone tensor的真实约束。保留该失败日志，移除不代表完整主干的重复toy正例，由上述实际1144-tensor Nr/Sr模型加载检查承担正例验证；生产加载约束未放宽。v2才是本次通过回执。

**数据与后续接续。** 本次重新核对正确mesh目录的1201train/312val文件SHA以及其他数据项inode；没有重报旧64个输入样本为本次新采样。原始32条预检选行与标注凭证保留其旧原生source SHA，新GPU预检显式绑定新原生source与标注来源；涉及输入的数据集代码沿用原文件。`run_native_candidate_range_preflight.py`和`launch_native_candidate_range_preflight.py`已准备，后者只有在当前固定范围正式评估及独立审计完整通过后，才允许用extent在Nr/Sr各16条真实训练输入做2次临时更新、验证原生过滤/选择与梯度。该启动器本轮没有执行，GPU预检和Nr/Sr正式训练均未启动；当前远端队列仍只负责原定ScanRefer终态与正式审计，未在运行中替换。

原生准备回执SHA`1202bee98539c35dc44861e9f8ea12ae587d83338bbf43779897fffca6977454`；原生618文件source manifest SHA`270843db4eeb20ac71966c06808134d4d494deba5e9aca567ea43bf03a04c42f`。目录`/root/autodl-tmp/mcln_native_range_preparation_20260907_v2`；最近磁盘剩余9374310400 bytes（8.731GiB）。两次CPU准备均无训练权重；之前累计清理约9.529GiB未计作本轮新增释放。现有空间足够本轮两个固定终点，未删除受保护或正在使用的权重。


### 20.106 范围配对零更新基线完整一致，实际进入训练（2026-09-07 03:39 CST）

03:36:41实际核验原screen47112仍存活，日志已完成6887条baseline并推进到每臂64/2482更新。零更新评估耗时1929.63秒，两臂保存的逐条row dict完全一致；本轮收集并独立复核6887个固定row ID、106个模块holdout物理场景及计数。REC6684/6426，Mask6511/6097，mIoU77.8108607870114%，与此前正确mesh起点相同。该检查是保存行与指标计数复核，没有重新根据框/GT计算IoU，不能冒充完整终态审计；formal_rows=0。

64次实际配对更新耗时184.90秒，约2.889秒/配对更新；control/local当前native loss分别10.2222729/10.2223883，裁剪前梯度范数1.14780/1.17615，均有限。它只证明固定GT训练路径正在更新，不是验证集改善。按首64步估算剩余训练约6985.82秒（116.4分钟），约05:30左右结束fit、写两个固定终点；随后另需约32分钟终态模块评估，正式三组9508评估与独立审计尚在其后。首次时间估算会随吞吐变化，不承诺固定完成时刻。没有中途改epoch、学习率、候选、权重或晋级规则。
...
### 20.108 范围读取来源核对；保持原训练配对（2026-09-07 04:13 CST）

等待既定训练期间，核对并锁定PV-RCNN/OpenPCDet及Box-DETR官方源码的两个commit、7个文件SHA与行定位。新增说明`docs/MCLN_RANGE_READER_SOURCE_AUDIT_2026-09-07.md`，未复制第三方完整源码，没有新增训练或改动正在运行的模型、预算及晋级门。

**方法边界修正。** RoI网格和空邻域识别均有现成上游实现，不能作为本轮独立创新。本轮八个支撑位置在5厘米半尺寸下限未生效时，等于2×2×2 RoI网格中心；不是六个边界面。两臂都使用同一分区读出，只比较中心近邻分配与每象限最多8个点的范围分配，64是槽位上限、并非相同有效点数。语言通过已有多模态Query间接影响读取，当前没有新增完整token接口或身份—范围解耦监督。Box-DETR使用动态的每头二维代理点，因此本轮也不等于其完整移植。上述为代码关系与代数推断，不是性能结果或完整查新结论。

**实际运行继续。** 04:12:09实查screen47112、47242、48128均存活；队列最近04:08:13记录每臂704/2482更新，elapsed2028.49秒，估计fit剩余5123.09秒。仍预计约05:30结束fit，随后完成终态模块评估和固定正式评估。原生Nr/Sr GPU预检尚未启动；只由既定48128在Scan通过后接续，不能重复启动。当前没有新增正式分数，受保护结果保持不变，目标继续active。

本次实际磁盘剩余9370193920 bytes（8.727GiB）；来源说明不产生权重，之前累计清理约9.529GiB未计作本轮新增。来源回执SHA`6b2234bf1c7df36d1afe7687c0ebc7871259de946d5d98dc372789b0e8664e2e`。


### 20.109 范围配对完成2482更新并保存固定终点；终态评估进行中（2026-09-07 05:37 CST）

05:34:42直接检查原screen47112及64KB日志尾部：两臂已完成2482/2482次更新，训练更新耗时7122.62秒；随后已进入6887条terminal模块留出评估。当时只看到首12行，不能将baseline的6684/6426或77.81%写成终态结果。本次文档写入前重新确认47112/47242/48128实际进程均存活，最新terminal日志为12/6887行，尚无完整terminal receipt或正式9508结果。

**固定终点已实际保存并复核。** control(center)与local(extent)各620352201 bytes，分别SHA`e97ff85721ae82c2f1a3a2dbe5627ad848e22a130050d070f0c1ff01d2f7ce8a`、`1d2034e5f819141a04c181d51a6af97e4e2990c6eb35651e355656f63de9800c`。两个文件位于原配对目录，保留完整model、原readout及optimizer，未传回本地或提交Git。本轮通过远端sha256sum独立核对内容摘要和实际文件大小；同时读取完整2482个批次记录，确认29778个固定fit row ID各出现一次、与holdout无交集、前2481批各12行、最后一批6行。此项检查未重新解释模型张量语义或计算预测IoU，完整终态独立审计仍由原队列接续。

**接续与结果边界不变。** 模块holdout仍是预训练主干见过的训练场景；终态评估完成后执行原定CPU审计和固定三组正式评估，再按历史REC5572/4797、同次保护不退化及ScanMask底线判定。本轮没有新增训练、延长步数、选择中途epoch或改动晋级标准。Nr/Sr原生GPU预检继续由48128等待正式通过后触发，当前尚未开始，整体目标仍active。

写出两个终点后，磁盘实查剩余8127053824 bytes（7.569GiB），本轮未再删除权重。fit_complete SHA`b496393feb13152715a8f69ac2b71197e78286b8e6df58dd65fee1f3729f2a3c`；fit批次记录SHA`15d99c6cc81310d453876fae619ed0e1b995dd76b18384c4032acc3534b90db7`；独立终点检查回执SHA`98d055ec5d7a07ecba52a2abd1fe81f57fa7351f285f1ea6d44d8a70929a371a`。


### 20.110 范围配对完整终态负结果；独立审计通过并启动固定正式三组评估（2026-09-07 06:19 CST）

原47112训练任务已正常结束，controller.exit=0。两臂各完成2482次更新和完整6887条terminal评估；独立CPU审计确认fit遍历及配对点身份一致、冻结核心参数/缓冲区和Parent/Geometry/V99权重及元数据未变，每臂76个有优化器状态的参数张量均为2482步。两个固定终点SHA与§20.109一致，未重新训练或更换epoch。
...
**本版本未通过模块REC双阈值不退化。** extent相对起点为−12/−13：@0.25修复16、破坏28；@0.50修复61、破坏74。extent相对center为−2/−3：修复/破坏分别6/8、16/19。按106个physical space聚类的2000次bootstrap，extent−center的95%区间为@0.25 [−0.1670,+0.1074]pp、@0.50 [−0.2766,+0.1924]pp，均跨0；区间只作诊断，不能把未过线改写为等效或晋级。此集合仍是主干见过的训练场景；不是9508条正式指标，也没有可靠的选中实例身份标签，不能从IoU变化直接宣布选错实例或仅框边界变差。结果未支持本次固定八区域、总64槽位读取优于匹配中心控制，不能外推为所有范围读取方法无效。

**固定正式评估已实际接续。** 独立完整性审计PASS后，原47242队列于06:08:32 CST启动flock PID50454，实际Python子进程50455；正式输入manifest绑定训练receipt、audit及两个终点。三组仍为protected_v99、center_v99、local_v99(extent)，唯一预定晋级候选仍是local_v99，不根据模块结果临时换center。原协议允许完整性通过后执行一次固定正式比较，模块负结果如实保留。06:16:44实查已输出首12/9508行，正式完整result/receipt尚未产生；本次文档写入前再查正式父子进程及两个接续队列均存活。正式脚本对同一批点同步保存原生与系统REC，结束后原队列独立审计修复/破坏、IoU区间转移和系统相对原生的收益变化。

Nr/Sr接续48128仍等待固定正式结果；只在ScanRefer历史REC5572/4797及同次保护不退化、ScanMask三底线均通过后运行原生GPU预检。目前Nr/Sr训练未开始，实际训练预算尚未锁定；不把已准备的CPU代码或条件队列当作训练成绩。范围读取尚无有效正证据，当前不叠加边界分布、质量头或兼容Loss，等待唯一正式结果。

06:16:44磁盘剩余8121761792 bytes（7.564GiB）；本轮未删当前终点或任何受保护权重。训练receipt SHA`8b970acd782a48b65db814e87ef94018729f810ff493f0503b680b453f5df660`；独立审计SHA`66bce72d4ad6ae6be31fa6c56fbf76081c81c3137595ddf228f6b6d3a2606c2f`；正式manifest SHA`2f19968db34b4b876d91b20460cbeb1efb2d816ab35c700281870cfd6fd8bd97`。


### 20.111 范围读取固定正式9508终态：完整性通过、REC未晋级，条件接续正常关闭（2026-09-07 06:57 CST）

本次正式评估于06:50:41完成，耗时2412.15秒；独立审计于06:50:50通过，正式及posttraining controller.exit均为0。614份固定来源文件、312份正确mesh验证superpoint、四份保护模型/读出及两份训练终点的实际SHA均经审计复核。本地另收取完整rows/native_rows/protocol/receipt，并重算9508行三组REC/Mask、行身份和采样点SHA，确认与审计一致；没有新GPU推理。

| 正式9508，同批输入 | 系统REC hits@0.25/@0.50 | 系统REC百分比 | 原生REC hits@0.25/@0.50 | Mask hits@0.25/@0.50 | Mask mIoU |
|---|---:|---:|---:|---:|---:|
| 同次受保护V99 | 5568/4794 | 58.561212% / 50.420698% | 5515/4411 | 5689/4974 | 45.92263519% |
| center机制控制 | 5555/4740 | 58.424485% / 49.852756% | 5510/4423 | 5692/4977 | 45.91705361% |
| extent预定候选 | 5543/4740 | 58.298275% / 49.852756% | 5502/4419 | 5690/4969 | 45.89673209% |

**判定：本版本REC失败封存，历史最好不变。** extent相对同次保护为−25/−54；@0.25修复41、破坏66，@0.50修复84、破坏138。相对历史V995572/4797为−29/−57。ScanMask三项仍超过原论文58.70/50.70/44.72底线，但REC四项历史/配对检查均未通过。同次保护5568/4794相对历史少4/3，属于本次控制复核差异；未重算或降低历史保护线，也没有把差异记成方法贡献。

**机制结论进一步明确。** 原生extent相对保护为−13/+8，而系统为−25/−54；系统相对各自原生的@0.50净收益由保护的+383变为extent的+321，减少62个命中。中心控制同样出现原生−5/+12、系统−13/−54，说明此现象并非extent分组独有。extent相对center为原生−8/−4、系统−12/0，未支持此次范围分组采样。系统@0.25 extent−center按141个physical space聚类的95%区间[−0.2185,−0.0301]pp；这些单seed区间只作诊断，不能代替多seed或逐阶段因果干预。系统仍给两个新核心带来正收益，并非全部失效；现有记录不能独立证明旧读出不兼容是唯一原因，也不能把IoU降低全归为选错实例。

**接续已真实结束。** 原生条件队列48128于06:53:01写出scanrefer_not_promoted，native_preflight_started=false、nr3d_sr3d_training_started=false、controller.exit=0。本次同步前实际检查训练、formal及两个队列PID均已不存在，并与完整退出回执对应；这是正常完成后的不晋级，不是运行阻塞或需要重启。未更换center为候选、没有补跑epoch或调整正式阈值。

**下一步收缩为部署兼容训练的独立问题。** 此次center/extent和旧local均不能同时保护系统REC，不在失败range终点继续加边界分布、质量头或新Gate。回到受保护E71和真实冻结Parent/Geometry/V99，先核验GT读出损失在冻结读出时是否向原生候选参数提供有效梯度，再决定固定预算的native-only与冻结读出GT耦合对照。它与旧JointRecReadout trial的明确区别是旧试验两个分支都更新读出，本方向固定旧读出解释规则；不新增range分支、不模仿旧错误答案、不绑定缓存Query编号。这只是下一项待验证工程机制，尚未启动新训练，也不声称取消后处理或形成网络创新。必须先完成实际梯度/候选一致性检查，再锁定训练输入、步数和唯一候选；通过ScanRefer保护线后仍立即推进Nr/Sr REC，不恢复已取消的长期baseline训练。

补充训练记录边界：40个日志批次中的extent−center瞬时loss均值−0.00678、median+0.000117，每臂10个新增参数张量实际改变；这仅排除分支从未更新，不能证明收敛、有效表征或加长训练会提升。完整样本loss均值未记录。

当前磁盘实查剩余8109797376 bytes（7.553GiB），本次未删除权重。正式receipt SHA`691ec0aa9df40f938d21999701504273350c526b1b2acb57eed9db0877c3502b`；独立audit SHA`fa4e8eb87740bf4f5b6a14c80247b26239cda41f73b176ab4b4d3ee3eda71bfe`；本地收取复核SHA`b5813956b82f4add6f14bbfa771c886819a4e5bf3ba902859e4a8a42f49f5685`。


### 20.112 冻结旧读出实际梯度检查通过；退休两份失败范围权重（2026-09-07 07:24 CST）

依照§20.111从受保护E71重新构造原网络，没有加载失败范围权重或安装范围模块。原bdetr环境的实际GPU检查于07:07:40启动screen51683，07:14:44完成，controller.exit=0；本次同步前确认screen和Python51686均已正常退出。共16条固定fit表达、两个批次12+4，原有完整场景表达保留以构造干扰物；1201份正确mesh训练superpoint经SHA核对。实际执行6次backbone forward和两组各2次一次性optimizer更新，零正式验证行、零新模块、零权重写出。9.572秒仅是数据准备之后的检查耗时，整个进程约7分钟。
...
冻结Parent/Geometry/V99的连续输出确实向当前原生候选参数传递GT监督；与detach输入的forward逐张量一致，detach后的辅助损失没有梯度。两组一次性更新后，三个读出器参数及归一化元数据均逐项保持不变，非训练核心参数及buffer亦不变；每次辅助标签均由当前候选框与训练root GT重新计算，不绑定旧缓存Query编号。这只是工程路径PASS，16条小样本不能证明全数据任务冲突、兼容性能或泛化。原生与辅助梯度在这两个批次夹角均为正，也不能预先声称本方法解决了梯度冲突。

**下一训练对照尚未启动。** 已在本地开始准备独立的native-only与frozen-readout-GT配对入口；同一E71、相同数据及更新步数，三个旧读出始终冻结。拟沿用原固定1epoch、29778条fit/6887条模块holdout、每臂2482步的短周期预算，核心LR1e-6、辅助权重1/3，更新范围限最后Decoder及预测头。正式启动前仍需完成该入口、终态独立审计和唯一正式候选的接续约定；不能把当前probe的PASS当作完成训练。辅助梯度乘1/3后在这两个批次仍约为原生的2.7—3.3倍，此事实应保留解释，当前未因此临时扫描权重。此方向保留原候选构造、几何变体及Pareto决策，属于固定旧读出的训练适配，不声称取消后处理或产生新架构创新。
...
**08:02:54实际执行补记。** 两个原screen持续正常，训练进程已结束文本预处理并完成首批12条baseline GPU评估，日志记录首批elapsed5.225秒，当前GPU7715MiB；不以首批速度推断整轮耗时。实际protocol确认1201份正确mesh训练superpoint、29778/6887固定行划分和68核心/0读出可训练张量。当前仍为零更新起点评估，不是训练终态；后台240秒队列已写出首次真实观察。磁盘剩余9349550080 bytes（约8.707GiB）。参照此前相近joint读出配对，2482更新训练阶段耗时10376.40秒（约2.9小时），这只是排程参考，本轮实际剩余时间待训练日志估计。本次没有新增启动、改动运行源码或训练参数。


### 20.114 等待ScanRefer期间完成原生权重接续检查；未占用GPU或启动Nr/Sr训练（2026-09-07 08:17 CST）

当前52529训练及52535接续持续正常；08:11:43最近日志已记录baseline1536/6887行、elapsed410.66秒，证明首批之后继续推进，尚无完整baseline指标或参数更新记录。按该阶段吞吐粗估零更新评估约08:32结束，实际以日志为准；不依据日志输出间隔认定卡住或重启。本次同步前再次确认两个原screen存活。
...
两个原生配置CPU加载回执SHA`e811f9ab5e79e4d63504aa92f7e9e5a3b1bdb6c2fccb952ca29232cd7f442a21`；权重/标注schema审计SHA`1acead96e1bee030c29ecd07b8f179d42f79b359343d833628996fa90f9f41df`。当前正式最好不变，Nr/Sr训练预算尚未因本次CPU检查而启动或锁定；仍由Scan正式保护线通过后推进。08:11:43磁盘剩余9349513216 bytes，约8.708GiB，本次未新增或删除权重。整体目标继续active。


### 20.115 冻结读出配对完成一致起点并进入实际更新（2026-09-07 08:37 CST）

本轮起点评估完整覆盖6887行，耗时1835.916秒；后台08:30:21观察已确认评估完成且两臂完成首个optimizer更新。没有更换配置或重新启动进程。
...
**实际训练继续。** 2026-09-07T08:36:16.618807+08:00观察，两臂各64/2482步，训练elapsed262.51秒，日志估计剩余9917.82秒；当前GPU为`0, 7835, 0`。该估计从最新已记录批次计算，存在日志间隔延迟，不是完成承诺。原52529和52535仍存活；训练达到固定预算后先保存两个终点，再做完整终态评估与原定独立审计。只有模块REC双阈值相对起点及native_only均不退化，才接续唯一固定9508正式评估。Nr/Sr未启动，目标继续active。

磁盘本次观察剩余9344630784 bytes（约8.703GiB），尚未写出训练终点，也没有新删除权重。baseline rows SHA`72cdac90a9be30aea83940519b2115be12460077e4a958cac8368ed593f29132`；跨运行审计SHA`548a8fb51c92a5cad4e13fd91500f986a8ea5a380b79bfc0cc2c80692094ba73`。本次完整起点文件和运行观察已收取并同步，历史正式最好保持不变。


### 20.116 冻结读出配对完成固定训练，终点已落盘（2026-09-07 11:26 CST）

原52529训练及52535接续队列持续运行，没有重启或改变设置。11:23:17 CST实际观察，两臂均完成2482/2482次更新，并已进入terminal评估；最新已打印记录为12/6887行，仅证明终点评估已启动，不能据此报告完整质量。实际训练更新耗时10171.826秒，含终点保存的fit记录为10178.659秒。

11:23:45的独立CPU收取检查逐批验证2482个记录、29778条fit表达各出现一次、末批6行、与6887条holdout无交集，并在服务器重新计算两份终点SHA256、核对大小与fit记录。没有下载权重到本地，没有增加GPU推理或训练。该检查尚不包括终点张量、冻结读出及optimizer状态的完整独立审计；这些按原队列在terminal结束后执行。

| 固定终点 | 文件大小 bytes | SHA256 |
|---|---:|---|
| native_only | 618602215 | `70d4a1494c7cddef83dab9b8f8cc9a2a3644cadc4024be5ba61e4a9ee6b99998` |
| frozen_gt | 618602215 | `fcf915d3fc9883865fd43aa5943f00701899319781472dd2e21b17423e10a70c` |
...
### 20.125 原生框转移起点逐行一致；固定两臂进入训练（2026-09-07T13:52:33.951206+08:00）

6887条零更新评估实际耗时1839.52秒，两臂所有行完全相同。CPU复核重新计算完整系统REC6684/6426、原生REC6572/5955，Mask6511/6097、mIoU77.810860787%。相对上一轮冻结读出配对的相同E71起点，6887条样本身份、点SHA、原生Query、原生/系统IoU、Mask IoU和所选几何变体均无一处差异；同一保护artifact、冻结来源、mesh目录和split已核对。这是输入与起点一致性，不是新方法增益或未见场景指标。

本轮观察2026-09-07T13:51:35.348439+08:00：原训练Python58023和接续Python58296均实际存活，每臂已记录64/2482更新，训练elapsed210.68秒，平均3.292秒/两臂同批更新。首步与当前记录梯度有限，周期检查确认冻结语义响应、对比表示及原始Mask未变，固定训练继续。按当前累计吞吐保守估计更新于16:04附近完成，再约31分钟终态评估，即16:34附近进入CPU审计；这是时间估计，不按到点判断完成。

磁盘剩余9298849792 bytes（约8.660GiB）；当前未生成终点或正式结果。保持仅16项回归头小终点设计。接续队列已按240秒实际观察原进程，完整训练后沿原已锁定流程执行审计及条件正式评估；不据训练loss提前选候选、调参或改epoch。Scan正式通过现行REC/Mask底线才接Nr/Sr REC，三数据集整体目标未完成。

起点rows SHA `b47a68e2a1841040ca6e1fd96dc0dfaca770777384d3577fa57bbd0afe9508e1`；CPU核对结果SHA `f25052ecd254f264a0f85d879e82726f3b93abac8ffbb67c311ab615abf1fb2e`。队列及监督对应分析已发布GitHub main `e97926c2ed658bc89200f52ab7decb37d6249578`；训练仍使用原启动manifest和source commit，未热修改。


### 20.126 小框头终点的原生初始化接续准备（2026-09-07T14:07:58.584371+08:00）

实际接口差异已由源码确认：当前终点只有16项框头参数，原生加载器需要带module.前缀的完整model。新增独立导出工具，在正式Scan通过后使用E71补全1144项state，仅覆盖16项回归参数；头内12项BatchNorm缓冲继续保持E71。复用现有model_only_initialization，不修改主训练加载器、不继承旧optimizer/scheduler。工具重新核对原正式行记录、晋级决定、来源和权重SHA，再写唯一初始化文件；不是现在运行新数据集训练。

原Python3.7/Torch1.10.2环境8项单元测试通过。使用真实E71和明确标注的合成参数差值，两个原生数据集模型工厂及加载器均实际执行：Nr/Sr各1144项逐值匹配重建模型，16项框参数正确覆盖，其余1128项保留；优化器状态0、调度器新建状态不变、start_epoch1。源618文件核对一致。该检查GPU前向0、真实训练步数0、正式行0，不是对未来训练终点的效果验证。

CPU检查先遇到暂存scripts包遮蔽，再遇到chdir后相对__file__目录失效；均发生在fixture写入前，仅修正检查脚本的源码包路径与绝对目录，原错误日志保留。最终临时完整模型599063649 bytes，按路径和SHA核对后已删除，新测试权重留存0。当前固定训练/接续队列的源码、manifest和设置没有改变。

正式通过后的真实终点导出、Nr/Sr实际加载、真实训练数据梯度与固定训练配置仍待接续；完整V99过线不自动证明原生网络达到V99。文档`docs/NATIVE_BOX_INITIALIZATION_2026-09-07.md`给出执行入口和边界。本轮CPU receipt SHA `d8f34b0c19d9dff7b44dcec66f4d75d8c12ee5522c7ef2ae19142b339635d5e5`；导出脚本SHA `9b76a5067ab4632028038b4f7886ec5e2fb3ff555d29f6ebb2329a372b013465`。三数据集目标仍未完成，未触发新正式评估。


### 20.127 清理已完成复核的正确mesh局部视觉终点（2026-09-07T14:32:02.301611+08:00）

正确mesh局部视觉配对、正式评估及逐级诊断均已退出0，正式REC未晋级，相关结果见§20.102—20.103。本次只删除该配对仍保留的两份完整终点；它们不属于当前原生教师框转移的输入，当前训练/队列manifest删除前后SHA一致，未重启或修改运行。

| 删除的失败终点 | 文件bytes | SHA-256 |
|---|---:|---|
| control | 618602343 | `7fcff034b713e1d05a319c51dc53c54887897e5bd9da634980b587604676b7a8` |
| local | 620352201 | `5a5bdf0a7f1d13b68a104e31f3a0fcf63d644ceca0b6b697b9eae2140d737294` |

删除限定于`/root/autodl-tmp/mcln_scanrefer_local_visual_mesh_pair_20260906_v1/`中的两个已核对文件；均只有一个hard link。实际释放1238966272 allocated bytes，约1.154GiB；14:30观察可用10537558016 bytes，约9.814GiB。E71、Parent、Geometry、V99、Nr平均E57及Nr续训E57六份受保护权重SHA删除前后逐项相同。训练/正式/逐级诊断的行级结果、审计、manifest和源码保留，核对11份证据SHA未变；Sr历史权重缺失与本次清理无关。

清理后14:30:11实查原训练58023、接续58296存活；两臂已记录768/2482更新，elapsed2535.59秒，无新终态或正式结果。继续原固定实验，终态/条件正式接续已自动排队；本节是磁盘维护，不是性能增益，也不更改Scan通过后接Nr/Sr的要求。receipt SHA `60646d13ea9e5e60e9dd40c277b0847a3de0cd2501e439421190ef41bce8ea44`，证据目录`refine-logs/weight_cleanup_20260907_v3/`。


### 20.128 原生教师框转移终态封存（2026-09-07T17:13:46.990118+08:00）

两臂实际各2482更新，16:38:50终态、16:39:03独立审计、16:40:04队列结束，均exit0。6887条主干曾见模块留出：起点系统6684/6426、原生6572/5955；GT-only系统6679/6440、原生6557/5979；教师框候选系统6678/6441、原生6556/5976。Mask仍6511/6097、mIoU77.810860787%。候选系统相对起点−6/+15、对GT-only−1/+1；原生分别−16/+21、−1/−3。固定REC筛选失败，正式0、Nr/Sr训练0，不替换候选、不扫描辅助权重/LR/epoch。只改16项回归参数，1144项完整重建及冻结输出审计通过；两份终点各4045205 bytes。

训练receipt SHA `a3b91dd91fe7a72106c468c9b928c2f8789661799ab40ad765156ad85996b075`；终态rows SHA `0e318fa605e795f19b960eb203c7d5634b0127d528e1d620d526fa012a9810aa`；独立审计SHA `cc62d9fe2ac64c349880f97a0538828b0d2800ba3fe176ad99efa612ad05a68a`。本地再次从逐行结果重算系统/原生/Mask一致。大批次输入清单原件保留远端与本地，Git只存SHA指针。已准备的小框头原生导出仅限未来正式晋级终点，当前失败候选不导出。

### 20.129 历史局部视觉几何归因：变体收益主要损失在修复减少（2026-09-07T17:13:46.990118+08:00）

CPU复用原9508-row逐级记录及141个真实场景的GT对象几何，重算2臂×8阶段共152128项。本地独立算术与映射核验、6项测试通过，新增GPU/优化/正式forward均0。最大GT重叠只是几何代理，嵌套对象与结构类也参与，不能据此证明真实语义身份。
...
### 20.137 Mask几何配对完整起点重算与跨运行一致性（2026-09-07T19:34:02.115482+08:00）

起点6887条模块留出已实际完成，两臂全部逐行记录完全一致。CPU独立重算完整V99为6684/6426，原生REC为6572/5955；Mask为6511/6097，mIoU 77.81086079%。与此前教师框转移实验的同E71/mesh起点相比，6887条行身份、场景、物理空间和输入点SHA差异0；原生Query索引、原生IoU、系统IoU、Mask IoU及实际几何变体位置差异也全部0。不是新模型增益，正式9508尚未执行。

本次新增GT匹配root Query几何记录：有效硬框6784/6887；软范围过线[6471, 5923]，实际硬范围过线[6568, 6046]；平均IoU软0.73624081、硬0.74274870（硬无效按0）。从保存的中心/尺寸独立重算软/硬IoU、中心尺寸L1、GIoU以及平均辅助loss，全部在预先沿用的数值容差内通过。这里的候选由训练GT匹配取得，不是实际部署REC，也不等同完整候选oracle。
...
### 20.140 Mask几何GT固定配对终态与软硬输出分解（2026-09-07T22:25:57.588002+08:00）
...
### 20.147 原生增强对象槽绑定检查通过；Scan外观配对1024步持续（2026-09-08T02:11:15.655793+08:00）

针对20.145的增强接口问题，新增独立CPU审计，使用此前固定32条Nr/Sr preflight记录及10个检测槽不连续场景，共42个case/37个不同扫描；每case固定seed0—3，实际168次调用原_get_pc及_get_scene_objects。本轮作业在02:07:35启动，02:09:19已观察audit.exit=0、receipt PASS；02:09:50本地独立重算PASS。48次Nr、48次Sr、72次检测检查，共4652槽次、3360可用槽次；所有原始slot_ids在增强后保持，点/框确实变化，原orig_pc和color保持不变。24次视角受限检查Z角在±5度内且无大角度翻转；其他原生小角度、噪声、缩放、RGB和box jitter未关闭。

新增src/object_appearance_inputs.py只处理已观测的非连续槽问题：compact feature按真实slot_ids散射，要求与当前valid mask索引严格一致；不足384点的槽与padding输出0。审计用人工槽编号标记验证放置位置，未使用真实OpenShape特征；不能将168次通过称为外观旋转不变、完整__getitem__或模型梯度通过。运行源码仍是冻结625文件加独立审计目录中的binding工具，现有Scan训练没有导入或修改这项helper。

后续Nr/Sr采用明确的两种观测约定：冻结外观从canonical原始XYZ/RGB和协议实例框编码，几何分支保留原增强；两者通过同一实例槽绑定。canonical点/框SHA在增强前核验，不能要求变换后的框与缓存坐标相等；不宣称G14特征旋转不变。验证期同样读取canonical外观。此选择利用已有预训练以减少编码成本，同时保留原训练增强；其有效性仍需真实embedding/full-loader/model零初始化一致性及梯度检查、最终REC实验，当前尚未启动Nr/Sr训练，也未增加推理GT目标或实例mask清洗。
...
### 20.150 Nr/Sr磁盘外观缓存封装及标准输入映射通过CPU检查（2026-09-08T03:06:14.672650+08:00）

本轮补齐了真实存在的输入缺口：TrainTester._get_inputs此前不传外观字段，现仅在原可选字段列表新增det_visual_features、det_visual_available两行。新增ReferItObjectAppearanceDataset封装原dataset，初始化时校验预期receipt、scene index、逐NPZ SHA、canonical点、原生框、预测类别和原始slot_ids，再一次加载所需场景特征。canonical验证使用浅拷贝dataset/scan并关闭拷贝的增强，实际native dataset及其原增强保持；取样调用原__getitem__后按原slot绑定。远端规范train_dist_mod.py在修改前与本地上一HEAD内容一致，未覆盖其他改动。

03:00:45启动独立CPU PID73435，03:04:18收集exit0。测试在独立目录生成明确标记synthetic_slot_marker_fixture的NPZ，使用真实canonical场景框和预测类别、人工槽编号特征，随后由新封装实际读盘验证。128组plain/wrapped比较、256次原生取样覆盖seed0/1及增强开/关；原37字段与随机状态一致。8个collated batch通过从修改后训练源码精确提取的_get_inputs方法体，新两字段原样到达、原有输入不变。这里执行的是实际纯映射方法体，不是整套TrainTester训练或MCLN前向；不得把人工文件当真实G14缓存。
...
### 20.153 ScanRefer预训练对象外观配对终态：完整留出评估失败，独立完整性审计通过（2026-09-08T04:08:55.007932+08:00）

两臂各2482步、29778条训练表达的固定拟合及6887条完整系统留出评估均已结束。receipt完成时间2026-09-08 03:52:32 CST；03:58:43实时核验controller.exit=0、audit_queue.exit=0，独立audit的integrity_pass=true、module_rec_screen_pass=false。6887条为新增模块留出、旧主干以前见过的训练场景，formal_rows=0，以下百分比不是9508条正式ScanRefer结果。

| 同一6887条模块留出 | REC hits@0.25/@0.50 | REC百分比@0.25/@0.50 | Mask hits@0.25/@0.50 | Mask mIoU |
|---|---:|---:|---:|---:|
| 受保护起点完整V99 | 6684/6426 | 97.0524/93.3062 | 6511/6097 | 77.81086079 |
| 同预算原生GT控制终点 | 6687/6431 | 97.0960/93.3788 | 6491/6083 | 77.70155849 |
| 预训练对象外观终点 | 6665/6342 | 96.7765/92.0865 | 6511/6057 | 77.36449878 |

外观相对起点REC净-19/-84，相对同预算控制净-22/-89。相对控制@0.25修复38、破坏60，@0.50修复80、破坏169；相对起点分别45/64、108/192。训练器及独立审计确认规定冻结状态和Parent/Geometry/V99未改变。本版本未满足预先固定的双REC不退化筛选，不能替换保护系统，也不能升级为正式9508推理或Nr/Sr训练。不能把这次已完成的负结果解释成训练未结束，也不扩大成所有预训练外观或所有点云主干无效。

03:58观测原生REC补诊断队列已接续，正在加载三臂模型；它使用相同6887条输入，分别测initial/control/appearance的实际原生evaluator输出，用于区分原生能力变化与冻结完整读出的变化，不是新的晋级门。正式Scan、ReferIt缓存、真实Nr/Sr模型probe队列仍等待前置退出；由于已固定的完整系统筛选失败，它们应按原条件拒绝，不生成新正式成绩。未修改运行协议、未重训或扫描损失权重/学习率。
...
### 20.155 原生与完整系统同输入逐行对照已准备并接续（2026-09-08T04:18:09.227198+08:00）

在恢复后的原生评估运行期间，新增只读analyze_scanrefer_appearance_readout_transfer.py。它必须读取完整native receipt、独立audit与baseline/terminal逐行文件并验证SHA；逐臂核对row_id、scan_id、physical_space及point_sha256，禁止按动态Query编号关联。完整系统旧记录只有所选variant位置与IoU，没有各级所有框，因此该分析仅做原生到完整系统的算术分解，不声称定位到Parent或Geometry的因果责任。

输出每臂原生/完整系统REC命中、完整系统相对原生的修复/破坏；再分解appearance相对initial/control的变化：完整系统净变化=原生净变化+系统增益变化。保存四种正确性状态的16格计数，以及系统破坏中原生仍正确、仍过0.25的数量；每条表达的CSV用于复核，不用于阈值或场景规则调参。

本地和实际bdetr CPU均通过独立4行决策算例、16格位序、算术分解和错误行顺序拒绝检查。真实6887分析尚未运行；screen接续脚本以240s间隔等待当前native_queue.exit，成功才执行，同一原生诊断未修改或重新启动。远端准备目录/root/autodl-tmp/mcln_scanrefer_appearance_readout_transfer_preparation_20260908_v1，代码与检查/接续记录归档refine-logs/scanrefer_appearance_readout_transfer_preparation_20260908_v1。仅CPU读既有输出，不增加GPU推理、训练或新权重，不改变已失败的模块晋级结论。

当前原生进程1129在04:14核验存活；04:16追加内存诊断时cgroup限额约77.31GB、当前约20.49GB、oom/oom_kill均0，仅能说明当前无OOM记录，不能据此解释此前进程退出。尚未有第一批原生REC进度，继续按实际进程及输出判断，不把旧ETA当完成证据。完整三数据集目标保持未完成。


### 20.156 已封存旧增量权重清理与预训练资源核验（2026-09-08T04:33:55.159375+08:00）

按用户“没用的权重删掉”的既有授权，核验两项已封存失败实验：native_box_transfer与mask_geometry的controller均退出0，独立审计status=pass且decision=seal_fixed_configuration、formal资格false，逐行/fit文件SHA与receipt匹配，无对应活跃进程。只删除下列四个精确路径文件，未递归删除目录：
- /root/autodl-tmp/mcln_scanrefer_mask_geometry_pair_20260907_v1/native_gt_core_delta_state.pt，42175633B，SHA 2b02de432c88ce098ef87fa03da4b66aa4d78ee4c9a27664f1f584a4b690c565
- /root/autodl-tmp/mcln_scanrefer_mask_geometry_pair_20260907_v1/native_gt_mask_geometry_core_delta_state.pt，42175697B，SHA a5ed787e96c20bbcb6094f45112666f63843f615fe54db45aef4103f044c5111
- /root/autodl-tmp/mcln_scanrefer_native_box_transfer_pair_20260907_v1/gt_only_native_box_head_state.pt，4045205B，SHA 282aa474a8d0926548b088eea78f15aafb63b3050434c06a4aab4eab89071138
- /root/autodl-tmp/mcln_scanrefer_native_box_transfer_pair_20260907_v1/gt_teacher_box_native_box_head_state.pt，4045205B，SHA 8f4eb86f7e7f837103ec0dc216a6bae639c4529345d25da3fd550b674a60140a

合计释放92441740B（约92.44MB）。数据盘free_before=7320825856B，free_after=7413268480B，差值还包含文件系统分配与记录写入，不能混作权重精确大小。两项实验的源码、日志、逐行输出、收据、独立审计和已完成结论全部保留；旧增量权重已不可重载，不能沿旧路径再次声称可复现前向。删除计划与逐文件SHA归档refine-logs/sealed_delta_cleanup_20260908_v1及远端/root/autodl-tmp/mcln_sealed_delta_cleanup_20260908_v1。

清理前后实际重算E71、Parent、Geometry、V99及当前control/appearance两份终点SHA，全部保持；Nr/Sr保护目录不在本次范围。当前原生评估仍需要两份约753/755MB终点，暂不删除。04:32:39实查PID1129和分析队列1474均存活，最新3072/6887、累计647.117s；尚无原生完整收据及新正式结果。

另核对原作者https://github.com/qzp2018/MCLN：git ls-remote得到main9744a4ed219062d448ed0dba587eeb864491f158，当前README Models仅列ScanRefer权重，公开Releases接口返回空列表。branch API遇未认证限流，main身份改由git协议核验；未下载新权重。此检查仅说明本次官方入口没有找到Nr3D/Sr3D终点，不声称所有其他渠道不存在。后续仍优先已有可信预训练初始化，不恢复取消的长期baseline重训。


### 20.157 ScanRefer外观原生评估与同输入系统增益分解完成（2026-09-08T05:22:26.262085+08:00）

恢复后的原生只读评估完成，receipt时间2026-09-08T04:42:25.830439+08:00，三臂各6887条，formal_rows=0、optimizer_steps=0、checkpoint_writes=0；文件和状态核验通过。该模块留出被旧主干训练见过，不作为正式新场景结果。原生路径由实际evaluator default_query_axis选择，不使用Parent/Geometry/V99；完整系统数值来自既有baseline/terminal逐行输出。

| 同一6887条 | 原生REC hits@0.25/@0.50 | 完整V99 hits@0.25/@0.50 | 完整系统相对原生净收益@0.25/@0.50 |
|---|---:|---:|---:|
|保护起点|6572/5955|6684/6426|112/471|
|同预算控制|6598/6144|6687/6431|89/287|
|预训练对象外观|6604/6099|6665/6342|61/243|

CPU分析按每行row_id/scan_id/physical_space/point SHA对齐，未按跨运行动态Query编号关联。本地另用独立标量AABB公式重算20661个保存框，三臂两阈值命中全部与receipt一致；最大IoU绝对差为{"initial": 2.1204818329012554e-06, "control": 2.583741437911513e-06, "appearance": 2.3635698279678152e-06}。真实CSV逐行值与两套源文件一致，原始/native/transfer收据SHA相互绑定。源读取、固定终点和模型均未修改。

原生REC中，外观终点相对保护起点为+32/+144，但相对同预算原生GT控制仅+6/-45；严格阈值修复131、破坏176。因此不能将相对旧起点的提升全部归因于外部预训练外观，也没有证据说明本版新入口优于同预算训练。完整系统相对起点为-19/-84，相对控制为-22/-89，既定晋级结论保持失败。

严格阈值的算术分解为：相对起点，-84 = +144 -228；相对控制，-89 = -45 -44。后两项分别表示完整系统相对各自原生输出的净收益变化，不是因果归因到某一级读出的样本数。@0.25相对起点为-19 = +32 -51，相对控制为-22 = +6 -28。

完整V99对于三个模型本身仍均有净收益：保护起点+112/+471、同预算控制+89/+287、外观终点+61/+243。应表述为系统收益减少，不能表述为完整后处理相对该模型自己的原生输出整体有害，也不能据此删掉V99。

与控制比较的169条完整系统@0.50破坏中，只有30条外观原生框本已过0.5，另139条外观原生框也未过线；127条外观最终框仍过0.25。与起点比较的192条严格破坏中，24条外观原生本已过线。这些是同输入的观察分类，尚未区分同一实例的低质量范围与跨实例误选，不能把IoU>0.25自动当作身份正确。

本轮固定外观方案封存，不启动其9508条正式评估和Nr/Sr迁移，不把事后观察到的控制组+3/+5完整系统命中用于替换预注册的唯一外观候选。保留两臂终点及逐行证据以支撑必要的错误归因，已有无用旧增量权重的清理记录不变。

下一项工作限定为已有终态输出的身份与范围分解：先核验保存的候选框、原始实例标注、场景坐标和root对齐是否足够，再将可可靠判定的跨实例变化与同实例几何变化分开。若旧输出缺少所需字段，明确记录不足；不从Query编号或一个IoU阈值虚构身份标签，不为补字段重新训练，不在已消费的正式/模块留出上搜索新的决策规则。新的训练方案必须改变实际信息或训练责任，并与此前已失败的GT质量、兼容和蒸馏方案核对后再固定。

所有原生框/root框/输入身份以native_rows.json.gz无损保存，paired_rows.csv.gz及transfer_receipt保存逐行与16格正确性计数；本地解压后SHA与远端原始行文件一致。归档refine-logs/scanrefer_object_appearance_pair_20260908_v1/native_completion，不将大权重上传Git。系统相对原生净变化仅是同输入的算术分解，不能把变化自动归因到Parent、Geometry或某一种variant，当前完整系统旧行记录没有保存各级全部候选框。

本轮已完成，不继续扫描同一外观模块学习率/权重/epoch，不用原生单项改善推翻既定完整系统筛选。保护V99仍58.6033/50.4523，Scan Mask底线及Nr/Sr REC目标保持，完整三数据集Goal未完成。


### 20.158 外观终态原生几何归因完成（2026-09-08T06:07:47.478963+08:00）

复用三臂各6887条、106个训练场景的原生终态输出，按原36665条注释序恢复target_id，验证每行root和50000点SHA，并与全部已标注对象框比较。CPU89.14s结束，无GPU前向、更新或新增正式评估。原运行环境与本地10项映射/几何测试通过；本地独立标量重算20661个选中框、762882次预测—对象比较，全类别/阈值/修复破坏一致，最大IoU差2.583741437911513e-6。

| 原生@0.50失败几何代理 | 起点 | 控制 | 外观 |
|---|---:|---:|---:|
| root唯一最大重叠 | 701 | 514 | 556 |
| 另一同原始标签对象唯一最大 | 188 | 192 | 197 |
| 另一不同标签对象唯一最大 | 42 | 37 | 35 |
| 无重叠 | 1 | 0 | 0 |

原生外观相对控制@0.50修复131/破坏176/净-45；两臂均root唯一最大的一组为95/139/净-44，其他组36/37/净-1。139/176只是几何代理占比，不能宣布139条的语义身份已经证明正确。总体root类失败514→556与配对净-44为不同统计。完整系统169条严格破坏中，外观原生133条root唯一最大，其中103条原生亦失败、30条已过线。完整系统缓存缺最终框，不能重建其逐级实例变化；原生仅保存选中框，也不能继续区分回归变化与同目标低质Query改选。

本次支持重视目标附近的范围质量，不支持将所有损失归到类别错误或冻结读出。固定外观方案仍封存；历史G0已在20.37完整结束，数据修复保留，性能路线不复跑。详见SCANREFER_APPEARANCE_NATIVE_GEOMETRY_RESULT_2026-09-08.md。summary SHA72fab673dfafe3bd4936188fdc48a61082b02e9d75a7bf82534c04de6f7ce5b1；归档scanrefer_appearance_native_overlap_20260908_v1。完整三数据集Goal未完成。

### 20.159 完整预训练几何资源核验与ScanRefer权重接入准备（2026-09-08T06:07:47.478963+08:00）
...
当前保护E71/V99结构与正式成绩未变，外部预训练资源不作为本项目新结果。后续仍ScanRefer先行，正式满足保护REC与Scan Mask底线后接Nr/Sr REC。未增加当前模型的Gate、IoU头或复活封存试验；不启动取消的150—240epoch baseline。外部完整组件的运行接口、点/体素坐标和已有权重对应尚需实际检查，不能把CPU权重读取当成模型前向成功。


### 20.160 完整预训练组件的隔离环境与实际算子核验（2026-09-08T07:12:25.351686+08:00）
...
### 20.161 官方ScanRefer完整权重严格加载及四场景前向通过（2026-09-08T07:12:25.351686+08:00）
...
### 20.167 原生REC读取与作者实际Evaluator的CPU一致性（2026-09-08T09:09:13.686422+08:00）

为后续Nr/Sr接入新增scripts/pvground_rec_readout.py，提取固定PV-Ground最后层root的bbs/bbf分数、尺寸clamp、对象重叠mask和256Query排序。它不是新评分头或新Loss，不接入当前Scan训练/正式队列，也不使用目标GT几何决定排名。

2026-09-08T09:04:26.948917+08:00，实际隔离runtime CPU集成检查退出0，用时8.66秒。固定seed2027的两种合成得分构造各检查过滤开/关，每组4行、256Query、19实际token和132非连续对象槽。通过Python trace读取固定作者BBox评估函数实际局部分数及Top-10；新函数8192个分数和320个索引逐元素相等，48项REC计数一致，输入张量未改，移除GT框/评估标记后结果不变。

一个明确合成构造中，合法候选均为负分，乘零的无效候选被两实现同时排到前面。这仅验证作者乘零与旧MCLN负无穷屏蔽不等价，不是实际Nr/Sr发生率。接口保留作者规则，不以此合成案例改变部署阈值或新增回退。当前只证明同CPU上的一致性，真实GPU和数据仍需后续验证。

Torch1.10.2+cu111环境直接复用；子进程屏蔽GPU，检查前后CUDA均未初始化，无模型前向、更新、正式评估或权重读取。原始小型证据保留于refine-logs/pvground_rec_readout_cpu_20260908_v1，readout SHA 3a4b77cbf41be2287f08f53f39f64ad44589a6932931f8e5895325fe88c4fefe，独立执行回执SHA f98cc24776b043724d5fcbb704284e37c339e750400d6890595d80f5341a5584。说明docs/PVG_NATIVE_REC_READOUT_2026-09-08.md明确标注合成输入及实际验证边界。

本轮不提前重复轮询Scan训练步数。原3723步训练、终态CPU审计和条件9508队列保持固定源，接近10:30 CST再观察实际进度；现有正式保护值没有变化。Nr/Sr的真实模型加载、输入前向/反向和训练仍在Scan正式晋级之后。完整三数据集Goal继续active。


### 20.168 Nr/Sr真实点到体素接入通过，并观察到增强后的Z范围截断（2026-09-08T09:28:51.427515+08:00）

为Scan正式晋级后的接续，复用已有固定32条训练样本（各12语言+4检测），使用已修复625-file原生数据、mesh superpoint及butd_cls输入。每条关闭/开启原生增强，seed2027+固定行索引，实际64次getitem；每次比较作者train/test体素处理，共128次CPU处理和16个四行batch。未加载权重、未执行模型/GPU/梯度或正式评估，未修改当前Scan固定源码。

实际完成2026-09-08T09:22:19.450818+08:00，用时132.82秒，审计/控制器退出0。50000点的坐标颜色和顺序保留；用独立float32量化、稳定分组及前五点参考逐元素核对实际体素坐标、计数、XYZRGB全部一致。批次前缀、对象框/预测类别/槽mask及superpoint均对齐，两模式预处理相同。该PASS仅指CPU输入接入，不是REC结果。

同批每组800000点，Nr关闭/开启增强分别164/95119点在体素范围外（0.0205%/11.889875%）；Sr为945/67502（0.118125%/8.43775%）。全部来自Z轴，X/Y为0。语言root目标范围内点：Nr4594→4202，Sr14413→13659；开启增强后4条Nr和3条Sr语言样本部分目标越界，无目标全空。scene0404_00的一条固定样本177目标点只余44点在范围内。完整点数组仍保留全部点，不能等同于模型完全看不到目标或全数据集发生率。

实际原生与作者源_augment AST相同，包含X/Y小旋转、三轴约±0.5米平移、噪声和缩放。新观察定位为既有增强组合与固定[-0.2,3.8]Z体素范围的覆盖限制，不是新批次映射错误；未隔离各增强项因果贡献。当前Scan协议保持不变，不根据该小样本临时移动点云或修改体素尺寸/网格。若后续研究范围适配，必须同步所有几何坐标并核验预训练结构。
...
### 20.170 完整PV-Ground原生微调终态退化，独立审计完成并跳过正式评估（2026-09-08T10:41:46.297591+08:00）

ScanRefer固定epoch81预训练起点、原生全路径GT微调完成：29778 fit表达恰好遍历一次，3723更新，seed2027，固定末步；实际完成时间2026-09-08T10:34:43.377995+08:00。759参数张量变化，冻结参数保持不变。终态6887条模块留出评估耗时676.61秒；这些场景曾参与作者预训练，不能作为正式新场景泛化结果。

| 模式 | 起点REC@0.25/@0.50命中 | 固定终点命中 | 净变化 | 起点/终点Mask mIoU |
|---|---:|---:|---:|---:|
| bbs（固定主输出） | 6189/5599 | 6123/5462 | -66/-137 | 74.67392596/74.37854809 |
| bbf（独立诊断） | 6206/5632 | 6146/5476 | -60/-156 | 74.82181476/74.47028082 |

主bbs两阈值分别修复132/296、破坏198/433，净-66/-137；bbf净-60/-156。bbs Mask命中6214/5867→6193/5829，mIoU下降0.29537787个百分点。终态receipt中的status=pass仅表示评估执行成功，性能晋级字段primary_rec_nonregression=false。

独立CPU审计integrity_pass=true，重新核对两阶段全候选/实际选择IoU、fit行遍历和修复/破坏，结果与训练回执一致。训练、审计和接续控制器均退出0。正式队列按既定条件写入skipped_primary_rec_regression，实际正式评估0行；未运行9508两臂，也未启动Nr/Sr训练。不能将本次6887指标标成ScanRefer正式结果。

本版本封存为完整预训练原生微调控制的负结果；不挑中间权重、不切换主模式、不扫描同一路线LR/额外遍历。该结果不证明所有PV-Ground或点—体素方法无效，也不能由小样本Z范围观察直接归因整个退化。ScanRefer受保护V99仍为58.6033/50.4523，Scan Mask59.8443/52.3349/45.9303；三数据集整体Goal尚未达到。下一步研究需要新的明确机制和预先固定对照，不能自动以失败终点迁移Nr/Sr。

证据保留在三个pvground_scanrefer_*_20260908_v1目录。预训练SHA 6f24c67cc3409f44befdef188f14383497a824d8ab309b8fd572a999ae8bdec3，固定终点SHA c0c904b95d17f0a50b4b9789f0a7345227fdcd67267e831f64fcbccb68b968c8，实际训练specSHA bf53de2790c19c81bc6ae31e1aac9bd911331c5a1312e95b141e623d551cd306，训练日志SHA 6f1e4c487a12c0ce5ed021588c7a66e40afa8a78d7993d45d5b30054f2dc197b。全流程观察器已实际收集10:23、10:29及终态记录，更新20.169中尚未执行的旧进度。RGB-D跟踪误发内容与本项目无关，不改变MCLN目标。


### 20.171 确认并修复输入检测框增强次序错位，启动单因素对照（2026-09-08T11:01:51.813587+08:00）

上一轮为实质进展：完整微调负结果及独立审计已封存。继续核对当前源码时发现，实际点云先YZ/XZ翻转再Z/X/Y旋转，而输入GroupFree框角点先旋转再翻转；非交换操作导致相同参数仍存在坐标错位。已从远端固定源4a8edacf2c59c8ded76697153d96e8eb078b70146222c187e98b0e34f64bc77e核实，不只依据本地源码推断。GT由增强点得到，因此这一问题会破坏训练中视觉点与对象记忆的对应。

固定SHA选取32个不同physical fit场景，各一条表达，真实1135个GroupFree输入框。通过实际点对在扣除逐点噪声后恢复仿射矩阵，独立计算八角点变换的AABB参考。2026-09-08 10:51:56完成CPU检查，用时60.82秒：旧顺序11/32场景、340/1135框中心偏差超过1厘米，最大6.15392265米；修正后全部框最大分量误差3.50966904e-7米。其他点云、GT、Mask、superpoint、文本、类别和随机状态逐项一致；关闭增强时检测框逐元素一致。该固定样本比例不代表全部训练数据发生率，更不是REC增幅。无模型/权重/更新/正式评估。

本地src/joint_det_dataset.py仅将三行检测框旋转移至翻转之后。Z平移和固定体素范围不变，避免同时研究两个因素。新625文件副本只有src/joint_det_dataset.py变化，文件SHA3afebfc69232f9547e6810563ba6dd4f34a0f770f929df2f4203b21493de0e3d；source manifest SHA75cd5f87a8e715c15b458ed1964c2e6f1d35046bb6beffca1b3388d5903bbe36。旧运行副本和旧负结果不覆盖。本项为几何输入修复，不包装成新网络贡献，也不能将此前全部退化归因于此。

按docs/SCANREFER_DETECTION_AUGMENTATION_ORDER_2026-09-08.md预先固定方案，从同一官方epoch81重新训练，只改变检测框增强次序；旧原顺序试验作为同预算控制，仍29778fit/6887模块留出、seed2027、batch8、3723更新、lr/backbone1e-5、完整原生损失、固定末步bbs。10:57:29启动训练控制器9515/实际模型9517，screen mcln_pvg_scan_detalign_v1；终态CPU审计控制器9522，正式接续控制器9634。正式队列先在13:45检查，然后300秒间隔；仅终态双阈值不退化且独立审计通过才运行9508两臂，不满足则跳过。复用相同正式评估源码，CPU计数已在上一轮6887初始输出重新核对，无新正式行。

末次实际观察2026-09-08T10:59:39.144581+08:00，三个链路进程均存在，模型已加载并处于数据/文本准备，GPU约2433MiB；尚无本轮起点或终态质量结果。依据上一轮实耗预计训练及模块评估约2小时50分，接近预计完成时再观察，不高频轮询。只读入口scripts/observe_pvground_detalign_pipeline.py。

已核验旧训练退出0及固定terminal SHA后，删除上一轮未使用的latest恢复文件，释放331178206字节；固定终点与预训练保留。新副本准备后磁盘剩余5743214592字节。清理文件路径/SHA及来源差异记录在新训练preparation_receipt.json。

源码、实际32行、仿射误差与回执见refine-logs/scanrefer_detection_augmentation_20260908_v1；新训练/审计/正式队列均在pvground_scanrefer_*_20260908_detalign_v1。无新正式指标，不启动Nr/Sr，受保护V99及原三数据集目标保持，Goal仍active。


### 20.172 修正顺序对照进入训练；起点审计通过但跨运行输出不完全一致（2026-09-08T11:25:40.232394+08:00）

本轮真实batch8完整前向/反向已通过，峰值allocated16430602752字节，容量检查不做优化器更新；其后实际执行state_dict逐项恢复断言。四条无增强参考输入、29778fit/6887holdout和physical scene划分检查通过。11:16:22完成6887初始评估，用时664.565秒；11:17:32独立CPU核对1763072候选框、13774选择，最大选中IoU误差3.17404e-6，通过。

| 模式 | 本轮起点REC命中@0.25/@0.50 | Mask命中@0.25/@0.50 | Mask mIoU |
|---|---:|---:|---:|
|bbs|6204/5605|6214/5867|74.73204293|
|bbf|6224/5626|6233/5879|74.85341774|

旧起点bbs6189/5599，本轮6204/5605，零更新差+15/+6，不能作为输入修复的训练收益。对两轮6887行逐项比较，row/scan/target ID、输入点SHA和root GT框全部一致；但全部行的256候选框并非逐元素一致，按Query编号对齐后的框差绝对均值0.032289；bbs/bbf最终Query编号分别1905/1873行不同。Query编号变化不直接等于实例变化。完整检测框/文本中间输入未逐行导出，不能仅由点SHA排除其所有差异。

已检查实际runner：容量测试后及初始评估后，模型state_dict均与原预训练逐项torch.equal，开始训练意味着断言已通过；Python/NumPy/Torch/CUDA seed均重设2027。作者GumbelSampling在eval仍调用F.gumbel_softmax，存在随机采样路径；目前没有同输入分层GPU重放，不能宣称跨运行差异已完整归因于Gumbel或CUDA。当前修正代码不作用于关闭增强的取框分支，32行同进程已证明其余输出和RNG一致，但这不能替代全模型跨进程可重复性验证。

本轮训练保持原固定规则和源码，不因起点差异临时重启、关闭Gumbel或重选seed。终态首先相对自身起点6204/5605判定；与旧训练末点的差不能全部解释为单因素因果收益，报告时必须带上各自起点及跨运行限制。如需归因运行差异，后续在不占用当前训练GPU的条件下做同输入/状态/RNG的分层重放，而不是用更多seed挑最好。

末次实际观察2026-09-08T11:23:50.170038+08:00，日志完成174/3723更新，累计415.69秒，三个控制器9515/9522/9634及模型9517仍运行；GPU19657MiB/81%，磁盘剩余5680799744字节。按前128步约2.40秒/步估计，训练及6887终态在13:50—14:05附近完成，接近ETA再查；条件9508两臂若启动再加约30—40分钟。

完整初始回执、CPU核对、训练日志和cross_run_initial比较已归档。此节不报告新正式指标，Scan保护V99与三数据集目标不变，Goal继续active。


### 20.173 首批真实输入冻结完成，分层重放排在主评估链之后（2026-09-08T11:44:09.614949+08:00）

上一自动目标回合产生了实际训练及起点核对证据；用户随后澄清RGB-D跟踪内容误发，本项目继续仅处理MCLN三数据集，不继承STTrack/M77规则。11:37:59实际核验模型9517及训练/审计/正式控制器9515/9522/9634存活；日志512/3723更新，累计1209.5265秒。按约2.36秒/步及终态评估预算，仍预计13:50—14:05完成训练及6887模块留出；条件正式两臂另需约30—40分钟。没有新正式指标。

针对20.172的零更新差异，冻结实际起点评估第一批row400—407，均来自scene0003_00。CPU执行在11:40:39完成，用时48.04秒、退出0；8行输入点SHA与root GT均和实际initial/rows.jsonl精确一致。检测框、预测类别、文本及superpoint一并冻结，目标监督独立存储，不作为模型输入。未加载模型或权重、未初始化GPU、无更新及正式行。单场景8表达不是泛化样本，旧6887未导出的检测框/文本中间量仍不能追溯逐项一致。输入文件SHA eeb6eab59f2b54a104d73f85d4bae08144e25e7ce6a23f1b3cfc41382964c91e。

已部署两个独立进程各三次前向的只读诊断：同batch同权重重置seed2027两次，再做随机状态推进控制。hook记录骨干、文本/对象投影、双向编码器、Gumbel前后、最后Decoder和框输出，检查状态不变及逐层差异；原始bbs分数标为raw，未冒充带对象过滤的最终部署分数。它不复现容量反向或全6887，不能仅凭首批结果宣称全部随机差异已归因。

GPU重放控制器10883、等待进程10884在11:42:40实际存活，尚无模型前向。首次13:50检查既有正式控制器9634退出回执，其后每300秒；整条主链成功退出后才尝试同一GPU锁。无自动重试、改seed或重启训练；运行代码和环境spec保持原版。完整计划见docs/PVG_INITIAL_REPLAY_2026-09-08.md，执行脚本/CPU回执/等待日志见refine-logs/pvground_initial_replay_20260908_v1。

发布前再次确认模型9517、正式队列9634、重放队列10884身份仍匹配；磁盘剩余5336535040字节，未新增权重副本。原始输入和后续trace仅留服务器，小型回执进仓库。Scan正式底线、Nr/Sr REC目标及Mask豁免不变，仍须正式Scan合格再转Nr/Sr；Goal未完成。


### 20.174 检测框增强对齐完整训练仍退化，独立审计完成并跳过正式评估（2026-09-08T14:02:39.171341+08:00）

此前等待回合逐次核验实际进程，日志512→1152→1600→2048→2560→3008→3328→3648持续推进，未重启或改变训练。固定3723更新、29778行一次遍历于13:42:02完成，训练用时8738.91秒；6887模块留出终态于13:53:19完成，用时677.46秒，运行最终退出0。759参数张量变化，冻结参数不变。这是主干预训练见过场景的模块留出，不是9508正式验证。

| 模式 | 本轮起点REC@0.25/@0.50 | 固定终点REC | 净变化 | 起点/终点Mask mIoU |
|---|---:|---:|---:|---:|
|bbs|6204/5605|6140/5500|-64/-105|74.73204293/74.20198581|
|bbf|6224/5626|6167/5530|-57/-96|74.85341774/74.37432684|

主bbs @0.25修复122、破坏186，净-64；@0.50修复277、破坏382，净-105。Mask命中6214/5867→6173/5819，mIoU下降0.53005712个百分点。bbf也退化，不将其改为主模式。回执status=pass仅指评估执行成功，性能字段primary_rec_nonregression=false。

独立CPU审计于2026-09-08T13:57:41.684781+08:00完成：integrity_pass=true，重新核对全候选框/实际选择IoU、训练行遍历、修复破坏和完整回执。正式队列按预先规则写入skipped_primary_rec_regression，正常退出0；本轮正式0行，未运行9508两臂，未启动Nr/Sr。V99正式58.6033/50.4523及Scan Mask59.8443/52.3349/45.9303保持原有保护。

原顺序控制终点6123/5462，本轮6140/5500，终点差+17/+38；但起点已有+15/+6差异，两轮相对自身起点变化之差为+2/+32。上述仅为算术比较，尚不能全部解释为增强修复因果收益，须保留20.172的跨运行限制。工程坐标修复仍有32场景/1135框独立几何证据；本结果说明该修复没有使本次固定微调达到晋级条件，不能据此恢复错误增强顺序或宣称所有体素网络无效。

本版本封存；不挑中间epoch、不扫同一路线LR/损失权重、不自动以该负结果终点迁移Nr/Sr。先完成已排定的起点分层重放，确认可观察的差异边界后再决定新对照；不把重复诊断当作三数据集目标已完成。

重放观察时刻2026-09-08T14:02:14.403888+08:00；已增加只读入口scripts/observe_pvground_initial_replay.py，收集两独立进程的退出码、日志、小型回执及跨进程comparison，不下载原始GPU trace。当前重放控制器退出状态为尚无退出回执；具体执行状态以该观察及原始文件为准，不能仅凭队列已部署称为通过。

核验训练已退出、终点SHA及审计/跳过决定后，删除本轮过时latest恢复权重，释放331178206字节；固定terminal未变，清理后磁盘剩余5273354240字节。只删除记录中的具体文件，不删除预训练、保护权重或本轮固定终点。

本轮预训练SHA 6f24c67cc3409f44befdef188f14383497a824d8ab309b8fd572a999ae8bdec3，终点SHA b158d88f635ea9a943675eb98fb6842fee0144317e0a8b5310b6c502257abf1b，specSHA a0ccb9e02e2b4946742dae261605fccea23838083c198448b96183f72f511bc6，训练日志SHA 355d88a1c1c80bb795ef1d1dca61fc8c5fa7b1308039010d86e0b9f523696e0e。完整训练/终态/独立审计/正式跳过回执保留三个detalign目录，Goal继续active。


### 20.175 固定重放定位到稀疏体素批次接口错误，修正后启动ScanRefer固定训练（2026-09-08T14:29:43.642193+08:00）

上一用户澄清回合只确认项目范围，未推进实验；本回合重新检查远端真实退出码和日志。首次重放v1在导入Transformers依赖时，部署目录queue.py遮蔽标准库queue，退出1，未进入模型前向。保留失败目录，仅将新目录的调度文件改为replay_queue.py；模型脚本、冻结输入、权重、种子和队列计算不变。v2两独立进程各三次完整前向正常退出0。

原版同进程两次重置seed2027仍不同，跨进程第一个可观察差异为backbone.point_features；点索引/坐标、文本投影、对象投影及采样前RNG一致。跨进程骨干最大差0.591557，中心1.116519米、尺寸5.617179米。因此不能把起点波动全部归因于Gumbel。

两次只读骨干追踪进一步发现conv2输出252256体素行，首遍220502行不属于按batch计数划出的连续段；conv3/4同样交错。对坐标在CPU按batch/z/y/x对齐，conv2/3/4/out的坐标集合和对应特征逐元素相同；VFE、BEV、raw_points及x_conv1聚合相同，x_conv2/3/4 VSA聚合不同。实际OpenPCDet堆叠ball_query CUDA按batch计数前缀和偏移输入指针，要求每个batch连续；原PV-Ground只计数，不重排稀疏输出。这是当前spconv到VSA接口的实际错误，不是笼统的浮点非确定性。

独立源码副本只改models/pv_utils.py：每个稀疏特征源进入VSA前，使用batch/z/y/x键对坐标和特征同步排序。保持batch连续，并固定批次内部first-nsample读取顺序；不改点/体素集合、不改网络参数数目、不加评分头、不升级依赖、不重编译CUDA。原runtime未改。修正SHA33279bc4e0302250025eb963f956f164cbe3f9b265ac57d9d4826abf6c370810。来源、完整修改文件和执行脚本保留在vsa_order_replay与sparse_order目录。

修正后同一8条表达六次完整前向于14:20:54完成，两个进程各自重置种子后全部记录tensor逐元素一致；跨进程仍不逐位一致，骨干最大差1.43051147e-6、中心3.98159027e-5米、尺寸4.67181206e-4米、raw bbs分数8.44597816e-5，8条选择相同。推进RNG控制仍改变候选，Gumbel随机路径保留。这里只测试同一场景首8条，不证明全6887完全复现或正式精度提升。

执行草案在重放已启动后整理时曾写跨进程逐位全等，实际未通过该额外要求，不能称为完全确定性。它不是预注册性能门槛，也不是用户要求；据实保留差异，不把不必要的逐位全等作为继续修正真实接口错误的前置条件。任何后续REC退化仍按原门槛判定，不通过重复运行挑结果。详见docs/PVG_VSA_BATCH_ORDER_2026-09-08.md。

14:26:18启动修正接口的固定Scan训练控制器14247、实际模型14249，screen mcln_pvg_scan_vsaorder_v1；CPU独立审计控制器14254，首次约14:41，之后300秒；正式条件队列控制器14358，首次17:12:18，之后300秒。修正预训练相同官方epoch81、seed2027、29778fit/6887模块留出、batch8、3723步、lr/backbone1e-5和原生GT损失；保留已验证的检测框增强顺序修正，唯一新模型改动是VSA行顺序。仍先执行实际batch8容量前向/反向并恢复权重，最后一步终点固定，不扫超参数。

正式计数准备已在已完成的旧6887输出上重新核对：完整重计数与评价汇总一致，0模型前向、0正式行。这只证明计数逻辑，不是新模型评估通过；新版evaluate.py改变了模型源码路径核验，不能把完整文件称为完全未改。终点双REC相对自身起点不退化且CPU审计通过后，才执行9508正式两臂；Scan正式5572/4797及Mask58.70/50.70/44.72底线不变，达标即转Nr/Sr REC。

末次观察2026-09-08T14:27:42.773659+08:00实际模型和三个控制器均存活，模型已加载，处于数据/文本准备，尚无本轮更新或质量终态。GPU 2433 MiB, 0 %，磁盘剩余5061505024字节；只复制约5.3MB模型源码，预训练复用，本回合没有删除保护权重。估计训练及起终点模块评估约3小时，约17:15—17:35附近检查终态；若正式接续启动另需约30—40分钟，以实际速率更新。

历史detalign负结果不撤销，但其接口已证实存在错误，不能将它当作正确批次邻域条件下的完整PV-Ground结论。新模型正式成绩尚无，V99 58.6033/50.4523及三数据集目标不变。Goal继续active，RGB-D跟踪内容继续排除。


### 20.176 VSA修正完整起点下降，独立核对通过，固定训练已进入更新（2026-09-08T14:49:12.679932+08:00）

上一Goal回合完成接口定位、修正、六次重放和固定任务启动，属于实际进展；本回合等待前核验原模型14249与三个控制器存活，未重启。真实batch8容量前向/反向于14:33:49通过，峰值allocated16433301504字节、0优化器更新；检查后恢复原权重。6887起点评估于14:44:54完成，用时663.94秒。

| 输出 | 修正前零更新REC | 本轮零更新REC | 净变化 | 本轮Mask@0.25/@0.50命中 | 本轮Mask mIoU |
|---|---:|---:|---:|---:|---:|
|bbs|6204/5605|6147/5549|-57/-56|6174/5824|74.14438183|
|bbf|6224/5626|6174/5583|-50/-43|6187/5836|74.32945913|

主bbs宽松阈值修复111/破坏168，严格阈值修复262/破坏318；未更新权重就净-57/-56。虽然批次顺序错误的代码与重放证据成立，旧预训练在修正接口下并未直接改善精度。不能把工程正确性记作REC收益，也不能把这张主干已经见过训练场景的模块留出表称为9508正式成绩。输入分布变化或具体读取差异如何造成下降尚未独立分解。

独立CPU审计14:46:21通过，重新核对1763072候选框、13774次选择，最大选中IoU误差3.91157637e-6；两模式计数与原回执一致。另一次CPU只读比较确认6887行row/scan/target ID、point SHA及root GT全部一致；不声称旧输出没有记录的检测框/文本张量也已逐项回溯。完整256框GT oracle从6873/6716变为6872/6697，分别-1/-19，是候选诊断而非部署成绩。比较没有模型前向、训练或正式行。

固定训练按原计划继续：最后一次14:47:19观察时模型14249与控制器14247/14254/14358均存活，保存的训练日志完成47/3723更新、累计114.27秒。后续终点首先与本轮真实起点6147/5549比较；不能使用较低的新起点抹去正式V99 5572/4797门槛，也不把跨起点差异算作训练收益。不改LR、seed、损失、epoch或主bbs模式，不恢复错误顺序。按早期约2.43秒/步估计，训练加终态模块评估约17:25—17:35；靠近预计终态再查，现有审计/正式依赖每300秒检查。

本轮没有新的正式9508指标，未启动Nr/Sr；目标仍是Scan正式达底线后尽快Nr/Sr REC。起点回执SHA 071f156730437ef46bcc5e616505907e1067a8e594336bb68e4fbcccbb08013b；CPU比較回执SHA 558377500d54fccc629bb15849c09f1840039cb0040ec0d88b82210896f1a697。新比较入口scripts/compare_pvground_vsa_initial.py及执行证据保留同一vsaorder训练目录。Goal继续active。


### 20.177 Nr/Sr独立REC核对通过，Scan修正VSA固定训练继续（2026-09-08）

用户澄清RGBD跟踪长文误发，本Goal只继续MCLN。上一回复仅澄清范围，未产生实验进展；本回合重新核验现有进程，没有重启。15:00:30 CST，原Scan模型14249、训练控制14247、独立审计14254及条件正式评估14358均存活；日志384/3723更新、累计906.88秒。磁盘空闲4999311360字节（约5.00GB），本轮未下载权重或删除保护文件。按约2.36秒/步估计训练加6887终点评估仍约17:25—17:35完成；靠近终点查看，不频繁查询GPU。没有新9508正式指标，受保护成绩及门槛不变。

利用等待时间补充Nr/Sr输出审计：独立NumPy重算对象重叠过滤、乘零后分数、实际选中Query是否为最大分数及root目标IoU。输入保持非连续对象槽mask，明确保留负分时无效零分候选可能胜出的作者行为，不新增负无穷过滤。CPU积分检查15:01:58正常退出0，8个合成行2048候选、16次选择、8项聚合REC计数与固定作者Evaluator一致；原8192分数/320个Top10索引/48计数检查也通过。完整行数下的Nr4726/4059与Sr12130/10157六项边界测试通过，Mask门关闭。

这是协议实现准备，非真实Nr/Sr模型推理、非模型精度提升、非全部GPU数值边界验证。0 GPU初始化、0模型前向、0优化器更新、0正式行，未动Scan运行源。未来Scan正式晋级后仍需加载实际Nr/Sr预训练、同结构前向/反向及完整REC；不能由该CPU检查宣布三数据集达标。新审计器为scripts/pvground_referit3d_rec_audit.py，复用verify_pvground_rec_readout.py固定受控输入，详情见docs/PVG_NATIVE_REC_READOUT_2026-09-08.md。
...
### 20.181 固定训练期间准备独立支持观测审计，未改当前网络（2026-09-08 16:05 CST）

上一Goal回合属于verified wait：15:58:18服务器实查同一模型14249及审计14257/正式队列14359仍存活，记录1856/3723更新，累计4355.79秒，磁盘3837407232字节；无终点和新正式指标。训练配置及预计17:25—17:35完成训练与6887终点评估的窗口未改，未重新启动任何任务。
...
### 20.185 Nr3D真实批次与父权重索引及文本标签绑定检查通过（2026-09-08 16:55 CST）
...
最新Scan实查仍为16:39:35：原模型14249与既有训练/审计/正式控制器存活，记录2880/3723步、累计6753.189秒，未见terminal，可用3701436416字节。17:12—17:15为训练预计终点，6887终态预计17:25—17:35；9508正式评估仅在通过既定筛选后另行接续，不计入前述窗口。当前没有新正式指标。支持采集队列16:44:12核实仍等待既有formal控制器，尚未执行。没有更改当前网络、LR、预算、晋级规则或启动Nr/Sr训练；Goal active。


### 20.186 Nr3D原生损失标签输入预检及Mask类型转换边界（2026-09-08 17:01 CST）

上一回合16:56:53确认原Scan进程14249及现有依赖队列存活，记录3328/3723步，属于verified wait；未终态，磁盘3701268480字节。本回合继续独立CPU准备，没有重复启动或修改当前训练。
...
### 20.188 Scan固定训练完成进入终态评估；后台准备Sr官方父权重（2026-09-08 17:15 CST）

上一回合确认训练完成并实际进入评估，属于verified wait。本回合利用评估空档推进第三数据集资源准备。

Scan修正接口训练已于17:10:27完成3723步/29778条，日志PVG_FIT_COMPLETE，耗时8732.11秒；终点权重331181534字节已写出。17:10:41实查同一模型14249仍在，terminal目录已创建boxes.npy、scores.npy和rows.jsonl，处于6887条终态评估，尚无receipt和终态指标。不是正式9508已完成；原起点同规模评估耗时663.94秒，预计17:22左右产生终态，再由独立审计/既有队列接续。预计17:25—17:35查看完整筛选结论，正式评估若获准另计。

17:10:27磁盘3369918464字节；现有latest331178206字节保留，未在活动任务中删除权重。终態及可能的两臂9508正式框/分数固定数组共212197376字节，另有逐行JSON/log。基于已有空间检查，启动Sr官方父权重CPU准备，下载前服务器3351695360字节，要求目标830MB外仍保留2GiB，不占GPU。

固定资源PV-Ground_SR3D.pth，829830168字节，SHAa4a14b0090947177a648703ad6de094246891d89174fffa56fe454f730dbe3dc，HF revision cf4a8b1eed045f1309e6a691ea948d1d6c54448e。复用已验证的本地HTTPS→SSH路线，完整字节和SHA通过后才重命名远端.part并在screen启动CPU inventory；不安装包、不训练、不做模型前向。Sr实际config及state schema由文件清点记录，不预先宣称与Nr完全相同；strict-load仍须后续独立执行。

本地隐藏进程20372于17:13:44启动；17:14:59核实存活，局部下载142606336字节，仅部分文件，尚未校验/传输完成。参考Nr实际下载252秒＋传输1377秒，预计17:40—17:45再检查完成态，期间不重复启动。服务器目录/root/autodl-tmp/mcln_pvground_sr_checkpoint_inspection_20260908_v1；本地临时副本在.codex/tmp/mcln_pv_sr_transfer_20260908，成功远端校验及CPU检查后再核实删除。权重不进入Git，现阶段只同步代码、plan、local launch及观察证据。

Scan先达现行正式底线再训练Nr/Sr的顺序不变；Nr真实反向入口仍未运行，支持采集仍排在当前formal队列之后。没有新正式REC、没有降低验收要求，Goal active。


### 20.189 修正接口PV微调终态未通过；正式评估跳过，Sr截断下载已校验恢复（2026-09-08 17:29 CST）

上一回合已启动Sr资源准备并验证Scan终态评估存活，属于progress。本回合获得固定训练与独立审计完整负结果，并处理实际下载截断。

当前修正检测框增强顺序＋VSA batch/z/y/x排序的固定微调已完整结束：17:10:27完成3723步/29778条，17:21:53完成6887条terminal，17:21:55总流程结束；controller退出0。17:26:30独立CPU审计通过，分别检查起点/终点各1763072个候选框及13774次选择，终点最大选框IoU误差2.998e-6。Mask审计仅复算导出的逐行IoU，不声称重新加载全部二值Mask。

主输出bbs：REC从6147/5549变为6119/5537，@0.25修复156/破坏184/净-28，@0.50修复332/破坏344/净-12。Mask6174/5824变为6166/5800，mIoU74.1443818→74.0307063，下降0.1136755个百分点。诊断bbf：6174/5583→6153/5550，净-21/-33。全部为6887模块留出，非9508正式指标，也非未见场景泛化成绩；不能从两种输出挑单项拼接。

17:27:21收集到formal decision=skipped_primary_rec_regression、controller.exit=0，正式行数0，未晋级Nr/Sr。固定版本封存为科学负结果，不追加epoch、扫LR或切换主输出挽救。父权重与终点SHA保留，terminal为6b951587f09674ec441b30d8c30627d94c6bdca24a8a4d9bee274cd9a3248c2f；759个参数张量变化，冻结参数保持，fit_seen_exactly_once通过。当前V99等正式保护结果不变。

另对绑定row SHA的原始导出做oracle汇总：bbs Top16从6614/6407变6610/6397；Full256从6872/6697变6865/6684，净-7/-13。故本轮不能描述成“候选更多合格但评分退化”，原生选择与GT辅助上界均略降；这是固定批次预算下的微调负结果，不否定全部点—体素或覆盖建模。此oracle不是可部署精度。下一步只接续已排队的实际VSA支持采集，核实各来源的真实空邻域和支撑；其使用固定8行父权重，不能据此直接归因本次终点净损失或声称全数据集覆盖率。后续机制仍按普通读取、仅空邻域屏蔽、覆盖条件读取逐项定义，不重复本轮纯微调。

Sr准备出现一次真实下载截断：原隐藏PID20372已退出，文件397533346字节，SHAcec75c80...，未上传。保留原stderr及initial_download_failure.json；针对已发生错误，单次HTTP Range从397533346续接，响应206、Content-Range准确为397533346—829830167/829830168，无自动重试或备用源。续传200.57秒后完整829830168字节及固定SHAa4a14b00...通过。新隐藏PID13440从17:21:08运行；17:28:26服务器仅.part为144474112字节，传输未完、CPU inventory未启动。预计17:45—17:50再检查，同一任务不重启；Sr实际config/strict-load仍未核实。

训练/审计/正式、Sr下载失败和恢复证据分目录保存，权重不上传Git。环境、Scan先行与三数据集最终门槛不变，Goal继续active；本次方法未达目标不等于外部阻塞。


### 20.190 实际VSA支持与候选中心几何审计；清理过期中间权重（2026-09-08 17:43 CST）

上一回合完成固定微调负结果与独立审计，属于progress。本回合完成支持采集归档和CPU几何分析，并释放过期权重空间，继续属于progress。
...
### 20.191 Sr完整预训练严格加载通过；16场景覆盖审计及最小空邻域控制（2026-09-08 18:18 CST）

上一回合完成单场景支持审计、权重清理和20.190同步，属于progress。本回合获得多训练场景实测、完成Sr资源与新控制实现/核验，继续属于progress；总体三数据集目标仍未达成。
...
### 20.194 空邻域控制initial完整比较及实际训练进度（2026-09-08 19:02 CST）
