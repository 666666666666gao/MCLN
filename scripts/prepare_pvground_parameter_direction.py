...
files = {'diagnose.py':(repo/'scripts/diagnose_pvground_parameter_direction.py').read_bytes(),
         ...
         'plan.md':(repo/'docs/PVG_PARAMETER_DIRECTION_PLAN_2026-09-17.md').read_bytes()}
...
spec = dict(training_root=training,training_spec_sha256=sha(train_spec_raw),terminal_sha256=receipt['terminal_sha256'],
    ...
    scope='fixed trained D shared-parameter direction; eval mode; no optimizer or formal evaluation',
...
command = 'screen -dmS mcln_pvg_parameter_direction_v1 '+shlex.quote('/root/miniconda3/envs/bdetr/bin/python')+' -u '+shlex.quote(root+'/controller.py')
...
record = dict(time_cst=datetime.datetime.now(datetime.timezone(datetime.timedelta(hours=8))).isoformat(),
    root=root,process=process,expected_seconds=600,first_check_after_seconds=300,
