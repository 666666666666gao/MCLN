"""Continue the fixed Mask geometry pair, without changing training or choosing a validation arm."""
import argparse
import datetime
import hashlib
import json
...
def validated_module_pass(receipt, audit):
    ...
    candidate = receipt['terminal_metrics']['native_gt_mask_geometry']
    references = [receipt['baseline_metrics']['native_gt_mask_geometry'], receipt['terminal_metrics']['native_gt']]
...
def main():
    ...
    assert spec['schema'] == 'mcln-mask-geometry-gt-posttraining-queue-v1'
    assert sha(__file__) == spec['queue_script_sha256']
    assert spec['interval_seconds'] == 240 and spec['candidate_predeclared'] == 'native_gt_mask_geometry'
    ...
    assert prep['candidate_predeclared'] == 'native_gt_mask_geometry_v99' and prep['formal_rows_executed'] == 0
    ...
    while not (training / 'controller.exit').exists():
        ...
        assert process.returncode == 0 and b'mcln_mask_geometry_pair_v1' in process.stdout, process.stdout.decode()
        ...
        progress = {}
        for line in lines:
            for label in ['SCANREFER MASK GEOMETRY GT EVAL ', 'SCANREFER MASK GEOMETRY GT EVAL COMPLETE ',
                          'SCANREFER MASK GEOMETRY GT TRAIN ', 'SCANREFER MASK GEOMETRY GT PAIR COMPLETE ']:
        ...
        print('MASK GEOMETRY GT QUEUE OBSERVED', json.dumps({'time_cst': value['time_cst'],
    ...
    formal_manifest = {
        'schema': 'mcln-scanrefer-mask-geometry-gt-official-input-v1', 'training_directory': str(training),
        'training_receipt_sha256': sha(training / 'receipt.json'), 'training_audit_sha256': sha(audit_path),
        ...
        'candidate_predeclared': 'native_gt_mask_geometry_v99', 'arms': ['protected_v99', 'native_gt_v99', 'native_gt_mask_geometry_v99'],
    ...
    command = ['flock', '-x', '/root/autodl-tmp/mcln_v99_backbone_gpu0.lock', PYTHON, '-u',
               'scripts/evaluate_scanrefer_mask_geometry_official.py', '--manifest', str(formal / 'input_manifest.json')]
    ...
    with (directory / 'formal_audit.txt').open('xb') as stream:
        process = subprocess.run([PYTHON, '-m', 'scripts.audit_scanrefer_mask_geometry_official',
