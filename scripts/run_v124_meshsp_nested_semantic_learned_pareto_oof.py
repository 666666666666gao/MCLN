#!/usr/bin/env python
"""Nested OOF screen of the V124 learned-Pareto sign gate."""
...
import models.rec_semantic_antisymmetric_utility as v124_model
...
V124_CRITIC_EPOCHS = 50
V124_CRITIC_BATCH_SIZE = 1024
V124_CRITIC_LEARNING_RATE = 1e-3
V124_CRITIC_WEIGHT_DECAY = 1e-3
...
def _pair_inputs(
    ...
    if count <= 0:
        raise ValueError("V124 pair population must be nonempty")
    for value in inputs.values():
        if (value.dtype != torch.float32 or value.device.type != "cpu"
                or value.shape[0] != count
                or not bool(torch.isfinite(value).all().item())):
            raise ValueError("V124 pair input layout changed")
...
def build_hard_pair_training_population(prediction, records):
    ...
    if (probabilities.dtype != torch.float32
        ...
        raise ValueError("V124 model probability layout changed")
    rows = torch.arange(count)
    ...
    if (tuple(ious.shape) != (count, QUERY_COUNT, VARIANT_COUNT)
            or not bool(torch.isfinite(ious).all().item())):
        raise ValueError("V124 candidate IoU layout changed")
    ...
    for column, suffix in enumerate(("025", "050")):
        ...
        if fixes <= 0 or breaks <= 0:
            raise ValueError("V124 hard pairs lack fix/break events")
        event_counts[suffix] = {
            "fix": fixes,
            "break": breaks,
...
def fit_semantic_pairwise_critic(prediction, records, device):
    ...
    if pair_count < 100:
        raise ValueError("V124 pairwise event population is too small")
    resolved = torch.device(device)
    _set_hierarchical_seed(resolved)
    model = v124_model.SemanticAntisymmetricUtilityRanker().to(resolved)
    optimizer = torch.optim.AdamW(
        model.parameters(),
        lr=V124_CRITIC_LEARNING_RATE,
        weight_decay=V124_CRITIC_WEIGHT_DECAY,
    )
    shuffle_generator = torch.Generator(device="cpu")
    shuffle_generator.manual_seed(0)
    epoch_receipts = []
    for epoch in range(V124_CRITIC_EPOCHS):
        model.train()
        order = torch.randperm(pair_count, generator=shuffle_generator)
        total_loss = 0.0
        total_events = 0
        for start in range(0, pair_count, V124_CRITIC_BATCH_SIZE):
            indices = order[start:start + V124_CRITIC_BATCH_SIZE]
            batch_inputs = {
    ...
    receipt.update({
        "epochs": V124_CRITIC_EPOCHS,
        "batch_size": V124_CRITIC_BATCH_SIZE,
        "epoch_receipts": epoch_receipts,
        "state_sha256": v99.tensor_sha256(*[
            value for _, value in sorted(model.state_dict().items())
        ]),
...
def predict_pairwise_utility(model, inputs, device):
    ...
    logits = []
    with torch.no_grad():
        for start in range(0, count, V124_CRITIC_BATCH_SIZE):
            stop = min(start + V124_CRITIC_BATCH_SIZE, count)
            logits.append(model(**{
                key: value[start:stop].to(resolved)
                for key, value in inputs.items()
            }).cpu().float())
    ...
    if (tuple(output.shape) != (count, 2)
        ...
        raise RuntimeError("V124 utility logits changed")
    return output
...
def pairwise_utility_accept_mask(base_accepted, utility_logits):
    if (base_accepted.dtype != torch.bool or base_accepted.dim() != 1
        ...
        raise ValueError("V124 utility gate tensors changed")
    return (
        base_accepted
        & utility_logits[:, 0].ge(0.0)
        & utility_logits[:, 1].ge(0.0)
    )
...
def main(argv=None):
    ...
    for held in range(5):
        ...
        if (set(records[index]["scan_id"] for index in inner_fit_indices)
            ...
            raise RuntimeError("scene leakage into V124 inner calibration")
        ...
        if not torch.equal(prediction["baselines"], baselines[held_indices]):
            raise RuntimeError("V124 OOF baseline identity changed")
        fold_utility_logits = predict_pairwise_utility(
            critic_model,
            selected_pair_inputs(prediction),
            args.device,
        )
        fold_accepted = pairwise_utility_accept_mask(
            prediction["base_accepted"], fold_utility_logits
        )
        ...
        utility_logits[held_indices] = fold_utility_logits
        fold_record = {
            "held_fold": held,
            ...
            "pairwise_accepted_switches": int(fold_accepted.sum().item()),
    ...
    pairwise_veto = base_accepted & ~accepted
    veto_reasons = {
        ...
        "nested_semantic_learned_pareto_vetoes": int(pairwise_veto.sum().item()),
    ...
    if protected_after != protected_before:
        raise RuntimeError("protected artifacts changed during V124")
    protected_metadata_after = {
        name: file_sha256(path)
        for name, path in protected_metadata_paths.items()
    }
    if protected_metadata_after != protected_metadata_before:
        raise RuntimeError("protected V124 cache metadata changed during OOF")
    script_sources = {
        "v124": str(Path(__file__).resolve()),
        ...
        "v124_model": str(Path(v124_model.__file__).resolve()),
        "v108": str(
    ...
    report = {
        "schema": "rec-v124-nested-semantic-learned-pareto-scene-oof-v1",
        "version": 1,
        "validation_data_accessed": False,
        ...
        "protocol": {
            "architecture": (
                ...
                "candidate-minus-baseline antisymmetry and a learned-Pareto "
                "dual-head sign gate"
            ),
            ...
            "acceptance": (
                "V115 Pareto acceptance plus nonnegative antisymmetric "
                "candidate-minus-baseline utilities at both REC thresholds"
            ),
            "grid_search": False,
            "selection_data": (
            ...
            "critic_candidate_dim": v124_model.V123_CANDIDATE_DIM,
            "critic_pair_structure": "shared_score_difference",
            "critic_model_gain_scale": "learned_positive_per_threshold",
            "critic_epochs": V124_CRITIC_EPOCHS,
            "critic_batch_size": V124_CRITIC_BATCH_SIZE,
            "critic_learning_rate": V124_CRITIC_LEARNING_RATE,
            "critic_weight_decay": V124_CRITIC_WEIGHT_DECAY,
            "critic_class_weights": None,
            "critic_neutral_events_in_loss": False,
            "critic_acceptance": "both_antisymmetric_utility_logits_nonnegative",
        ...
        "prediction_sha256": v99.tensor_sha256(
            ...
            base_accepted, utility_logits, accepted
        ),
        "oof": {
    ...
    try:
        ...
        while written < len(payload):
            ...
            if count <= 0:
                raise OSError("V124 output write made no progress")
            written += count
        os.fsync(descriptor)
    finally:
        os.close(descriptor)
