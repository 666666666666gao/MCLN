"""Fixed GT-only versus GT-supported teacher-box transfer into native box heads."""
...
def main():
    ...
    assert manifest['schema'] == 'mcln-scanrefer-native-box-transfer-training-input-v1'
    ...
    assert native_probe['teacher_and_readouts_unchanged'] and native_probe['formal_rows'] == 0
    ...
    assert manifest['teacher_box_loss_weight'] == 1.
    os.chdir(str(source))
    ...
    models = {'gt_only': first_model, 'gt_teacher_box': copy.deepcopy(first_model)}
    teacher = copy.deepcopy(first_model).requires_grad_(False)
    ...
    readouts = {'gt_only': first_readout, 'gt_teacher_box': first_readout}
    ...
    assert core_names == native_probe['trainable_tensors']
    ...
    class FitDataset(Joint3DDataset):
        def _scene_graph_parse(self, annos):
            ...
            for index, row in enumerate(annos):
                row['_box_transfer_training_id'] = index
        ...
        def __getitem__(self, index):
            ...
            result['joint_training_id'] = self.annos[index]['_box_transfer_training_id']
            return result
    ...
    assert [row['_box_transfer_training_id'] for row in dataset.annos] == list(range(36665))
    ...
    write_json(directory / 'protocol.json', {'row_ids': partitions, 'physical_spaces': spaces,
        ...
        'readout_trainable_tensors': [], 'teacher_trainable_tensors': [],
        'teacher_box_loss_weight': manifest['teacher_box_loss_weight'], 'data_root': manifest['data_root'], 'superpoint_inputs': data_inputs})
    ...
    tester.logger = logging.getLogger('scanrefer-native-box-transfer-pair')
    ...
    def evaluate(stage):
        ...
        with torch.no_grad():
            for batch_index, raw in enumerate(loader('holdout', 1000, False)):
                ...
                if batch_index == 0 or (batch_index + 1) % 128 == 0:
                    print('SCANREFER NATIVE BOX TRANSFER EVAL', json.dumps({'stage': stage,
                        'rows': len(records['gt_teacher_box']), 'total': len(partitions['holdout']),
        ...
        print('SCANREFER NATIVE BOX TRANSFER EVAL COMPLETE', json.dumps({'stage': stage, 'metrics': metrics,
    ...
    assert baseline_metrics['gt_only'] == baseline_metrics['gt_teacher_box']
    ...
    for raw in loader('fit', 0, True):
        ...
        with torch.no_grad():
            ...
            runtime = first_readout(teacher_outputs, inputs)['runtime']
            teacher_selected = runtime['rec_geometry_scores'].masked_fill(
                ~runtime['rec_geometry_valid_mask'], -float('inf')).argmax(dim=1)
            teacher_boxes = runtime['rec_geometry_boxes'][
                torch.arange(len(teacher_selected), device=teacher_selected.device), teacher_selected].detach()
        for arm, model in models.items():
            optimizer = optimizers[arm]
            optimizer.zero_grad(set_to_none=True)
            outputs = model(inputs)
            if step == 0 or (step + 1) % 64 == 0:
                for key in ['last_sem_cls_scores', 'last_proj_queries', 'proj_tokens', 'last_pred_masks', 'sp_last_pred_masks']:
                    a, b = outputs[key], teacher_outputs[key]
                    if torch.is_tensor(a):
                        assert torch.equal(a, b), (arm, key, step)
                    else:
                        assert len(a) == len(b) and all(torch.equal(x, y) for x, y in zip(a, b)), (arm, key, step)
            ...
            auxiliary, auxiliary_stats = native_teacher_box_loss(boxes, teacher_boxes, roots[:, 0], captured[0])
            loss = native + (manifest['teacher_box_loss_weight'] * auxiliary if arm == 'gt_teacher_box' else auxiliary * 0.)
            ...
            statistics[arm] = {'loss': float(loss), 'native_loss': float(native),
                'teacher_box_loss': float(auxiliary),
                'teacher_box_stats': {name: value.detach().cpu().tolist() for name, value in auxiliary_stats.items()},
                'gradient_norm_before_clip': float(gradient_norm)}
            del outputs, auxiliary, native, loss, boxes, auxiliary_stats, captured
        step += 1
        batches.append({'step': step, 'row_ids': raw['joint_training_id'].tolist(),
            'teacher_selected': teacher_selected.cpu().tolist(), 'teacher_boxes': teacher_boxes.cpu().tolist(),
            'roots': roots[:, 0].cpu().tolist(), 'arms': statistics,
            'point_sha256': [hashlib.sha256(cloud.cpu().numpy().tobytes()).hexdigest()
                            for cloud in inputs['point_clouds']]})
        del teacher_outputs, runtime, teacher_boxes, teacher_selected
        if step == 1 or step % 64 == 0 or step == manifest['steps_per_arm']:
            elapsed = time.time() - begin_training
            print('SCANREFER NATIVE BOX TRANSFER TRAIN', json.dumps({'step': step, 'total': manifest['steps_per_arm'],
    ...
    for arm, model in models.items():
        ...
        assert len(optimizer.state) == 16 and all(float(value['step']) == step for value in optimizer.state.values())
        checkpoint = directory / (arm + '_native_box_head_state.pt')
        assert not checkpoint.exists()
        # The frozen E71 already exists; save only the 16 changed head parameters and optimizer.
        torch.save({'schema': 'mcln-scanrefer-native-box-head-state-v1', 'arm': arm, 'steps': step,
            'head_parameters': {name: model.state_dict()[name].detach().cpu() for name in core_names},
            'optimizer': optimizer.state_dict(), 'core_trainable_tensors': core_names,
            'manifest_sha256': file_sha(option.manifest), 'pretrained_artifacts': manifest['artifacts'],
            'decision_rule': 'native E71 with transferred final box heads; frozen V99 for system comparison only'}, checkpoint)
        saved = torch.load(checkpoint, map_location='cpu')
        assert sorted(saved['head_parameters']) == sorted(core_names)
        reconstructed = dict(initial)
        reconstructed.update(saved['head_parameters'])
    ...
    assert all(torch.equal(value.detach().cpu(), initial[name]) for name, value in teacher.state_dict().items())
    ...
    effects = {reference: {str(t): paired_effect(
        baseline['gt_teacher_box'] if reference == 'baseline' else terminal['gt_only'],
        terminal['gt_teacher_box'], 'rec_iou', t) for t in (.25, .5)} for reference in ('baseline', 'gt_only')}
    ...
    native_effects = {reference: {str(t): paired_effect(
        baseline['gt_teacher_box'] if reference == 'baseline' else terminal['gt_only'],
        terminal['gt_teacher_box'], 'native_rec_iou', t) for t in (.25, .5)} for reference in ('baseline', 'gt_only')}
    receipt = {'schema': 'mcln-scanrefer-native-box-transfer-pair-v1', 'status': 'complete',
        ...
        'readout_parameters_and_metadata_unchanged': True, 'teacher_parameters_and_buffers_unchanged': True, 'data_root': manifest['data_root'], 'superpoint_inputs': data_inputs,
    ...
    print('SCANREFER NATIVE BOX TRANSFER PAIR COMPLETE', json.dumps(receipt), flush=True)
