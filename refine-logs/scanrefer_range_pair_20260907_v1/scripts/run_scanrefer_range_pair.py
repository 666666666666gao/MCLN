"""Fixed ScanRefer comparison of center and extent sampling with matched region readers."""
...
def main():
    ...
    assert manifest['mode'] == 'train' and manifest['schema'] == 'mcln-scanrefer-range-pair-input-v1'
    ...
    assert native_probe['schema'] == 'mcln-scanrefer-range-preflight-result-v1'
    assert native_probe['status'] == 'pass' and native_probe['disposable_optimizer_steps_per_arm'] == 2
    assert native_probe['data_root'] == manifest['data_root']
    assert file_sha(manifest['native_probe_manifest']) == native_probe['manifest_sha256']
    probe_manifest = json.loads(Path(manifest['native_probe_manifest']).read_text())
    assert probe_manifest['superpoint_files'] == manifest['superpoint_files']
    assert probe_manifest['source_manifest_sha256'] == manifest['source_manifest_sha256']
    assert probe_manifest['files']['models/candidate_range_visual.py'] == manifest['files']['models/candidate_range_visual.py']
    ...
    first_model.decoder[-1].local_visual = CandidateRangeVisual('center').cuda().eval()
    models = {'control': first_model, 'local': copy.deepcopy(first_model)}
    models['local'].decoder[-1].local_visual.sampling = 'extent'
    ...
    assert not set(core_names).intersection(local_initial)
    ...
    tester.logger = logging.getLogger('scanrefer-range-pair')
    ...
    def evaluate(stage):
        ...
        with torch.no_grad():
            for batch_index, raw in enumerate(loader('holdout', 1000, False)):
                ...
                if batch_index == 0 or (batch_index + 1) % 128 == 0:
                    print('SCANREFER RANGE EVAL', json.dumps({'stage': stage,
        ...
        print('SCANREFER RANGE EVAL COMPLETE', json.dumps({'stage': stage, 'metrics': metrics,
              'elapsed_seconds': time.time() - begin}), flush=True)
    ...
    for raw in loader('fit', 0, True):
        ...
        if step == 1 or step % 64 == 0 or step == manifest['steps_per_arm']:
            ...
            print('SCANREFER RANGE TRAIN', json.dumps({'step': step, 'total': manifest['steps_per_arm'],
    ...
    for arm, model in models.items():
        ...
        checkpoint = directory / (arm + '_range_visual_state.pt')
        assert not checkpoint.exists()
        # Preserve the completed fit endpoint before the potentially long terminal evaluation.
        torch.save({'schema': 'mcln-scanrefer-range-trained-state-v1', 'arm': arm, 'steps': step,
            ...
            'model_config': {'candidate_range_visual': 'extent' if arm == 'local' else 'center'},
    ...
    receipt = {'schema': 'mcln-scanrefer-range-pair-v1', 'status': 'complete',
        'data_root': manifest['data_root'], 'superpoint_files_sha256': superpoint_files_sha256,
    ...
    print('SCANREFER RANGE PAIR COMPLETE', json.dumps(receipt), flush=True)
