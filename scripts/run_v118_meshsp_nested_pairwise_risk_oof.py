#!/usr/bin/env python
"""Nested scene-OOF screen of the V118 pairwise switch-risk head."""
...
def main(argv=None):
    ...
    for held in range(5):
        ...
        if (set(records[index]["scan_id"] for index in inner_fit_indices)
            ...
            raise RuntimeError("scene leakage into V118 inner calibration")
        ...
        if not torch.equal(prediction["baselines"], baselines[held_indices]):
            raise RuntimeError("V118 OOF baseline identity changed")
        fold_risk_scores = predict_pairwise_risk(
            risk_model, prediction["features"], args.device
        )
        fold_accepted = (
            prediction["base_accepted"]
            & fold_risk_scores[:, 0].gt(V118_ACCEPTANCE_THRESHOLD)
            & fold_risk_scores[:, 1].gt(V118_ACCEPTANCE_THRESHOLD)
        )
    ...
    if protected_after != protected_before:
        raise RuntimeError("protected artifacts changed during V118")
    protected_metadata_after = {
        name: file_sha256(path)
        for name, path in protected_metadata_paths.items()
    }
    if protected_metadata_after != protected_metadata_before:
        raise RuntimeError("protected V118 cache metadata changed during OOF")
    script_sources = {
        "v118": str(Path(__file__).resolve()),
    ...
    report = {
        "schema": "rec-v118-nested-pairwise-switch-risk-scene-oof-v1",
        "version": 1,
        "validation_data_accessed": False,
        ...
        "protocol": {
            ...
            "acceptance": (
                "V115 Pareto acceptance and independently calibrated positive "
                "signed risk at both thresholds"
            ),
            "grid_search": False,
            "selection_data": (
            ...
            "risk_acceptance_threshold": V118_ACCEPTANCE_THRESHOLD,
    ...
    try:
        ...
        while written < len(payload):
            ...
            if count <= 0:
                raise OSError("V118 output write made no progress")
            written += count
        os.fsync(descriptor)
    finally:
        os.close(descriptor)
