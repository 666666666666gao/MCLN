...
OUTPUT=$BASE/v114_meshsp_language_spatial_oof.json
LOG=$BASE/v114_meshsp_language_spatial_oof.log
EXIT=$BASE/v114_meshsp_language_spatial_oof.exit
SESSION=v114_language_spatial_oof
SELF=$(readlink -f "$0")
...
if [[ "${1:-}" == "--inner" ]]; then
    ...
    env CUDA_VISIBLE_DEVICES=0 \
        ...
        "$PYTHON" scripts/run_v114_meshsp_language_spatial_oof.py \
...
cd "$REPO"
require_sha 4ee9fa146d9215745eaa997230727e144230a5bb2ecb4c56f32f1c9a6eb11a66 \
    scripts/run_v114_meshsp_language_spatial_oof.py
require_sha e26d2880d7c787531feeb49c2b1ecb0021bedf2bc801a04af761f1b0a127acfe \
    models/rec_language_spatial_context.py
...
for path in "$OUTPUT" "$LOG" "$EXIT"; do
    if [[ -e "$path" ]]; then
        echo "Refusing to overwrite existing V114 output: $path" >&2
        exit 74
    fi
done
if [[ $(nvidia-smi -L | wc -l) -ne 1 ]]; then
    echo "V114 requires the frozen single-GPU topology" >&2
    exit 75
fi
...
if pgrep -af '[r]un_v114_meshsp_language_spatial_oof.py' >/dev/null; then
    echo "V114 process already exists" >&2
    exit 75
fi
if screen -ls 2>/dev/null | grep -q "[.]$SESSION"; then
    echo "V114 screen session already exists" >&2
    exit 75
fi
available_kb=$(df -Pk /root/autodl-tmp | awk 'NR==2 {print $4}')
if [[ "$available_kb" -lt 2621440 ]]; then
    echo "V114 requires at least 2.5 GiB free on /root/autodl-tmp" >&2
    exit 75
fi
...
if ! screen -ls 2>/dev/null | grep -q "[.]$SESSION" \
        && [[ ! -e "$EXIT" ]]; then
    echo "V114 screen failed to remain alive and produced no exit receipt" >&2
    exit 75
fi
