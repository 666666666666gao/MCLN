...
def main():
    ...
    model_source = Path('/root/autodl-tmp/mcln_pvground_vsa_order_source_20260908_v1/PV-Ground')
    ...
    for name, info in source['sources']['PV-Ground']['files'].items():
    ...
    from pvground_empty_pool_mask import install_empty_pool_mask
    ...
    for step, start in enumerate((0,2),1):
        ...
        record = {'step':step,'training_row_ids':[r['training_row_id'] for r in fixture_receipt['rows'][start:start+2]],
            ...
            'all_present_gradients_finite':True,'seconds':time.time()-begin,
    ...
    receipt = {'status':'pass','time_cst':datetime.datetime.now(datetime.timezone(datetime.timedelta(hours=8))).isoformat(),
        'strict_state_tensors':len(state),'env_spec_sha256':build['spec_sha256'],'checkpoint_sha256':checkpoint['sha256'],
        ...
        'eval_forwards':2,'train_forwards':2,'trainable_tensors':len(trainable),
    ...
    receipt.update(empty_pool_mask=True,model_source=str(model_source),module_sha256=sha(Path(__file__).parent/'pvground_empty_pool_mask.py'))
    (args.output/'receipt.json').write_text(json.dumps(receipt,indent=2,sort_keys=True)+'\n')
    print('PVG_EMPTY_POOL_TRAINING_INTERFACE_PASS '+json.dumps({k:v for k,v in receipt.items() if k not in ['steps','changed_parameters','changed_buffers']}),flush=True)
