"""Fixed native GT versus current-root Mask geometry GT; frozen deployed V99."""

import argparse
import datetime
import hashlib
import json
import logging
import math
import os
from pathlib import Path
import random
import sys
import time


def file_sha(path):
    digest = hashlib.sha256()
    with Path(path).open('rb') as stream:
        for block in iter(lambda: stream.read(8 * 1024 * 1024), b''):
            digest.update(block)
    return digest.hexdigest()


def write_json(path, value):
    with path.open('x') as stream:
        json.dump(value, stream, sort_keys=True, allow_nan=False)
        stream.write('\n')


def paired_effect(reference, candidate, field, threshold):
    before = [row[field] > threshold for row in reference]
    after = [row[field] > threshold for row in candidate]
    repair = sum(new and not old for old, new in zip(before, after))
    damage = sum(old and not new for old, new in zip(before, after))
    return {'repair': repair, 'damage': damage, 'net': repair - damage}


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument('--manifest', type=Path, required=True)
    option = parser.parse_args()
    directory = option.manifest.parent
    manifest = json.loads(option.manifest.read_text())
    assert manifest['schema'] == 'mcln-scanrefer-mask-geometry-gt-training-input-v1'
    source = Path(manifest['model_source'])
    assert file_sha(source / 'local_visual_source_manifest.json') == manifest['source_manifest_sha256']
    for name, digest in json.loads((source / 'local_visual_source_manifest.json').read_text())['files'].items():
        assert file_sha(source / name) == digest, name
    for name, digest in manifest['files'].items():
        assert file_sha(directory / name) == digest, name
    for name, item in manifest['artifacts'].items():
        assert file_sha(item['path']) == item['sha256'], name
    assert file_sha(manifest['native_probe_receipt']) == manifest['native_probe_receipt_sha256']
    native_probe = json.loads(Path(manifest['native_probe_receipt']).read_text())
    assert native_probe['status'] == 'pass' and native_probe['disposable_optimizer_steps_per_arm'] == 2
    assert native_probe['readouts_unchanged'] and native_probe['formal_rows'] == 0
    assert file_sha(manifest['split_protocol']) == manifest['split_protocol_sha256']
    partitions = json.loads(Path(manifest['split_protocol']).read_text())['row_ids']
    assert manifest['steps_per_arm'] == math.ceil(len(partitions['fit']) / 12)
    assert manifest['epochs'] == 1 and manifest['batch_size'] == 12
    assert manifest['core_learning_rate'] == 1e-6 and manifest['readouts_frozen']
    assert manifest['weight_decay'] == .0005 and manifest['clip_norm'] == .1
    assert manifest['mask_geometry_loss_weight'] == 1.
    os.chdir(str(source))
    sys.path.insert(0, str(source))

    import copy
    import numpy as np
    import torch
    import scripts
    scripts.__path__ = [str(directory / 'scripts'), str(source / 'scripts')]
    from main_utils import parse_option
    from train_dist_mod import TrainTester
    from src.joint_det_dataset import Joint3DDataset
    from models.rec_reranker import compute_query_ious
    from scripts.run_frozen_v99_pareto_contextual_official import build_authoritative_command
    from scripts.scanrefer_joint_readout import JointRecReadout
    from scripts.native_mask_geometry_supervision import native_mask_geometry_loss
    from scripts.mask_geometry_pair_support import CORE_PREFIXES, hard_root_geometry, optimizer_presence_counts
    from scripts.scanrefer_rec_evaluation import rec_evaluation_view
    from scripts.scanrefer_data_contract import set_scanrefer_data_root, verify_scanrefer_superpoints
    from scripts.audit_scanrefer_joint_readout_pair import assert_metadata_equal

    def seed_everything(seed):
        random.seed(seed)
        np.random.seed(seed)
        torch.manual_seed(seed)
        torch.cuda.manual_seed_all(seed)

    seed_everything(0)
    torch.backends.cudnn.benchmark = False
    torch.backends.cudnn.deterministic = True
    torch.backends.cuda.matmul.allow_tf32 = True
    torch.backends.cudnn.allow_tf32 = True
    command = set_scanrefer_data_root(build_authoritative_command(directory / 'unused_official_output'), manifest['data_root'])
    data_inputs = verify_scanrefer_superpoints(manifest['data_root'], 'train', manifest['train_superpoint_files'])
    sys.argv = [sys.argv[0]] + command[command.index('train_dist_mod.py') + 1:]
    args = parse_option()
    assert args.dataset == ['scanrefer'] and args.test_dataset == 'scanrefer'
    assert args.butd and not args.butd_cls and not args.butd_gt
    assert args.use_color and not args.use_height and not args.use_multiview
    assert args.batch_size == 12 and args.num_decoder_layers == 6
    assert args.checkpoint_path == manifest['artifacts']['backbone']['path']
    payload = torch.load(args.checkpoint_path, map_location='cpu')
    initial = {name[7:]: value for name, value in payload['model'].items()}
    first_model = TrainTester.get_model(args).cuda().eval()
    first_model.load_state_dict(initial, strict=True)
    assert first_model.decoder[-1].local_visual is None and first_model.query_mask_fusion_calibrator is None
    for name, parameter in first_model.named_parameters():
        parameter.requires_grad_(name.startswith(CORE_PREFIXES))
    models = {'native_gt': first_model, 'native_gt_mask_geometry': copy.deepcopy(first_model)}
    artifacts = {name: torch.load(item['path'], map_location='cpu')
                 for name, item in manifest['artifacts'].items() if name != 'backbone'}
    first_readout = JointRecReadout(artifacts).cuda().eval().requires_grad_(False)
    readouts = {'native_gt': first_readout, 'native_gt_mask_geometry': first_readout}
    readout_metadata = copy.deepcopy(first_readout.metadata)
    readout_initial = {name: value.detach().cpu().clone() for name, value in first_readout.state_dict().items()}
    core_names = [name for name, value in first_model.named_parameters() if value.requires_grad]
    assert core_names == native_probe['requires_grad_parameters']
    assert len(core_names) == 84
    assert all(not parameter.requires_grad for parameter in first_readout.parameters())
    trainable, optimizers = {}, {}
    for arm, model in models.items():
        core_parameters = [value for value in model.parameters() if value.requires_grad]
        trainable[arm] = core_parameters
        optimizers[arm] = torch.optim.AdamW(core_parameters, lr=manifest['core_learning_rate'],
                                             weight_decay=manifest['weight_decay'])
    criterion, set_criterion = TrainTester.get_criterion(args)

    class FitDataset(Joint3DDataset):
        def _scene_graph_parse(self, annos):
            assert len(annos) == 36665
            actual = {'fit': [], 'holdout': []}
            for index, row in enumerate(annos):
                row['_mask_geometry_training_id'] = index
                code = (manifest['split_salt'] + '\0' + row['scan_id'].split('_')[0]).encode()
                fold = int(hashlib.sha256(code).hexdigest()[:8], 16) % 5
                actual['holdout' if fold == 0 else 'fit'].append(index)
            assert actual == partitions
            # Preserve all scene expressions for native distractor construction before partitioning.
            super()._scene_graph_parse(annos)

        def __getitem__(self, index):
            result = super().__getitem__(index)
            result['joint_training_id'] = self.annos[index]['_mask_geometry_training_id']
            return result

    dataset = FitDataset(dataset_dict={'scanrefer': 1}, test_dataset='scanrefer', split='train',
        data_path=args.data_root, use_color=args.use_color, use_height=args.use_height,
        use_multiview=args.use_multiview, detect_intermediate=args.detect_intermediate,
        butd=args.butd, butd_gt=args.butd_gt, butd_cls=args.butd_cls,
        augment_det=False, skip_missing_superpoints=args.skip_missing_superpoints)
    assert [row['_mask_geometry_training_id'] for row in dataset.annos] == list(range(36665))
    dataset.augment = False
    spaces = {part: sorted({dataset.annos[i]['scan_id'].split('_')[0] for i in ids})
              for part, ids in partitions.items()}
    assert not set(spaces['fit']).intersection(spaces['holdout'])
    write_json(directory / 'protocol.json', {'row_ids': partitions, 'physical_spaces': spaces,
        'source_protocol_sha256': manifest['split_protocol_sha256'],
        'previous_pretraining_has_seen_development_holdout': True,
        'steps_per_arm': manifest['steps_per_arm'], 'epochs': 1,
        'authoritative_base_command': command, 'core_trainable_tensors': core_names,
        'readout_trainable_tensors': [],
        'mask_geometry_loss_weight': manifest['mask_geometry_loss_weight'], 'data_root': manifest['data_root'], 'superpoint_inputs': data_inputs})

    def loader(part, seed, shuffle):
        return torch.utils.data.DataLoader(torch.utils.data.Subset(dataset, partitions[part]),
            batch_size=12, shuffle=shuffle, num_workers=0,
            generator=torch.Generator().manual_seed(seed))

    tester = object.__new__(TrainTester)
    tester.logger = logging.getLogger('scanrefer-mask-geometry-gt-pair')

    def losses(outputs, inputs, batch):
        boxes = torch.cat([outputs['last_center'], outputs['last_pred_size']], dim=-1)
        roots = torch.cat([batch['center_label'][:, 0], batch['size_gts'][:, 0]], dim=-1)
        captured = []
        def capture(module, arguments, result):
            if torch.equal(arguments[0]['pred_boxes'], boxes):
                assert all(torch.equal(target['boxes'][0], roots[i])
                           for i, target in enumerate(arguments[1]))
                captured.append(result)
        handle = set_criterion.matcher.register_forward_hook(capture)
        outputs.update(batch)
        native, outputs = TrainTester._compute_loss(outputs, criterion, set_criterion, args)
        handle.remove()
        assert len(captured) == 1
        auxiliary, stats = native_mask_geometry_loss(outputs, inputs, roots, captured[0])
        return native, auxiliary, stats

    def evaluate(stage):
        seed_everything(1000)
        records = {arm: [] for arm in models}
        evaluators = {arm: tester._build_grounding_evaluator(args, ['last_']) for arm in models}
        native_evaluators = {arm: copy.deepcopy(evaluator) for arm, evaluator in evaluators.items()}
        for evaluator in native_evaluators.values():
            evaluator.eval_use_selector_choice_scores = False
            evaluator.eval_use_rec_reranker_scores = False
            evaluator.eval_use_rec_geometry_reranker_scores = False
            evaluator.eval_use_rec_joint_box_mask = False
            assert not evaluator.filter_non_gt_boxes
        begin = time.time()
        with torch.no_grad():
            for batch_index, raw in enumerate(loader('holdout', 1000, False)):
                batch = TrainTester._to_gpu(raw)
                inputs = TrainTester._get_inputs(batch)
                inputs['train'] = False
                roots = torch.cat([batch['center_label'][:, :1], batch['size_gts'][:, :1]], dim=-1)
                root_valid = batch['box_label_mask'][:, :1].bool()
                point_hashes = [hashlib.sha256(cloud.cpu().numpy().tobytes()).hexdigest()
                                for cloud in inputs['point_clouds']]
                for arm, model in models.items():
                    outputs = model(inputs)
                    native_outputs = rec_evaluation_view(dict(outputs, **batch))
                    native_evaluator = native_evaluators[arm]
                    native_selected = []
                    original_top = native_evaluator._position_top_indices

                    def record_top(scores, valid, axis_mode, max_topk):
                        top = original_top(scores, valid, axis_mode, max_topk)
                        native_selected.append(int(top[0, 0]))
                        return top

                    native_evaluator._position_top_indices = record_top
                    native_evaluator.evaluate_bbox_by_pos_align(native_outputs, 'last_')
                    native_evaluator._position_top_indices = original_top
                    assert len(native_selected) == len(roots)
                    native_boxes = torch.cat([native_outputs['last_center'], native_outputs['last_pred_size']], dim=-1)
                    native_ious = compute_query_ious(native_boxes, roots, root_valid)
                    native_chosen = [float(native_ious[i, q]) for i, q in enumerate(native_selected)]
                    del native_outputs, native_boxes, native_ious
                    readout = readouts[arm](outputs, inputs)
                    outputs.update(batch)
                    outputs.update(readout['runtime'])
                    negative_sizes = (outputs['last_pred_size'] < 0).any(dim=-1).sum(dim=-1).tolist()
                    evaluated = rec_evaluation_view(outputs)
                    evaluator = evaluators[arm]
                    evaluator.evaluate_bbox_by_pos_align(evaluated, 'last_')
                    # Call the native semantic Mask evaluator and capture its actual per-row IoUs.
                    mask_ious = []
                    original_iou = evaluator.calculate_masks_iou

                    def record_iou(prediction, target):
                        value = original_iou(prediction, target)
                        mask_ious.append(float(value))
                        return value

                    evaluator.calculate_masks_iou = record_iou
                    evaluator.evaluate_masks_by_sem_align(evaluated, 'last_')
                    evaluator.calculate_masks_iou = original_iou
                    assert len(mask_ious) == len(roots)
                    runtime = readout['runtime']
                    selected = runtime['rec_geometry_scores'].masked_fill(
                        ~runtime['rec_geometry_valid_mask'], -float('inf')).argmax(dim=1)
                    all_ious = compute_query_ious(runtime['rec_geometry_boxes'], roots, root_valid)
                    chosen_ious = all_ious.gather(1, selected[:, None])[:, 0]
                    # GT diagnostics run only after both deployed decisions have been recorded.
                    diagnostic_native, diagnostic_auxiliary, geometry = losses(outputs, inputs, batch)
                    hard_boxes, hard_valid = hard_root_geometry(outputs, inputs, geometry['query_indices'])
                    soft_ious = compute_query_ious(geometry['soft_boxes'][:, None], roots, root_valid)[:, 0]
                    hard_ious = compute_query_ious(hard_boxes[:, None], roots, root_valid)[:, 0]
                    hard_ious = hard_ious.masked_fill(~hard_valid, 0.)
                    for index, row_id in enumerate(raw['joint_training_id'].tolist()):
                        records[arm].append({'row_id': row_id, 'scan_id': raw['scan_ids'][index],
                            'physical_space': raw['scan_ids'][index].split('_')[0],
                            'point_sha256': point_hashes[index], 'rec_iou': float(chosen_ious[index]),
                            'native_rec_iou': native_chosen[index], 'native_query_index': native_selected[index],
                            'mask_iou': mask_ious[index], 'selected_variant_position': int(selected[index]),
                            'raw_negative_size_queries': negative_sizes[index],
                            'matched_root_query': int(geometry['query_indices'][index]),
                            'root_box': roots[index, 0].cpu().tolist(),
                            'soft_box': geometry['soft_boxes'][index].cpu().tolist(),
                            'hard_box': hard_boxes[index].cpu().tolist(), 'hard_valid': bool(hard_valid[index]),
                            'soft_iou': float(soft_ious[index]), 'hard_iou': float(hard_ious[index]),
                            'soft_l1': float(geometry['l1_per_row'][index]),
                            'soft_giou_loss': float(geometry['giou_per_row'][index])})
                    del outputs, readout, runtime, evaluated, geometry, diagnostic_native, diagnostic_auxiliary, hard_boxes, hard_valid, soft_ious, hard_ious
                if batch_index == 0 or (batch_index + 1) % 128 == 0:
                    print('SCANREFER MASK GEOMETRY GT EVAL', json.dumps({'stage': stage,
                        'rows': len(records['native_gt_mask_geometry']), 'total': len(partitions['holdout']),
                        'elapsed_seconds': time.time() - begin}), flush=True)
        metrics, native_metrics, geometry_metrics = {}, {}, {}
        for arm, rows in records.items():
            evaluator = evaluators[arm]
            count = len(rows)
            assert [row['row_id'] for row in rows] == partitions['holdout']
            metric = {'rows': count, 'mask_miou': sum(row['mask_iou'] for row in rows) / count * 100.}
            for threshold, suffix in [(.25, '025'), (.5, '050')]:
                rec_hits = sum(row['rec_iou'] > threshold for row in rows)
                mask_hits = sum(row['mask_iou'] > threshold for row in rows)
                assert evaluator.gts[('last_', threshold, 1, 'bbs')] == count
                assert evaluator.dets[('last_', threshold, 1, 'bbs')] == rec_hits
                key = 'overall_mask' if threshold == .25 else 'overall50_mask'
                assert evaluator.dets[key] == mask_hits
                metric['rec_hits' + suffix] = rec_hits
                metric['mask_hits' + suffix] = mask_hits
            assert abs(evaluator.dets['mask_sem'] - sum(row['mask_iou'] for row in rows)) < 1e-8
            metrics[arm] = metric
            native_metric = {'rows': count}
            for threshold, suffix in [(.25, '025'), (.5, '050')]:
                hits = sum(row['native_rec_iou'] > threshold for row in rows)
                assert native_evaluators[arm].dets[('last_', threshold, 1, 'bbs')] == hits
                assert native_evaluators[arm].gts[('last_', threshold, 1, 'bbs')] == count
                native_metric['rec_hits' + suffix] = hits
            native_metrics[arm] = native_metric
            geometry_metrics[arm] = {
                'rows': count, 'hard_valid': sum(row['hard_valid'] for row in rows),
                'soft_mean_iou': sum(row['soft_iou'] for row in rows) / count,
                'hard_mean_iou_invalid_as_zero': sum(row['hard_iou'] for row in rows) / count,
                'soft_hits': [sum(row['soft_iou'] > t for row in rows) for t in [.25, .5]],
                'hard_hits': [sum(row['hard_iou'] > t for row in rows) for t in [.25, .5]],
                'mean_soft_geometry_loss': sum(5. * row['soft_l1'] + row['soft_giou_loss'] for row in rows) / count,
                'scope': 'GT-matched root Query diagnostics; not deployed REC'}
        write_json(directory / (stage + '_rows.json'), records)
        write_json(directory / (stage + '_metrics.json'), metrics)
        write_json(directory / (stage + '_native_metrics.json'), native_metrics)
        write_json(directory / (stage + '_geometry_metrics.json'), geometry_metrics)
        print('SCANREFER MASK GEOMETRY GT EVAL COMPLETE', json.dumps({'stage': stage, 'metrics': metrics,
              'native_rec_metrics': native_metrics, 'elapsed_seconds': time.time() - begin}), flush=True)
        return records, metrics

    baseline, baseline_metrics = evaluate('baseline')
    assert baseline['native_gt'] == baseline['native_gt_mask_geometry']
    assert baseline_metrics['native_gt'] == baseline_metrics['native_gt_mask_geometry']
    assert all(torch.equal(value.detach().cpu(), initial[name])
               for model in models.values() for name, value in model.state_dict().items())
    assert all(torch.equal(value.detach().cpu(), readout_initial[name])
               for readout in readouts.values() for name, value in readout.state_dict().items())
    seed_everything(0)
    batches = []
    begin_training = time.time()
    previous_report = begin_training
    step = 0
    for raw in loader('fit', 0, True):
        batch = TrainTester._to_gpu(raw)
        inputs = TrainTester._get_inputs(batch)
        inputs['train'] = False
        roots = torch.cat([batch['center_label'][:, :1], batch['size_gts'][:, :1]], dim=-1)
        root_valid = batch['box_label_mask'][:, :1].bool()
        statistics = {}
        for arm, model in models.items():
            optimizer = optimizers[arm]
            optimizer.zero_grad(set_to_none=True)
            outputs = model(inputs)
            native, auxiliary, auxiliary_stats = losses(outputs, inputs, batch)
            loss = native + auxiliary if arm == 'native_gt_mask_geometry' else native
            assert torch.isfinite(loss), (arm, step)
            loss.backward()
            assert all(torch.isfinite(parameter.grad).all() for parameter in trainable[arm]
                       if parameter.grad is not None), (arm, step)
            gradient_norm = torch.nn.utils.clip_grad_norm_(trainable[arm], manifest['clip_norm'])
            assert torch.isfinite(gradient_norm), (arm, step)
            presence = ''.join('1' if parameter.grad is not None else '0' for parameter in trainable[arm])
            optimizer.step()
            statistics[arm] = {'loss': float(loss), 'native_loss': float(native),
                'mask_geometry_loss': float(auxiliary),
                'mask_geometry_stats': {name: value.detach().cpu().tolist() for name, value in auxiliary_stats.items()},
                'gradient_norm_before_clip': float(gradient_norm), 'gradient_presence': presence}
            del outputs, auxiliary, native, loss, auxiliary_stats
        step += 1
        batches.append({'step': step, 'row_ids': raw['joint_training_id'].tolist(),
            'roots': roots[:, 0].cpu().tolist(), 'arms': statistics,
            'point_sha256': [hashlib.sha256(cloud.cpu().numpy().tobytes()).hexdigest()
                            for cloud in inputs['point_clouds']]})
        if step == 1 or step % 64 == 0 or step == manifest['steps_per_arm']:
            elapsed = time.time() - begin_training
            print('SCANREFER MASK GEOMETRY GT TRAIN', json.dumps({'step': step, 'total': manifest['steps_per_arm'],
                'arms': {arm: {key: value for key, value in stats.items() if key != 'mask_geometry_stats'} for arm, stats in statistics.items()}, 'elapsed_seconds': elapsed,
                'since_previous_report_seconds': time.time() - previous_report,
                'estimated_training_remaining_seconds': (manifest['steps_per_arm'] - step) * elapsed / step}), flush=True)
            previous_report = time.time()
    assert step == manifest['steps_per_arm']
    assert sorted(row_id for item in batches for row_id in item['row_ids']) == partitions['fit']
    assert all(len(item['row_ids']) == 12 for item in batches[:-1])
    write_json(directory / 'fit_point_batches.json', batches)
    changed, checkpoints = {}, {}
    for arm, model in models.items():
        changed[arm] = []
        for name, value in model.state_dict().items():
            same = torch.equal(value.detach().cpu(), initial[name])
            if name not in core_names:
                assert same, (arm, name)
            elif not same:
                changed[arm].append(name)
        assert changed[arm]
        optimizer = optimizers[arm]
        assert all(torch.isfinite(value[key]).all() for value in optimizer.state.values()
                   for key in ['exp_avg', 'exp_avg_sq'])
        counts = optimizer_presence_counts([item['arms'][arm]['gradient_presence'] for item in batches], core_names)
        assert len(optimizer.state) == sum(count > 0 for count in counts.values())
        for name, parameter in zip(core_names, trainable[arm]):
            if counts[name]:
                assert float(optimizer.state[parameter]['step']) == counts[name], (arm, name)
            else:
                assert parameter not in optimizer.state, (arm, name)
        checkpoint = directory / (arm + '_core_delta_state.pt')
        assert not checkpoint.exists()
        # Keep E71 once; save the allowed core delta and actual per-parameter optimizer state.
        torch.save({'schema': 'mcln-scanrefer-mask-geometry-core-state-v1', 'arm': arm, 'steps': step,
            'core_parameters': {name: model.state_dict()[name].detach().cpu() for name in core_names},
            'optimizer': optimizer.state_dict(), 'optimizer_parameter_steps': counts, 'core_trainable_tensors': core_names,
            'manifest_sha256': file_sha(option.manifest), 'pretrained_artifacts': manifest['artifacts'],
            'decision_rule': 'Native E71 architecture; frozen V99 hard geometry and final decision unchanged'}, checkpoint)
        saved = torch.load(checkpoint, map_location='cpu')
        assert sorted(saved['core_parameters']) == sorted(core_names)
        assert saved['optimizer_parameter_steps'] == counts
        reconstructed = dict(initial)
        reconstructed.update(saved['core_parameters'])
        assert set(reconstructed) == set(model.state_dict())
        assert all(torch.equal(value.detach().cpu(), reconstructed[name])
                   for name, value in model.state_dict().items())
        # Terminal evaluation uses a model actually reconstructed from the disk endpoint plus E71.
        model.load_state_dict(reconstructed, strict=True)
        checkpoints[arm] = {'path': str(checkpoint), 'bytes': checkpoint.stat().st_size,
                            'sha256': file_sha(checkpoint), 'reconstruction_equal_all_model_tensors': True}
    assert all(not p.requires_grad and p.grad is None for p in first_readout.parameters())
    assert all(torch.equal(value.cpu(), readout_initial[name]) for name, value in first_readout.state_dict().items())
    assert_metadata_equal(first_readout.metadata, readout_metadata)
    write_json(directory / 'fit_complete.json', {'steps_per_arm': step, 'checkpoints': checkpoints,
        'changed_core_tensors': changed, 'training_seconds': time.time() - begin_training})
    terminal, terminal_metrics = evaluate('terminal')
    for arm in models:
        for before, after in zip(baseline[arm], terminal[arm]):
            for key in ['row_id', 'scan_id', 'physical_space', 'point_sha256']:
                assert before[key] == after[key], (arm, key, before['row_id'])
    effects = {reference: {str(t): paired_effect(
        baseline['native_gt_mask_geometry'] if reference == 'baseline' else terminal['native_gt'],
        terminal['native_gt_mask_geometry'], 'rec_iou', t) for t in (.25, .5)} for reference in ('baseline', 'native_gt')}
    eligible = all(effect['net'] >= 0 for comparison in effects.values() for effect in comparison.values())
    native_effects = {reference: {str(t): paired_effect(
        baseline['native_gt_mask_geometry'] if reference == 'baseline' else terminal['native_gt'],
        terminal['native_gt_mask_geometry'], 'native_rec_iou', t) for t in (.25, .5)} for reference in ('baseline', 'native_gt')}
    # Evaluation must not alter frozen buffers or the saved endpoint.
    for arm, model in models.items():
        saved = torch.load(checkpoints[arm]['path'], map_location='cpu')
        expected = dict(initial)
        expected.update(saved['core_parameters'])
        assert all(torch.equal(value.detach().cpu(), expected[name]) for name, value in model.state_dict().items())
    assert all(torch.equal(value.detach().cpu(), readout_initial[name]) for name, value in first_readout.state_dict().items())
    assert_metadata_equal(first_readout.metadata, readout_metadata)
    receipt = {'schema': 'mcln-scanrefer-mask-geometry-gt-pair-v1', 'status': 'complete',
        'time_cst': datetime.datetime.now(datetime.timezone(datetime.timedelta(hours=8))).isoformat(),
        'steps_per_arm': step, 'fit_rows': len(partitions['fit']), 'holdout_rows': len(partitions['holdout']),
        'previous_pretraining_has_seen_development_holdout': True, 'formal_rows': 0,
        'readout_parameters_and_metadata_unchanged': True, 'data_root': manifest['data_root'], 'superpoint_inputs': data_inputs,
        'baseline_metrics': baseline_metrics, 'terminal_metrics': terminal_metrics,
        'system_rec_effects': effects, 'native_rec_effects': native_effects, 'eligible_for_fixed_terminal_formal_evaluation': eligible,
        'changed_core_tensors': changed, 'frozen_core_parameters_and_buffers_unchanged_after_fit': True,
        'checkpoints': checkpoints, 'max_gpu_mib': torch.cuda.max_memory_allocated() / 1024**2,
        'manifest_sha256': file_sha(option.manifest),
        'evaluation_extent_policy': 'existing rec_candidate_adapter floor at 1e-6;raw loss inputs unchanged',
        'fit_batches_sha256': file_sha(directory / 'fit_point_batches.json'),
        'baseline_rows_sha256': file_sha(directory / 'baseline_rows.json'),
        'terminal_rows_sha256': file_sha(directory / 'terminal_rows.json')}
    write_json(directory / 'receipt.json', receipt)
    print('SCANREFER MASK GEOMETRY GT PAIR COMPLETE', json.dumps(receipt), flush=True)


if __name__ == '__main__':
    main()
