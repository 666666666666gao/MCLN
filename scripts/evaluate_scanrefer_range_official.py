"""One fixed9508-row evaluation of protected V99, center control and extent candidate."""
...
def verify_training_endpoint(directory):
    ...
    assert baseline['local'] == baseline['control']
...
def main():
    ...
    assert manifest['schema'] == 'mcln-scanrefer-range-official-input-v1'
    ...
    assert manifest['files']['models/candidate_range_visual.py'] == train_manifest['files']['models/candidate_range_visual.py']
    ...
    tester.logger = logging.getLogger('scanrefer-range-official')
    ...
    write_json(result_directory / 'protocol.json', {'authoritative_base_command': command,
        ...
        'native_loader_and_worker_seeding': True, 'formal_checkpoint_arm': 'local', 'formal_control_arm': 'control', 'candidate_predeclared': 'extent',
    ...
    for arm, sampling, label in [('control', 'center', 'center_v99'), ('local', 'extent', 'local_v99')]:
        trained = torch.load(receipt['checkpoints'][arm]['path'], map_location='cpu')
        assert trained['schema'] == 'mcln-scanrefer-range-trained-state-v1'
        assert trained['arm'] == arm and trained['steps'] == 2482
        assert trained['manifest_sha256'] == receipt['manifest_sha256']
        assert trained['model_config'] == {'candidate_range_visual': sampling}
        model = copy.deepcopy(protected)
        model.decoder[-1].local_visual = CandidateRangeVisual(sampling).cuda().eval()
    ...
    with torch.no_grad():
        for batch_index, raw in enumerate(test_loader):
            ...
            if batch_index == 0 or (batch_index + 1) % 128 == 0:
                ...
                print('SCANREFER RANGE OFFICIAL', json.dumps({'rows': count, 'total': 9508,
    ...
    for candidate_arm in ['center_v99', 'local_v99']:
    ...
    result = {'schema': 'mcln-scanrefer-range-official-v1', 'status': 'complete',
        ...
        'candidate_predeclared': 'local_v99', 'center_is_mechanism_control_not_validation_selected_candidate': True,
        'data_root': args.data_root, 'superpoint_inputs': data_inputs,
    ...
    print('SCANREFER RANGE OFFICIAL COMPLETE', json.dumps(result), flush=True)
