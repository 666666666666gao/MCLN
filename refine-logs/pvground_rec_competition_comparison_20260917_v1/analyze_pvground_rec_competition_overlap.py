"""Attribute the completed D/F selected boxes to verified scene geometry on CPU."""
import argparse
import datetime
import gzip
import hashlib
...
def analyze(output):
    ...
    locations = {
        'F_initial': ('rec_competition', 'initial'),
        'D_terminal': ('task_observation', 'terminal'),
        'F_terminal': ('rec_competition', 'terminal'),
    }
    pair_root = base/'mcln_pvground_rec_competition_comparison_20260917_v1'
    pair_result = json.loads((pair_root/'terminal_D_F.json').read_bytes())
    assert pair_result['status'] == 'complete' and pair_result['training_row_order_identical']
    inputs[str(pair_root/'terminal_D_F.json')] = sha(pair_root/'terminal_D_F.json')
    analyzed = {}; source_rows = {}; metrics = {}; maximum_error = 0.
    for name, (trial, stage) in locations.items():
        root = base/('mcln_pvground_scanrefer_finetune_20260917_'+trial+'_v1')/stage
    ...
    comparisons = {}
    for before, after in [('F_initial', 'F_terminal'),
                          ('D_terminal', 'F_terminal')]:
    ...
    for threshold, suffix in [('0.25', '025'), ('0.5', '050')]:
        assert metrics['F_terminal']['bbs'][suffix]['hits'] - metrics['D_terminal']['bbs'][suffix]['hits'] == pair_result['transitions']['bbs'][threshold]['net']
    ...
    result = dict(status='complete', time_cst=datetime.datetime.now(datetime.timezone(datetime.timedelta(hours=8))).isoformat(),
        rows_per_snapshot=6887, scenes=len({r['scan_id'] for r in source_rows['F_initial']}),
