"""Prepare and gate the Nr F batch check; no cross-dataset training is launched."""
import datetime,hashlib,json,os,shlex
from pathlib import Path
import paramiko
repo=Path(__file__).resolve().parents[1]
root='/root/autodl-tmp/mcln_pvground_nr_rec_competition_preparation_20260917_v1'
...
files={'check.py':(repo/'scripts/check_pvground_nr_rec_competition_backward.py').read_bytes(),
...
run=subprocess.run([sys.executable,str(root/'tests/test_pvground_referit_rec_competition.py')],stdout=subprocess.PIPE,stderr=subprocess.PIPE,env=dict(os.environ,CUDA_VISIBLE_DEVICES='',OMP_NUM_THREADS='1'))
(root/'cpu_test.log').write_bytes(run.stdout+run.stderr);assert run.returncode==0,run.stderr.decode()
assets=json.loads(Path('/root/autodl-tmp/mcln_pvground_nr_fixed_memory_preparation_20260917_v1/receipt.json').read_bytes())['assets']
for name,item in assets.items():assert sha(item['path'])==item['sha256'],name
import torch
torch.set_num_threads(1)
batch=torch.load(assets['nr_batch']['path'],map_location='cpu')
sources=batch['native_batch']['sample_dataset']
assert sources==[row['dataset'] for row in batch['rows']]==['nr3d']*4+['scannet']*4
assert batch['native_batch']['language_dataset']==['nr3d']*8
assert not torch.cuda.is_initialized()
record=dict(status='cpu_preparation_pass',time_cst=datetime.datetime.now(datetime.timezone(datetime.timedelta(hours=8))).isoformat(),assets=assets,actual_batch_sources=sources,gate_rejected_before_torch_import=True,model_forwards=0,backward_calls=0,optimizer_steps=0,formal_rows=0,new_checkpoints=0,missing_actual_nr_f_backward=True,missing_sr_real_batch_probe=True,training_jobs_launched=0,script_sha256=sha(root/'check.py'))
...
for name in ['prepare.log','prepare.exit','receipt.json','gate_rejection.log','cpu_test.log']:s.get(root+'/'+name,str(archive/name))
assert exit_code==0
