...
import scripts
scripts.__path__=[str(source/'scripts')]
import torch
...
spec=importlib.util.spec_from_file_location('export_fixture',root/'scripts/export_mask_geometry_initialization.py')
...
names=[name[7:] for name in backbone['model'] if name[7:].startswith(exporter.PREFIXES) and name.endswith(('.weight','.bias','.in_proj_weight','.in_proj_bias'))]
assert len(names)==84 and len(backbone['model'])==1144
endpoint={'schema':'mcln-scanrefer-mask-geometry-core-state-v1','arm':'native_gt_mask_geometry','steps':2482,
          'pretrained_artifacts':{'backbone':item},'manifest_sha256':'synthetic-format-test-not-a-training-run',
          'core_trainable_tensors':names,'core_parameters':{name:backbone['model']['module.'+name].clone()+1e-4 for name in names}}
...
assert any(name.endswith('in_proj_weight') for name in names)
...
for dset,count in [('nr3d',7899),('sr3d',17726)]:
    ...
    argv+=['--model_only_initialization'];sys.argv=['mask-geometry-initialization-cpu-test']+argv
    ...
    actual_names={name for name,_ in model.named_parameters() if name.startswith(exporter.PREFIXES)}
    ...
    results[dset]={'native_factory_and_checkpoint_loader_used':True,'all1144_tensors_equal_reconstructed_fixture':True,
                   'changed_core_parameters_loaded':84,'frozen_other_tensors_preserved':1060,
...
fixture_path.unlink()
result={'schema':'mcln-mask-geometry-initialization-cpu-test-v1','status':'pass',
        'time_cst':datetime.datetime.now(datetime.timezone(datetime.timedelta(hours=8))).isoformat(),
        'elapsed_seconds':time.time()-started,'protocols':results,'source_files_verified':len(native_manifest['files']),
        'backbone_sha256':item['sha256'],'export_script_sha256':sha(root/'scripts/export_mask_geometry_initialization.py'),
...
print('MASK GEOMETRY INITIALIZATION CPU TEST '+json.dumps(result),flush=True)
