"""Launch one read-only CPU check after the first E checkpoint exists."""
import datetime
import hashlib
import json
import os
...
root='/root/autodl-tmp/mcln_pvground_fixed_memory_checkpoint_check_20260917_v1'
training='/root/autodl-tmp/mcln_pvground_scanrefer_finetune_20260917_fixed_memory_v1'
archive=repo/'refine-logs/pvground_fixed_memory_checkpoint_check_20260917_v1'
train_spec=json.loads((repo/'refine-logs/pvground_scanrefer_finetune_20260917_fixed_memory_v1/spec.json').read_bytes())
files={'check.py':(repo/'scripts/check_pvground_fixed_memory_latest.py').read_bytes()}
...
files['plan.md']=b'One actual E periodic checkpoint CPU strict restore; require204 backbone state tensors exactly equal parent and718 optimizer parameter IDs with empty backbone/text groups. Pin one open inode during atomic writer replacements; bind parent/spec/source and all 37 reader states, verify both task matrices updated. No GPU forward, optimizer update, new weights, quality inference or training changes. Record observed step rather than assume512.\n'
...
_,out,err=c.exec_command('screen -dmS mcln_pvg_fixed_memory_checkpoint_check_v1 bash -c '+shlex.quote(inner),timeout=30)
