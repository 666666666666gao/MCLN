#!/usr/bin/env bash
# Frozen formal-pipeline implementation for the Nr3D tier hard-query branch.
...
BACKBONE_SANDBOX_SCRIPT="${BACKBONE_SANDBOX_SCRIPT:-}"
BACKBONE_SANDBOX_CODE_ROOT="${BACKBONE_SANDBOX_CODE_ROOT:-}"
BACKBONE_SANDBOX_CODE_MANIFEST_SHA256="${BACKBONE_SANDBOX_CODE_MANIFEST_SHA256:-}"
BACKBONE_SANDBOX_INPUT_ROOT="${BACKBONE_SANDBOX_INPUT_ROOT:-}"
BACKBONE_SANDBOX_INPUT_MANIFEST_SHA256="${BACKBONE_SANDBOX_INPUT_MANIFEST_SHA256:-}"
if [[ -n "${BACKBONE_SANDBOX_SCRIPT}" ]]; then
  [[ -f "${BACKBONE_SANDBOX_SCRIPT}"
      && -d "${BACKBONE_SANDBOX_CODE_ROOT}"
      && "${BACKBONE_SANDBOX_CODE_MANIFEST_SHA256}" =~ ^[0-9a-f]{64}$
      && -d "${BACKBONE_SANDBOX_INPUT_ROOT}"
      && "${BACKBONE_SANDBOX_INPUT_MANIFEST_SHA256}" =~ ^[0-9a-f]{64}$ ]] || {
    echo "complete backbone sandbox provenance is required" >&2
    exit 2
  }
fi
...
run_backbone() {
  ...
  local -a execution_prefix=()
  local -a single_rank_prefix=(
    /usr/bin/env
    RANK=0 WORLD_SIZE=1 LOCAL_RANK=0 LOCAL_WORLD_SIZE=1 LOCAL_SIZE=1
    MASTER_ADDR=127.0.0.1 MASTER_PORT="${MASTER_PORT}"
  )
  if [[ -n "${BACKBONE_SANDBOX_SCRIPT}" ]]; then
    mkdir -p "${BACKBONE_RUN_DIR}/runtime_home"
    export HOME="${BACKBONE_RUN_DIR}/runtime_home"
    execution_prefix=(
      "${PYTHON_BIN}" "${BACKBONE_SANDBOX_SCRIPT}"
      --code-root "${BACKBONE_SANDBOX_CODE_ROOT}"
      --code-manifest-sha256 "${BACKBONE_SANDBOX_CODE_MANIFEST_SHA256}"
      --input-root "${BACKBONE_SANDBOX_INPUT_ROOT}"
      --input-manifest-sha256 "${BACKBONE_SANDBOX_INPUT_MANIFEST_SHA256}"
      --allow-write "${BACKBONE_RUN_DIR}"
      --allow-write "${BACKBONE_SANDBOX_CODE_ROOT}/tensorboard_output"
  ...
  local -a train_args=(
    ...
    --pp_checkpoint "${SOURCE_CHECKPOINT}"
  ...
  if [[ -n "${BACKBONE_RESUME_CHECKPOINT}"
    ...
    "${execution_prefix[@]}" "${single_rank_prefix[@]}" \
      "${PYTHON_BIN}" "${eval_args[@]}" --local_rank 0
  fi
  "${execution_prefix[@]}" "${single_rank_prefix[@]}" \
    "${PYTHON_BIN}" "${train_args[@]}" --local_rank 0
