...
from scripts.run_v127_meshsp_nested_semantic_prior_corrected_oof import (
    all_hard_candidate_inputs,
    build_listwise_training_population,
    listwise_abstain_loss,
    prior_corrected_positive_row_weight,
...
def test_v127_antisymmetric_ranker_shape_is_finite_and_trainable():
...
def test_v127_ranker_is_strictly_antisymmetric_under_pair_swap():
...
def test_v127_listwise_population_uses_fix_actions_or_baseline_target():
    count = 2
    components = {
...
def test_v127_prior_corrected_loss_preserves_four_to_one_budget():
    logits = torch.tensor(
    ...
    population = {
        "row_count": 2,
        "candidate_positive_rows": {"025": 1, "050": 1},
        "baseline_target_rows": {"025": 1, "050": 1},
    }
    positive_row_weight = prior_corrected_positive_row_weight(population)
    assert torch.equal(positive_row_weight, torch.tensor([0.25, 0.25]))
    loss, baseline_target, row_weight = listwise_abstain_loss(
        logits, positive, positive_row_weight
    )
    expected = (
        0.25 * torch.nn.functional.softplus(torch.tensor(-2.0))
...
def test_v127_selector_ranks_all_hard_candidates_then_abstains():
    count = 2
