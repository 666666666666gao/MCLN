...
OUTPUT=$BASE/v119_meshsp_nested_break_veto_oof.json
LOG=$BASE/v119_meshsp_nested_break_veto_oof.log
EXIT=$BASE/v119_meshsp_nested_break_veto_oof.exit
SESSION=v119_nested_break_veto_oof
SELF=$(readlink -f "$0")
...
if [[ "${1:-}" == "--inner" ]]; then
    ...
    env CUDA_VISIBLE_DEVICES=0 \
        ...
        "$PYTHON" scripts/run_v119_meshsp_nested_break_veto_oof.py \
...
cd "$REPO"
require_sha 65938d1678f1ef53d5759f13feb187975b71ec1da302f43063432269f220b9a2 \
    scripts/run_v119_meshsp_nested_break_veto_oof.py
...
for path in "$OUTPUT" "$LOG" "$EXIT"; do
    if [[ -e "$path" ]]; then
        echo "Refusing to overwrite existing V119 output: $path" >&2
        exit 74
    fi
done
if [[ $(nvidia-smi -L | wc -l) -ne 1 ]]; then
    echo "V119 requires the frozen single-GPU topology" >&2
    exit 75
fi
...
if pgrep -af '[r]un_v119_meshsp_nested_break_veto_oof.py' >/dev/null; then
    echo "V119 process already exists" >&2
    exit 75
fi
if screen -ls 2>/dev/null | grep -q "[.]$SESSION"; then
    echo "V119 screen session already exists" >&2
    exit 75
fi
available_kb=$(df -Pk /root/autodl-tmp | awk 'NR==2 {print $4}')
if [[ "$available_kb" -lt 2621440 ]]; then
    echo "V119 requires at least 2.5 GiB free on /root/autodl-tmp" >&2
    exit 75
fi
...
if ! screen -ls 2>/dev/null | grep -q "[.]$SESSION" \
        && [[ ! -e "$EXIT" ]]; then
    echo "V119 screen failed to remain alive and produced no exit receipt" >&2
    exit 75
fi
