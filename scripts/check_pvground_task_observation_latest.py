...
assert len(parameter_ids)==820 and len(set(parameter_ids))==820
...
assert not torch.cuda.is_initialized()
result=dict(task_query_max_absolute_change=task_changes,task_module_sha256=data['task_module_sha256'],status='pass',time_cst=now(),step=data['step'],fit_rows=len(data['row_ids']),
...
print('PVG_TASK_OBSERVATION_LATEST_CPU_PASS '+json.dumps(result),flush=True)
