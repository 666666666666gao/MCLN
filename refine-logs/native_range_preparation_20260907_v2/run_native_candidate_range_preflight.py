...
def main():
    ...
    tester.logger = logging.getLogger('native-range-preflight')
    ...
    for dset in ['nr3d', 'sr3d']:
        ...
        for batch_index, raw in enumerate(loader()):
            ...
            records.append({'phase': 'train', 'batch': batch_index, 'rows': len(raw['scan_ids']),
                            ...
                            'point_sha256': [hashlib.sha256(p.cpu().numpy().tobytes()).hexdigest() for p in inputs['point_clouds']]})
            print('NATIVE RANGE PREFLIGHT', json.dumps({'dataset': dset, 'step': batch_index + 1,
    ...
    result = {'schema': 'mcln-native-candidate-range-gpu-preflight-v1',
              'candidate_local_visual_variant': manifest['candidate_local_visual_variant'], 'status': 'pass',
    ...
    print('NATIVE RANGE PREFLIGHT COMPLETE', json.dumps({k: result[k] for k in [
