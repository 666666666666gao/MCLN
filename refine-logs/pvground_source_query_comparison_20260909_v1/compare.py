"""Compare saved native-control and source-query outputs; never run the model."""
import argparse
import datetime
import hashlib
import json
...
def compare(spec, stage):
    ...
    assert 'source_query_read' not in configs[0] and configs[1]['source_query_read'] is True
    assert sha(roots[1]/'pvground_source_query.py')==configs[1]['source_query_module_sha256']
    for name in ['rows.jsonl','boxes.npy','scores.npy']:
        assert sha(roots[0]/'initial'/name)==sha(roots[1]/'initial'/name),name
    for root in roots:
    ...
    if stage=='terminal':
        ...
        output['initial_cross_control_net']={str(t):initial['transitions']['bbs'][str(t)]['net'] for t in [.25,.5]}
        output['difference_in_training_changes']={str(t):output['transitions']['bbs'][str(t)]['net']-initial['transitions']['bbs'][str(t)]['net'] for t in [.25,.5]}
