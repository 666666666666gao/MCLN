#!/usr/bin/env python
"""Nested OOF screen of V125 all-hard-candidate utility selection."""
...
import models.rec_semantic_antisymmetric_utility as v125_model
...
V125_CRITIC_EPOCHS = 50
V125_CRITIC_BATCH_SIZE = 1024
V125_CRITIC_LEARNING_RATE = 1e-3
V125_CRITIC_WEIGHT_DECAY = 1e-3
...
def _pair_inputs(
    ...
    if count <= 0:
        raise ValueError("V125 pair population must be nonempty")
    for value in inputs.values():
        if (value.dtype != torch.float32 or value.device.type != "cpu"
                or value.shape[0] != count
                or not bool(torch.isfinite(value).all().item())):
            raise ValueError("V125 pair input layout changed")
    return {key: value.contiguous() for key, value in inputs.items()}


def build_hard_pair_training_population(prediction, records):
    """Build label-bearing V115-Pareto hard pairs on inner-held scenes."""
    ...
    if (probabilities.dtype != torch.float32
        ...
        raise ValueError("V125 model probability layout changed")
    rows = torch.arange(count)
    ...
    if (tuple(ious.shape) != (count, QUERY_COUNT, VARIANT_COUNT)
            or not bool(torch.isfinite(ious).all().item())):
        raise ValueError("V125 candidate IoU layout changed")
    ...
    event = candidate_hit.ne(baseline_hit[:, None, None, :])
    pair_mask = eligible & event.any(dim=-1)
    pair_rows, proposal_query, proposal_variant = pair_mask.nonzero(
        as_tuple=True
    )
    ...
    for column, suffix in enumerate(("025", "050")):
        active = event_mask[:, column]
        values = targets[active, column]
        fixes = int(values.sum().item())
        breaks = int(values.numel() - fixes)
        if fixes <= 0 or breaks <= 0:
            raise ValueError("V125 hard pairs lack fix/break events")
        event_counts[suffix] = {
            "fix": fixes,
            "break": breaks,
            "events": int(values.numel()),
        }
    return inputs, targets, event_mask, {
        "eligible_pair_count": int(eligible.sum().item()),
        "event_pair_count": int(pair_mask.sum().item()),
        "event_counts": event_counts,
    }


def fit_semantic_pairwise_critic(prediction, records, device):
    """Fit the fixed event-only pairwise utility critic."""
    inputs, targets, event_mask, population = (
        build_hard_pair_training_population(prediction, records)
    )
    pair_count = targets.shape[0]
    if pair_count < 100:
        raise ValueError("V125 pairwise event population is too small")
    resolved = torch.device(device)
    _set_hierarchical_seed(resolved)
    model = v125_model.SemanticAntisymmetricUtilityRanker().to(resolved)
    optimizer = torch.optim.AdamW(
        model.parameters(),
        lr=V125_CRITIC_LEARNING_RATE,
        weight_decay=V125_CRITIC_WEIGHT_DECAY,
    )
    shuffle_generator = torch.Generator(device="cpu")
    shuffle_generator.manual_seed(0)
    epoch_receipts = []
    for epoch in range(V125_CRITIC_EPOCHS):
        model.train()
        order = torch.randperm(pair_count, generator=shuffle_generator)
        total_loss = 0.0
        total_events = 0
        for start in range(0, pair_count, V125_CRITIC_BATCH_SIZE):
            indices = order[start:start + V125_CRITIC_BATCH_SIZE]
            batch_inputs = {
                key: value[indices].to(resolved)
                for key, value in inputs.items()
            }
            batch_targets = targets[indices].to(resolved)
            batch_event = event_mask[indices].to(resolved)
            logits = model(**batch_inputs)
            raw_loss = F.binary_cross_entropy_with_logits(
                logits, batch_targets, reduction="none"
            )
            event_count = int(batch_event.sum().item())
            loss = raw_loss[batch_event].sum() / event_count
            optimizer.zero_grad(set_to_none=True)
            loss.backward()
            clip_grad_norm_(model.parameters(), 1.0)
            optimizer.step()
            total_loss += float(loss.detach().item()) * event_count
    ...
    receipt.update({
        "epochs": V125_CRITIC_EPOCHS,
        "batch_size": V125_CRITIC_BATCH_SIZE,
        "epoch_receipts": epoch_receipts,
        "state_sha256": v99.tensor_sha256(*[
            value for _, value in sorted(model.state_dict().items())
        ]),
...
def all_hard_candidate_inputs(prediction):
    ...
    if (probabilities.dtype != torch.float32
            ...
            or tuple(baselines.shape) != (count,)
            or not bool(torch.isfinite(probabilities).all().item())):
        raise ValueError("V125 all-candidate probability layout changed")
    ...
    if pair_rows.numel() <= 0:
        raise ValueError("V125 all-candidate population must be nonempty")
    pair_baselines = baselines[pair_rows]
    baseline_query = pair_baselines.div(
        VARIANT_COUNT, rounding_mode="floor"
    )
...
def predict_pairwise_utility(model, inputs, device):
    ...
    logits = []
    with torch.no_grad():
        for start in range(0, count, V125_CRITIC_BATCH_SIZE):
            stop = min(start + V125_CRITIC_BATCH_SIZE, count)
            logits.append(model(**{
                key: value[start:stop].to(resolved)
                for key, value in inputs.items()
            }).cpu().float())
    ...
    if (tuple(output.shape) != (count, 2)
        ...
        raise RuntimeError("V125 utility logits changed")
    return output
...
def select_all_hard_candidates(model, prediction, device):
    ...
    if (proposals.dtype != torch.long
        ...
        raise RuntimeError("V125 all-candidate selection contract changed")
    if bool((accepted & proposals.eq(baselines)).any().item()):
        raise RuntimeError("V125 accepted a baseline as a candidate")
    return {
        "proposals": proposals,
        "selected": selected,
        "accepted": accepted,
...
def pairwise_utility_accept_mask(base_accepted, utility_logits):
    if (base_accepted.dtype != torch.bool or base_accepted.dim() != 1
        ...
        raise ValueError("V125 utility gate tensors changed")
...
def main(argv=None):
    ...
    for held in range(5):
        ...
        if (set(records[index]["scan_id"] for index in inner_fit_indices)
            ...
            raise RuntimeError("scene leakage into V125 inner calibration")
        ...
        critic_model, critic_training = fit_semantic_pairwise_critic(
            inner_prediction,
            inner_records,
            args.device,
        )
        ...
        if not torch.equal(prediction["baselines"], baselines[held_indices]):
            raise RuntimeError("V125 OOF baseline identity changed")
    ...
    if (veto_reasons["selected_margin_violations"] != 0
        ...
        raise RuntimeError("V125 all-candidate Pareto contract changed")
    protected_after = capture_immutable_artifact_identities(protected_paths)
    if protected_after != protected_before:
        raise RuntimeError("protected artifacts changed during V125")
    protected_metadata_after = {
        name: file_sha256(path)
        for name, path in protected_metadata_paths.items()
    }
    if protected_metadata_after != protected_metadata_before:
        raise RuntimeError("protected V125 cache metadata changed during OOF")
    script_sources = {
        "v125": str(Path(__file__).resolve()),
        ...
        "v125_model": str(Path(v125_model.__file__).resolve()),
        "v108": str(
    ...
    report = {
        "schema": "rec-v125-nested-semantic-all-hard-candidate-scene-oof-v1",
        "version": 1,
        "validation_data_accessed": False,
        ...
        "protocol": {
            "architecture": (
                ...
                "candidate-minus-baseline antisymmetry; outer inference "
                "ranks every deployable V115-Pareto hard candidate"
            ),
            "adapter_objective": "frozen V95 bounded threshold-aligned listwise",
            "critic_objective": (
                "unweighted event-only binary cross entropy for fix versus "
                "break on every inner-held V115-Pareto hard candidate pair"
            ),
            ...
            "critic_candidate_dim": v125_model.V123_CANDIDATE_DIM,
            "critic_pair_structure": "shared_score_difference",
            "critic_model_gain_scale": "learned_positive_per_threshold",
            "critic_epochs": V125_CRITIC_EPOCHS,
            "critic_batch_size": V125_CRITIC_BATCH_SIZE,
            "critic_learning_rate": V125_CRITIC_LEARNING_RATE,
            "critic_weight_decay": V125_CRITIC_WEIGHT_DECAY,
            "critic_class_weights": None,
            "critic_neutral_events_in_loss": False,
            "critic_acceptance": (
    ...
    try:
        ...
        while written < len(payload):
            ...
            if count <= 0:
                raise OSError("V125 output write made no progress")
            written += count
        os.fsync(descriptor)
    finally:
        os.close(descriptor)
