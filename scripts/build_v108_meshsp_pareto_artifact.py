#!/usr/bin/env python
"""Fit and freeze the V108 MeshSP-aligned Pareto hierarchy."""
...
V108_ARTIFACT_SCHEMA = "rec-pareto-contextual-meshsp-full-train-artifact-v1"
V108_ARTIFACT_VERSION = 1
V108_OOF_SCHEMA = "rec-v108-meshsp-pareto-full-train-scene-oof-v1"
...
def _validate_oof_result(path, script_path):
    ...
    if (result.get("schema") != V108_OOF_SCHEMA
            ...
            or set(subgroups) != {"corrected", "regular"}
            or result.get("source_sha256", {}).get("v108") != script_sha256
        ...
        raise ValueError("V108 OOF result does not satisfy frozen gate")
    ...
    if len(diagnostics.get("fold_deltas", {})) != 5 or not all(
        ...
        raise ValueError("V108 OOF folds are not all strictly positive")
    expected_scenes = {
    ...
    for name, scene_count in expected_scenes.items():
        subgroup = subgroups.get(name)
        subgroup_diagnostics = (
            subgroup.get("diagnostics") if isinstance(subgroup, dict) else None
        )
        if (not isinstance(subgroup_diagnostics, dict)
                or subgroup.get("scene_count") != scene_count
                or not isinstance(subgroup.get("row_count"), int)
                or subgroup["row_count"] <= 0
                or subgroup_diagnostics.get("delta_hits025", -1) < 0
                or subgroup_diagnostics.get("delta_hits050", -1) < 0):
            raise ValueError("V108 OOF subgroup gate changed")
...
def validate_v108_artifact(
    ...
    if (artifact.get("normalization_sha256") != normalization["sha256"]
        ...
        raise ValueError("V108 normalization binding changed")
    if artifact.get("training_contract") != _training_contract():
        raise ValueError("V108 training contract changed")
    if artifact.get("policy") != {
        ...
        raise ValueError("V108 Pareto policy changed")
    ...
    if (not isinstance(fit, dict) or set(fit) != {
            ...
            or any(not b99._is_sha256(fit.get(name)) for name in (
                ...
                "model_state_sha256"))):
        raise ValueError("V108 fit evidence is invalid")
    ...
    if (not isinstance(evidence, dict) or set(evidence) != evidence_fields
            ...
            or not b99._is_sha256(evidence.get("sha256"))
            or not b99._is_sha256(evidence.get("script_sha256"))
            or not isinstance(evidence.get("switches"), int)
            or isinstance(evidence.get("switches"), bool)
            or evidence["switches"] < 0
            or not isinstance(evidence.get("delta_hits025"), int)
            or evidence["delta_hits025"] < MIN_DELTA_025
            or not isinstance(evidence.get("delta_hits050"), int)
            or evidence["delta_hits050"] < MIN_DELTA_050
            or not isinstance(
                evidence.get("bootstrap025_lower_bound_95"), int)
            or evidence["bootstrap025_lower_bound_95"] <= 0
            or not isinstance(
                evidence.get("bootstrap050_lower_bound_95"), int)
            or evidence["bootstrap050_lower_bound_95"] <= 0
            ...
            or not all(value is True for value in evidence["predicates"].values())):
        raise ValueError("V108 OOF evidence is invalid")
    subgroups = evidence.get("subgroups")
    expected_subgroup_scenes = {
        "corrected": EXPECTED_CORRECTED_SCENE_COUNT,
        "regular": EXPECTED_REGULAR_SCENE_COUNT,
    }
    if not isinstance(subgroups, dict) or set(subgroups) != set(
            expected_subgroup_scenes):
        raise ValueError("V108 OOF subgroup evidence is invalid")
    for name, scene_count in expected_subgroup_scenes.items():
        subgroup = subgroups[name]
        if (not isinstance(subgroup, dict) or set(subgroup) != {
                "scene_count", "row_count", "delta_hits025", "delta_hits050",
            } or subgroup.get("scene_count") != scene_count
                or not isinstance(subgroup.get("row_count"), int)
                or subgroup["row_count"] <= 0
                or not isinstance(subgroup.get("delta_hits025"), int)
                or subgroup["delta_hits025"] < 0
                or not isinstance(subgroup.get("delta_hits050"), int)
                or subgroup["delta_hits050"] < 0):
            raise ValueError("V108 OOF subgroup evidence is invalid")
    ...
    if (not isinstance(state, dict) or not state
        ...
        raise ValueError("V108 model state evidence is invalid")
    model = ParetoContextualHierarchicalReranker(**artifact["model_config"])
    try:
        model.load_state_dict(state, strict=True)
    except (RuntimeError, TypeError) as error:
        raise ValueError("V108 model state is incompatible") from error
    return copy.deepcopy(artifact["model_config"])


def load_v108_artifact(
    ...
    if expected_artifact_sha256 is not None and sha256 != expected_artifact_sha256:
        raise ValueError("V108 artifact SHA-256 mismatch")
    try:
        artifact = torch.load(io.BytesIO(payload), map_location="cpu")
    except Exception as error:
        raise ValueError("could not deserialize V108 artifact") from error
    config = validate_v108_artifact(
        artifact,
    ...
    if resolved_device.type == "cuda" and not torch.cuda.is_available():
        raise ValueError("CUDA V108 artifact load requested but unavailable")
...
def main(argv=None):
    ...
    parser.add_argument("--v108-result", required=True)
    parser.add_argument("--v108-script", required=True)
    ...
    if artifact_output.exists() or receipt_output.exists():
        raise FileExistsError("V108 artifact or receipt output already exists")
    result, result_sha256, script_sha256 = _validate_oof_result(
        args.v108_result, args.v108_script
    )
    protected_paths = {
    ...
    if loaded["input_sha256"] != result["input_sha256"]:
        raise ValueError("V108 live input provenance differs from OOF")
    ...
    if (len(records) != EXPECTED_ROW_COUNT
        ...
        raise ValueError("V108 full-train materialization changed")
    ...
    artifact = {
        "schema": V108_ARTIFACT_SCHEMA,
        "version": V108_ARTIFACT_VERSION,
        "deployable": True,
        ...
        "oof_evidence": {
            "path": str(Path(args.v108_result).expanduser().absolute()),
    ...
    loaded_model, reloaded = load_v108_artifact(
    ...
    if b99._state_sha256(loaded_model.state_dict()) != artifact[
        ...
        raise RuntimeError("V108 strict reload changed model state")
    protected_after = capture_immutable_artifact_identities(protected_paths)
    if protected_after != protected_before:
        raise RuntimeError("protected artifacts changed during V108 refit")
    protected_metadata_after = {
        name: b99._sha256(path)
        for name, path in protected_metadata_paths.items()
    }
    if protected_metadata_after != protected_metadata_before:
        raise RuntimeError("protected V108 cache metadata changed during fit")
    receipt = {
        "schema": "rec-pareto-contextual-meshsp-full-train-artifact-receipt-v1",
        "version": 1,
        "artifact": {
