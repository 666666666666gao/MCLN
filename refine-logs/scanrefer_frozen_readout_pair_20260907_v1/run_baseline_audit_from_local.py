...
code='''import datetime,hashlib,json,math
from pathlib import Path
...
assert 'baseline_metrics.json' in sftp.listdir(remote)
assert 'baseline_cross_run_audit.json' not in sftp.listdir(remote)
for name,raw in [('baseline_cross_run_audit.py',code.encode()),('run_baseline_audit_from_local.py',Path(__file__).read_bytes())]:
...
for name in ['baseline_cross_run_audit.json','baseline_rows.json']:
