...
train='/root/autodl-tmp/mcln_pvground_scanrefer_finetune_20260917_task_observation_v1'
...
oldtrain=repo/'refine-logs/pvground_scanrefer_finetune_20260909_observation_v1'
oldaudit=repo/'refine-logs/pvground_scanrefer_endpoint_audit_20260909_observation_v1'
...
for dependency in ['finetune','endpoint_audit','formal']:
    with s.open('/root/autodl-tmp/mcln_pvground_scanrefer_'+dependency+'_20260909_observation_v1/controller.exit','rb') as f:assert f.read().strip()==b'0'
...
spec.update(root=train,model_source=model_source,source_port=source_port,
    ...
    comparison='own initial and same-budget C/A/B; primary bbs; shared identity and task reads; no quality or generalization claim')
files={'train.py':(repo/'scripts/run_pvground_scanrefer_task_observation.py').read_bytes(),
       'pvground_task_observation_query.py':task_module,'pvground_observation_query.py':module,'pvground_source_query.py':(repo/'models/pvground_source_query.py').read_bytes(),'plan.md':(repo/'docs/PVG_TASK_OBSERVATION_CONTROL_2026-09-17.md').read_bytes()}
...
training_launch=launch(train,'mcln_pvg_scan_task_observation_v1')
...
install(audit,audit_files);audit_launch=launch(audit,'mcln_pvg_task_observation_audit_v1')
