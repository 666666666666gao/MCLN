...
root=Path('/root/autodl-tmp/mcln_pvground_scanrefer_finetune_20260917_task_observation_v1').resolve()
audit_root=Path('/root/autodl-tmp/mcln_pvground_scanrefer_endpoint_audit_20260917_task_observation_v1')
...
comparison=Path('/root/autodl-tmp/mcln_pvground_task_observation_comparison_20260917_v1')
for directory in [root,audit_root,formal,comparison]:
    assert (directory/'controller.exit').read_text().strip()=='0'
    pid=int(json.loads((directory/'launch.json').read_bytes())['process'].split()[0])
...
for key in ['source_query_read','source_query_module_sha256','source_port_sha256','observation_state','observation_module_sha256','task_read','task_module_sha256']:
    assert payload[key]==receipt[key],key
