#!/usr/bin/env python
"""Run and seal the sole frozen V101 ScanRefer mesh-superpoint validation."""
...
SAMPLE_COUNT = 9508
EXPERIMENT = "epoch71_v101_meshsp_full9508"
MASTER_PORT = 29731
ARTIFACT_PATH = Path(
    "/root/autodl-tmp/DATA_ROOT/output/rec_reranker/e71_top16/"
    "v101_artifacts/pareto_contextual_h128_seed0_fulltrain.pth"
)
ARTIFACT_SHA256 = (
    "2c969a6c28a0c9315b53f0f847567345e47da8c912091344b23612680643a2ae"
)
CLAIM_PATH = ARTIFACT_PATH.parent / (
    "v101_meshsp_official_once_retry_after_path_preflight.claim.json"
)
RESULT_SCHEMA = "rec-v101-meshsp-official-validation-result-v1"
...
def validate_authoritative_command(command, output_dir):
    ...
    if list(command) != expected:
        raise ValueError("V101 meshsp official command changed")
    required = {
    ...
    if not required.issubset(command) or FORBIDDEN.intersection(command):
        raise ValueError("V101 meshsp official command policy is invalid")
    if "--eval_use_rec_selective_residual_scores" in command:
        raise ValueError("V101 cannot combine the selective residual policy")
    return list(command)
...
def _protected_snapshots():
    result = {
        ...
        "v101": _snapshot(ARTIFACT_PATH, "V101 artifact", True),
    }
    expected = {
    ...
    if (artifact.get("deployable") is not True
        ...
        raise ValueError("V101 deployable artifact contract is invalid")
    return result
...
def run_official(output_dir, dry_run=False):
    ...
    if not dry_run and (CLAIM_PATH.exists() or CLAIM_PATH.is_symlink()):
        raise FileExistsError("V101 meshsp validation claim already exists")
    ...
    if protected_after != protected_before:
        raise RuntimeError("protected artifact changed during V101 official run")
    if code_after != code_before:
        raise RuntimeError("Python source tree changed during V101 official run")
    text = stdout_path.read_text(encoding="utf-8", errors="replace")
    if returncode != 0:
        raise RuntimeError("V101 official subprocess failed with code {}".format(returncode))
    populations = re.findall(r"length of testing dataset:\s*(\d+)", text)
    if len(populations) != 1 or populations[0] != str(SAMPLE_COUNT):
        raise ValueError("V101 official validation population is not 9508")
    ...
    if len(timestamp_runs) != 1:
        raise ValueError("V101 official run directory is ambiguous")
