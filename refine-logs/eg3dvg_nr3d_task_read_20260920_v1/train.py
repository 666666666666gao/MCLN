"""Fixed one-pass EG Nr3D adaptation; author model/losses, new optimizer, no validation selection."""
import argparse
import datetime
import hashlib
import json
...
def main():
    ...
    assert len(dataset) == spec['fit_rows'] == 44909 and dataset.augment
    ...
    for batch in loader:
        ...
        record = {'step': steps, 'rows': rows, 'loss': float(loss.detach()),
                  'task_gradient_norms': task_grad_norms,
    ...
    log.close()
    assert steps == (2 if args.stage == 'preflight' else 5614)
    assert rows == (16 if args.stage == 'preflight' else 44909)
