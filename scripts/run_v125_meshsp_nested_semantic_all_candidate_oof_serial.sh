...
OUTPUT=$BASE/v125_meshsp_nested_semantic_all_candidate_oof.json
LOG=$BASE/v125_meshsp_nested_semantic_all_candidate_oof.log
EXIT=$BASE/v125_meshsp_nested_semantic_all_candidate_oof.exit
SESSION=v125_nested_semantic_all_candidate_oof
SELF=$(readlink -f "$0")
...
if [[ "${1:-}" == "--inner" ]]; then
    ...
    env CUDA_VISIBLE_DEVICES=0 \
        ...
        "$PYTHON" scripts/run_v125_meshsp_nested_semantic_all_candidate_oof.py \
...
cd "$REPO"
require_sha 6cee94ab0ef3e06da6c22a412304ab33877c19517f3412b232f238a48d08e9a0 \
    scripts/run_v125_meshsp_nested_semantic_all_candidate_oof.py
...
for path in "$OUTPUT" "$LOG" "$EXIT"; do
    if [[ -e "$path" ]]; then
        echo "Refusing to overwrite existing V125 output: $path" >&2
        exit 74
    fi
done
if [[ $(nvidia-smi -L | wc -l) -ne 1 ]]; then
    echo "V125 requires the frozen single-GPU topology" >&2
    exit 75
fi
...
if pgrep -af '[r]un_v125_meshsp_nested_semantic_all_candidate_oof.py' >/dev/null; then
    echo "V125 process already exists" >&2
    exit 75
fi
if screen -ls 2>/dev/null | grep -q "[.]$SESSION"; then
    echo "V125 screen session already exists" >&2
    exit 75
fi
available_kb=$(df -Pk /root/autodl-tmp | awk 'NR==2 {print $4}')
if [[ "$available_kb" -lt 2621440 ]]; then
    echo "V125 requires at least 2.5 GiB free on /root/autodl-tmp" >&2
    exit 75
fi
...
if ! screen -ls 2>/dev/null | grep -q "[.]$SESSION" \
        && [[ ! -e "$EXIT" ]]; then
    echo "V125 screen failed to remain alive and produced no exit receipt" >&2
    exit 75
fi
