...
for key in ['source_query_read','source_query_module_sha256','source_port_sha256','observation_state','observation_module_sha256','task_read','task_module_sha256']:
...
assert sha(formal/'pvground_task_observation_query.py')==data['task_module_sha256']
assert len(added)==37
assert spec['fixed_visual_memory'] and data['fixed_visual_memory']
policy=json.loads((training/'memory_policy.json').read_bytes())
assert data['memory_policy']==policy
visual={name for name in parent if name.startswith('backbone_net.')}
assert len(visual)==policy['visual_state_tensors']==204
assert all(name in data['state_delta'] and torch.equal(data['state_delta'][name],parent[name]) for name in visual)
...
assert len(parameter_ids)==718 and len(set(parameter_ids))==718
assert [len(group['params']) for group in optimizer['param_groups']]==[718,0,0]
...
task_name='decoder.5.source_query_read.task_queries'
task_changes=[float((state[task_name][i]-initial[task_name][i]).abs().max()) for i in [0,1]]
assert all(value>0 for value in task_changes)
assert not torch.cuda.is_initialized()
result=dict(fixed_visual_memory=True,visual_state_tensors=len(visual),visual_state_exactly_parent=True,optimizer_group_sizes=[len(group['params']) for group in optimizer['param_groups']],task_query_max_absolute_change=task_changes,task_module_sha256=data['task_module_sha256'],status='pass',time_cst=now(),step=data['step'],fit_rows=len(data['row_ids']),
...
print('PVG_FIXED_MEMORY_LATEST_CPU_PASS '+json.dumps(result),flush=True)
