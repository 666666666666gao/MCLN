...
from .candidate_local_visual_training import LOCAL_VISUAL_PREFIX
...
def build_mcln_optimizer_param_groups(
    ...
    candidate_local_visual_lr=None,
):
    ...
    if candidate_local_visual_lr is not None:
        candidate_local_visual_lr = _positive_finite_float(
            "candidate_local_visual_lr", candidate_local_visual_lr
        )
        group_order = group_order + ("candidate_local_visual",)
    ...
    for name, parameter in _named_parameters_with_all_paths(model):
        ...
        if (candidate_local_visual_lr is not None
                and bare_name.startswith(LOCAL_VISUAL_PREFIX)):
            group_name = "candidate_local_visual"
    ...
    if candidate_local_visual_lr is not None:
        lr_by_group["candidate_local_visual"] = candidate_local_visual_lr
    groups = []
