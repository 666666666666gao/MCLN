...
OUTPUT=$BASE/v115r1_meshsp_anchored_spatial_adapter_oof.json
LOG=$BASE/v115r1_meshsp_anchored_spatial_adapter_oof.log
EXIT=$BASE/v115r1_meshsp_anchored_spatial_adapter_oof.exit
SESSION=v115r1_anchored_spatial_adapter_oof
SELF=$(readlink -f "$0")
...
for path in "$OUTPUT" "$LOG" "$EXIT"; do
    if [[ -e "$path" ]]; then
        echo "Refusing to overwrite existing V115r1 output: $path" >&2
        exit 74
    fi
done
if [[ $(nvidia-smi -L | wc -l) -ne 1 ]]; then
    echo "V115r1 requires the frozen single-GPU topology" >&2
    exit 75
fi
...
if screen -ls 2>/dev/null | grep -q "[.]$SESSION"; then
    echo "V115r1 screen session already exists" >&2
    exit 75
fi
available_kb=$(df -Pk /root/autodl-tmp | awk 'NR==2 {print $4}')
if [[ "$available_kb" -lt 2621440 ]]; then
    echo "V115r1 requires at least 2.5 GiB free on /root/autodl-tmp" >&2
    exit 75
fi
...
if ! screen -ls 2>/dev/null | grep -q "[.]$SESSION" \
        && [[ ! -e "$EXIT" ]]; then
    echo "V115r1 screen failed to remain alive and produced no exit receipt" >&2
    exit 75
fi
