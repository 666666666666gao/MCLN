...
from scripts.queue_scanrefer_mask_geometry_posttraining import validated_module_pass
...
def completed(baseline, control, candidate, expected):
    ...
    receipt = {'schema': 'mcln-scanrefer-mask-geometry-gt-pair-v1', 'status': 'complete',
        ...
        'baseline_metrics': {'native_gt_mask_geometry': metrics(baseline)},
        'terminal_metrics': {'native_gt': metrics(control), 'native_gt_mask_geometry': metrics(candidate)},
...
def test_no_formal_when_candidate_loses_to_native_gt_despite_improving_start():
