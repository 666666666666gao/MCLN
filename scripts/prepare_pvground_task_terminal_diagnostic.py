"""Stage and launch only the predeclared fixed-terminal diagnostic, with no training."""
import datetime
import hashlib
import json
import os
...
assert read('/root/autodl-tmp/mcln_pvground_task_terminal_diagnostic_20260917_v1/controller.exit').strip() == b'1'
...
files = {'diagnose.py':(repo/'scripts/diagnose_pvground_task_terminal.py').read_bytes(),
...
spec = dict(training_root=training,training_spec_sha256=sha(train_spec_raw),terminal_sha256=receipt['terminal_sha256'],
    ...
    conditions=['normal','normal_repeat','zero_semantic','zero_geometry','zero_both'],
    training_rows=[0,173,237,455],batch_size=2,model_forwards=10,optimizer_steps=0,formal_rows=0,
    scope='fixed trained D mechanism diagnostic, not performance evaluation or model selection',
...
command = 'screen -dmS mcln_pvg_task_terminal_diagnostic_v2 '+shlex.quote('/root/miniconda3/envs/bdetr/bin/python')+' -u '+shlex.quote(root+'/controller.py')
...
record = dict(time_cst=datetime.datetime.now(datetime.timezone(datetime.timedelta(hours=8))).isoformat(),
    root=root,process=process,expected_seconds=180,first_check_after_seconds=180,
