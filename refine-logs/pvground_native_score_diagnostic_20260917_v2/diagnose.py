...
def main():
    ...
    for raw in loader:
        ...
        logits = prediction['last_sem_cls_scores'].detach().requires_grad_()
        ...
        text_weights = (batch['positive_map'][:,0]>0).float()
        for key in ['modify_positive_map','pron_positive_map','rel_positive_map']:
            text_weights += batch[key][:,0]
        text_weights -= batch['other_entity_map'][:,0]
        ...
        # Preserve the native loss's float32 logits, labels and EOS constant.
        ...
        for b,(query_ids,target_ids) in enumerate(indices):
            ...
            target_sim[b,query_ids] = weighted[target_ids]
            eos[b,query_ids] = 1
        ...
        # Check weighted-sum score algebra against the deployed sequence of float32 sums.
        ...
        selection_formula = scores.argsort(-1,descending=True)[:,0]
        ...
        for b,(query_ids,target_ids) in enumerate(indices):
            ...
            record = dict(row_id=row_id,scan_id=dataset.annos[row_id]['scan_id'],point_sha256=point_hashes[b],
                matched_root=matched,selected=choice,best_iou_query=best,formula_selection=int(selection_formula[b]),
                ...
                matched_score_rank=int((deployed_scores[b]>deployed_scores[b,matched]).sum())+1,
                best_score_rank=int((deployed_scores[b]>deployed_scores[b,best]).sum())+1)
    ...
    result = dict(status='complete',time_cst=now(),elapsed_seconds=time.time()-begin,training_rows=128,physical_scenes=128,
        ...
        score_formula_selection_differences=sum(r['selected']!=r['formula_selection'] for r in records),
        analysis_dtype='float32',gradient_analytic_tolerance=1e-6,
